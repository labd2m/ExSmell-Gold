...
- **Expected smell location:** Modules `FleetRegistrar`, `TelemetryIngester`, `MaintenanceScheduler`, and `FleetMonitor`
- **Affected functions:** `FleetRegistrar.add_vehicle/2`, `TelemetryIngester.ingest/3`, `MaintenanceScheduler.schedule/3`, `FleetMonitor.vehicles_needing_attention/1`
- **Short explanation:** Each of the four fleet-management modules directly accesses the shared vehicle-state Agent instead of routing through a centralised fleet state owner, scattering the knowledge of the internal vehicles map across the system.
...
# VALIDATION: This is a smell because FleetRegistrar directly calls Agent.update to
# insert new vehicle records into the Agent, owning the vehicles map format without
# any centralised fleet-state interface.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because TelemetryIngester directly calls Agent.update to
# both append telemetry records to the list and update fields on the vehicle record,
# spreading state-mutation logic across a module that should only care about data ingestion.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because MaintenanceScheduler directly calls Agent.update
# to append maintenance entries and update vehicle status, yet another module that
# manipulates both the vehicles map and the maintenance_schedule list independently.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because FleetMonitor directly calls Agent.get to scan the
# vehicles map for specific status values, tightly coupling operational monitoring to
# the Agent's internal vehicle record structure.
