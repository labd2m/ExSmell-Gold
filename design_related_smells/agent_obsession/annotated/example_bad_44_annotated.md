...
- **Expected smell location:** Modules `UserRegistrar`, `UserProfileEditor`, `UserSuspender`, and `UserDirectory`
- **Affected functions:** `UserRegistrar.register/2`, `UserProfileEditor.update_profile/3`, `UserSuspender.suspend/3`, `UserDirectory.search/2`
- **Short explanation:** All four modules independently call `Agent.get` or `Agent.update` on the shared user store without going through a single owner module, spreading knowledge of the user map structure everywhere.
...
# VALIDATION: This is a smell because UserRegistrar directly calls Agent.get to check
# for duplicate emails and Agent.update to persist new users, taking full responsibility
# for the internal users and email_index maps without any centralised interface.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because UserProfileEditor directly calls Agent.update to
# merge profile changes into the users map, independently owning the update semantics
# for user records inside the Agent.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because UserSuspender directly calls Agent.update to change
# user status to :suspended and append a suspension reason, yet another module that
# manipulates the shared Agent's internal user records independently.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because UserDirectory directly calls Agent.get to scan the
# users map for search results, coupling directory-query logic to the raw Agent state
# structure.
