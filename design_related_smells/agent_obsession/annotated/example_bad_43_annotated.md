...
- **Expected smell location:** Modules `AppointmentBooker`, `AppointmentCanceller`, `AvailabilityChecker`, and `ScheduleExporter`
- **Affected functions:** `AppointmentBooker.book/2`, `AppointmentCanceller.cancel/2`, `AvailabilityChecker.slots_available/3`, `ScheduleExporter.export_day/2`
- **Short explanation:** All four modules reach directly into the shared schedule Agent to read and write appointments, spreading ownership of the schedule map and slot logic across unrelated modules.
...
# VALIDATION: This is a smell because AppointmentBooker directly calls Agent.get to
# check conflicts and Agent.update to persist new appointments, embedding slot-booking
# logic with direct Agent manipulation instead of using a single owner module.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because AppointmentCanceller directly calls Agent.update
# to transition an appointment to :cancelled status and appends to the cancellations list,
# taking independent ownership of those state fields.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because AvailabilityChecker directly calls Agent.get to
# compute free slots by scanning the appointments map, coupling availability logic
# directly to the Agent's internal representation.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because ScheduleExporter directly calls Agent.get to read
# the appointments map and filter by day, binding export/reporting logic to the raw Agent
# state format.
