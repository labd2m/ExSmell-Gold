...
- **Expected smell location:** Modules `FeatureFlagWriter`, `FeatureFlagReader`, `FeatureFlagRollout`, and `FeatureFlagAuditor`
- **Affected functions:** `FeatureFlagWriter.upsert/2`, `FeatureFlagReader.enabled_for?/3`, `FeatureFlagRollout.set_percentage/3`, `FeatureFlagAuditor.change_log/1`
- **Short explanation:** The shared feature-flag Agent is directly accessed by four modules, each reading or writing the flags map without routing through a single owner, spreading internal flag data format knowledge across the system.
...
# VALIDATION: This is a smell because FeatureFlagWriter directly calls Agent.update to
# insert or update flag definitions in the shared Agent's flags map, independently
# owning the flag record format without a centralised owner module.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because FeatureFlagReader directly calls Agent.get to
# read a flag and evaluate it for a given user, coupling evaluation logic to the raw
# flag record structure stored inside the Agent.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because FeatureFlagRollout directly calls Agent.get and
# Agent.update to gradually increment the rollout percentage on a flag, independently
# mutating the Agent's internal flag map without a single state-owner module.
...
# VALIDATION: SMELL START - Agent Obsession
# VALIDATION: This is a smell because FeatureFlagAuditor directly calls Agent.get to
# read the change_log list out of the Agent, binding audit-trail functionality to
# the internal Agent data structure.
