...
- **smell**: Agent Obsession
- **expected_smell_location**: `Billing.InvoiceCreator`, `Billing.InvoiceApprover`, `Billing.PaymentCollector`, and `Billing.InvoiceReporter` — all directly calling `Agent.get`, `Agent.update`, or `Agent.get_and_update` on the `Billing.InvoiceStore` Agent.
- **affected_functions**: `Billing.InvoiceCreator.create/3`, `Billing.InvoiceApprover.approve/2`, `Billing.InvoiceApprover.void/2`, `Billing.PaymentCollector.record_payment/3`, `Billing.InvoiceReporter.financial_summary/0`, `Billing.InvoiceReporter.for_customer/1`
- **explanation**: The responsibility for interacting with the shared `InvoiceStore` Agent is spread across four different modules instead of being fully encapsulated within `Billing.InvoiceStore`. This makes it difficult to reason about state transitions, increases the risk of data corruption from mismatched formats, and forces developers to track Agent usage across the entire billing subsystem.
...
defmodule Billing.InvoiceCreator do
  ...
  def create(order_id, customer_id, line_items) when is_list(line_items) do
    ...
    # VALIDATION: This is a smell because Billing.InvoiceCreator directly calls
    # Agent.get_and_update on the InvoiceStore Agent instead of delegating that
    # responsibility to InvoiceStore itself. This spreads Agent interaction across
    # multiple modules, breaking encapsulation and making state transitions harder
...
defmodule Billing.InvoiceApprover do
  ...
  def approve(invoice_id, approved_by) do
    # VALIDATION: [Agent Obsession] Billing.InvoiceApprover also directly calls
    # Agent.get_and_update on the InvoiceStore, adding another module that bypasses
    # the store's encapsulation boundary.
    result =
      Agent.get_and_update(InvoiceStore, fn state ->
...
defmodule Billing.PaymentCollector do
  ...
  def record_payment(invoice_id, payment_ref, paid_amount) when is_float(paid_amount) do
    # VALIDATION: [Agent Obsession] Billing.PaymentCollector directly calls
    # Agent.get_and_update on the InvoiceStore Agent, further spreading
    # the Agent interaction responsibility across the billing subsystem.
    Agent.get_and_update(InvoiceStore, fn state ->
      case Map.fetch(state.invoices, invoice_id) do
...
defmodule Billing.InvoiceReporter do
  ...
  def financial_summary do
    # VALIDATION: [Agent Obsession] Billing.InvoiceReporter also reads directly
    # from the Agent via Agent.get, adding read-side responsibility to a fourth
    # module that should not need to know about the Agent at all.
