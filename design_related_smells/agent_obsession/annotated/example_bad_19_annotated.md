...
| **Expected smell location** | Multiple modules: `BillingCart`, `BillingTax`, `BillingDiscount`, `BillingCheckout` |
| **Affected functions** | `BillingCart.add_item/2`, `BillingTax.apply/2`, `BillingDiscount.apply_coupon/3`, `BillingCheckout.finalize/2` |
| **Short explanation** | Direct `Agent` calls are spread across four unrelated billing modules. Each module reads and writes the shared cart state through raw `Agent.get/2` and `Agent.update/2` calls, bypassing any centralized ownership or encapsulation of the agent state. |
...
defmodule BillingCart do
  ...
  # VALIDATION: This is a smell because BillingCart directly calls Agent.update/2 and Agent.get/2,
  # spreading the responsibility for interacting with the agent state into this module
  # instead of centralizing all agent access in a single dedicated module.
...
defmodule BillingTax do
  ...
  # VALIDATION: SMELL START - Agent Obsession
  # VALIDATION: This is a smell because BillingTax also directly calls Agent.update/2,
  # adding another module that takes ownership of the shared agent state without going through
  # a centralized interface, increasing maintenance difficulty and risk of inconsistent state.
...
defmodule BillingDiscount do
  ...
  # VALIDATION: SMELL START - Agent Obsession
  # VALIDATION: This is a smell because BillingDiscount directly manipulates the agent state
  # via Agent.update/2, further spreading agent interaction responsibility across the system.
  # Any change to the state structure requires updates in all four modules.
...
defmodule BillingCheckout do
  ...
  # VALIDATION: SMELL START - Agent Obsession
  # VALIDATION: This is a smell because BillingCheckout is a fourth module that directly
  # reads the agent state via Agent.get/2. The agent's internal structure is now implicitly
  # depended on by all four modules, making refactoring fragile and bug-prone.
