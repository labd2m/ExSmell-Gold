...
- **Expected smell location:** Modules `InventoryWriter`, `InventoryReader`, `InventoryAlerts`, and `InventorySync`
- **Affected functions:** `InventoryWriter.restock/3`, `InventoryReader.available_quantity/2`, `InventoryAlerts.check_low_stock/2`, `InventorySync.sync_warehouse/2`
- **Short explanation:** Agent state representing warehouse inventory is accessed and mutated directly from four different modules. The lack of a single ownership boundary makes the data contract implicit and difficult to evolve.
...
defmodule Inventory.InventoryWriter do
  ...
  def restock(pid, sku, quantity) do
    ...
    # VALIDATION: This is a smell because InventoryWriter directly calls Agent.update/2
    # to manage inventory state. This responsibility should be centralized in one module.
...
defmodule Inventory.InventoryReader do
  ...
  def available_quantity(pid, sku) do
    # VALIDATION: SMELL START - Agent Obsession
    # VALIDATION: This is a smell because InventoryReader directly calls Agent.get/2,
    # knowing the internal shape of the stock map independently of InventoryWriter.
...
defmodule Inventory.InventoryAlerts do
  ...
  def check_low_stock(pid, threshold) do
    # VALIDATION: SMELL START - Agent Obsession
    # VALIDATION: This is a smell because InventoryAlerts directly reads Agent state
    # without going through any shared interface, coupling it to the internal state structure.
    low_stock_items =
      Agent.get(pid, fn state ->
...
defmodule Inventory.InventorySync do
  ...
  def sync_warehouse(pid, warehouse_data) do
    # VALIDATION: SMELL START - Agent Obsession
    # VALIDATION: This is a smell because InventorySync directly replaces Agent state,
    # adding yet another module that owns agent interaction logic independently.
    Agent.update(pid, fn state ->
      merged_stock =
