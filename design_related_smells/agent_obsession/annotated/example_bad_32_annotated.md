...
- **Expected smell location:** Modules `RouteTracker`, `RouteOptimizer`, `RouteReporter`, and `RouteAlerter`
- **Affected functions:** `RouteTracker.update_position/3`, `RouteOptimizer.recalculate/2`, `RouteReporter.progress_summary/1`, `RouteAlerter.check_delays/2`
- **Short explanation:** Logistics route state shared via an Agent is accessed and mutated by four independent modules. Each module directly encodes knowledge of the agent's internal state map, creating fragile coupling and making state evolution risky.
...
defmodule Logistics.RouteTracker do
  ...
  def update_position(pid, coords, timestamp) do
    ...
    # VALIDATION: This is a smell because RouteTracker directly calls Agent.update/2
    # to mutate route state. This creates a direct coupling to the agent that should
    # be owned by a single dedicated module.
...
defmodule Logistics.RouteOptimizer do
  ...
  def recalculate(pid, new_waypoints) do
    # VALIDATION: SMELL START - Agent Obsession
    # VALIDATION: This is a smell because RouteOptimizer directly calls Agent.update/2
    # to replace the waypoint list, independently mutating the same state structure
    # as RouteTracker without going through any shared interface.
    Agent.update(pid, fn state ->
...
defmodule Logistics.RouteReporter do
  ...
  def progress_summary(pid) do
    # VALIDATION: SMELL START - Agent Obsession
    # VALIDATION: This is a smell because RouteReporter directly reads Agent state
    # without routing through a centralized interface, coupling it to the agent's
...
defmodule Logistics.RouteAlerter do
  ...
  def check_delays(pid, expected_arrival) do
    # VALIDATION: SMELL START - Agent Obsession
    # VALIDATION: This is a smell because RouteAlerter directly reads Agent state
    # to determine delay conditions, becoming a fourth module tightly coupled to
    # the shared mutable state of the agent.
