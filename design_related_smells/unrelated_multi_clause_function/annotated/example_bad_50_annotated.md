...
- **Expected smell location:** `InventoryCoordinator.execute/1`
- **Affected function(s):** `execute/1`
- **Short explanation:** The `execute/1` function conflates three unrelated inventory operations — transferring stock between warehouses, writing off damaged goods, and creating a new product listing — under a single multi-clause function. Each clause interacts with different entities, applies different validation rules, triggers different downstream events, and targets different business stakeholders. There is no shared logic between them; only a superficial common verb ties them together, making the function an obstacle to readability, testing, and documentation.
...
defmodule MyApp.InventoryCoordinator do
  ...
  # unrelated inventory workflows: moving stock between locations, recording
  # physical damage losses, and publishing a new product to the catalogue.
  # Each has distinct validation rules, different side effects, different
  # notification targets, and different failure paths. They share only a
  # function name, not any actual logic or domain concept.
