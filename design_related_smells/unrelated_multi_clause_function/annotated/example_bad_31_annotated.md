...
- **Expected smell location:** `LoanProcessor.evaluate/1`
- **Affected function(s):** `evaluate/1`
- **Short explanation:** `evaluate/1` handles personal loan applications, credit limit adjustment requests, and loan early repayment processing — three unrelated financial lending operations — under one multi-clause function. Each has distinct underwriting rules, risk models, and regulatory reporting obligations.
...
defmodule LoanProcessor do
  ...
  # VALIDATION: This is a smell because underwriting a new loan, adjusting
  # a customer's credit limit, and processing an early repayment are entirely
  # different lending operations with different risk models, regulatory
  # reporting codes, and customer-facing outcomes. Grouping them under
  # `evaluate/1` conflates unrelated financial workflows.
