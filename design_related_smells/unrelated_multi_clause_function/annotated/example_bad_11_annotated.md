...
- **Short explanation:** `apply/1` handles stock replenishment events, warehouse transfers, and product write-offs — three independent inventory operations — fused into a single multi-clause function. Each clause represents an entirely different business workflow with no shared logic.
...
defmodule InventoryManager do
  ...
  # VALIDATION: This is a smell because replenishment, transfer, and write-off
  # are unrelated inventory workflows bundled into one `apply/1` function.
  # They touch different records, involve different validation rules, and
  # emit different audit entries. Each should be its own named function.
