...
- **Expected smell location:** `InsurancePolicy.apply/1`
- **Affected function(s):** `apply/1`
- **Short explanation:** `apply/1` handles new policy issuance, claim filing, and policy renewal — three distinct insurance lifecycle operations — merged into a single multi-clause function. Each clause involves different actuarial logic, documentation requirements, and regulatory steps.
...
defmodule InsurancePolicy do
  ...
  # VALIDATION: This is a smell because issuing a new insurance policy,
  # filing a claim against an existing policy, and processing a policy renewal
  # are entirely different insurance lifecycle operations. Each involves
  # different data (underwriting vs incident details vs premium recalculation),
  # different regulatory submissions, and different customer communications.
