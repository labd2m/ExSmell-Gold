...
- **Expected smell location:** `DataSyncWorker.sync/1`
- **Affected function(s):** `sync/1`
- **Short explanation:** `sync/1` handles CRM contact synchronization, ERP inventory sync, and payment gateway reconciliation — three entirely different integration sync workflows — under one multi-clause function. Each clause targets a different third-party system with its own API, error handling, and conflict-resolution strategy.
...
defmodule DataSyncWorker do
  ...
  # VALIDATION: This is a smell because synchronising CRM contacts, syncing
  # ERP inventory levels, and reconciling payment transactions are completely
  # different integration workflows with separate APIs, conflict strategies,
  # and failure semantics. Bundling them under one `sync/1` multi-clause
  # function obscures each workflow's intent and prevents independent docs.
