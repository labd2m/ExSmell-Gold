...
- **Short explanation:** The `dispatch/1` function uses pattern matching to handle dispatching for completely unrelated entity types — a password reset request, a shipment alert, and a promotional campaign. These are separate notification concerns that mix domain logic, making each clause impossible to document, test, or evolve independently.
...
defmodule NotificationDispatcher do
  ...
  # VALIDATION: This is a smell because the clauses handle three completely different
  # notification domains (account security, logistics, marketing) in a single function.
  # Each clause has its own channel logic, retry policy, and side effects, and they
  # share nothing in common except the function name.
