...
- **Expected smell location:** `TaxEngine.compute/1`
- **Affected function(s):** `compute/1`
- **Short explanation:** `compute/1` handles VAT calculation for e-commerce orders, payroll tax withholding calculation, and capital gains tax estimation — three entirely unrelated tax domains — under one multi-clause function. Each operates under completely different tax codes, rates, and jurisdictional rules.
...
defmodule TaxEngine do
  ...
  # VALIDATION: This is a smell because VAT/sales tax on goods, income tax
  # withholding on wages, and capital gains tax on investment disposals are
  # governed by completely separate tax codes, with different rates, bracket
  # tables, exemptions, and regulatory filing obligations. Grouping them under
  # `compute/1` conflates entirely unrelated tax domains in one function.
