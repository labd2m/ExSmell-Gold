...
- **Expected smell location:** `PlatformMigration.run/1`
- **Affected function(s):** `run/1`
- **Short explanation:** `run/1` handles database schema migrations, feature flag rollouts, and legacy data backfills — three entirely different platform engineering operations — merged under one multi-clause function. Each has distinct rollback strategies, safety checks, and infrastructure side effects.
...
defmodule PlatformMigration do
  ...
  # VALIDATION: This is a smell because running a database DDL migration,
  # rolling out a feature flag to a user segment, and backfilling legacy data
  # records are entirely different platform engineering operations. Each has
  # different safety requirements (schema lock vs canary rollout vs batch
  # throughput limits), different rollback mechanisms, and different monitoring
  # needs. Grouping all three under `run/1` conflates unrelated concerns.
