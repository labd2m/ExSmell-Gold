...
| **Expected location**  | `NotificationDispatcher.dispatch/1`                                                                                                                               |
| **Affected function(s)**| `dispatch/1`                                                                                                                                                     |
| **Short explanation**  | The `dispatch/1` function uses pattern-matched multi-clause definitions to handle three completely unrelated business entities — `InvoiceAlert`, `ShipmentUpdate`, and `PasswordResetRequest` — each carrying distinct logic, side-effects, and domain rules. Because these concerns share nothing beyond the function name, they should be separate, individually-documented functions. The single `@doc` annotation on the first clause cannot describe the behaviour of the other two, forcing the author to fall back to plain comments, which are untestable and easy to ignore. |
...
defmodule MyApp.NotificationDispatcher do
  ...
  # VALIDATION: This is a smell because `dispatch/1` groups three completely
  # VALIDATION: unrelated business concerns — invoice billing alerts, logistics
  # VALIDATION: shipment tracking, and security-sensitive password resets — under
  # VALIDATION: a single function name purely through pattern matching. Each clause
  # VALIDATION: has distinct domain rules, channels, side-effects, and urgency
  # VALIDATION: levels. The single @doc can only describe the first clause; the
  # VALIDATION: remaining two are documented with plain comments, making their
  # VALIDATION: behaviour invisible to ExDoc and untestable via doctests.
  # ---------------------------------------------------------------------------
