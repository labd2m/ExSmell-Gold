...
- **Short explanation:** The `process/1` function groups entirely unrelated business operations (processing an invoice, handling a refund, and activating a subscription plan) under the same function name using pattern matching on unrelated structs. Each clause belongs to a different domain concern and should be an independent, well-named function.
...
defmodule BillingProcessor do
  ...
  # VALIDATION: This is a smell because the three clauses handle completely unrelated
  # business concerns (invoice payment, refund issuance, subscription activation)
  # under the same function name, making it impossible to document each independently
  # and forcing readers to understand all three domains to reason about any one of them.
