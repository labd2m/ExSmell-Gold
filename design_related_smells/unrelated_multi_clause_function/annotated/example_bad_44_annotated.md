...
- **Expected smell location:** `AuthHandler.execute/1`
- **Affected function(s):** `execute/1`
- **Short explanation:** The `execute/1` function conflates three distinct authentication operations — user login, password reset initiation, and API token revocation — into one multi-clause function. These operations have different inputs, different side effects, different failure modes, and serve entirely different user journeys. Grouping them under a single name obscures each operation's intent and makes documentation, testing, and maintenance significantly harder.
...
defmodule MyApp.AuthHandler do
  ...
  # VALIDATION: This is a smell because the three clauses perform completely
  # unrelated operations (credential verification, password reset email dispatch,
  # and API token revocation). They share no logic, serve different user journeys,
  # and have unrelated failure modes. Only the function name ties them together.
