...
- **Expected smell location:** `WarehouseRouter.route/1`
- **Affected function(s):** `route/1`
- **Short explanation:** `route/1` handles pick-list assignment for warehouse pickers, inbound receiving dock assignment, and outbound carrier lane allocation — three unrelated warehouse operations — under one multi-clause function. Each clause applies different optimization criteria and communicates with different warehouse subsystems.
...
defmodule WarehouseRouter do
  ...
  # VALIDATION: This is a smell because assigning a pick list to a warehouse
  # picker, allocating a receiving dock for an inbound shipment, and routing
  # an outbound shipment to a carrier lane are completely different warehouse
  # operations. Each optimises for different constraints (picker proximity,
  # dock capacity, carrier cutoff time) with distinct downstream systems.
