...
- **Expected smell location:** `MarketplaceAction.perform/1`
- **Affected function(s):** `perform/1`
- **Short explanation:** `perform/1` handles seller product listing creation, buyer offer submission, and dispute filing — three unrelated marketplace operations with different actors and workflows — merged under one multi-clause function with no shared abstraction.
...
defmodule MarketplaceAction do
  ...
  # VALIDATION: This is a smell because creating a product listing, submitting
  # a buyer offer, and filing a transaction dispute are entirely different
  # marketplace operations involving different actors (seller vs buyer vs
  # arbitrator), different validation rules (content moderation vs pricing
  # vs evidence review), and different downstream workflows. Merging them
  # under `perform/1` conflates completely unrelated commerce operations.
