...
- **Expected smell location:** `SupportTicketHandler.handle/1`
- **Affected function(s):** `handle/1`
- **Short explanation:** `handle/1` processes new ticket creation, ticket escalation, and ticket closure — three independent customer support workflows — under a single multi-clause function. Each clause has its own SLA rules, routing logic, and notification targets with no shared abstraction.
...
defmodule SupportTicketHandler do
  ...
  # VALIDATION: This is a smell because creating a new ticket, escalating an
  # existing ticket, and closing a ticket are completely different support
  # operations. Each involves different routing logic, SLA rules, and
  # stakeholders. There is no shared logic justifying a single `handle/1`
