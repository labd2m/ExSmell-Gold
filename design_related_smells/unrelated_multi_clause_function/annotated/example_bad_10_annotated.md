...
- **Short explanation:** `dispatch/1` handles push notifications, email alerts, and SMS verifications — three unrelated notification concerns — mixed into one multi-clause function, preventing proper documentation and obscuring each clause's purpose.
...
defmodule NotificationDispatcher do
  ...
  # VALIDATION: This is a smell because three completely different notification
  # domains (mobile push, email, and SMS OTP) are bundled under a single
  # `dispatch/1` name. Each clause has its own provider, retry logic, and
  # side effects, but they share no meaningful abstraction.
