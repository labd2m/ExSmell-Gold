...
- **Expected smell location:** `PaymentOrchestrator.run/1`
- **Affected function(s):** `run/1`
- **Short explanation:** The `run/1` function handles three unrelated payment operations: initiating a new charge, processing a payout to a vendor, and reconciling a disputed chargeback. These involve different entities, different gateway operations, different regulatory considerations, and entirely different success/failure semantics. Grouping them under a single `run/1` multi-clause function creates confusion and prevents meaningful per-function documentation.
...
defmodule MyApp.PaymentOrchestrator do
  ...
  # VALIDATION: This is a smell because the three clauses implement distinct and
  # unrelated payment workflows: charging a customer, disbursing funds to a vendor,
  # and handling a disputed chargeback. They use different Stripe API calls,
  # different ledger entries, and have no shared business logic. Only the verb
  # "run" links them, which obscures intent and prevents granular documentation.
