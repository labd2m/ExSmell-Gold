...
- **Expected smell location:** `EventBus.publish/1`
- **Affected function(s):** `publish/1`
- **Short explanation:** `publish/1` publishes domain events to Kafka, broadcasts WebSocket notifications to clients, and writes webhook deliveries to queues — three unrelated event delivery mechanisms — grouped under one multi-clause function with no shared logic and different reliability guarantees.
...
defmodule EventBus do
  ...
  # VALIDATION: This is a smell because publishing to Kafka (durable, ordered
  # message stream), broadcasting over WebSockets (ephemeral, real-time push),
  # and queuing webhook deliveries (HTTP callbacks with retry logic) are
  # entirely different event delivery mechanisms with different durability,
  # ordering, and failure-recovery semantics. Collapsing all three into a
  # single `publish/1` multi-clause function conflates unrelated concerns.
