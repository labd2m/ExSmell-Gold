...
- **Short explanation:** The `process/1` function uses multi-clause pattern matching to handle completely unrelated entities — invoices, refund requests, and subscription renewals — mixing distinct business logic under a single function name, making intent unclear and preventing per-clause documentation.
...
defmodule BillingProcessor do
  ...
  # VALIDATION: This is a smell because `process/1` groups three completely
  # unrelated business operations (invoice payment, refund issuance, and
  # subscription renewal) under one function name using pattern matching,
  # making each clause's intent opaque and preventing independent @doc annotations.
