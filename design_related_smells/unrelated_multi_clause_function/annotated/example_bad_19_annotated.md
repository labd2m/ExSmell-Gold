...
- **Expected smell location:** `TenantProvisioner.provision/1`
- **Affected function(s):** `provision/1`
- **Short explanation:** `provision/1` handles new tenant onboarding, tenant plan upgrades, and tenant offboarding — three unrelated SaaS lifecycle operations — collapsed into one multi-clause function, preventing independent documentation and mixing distinct infrastructure concerns.
...
defmodule TenantProvisioner do
  ...
  # VALIDATION: This is a smell because tenant onboarding (provisioning
  # databases, DNS, and feature flags), plan upgrading (billing and feature
  # changes), and offboarding (data export and teardown) are entirely separate
  # SaaS operations with different infrastructure impacts. Using one `provision/1`
  # clause set for all three is an unrelated multi-clause function abuse.
