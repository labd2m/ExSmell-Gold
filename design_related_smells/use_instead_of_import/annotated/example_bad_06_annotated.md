# example_bad_06_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Inventory.StockManager` module, `use Inventory.ValidationHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Inventory.StockManager` module)
- **Short Explanation:** `Inventory.StockManager` uses `use Inventory.ValidationHelpers` solely to get access to stock validation functions. The `__using__/1` macro also silently injects `import Inventory.UnitUtils` into the caller, propagating unit-conversion helpers into `StockManager`'s namespace without any explicit declaration. Since only validation functions are needed, `import Inventory.ValidationHelpers` would be the transparent alternative.
...
defmodule Inventory.StockManager do
  ...
  # VALIDATION: This is a smell because `use Inventory.ValidationHelpers` triggers
  # VALIDATION: `__using__/1`, which injects `import Inventory.UnitUtils` into
  # VALIDATION: `StockManager`. This silently brings unit-conversion functions
  # VALIDATION: (e.g., `lbs_to_kg/1`, `inches_to_cm/1`) into scope even though
  # VALIDATION: `StockManager` never uses them. An `import Inventory.ValidationHelpers`
  # VALIDATION: would make the dependency explicit and avoid polluting the
  # VALIDATION: module's function namespace.
