# Annotated Bad Example 27

**Smell:** "Use" instead of "import"
**Expected Smell Location:** `Shipping.RateEstimator`, `use Shipping.WeightHelpers` directive
**Affected Functions:** `estimate/2`, `cheapest_option/2`, `dimensional_weight/1`, `surcharge_total/2`
**Explanation:** `Shipping.RateEstimator` uses `use Shipping.WeightHelpers` to access weight-conversion and dimensional-weight calculation utilities. The `__using__/1` macro silently injects an alias for `Shipping.CarrierPricing` and sets `@dim_divisor` and `@weight_unit` module attributes. The caller did not explicitly declare a dependency on `CarrierPricing`, `@dim_divisor`, or `@weight_unit`; these are propagated invisibly by the macro. A plain `import Shipping.WeightHelpers` would expose only the weight utility functions, keeping dependencies explicit and readable.
...
defmodule Shipping.WeightHelpers do
  ...
  # VALIDATION: This is a smell because __using__/1 secretly injects alias
  # Shipping.CarrierPricing and two module attributes (@dim_divisor and @weight_unit)
  # into the calling module. The caller never explicitly declared these dependencies,
  # making the module's dependency surface opaque without inspecting this macro.
...
defmodule Shipping.RateEstimator do
  # VALIDATION: SMELL START - "Use" instead of "import"
  # VALIDATION: This is a smell because `use Shipping.WeightHelpers` silently injects
  # alias Shipping.CarrierPricing, @dim_divisor, and @weight_unit via __using__/1.
  # A reader of RateEstimator sees no explicit declaration of these dependencies.
  # `import Shipping.WeightHelpers` would expose only the weight utility functions,
  # and aliases and module attributes could be declared explicitly if needed.
