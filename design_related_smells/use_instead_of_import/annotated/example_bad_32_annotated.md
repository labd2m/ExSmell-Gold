...
- **Expected smell location:** `TaxCalculator` module, top-level directive
- **Affected function(s):** `compute/2`, `breakdown/2`, `effective_rate/2`
- **Short explanation:** `TaxCalculator` calls `use RoundingHelpers` to access rounding and precision utilities. The `__using__/1` macro of `RoundingHelpers` silently injects an `import` of `PrecisionMath` into `TaxCalculator`, making `ceil_at/2`, `floor_at/2`, and `banker_round/2` available without any visible declaration. Replacing `use RoundingHelpers` with `import RoundingHelpers` would keep every imported function's origin traceable from `TaxCalculator` alone.
...
defmodule RoundingHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 injects `import PrecisionMath`
      # VALIDATION: into TaxCalculator. ceil_at/2, floor_at/2, banker_round/2, and
      # VALIDATION: truncate_at/2 become available in TaxCalculator without any visible
      # VALIDATION: import statement there. A maintainer reading TaxCalculator cannot
      # VALIDATION: identify the source of these precision helpers without inspecting
      # VALIDATION: RoundingHelpers. Using `import RoundingHelpers` at the call site
      # VALIDATION: would be transparent and sufficient.
