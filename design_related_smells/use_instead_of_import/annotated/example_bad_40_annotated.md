...
- **Expected smell location:** `InvoiceProcessor` module, top-level directive
- **Affected function(s):** All functions in `InvoiceProcessor` that call helpers from `InvoiceHelpers`
- **Short explanation:** `InvoiceProcessor` uses `use InvoiceHelpers` to gain access to formatting and calculation functions. However, the `__using__/1` macro in `InvoiceHelpers` injects code (including an `import` of `NumberFormatter`) that propagates hidden dependencies into `InvoiceProcessor`. A plain `import InvoiceHelpers` would make the dependency explicit and prevent silent code injection.
...
defmodule InvoiceHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 propagates a hidden dependency
      # VALIDATION: on NumberFormatter into every module that calls `use InvoiceHelpers`.
      # VALIDATION: The caller (InvoiceProcessor) has no visibility into this injection
      # VALIDATION: without reading InvoiceHelpers internals. A plain `import InvoiceHelpers`
      # VALIDATION: would be sufficient and transparent.
