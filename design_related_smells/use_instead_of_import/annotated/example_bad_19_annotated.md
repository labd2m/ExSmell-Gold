...
**Expected Smell Location:** `Reporting.RevenueReport`, `use Reporting.NumberHelpers` directive
**Affected Functions:** `generate/2`, `summarise_by_period/2`, `format_row/1`, `totals/1`
**Explanation:** `Reporting.RevenueReport` uses `use Reporting.NumberHelpers` to gain access to numeric formatting and rounding functions. The `__using__/1` macro in `NumberHelpers` also silently injects an alias for `Reporting.DataSource` and sets `@report_timezone` and `@decimal_places` module attributes. A reader of `RevenueReport` would not know where `DataSource`, `@report_timezone`, or `@decimal_places` come from without tracing through the library macro. `import Reporting.NumberHelpers` would have been sufficient and transparent.
...
defmodule Reporting.NumberHelpers do
  ...
  # VALIDATION: This is a smell because __using__/1 injects an alias to
  # Reporting.DataSource and two module attributes into any caller. The caller
  # never explicitly chose to depend on DataSource, @report_timezone, or
  # @decimal_places; these are hidden side-effects of the `use` directive.
...
defmodule Reporting.RevenueReport do
  # VALIDATION: SMELL START - "Use" instead of "import"
  # VALIDATION: This is a smell because `use Reporting.NumberHelpers` expands
  # __using__/1 and silently brings alias Reporting.DataSource, @report_timezone,
  # and @decimal_places into scope. A reader of RevenueReport cannot determine
  # the origin of DataSource or the module attributes without inspecting the macro.
  # `import Reporting.NumberHelpers` alone would suffice.
