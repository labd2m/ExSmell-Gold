# example_bad_05_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Reporting.SalesReport` module, `use Reporting.ChartHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Reporting.SalesReport` module)
- **Short Explanation:** `Reporting.SalesReport` uses `use Reporting.ChartHelpers` to access chart and table rendering functions. The `__using__/1` macro additionally injects `import Reporting.MathUtils` into the caller, silently making statistical helpers available in `SalesReport`'s namespace. Since the module only needs the chart rendering functions, `import Reporting.ChartHelpers` would be sufficient and would avoid the hidden dependency on `MathUtils`.
...
defmodule Reporting.SalesReport do
  ...
  # VALIDATION: This is a smell because `use Reporting.ChartHelpers` triggers
  # VALIDATION: `__using__/1`, which injects `import Reporting.MathUtils` into
  # VALIDATION: `SalesReport`. This silently makes `mean/1`, `median/1`,
  # VALIDATION: `percentage/2`, and `growth_rate/2` available in this module
  # VALIDATION: without any explicit declaration. The module only needs
  # VALIDATION: `bar_chart/2` and `data_table/2`; using
  # VALIDATION: `import Reporting.ChartHelpers` would make that clear.
