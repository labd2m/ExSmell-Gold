...
- **Expected smell location:** `AuthService` module, top-level directive
- **Affected function(s):** `hash_password/1`, `verify_password/2`, `generate_token/1`
- **Short explanation:** `AuthService` uses `use CryptoHelpers` to pull in password-hashing and token utilities. The `__using__/1` macro silently injects an `import` of `SecureRandom` into `AuthService`, propagating a hidden dependency. Using `import CryptoHelpers` instead would make all imported names explicit, preventing confusion about where functions like `random_bytes/1` originate.
...
defmodule CryptoHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 silently imports SecureRandom
      # VALIDATION: into AuthService. A developer reading AuthService will not know that
      # VALIDATION: random_bytes/1, hex/1, and url_safe_base64/1 come from SecureRandom
      # VALIDATION: without inspecting CryptoHelpers internals. Using
      # VALIDATION: `import CryptoHelpers` at the call site would make the dependency graph
      # VALIDATION: transparent and avoid unintended name conflicts.
      import SecureRandom
