...
- **Expected smell location:** `UserRegistrationService` module, top-level directive
- **Affected function(s):** `register/1`, `update_profile/2`, `validate_email/1`
- **Short explanation:** `UserRegistrationService` calls `use ValidationHelpers` to access input-validation utilities. The `__using__/1` macro of `ValidationHelpers` silently injects an `import` of `RegexPatterns` into the caller, propagating a hidden dependency on compiled regex constants and helper predicates. Replacing `use ValidationHelpers` with `import ValidationHelpers` would make every imported name traceable to its source without diving into `ValidationHelpers` internals.
...
defmodule ValidationHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 injects `import RegexPatterns`
      # VALIDATION: into UserRegistrationService. valid_email?/1, valid_phone?/1,
      # VALIDATION: valid_username?/1, and valid_zip?/1 appear available inside
      # VALIDATION: UserRegistrationService with no visible import for RegexPatterns.
      # VALIDATION: A developer must inspect ValidationHelpers to understand this.
      # VALIDATION: `import ValidationHelpers` at the call site would be transparent.
