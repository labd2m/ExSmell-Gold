# example_bad_07_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Payments.TransactionHandler` module, `use Payments.CurrencyHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Payments.TransactionHandler` module)
- **Short Explanation:** `Payments.TransactionHandler` uses `use Payments.CurrencyHelpers` to access currency formatting and conversion functions. The `__using__/1` macro also silently injects `import Payments.RoundingUtils` into the caller, propagating low-level rounding helpers into `TransactionHandler` without any explicit import. Since the client only needs the currency helper functions, `import Payments.CurrencyHelpers` would be the appropriate and transparent alternative.
...
defmodule Payments.TransactionHandler do
  ...
  # VALIDATION: This is a smell because `use Payments.CurrencyHelpers` triggers
  # VALIDATION: `__using__/1`, which silently injects `import Payments.RoundingUtils`
  # VALIDATION: into `TransactionHandler`. Low-level rounding functions such as
  # VALIDATION: `round_half_even/2` and `ceil_to_cent/1` become available in this
  # VALIDATION: module without any visible declaration. Since `TransactionHandler`
  # VALIDATION: only needs the currency helper functions, `import Payments.CurrencyHelpers`
  # VALIDATION: would make dependencies explicit and avoid the hidden coupling to
  # VALIDATION: `RoundingUtils`.
