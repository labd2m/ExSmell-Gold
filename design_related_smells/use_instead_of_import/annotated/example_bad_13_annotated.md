# example_bad_13_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `HR.PayrollCalculator` module, `use HR.ComplianceHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `HR.PayrollCalculator` module)
- **Short Explanation:** `HR.PayrollCalculator` uses `use HR.ComplianceHelpers` to access labor-law compliance validation helpers. The `__using__/1` macro additionally injects `import HR.TaxUtils` into the caller, propagating tax-calculation functions into `PayrollCalculator` without any explicit declaration. The calculator needs both sets of helpers in this example, but the coupling to `TaxUtils` is hidden and opaque: it arrives via the `use` of `ComplianceHelpers`, not through a visible `import`. Using `import HR.ComplianceHelpers` and `import HR.TaxUtils` separately would make all dependencies transparent.
...
defmodule HR.PayrollCalculator do
  ...
  # VALIDATION: This is a smell because `use HR.ComplianceHelpers` triggers
  # VALIDATION: `__using__/1`, which injects `import HR.TaxUtils` into
  # VALIDATION: `PayrollCalculator`. Tax functions like `federal_withholding/1`,
  # VALIDATION: `fica_employee/1`, and `state_withholding/2` become available
  # VALIDATION: in this module through a hidden chain: the reader must open
  # VALIDATION: `HR.ComplianceHelpers.__using__/1` to discover that `HR.TaxUtils`
  # VALIDATION: is imported. Declaring both `import HR.ComplianceHelpers` and
  # VALIDATION: `import HR.TaxUtils` explicitly would eliminate the opacity.
