# example_bad_12_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Healthcare.AppointmentManager` module, `use Healthcare.CalendarHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Healthcare.AppointmentManager` module)
- **Short Explanation:** `Healthcare.AppointmentManager` uses `use Healthcare.CalendarHelpers` to access appointment-slotting and availability helpers. The `__using__/1` macro also silently injects `import Healthcare.TimeZoneUtils` into the caller, propagating timezone-conversion functions into `AppointmentManager` without any explicit declaration. Since only the calendar helper functions are needed, `import Healthcare.CalendarHelpers` would be the explicit and correct alternative.
...
defmodule Healthcare.AppointmentManager do
  ...
  # VALIDATION: This is a smell because `use Healthcare.CalendarHelpers` triggers
  # VALIDATION: `__using__/1`, which injects `import Healthcare.TimeZoneUtils`
  # VALIDATION: into `AppointmentManager`. Functions like `to_utc/2`, `from_utc/2`,
  # VALIDATION: and `format_with_tz/2` land in this module's namespace without
  # VALIDATION: any explicit declaration. The manager only needs the slot helpers;
  # VALIDATION: `import Healthcare.CalendarHelpers` would make that intent clear.
