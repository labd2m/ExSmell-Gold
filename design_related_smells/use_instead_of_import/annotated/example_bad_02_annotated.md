# example_bad_02_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Logistics.ShipmentTracker` module, `use Logistics.AddressHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Logistics.ShipmentTracker` module)
- **Short Explanation:** `Logistics.ShipmentTracker` uses `use Logistics.AddressHelpers` solely to access address-formatting and validation functions. The `__using__/1` macro, however, also silently injects `import Logistics.GeoUtils` into the caller, creating a hidden transitive dependency. An `import Logistics.AddressHelpers` would provide the needed functions without the opaque side effects.
...
defmodule Logistics.ShipmentTracker do
  ...
  # VALIDATION: This is a smell because `use Logistics.AddressHelpers` executes
  # VALIDATION: `__using__/1`, which secretly injects `import Logistics.GeoUtils`
  # VALIDATION: into this module. The tracker only needs the address helper
  # VALIDATION: functions; it never calls geo functions directly. Using
  # VALIDATION: `import Logistics.AddressHelpers` would make dependencies
  # VALIDATION: explicit and prevent accidental coupling to GeoUtils.
