...
- **Expected smell location:** `AppointmentScheduler` module, top-level directive
- **Affected function(s):** `schedule/2`, `available_slots/2`, `reschedule/3`
- **Short explanation:** `AppointmentScheduler` calls `use SchedulingHelpers` to obtain slot-calculation and conflict-detection utilities. The `__using__/1` macro of `SchedulingHelpers` silently injects an `import` of `CalendarUtils` into the caller, propagating a hidden dependency on functions like `next_weekday/1` and `overlap?/4`. Replacing `use SchedulingHelpers` with `import SchedulingHelpers` would make every dependency explicit and allow readers to understand the module without inspecting `SchedulingHelpers` internals.
...
defmodule SchedulingHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 injects `import CalendarUtils`
      # VALIDATION: into AppointmentScheduler. next_weekday/1, overlap?/4, slot_end/2,
      # VALIDATION: and same_day?/2 appear in AppointmentScheduler as if they were local,
      # VALIDATION: but they actually come from CalendarUtils. Reading
      # VALIDATION: AppointmentScheduler alone is insufficient to understand its full
      # VALIDATION: dependency surface. Using `import SchedulingHelpers` instead would
      # VALIDATION: keep injected names visible and scoped only to SchedulingHelpers.
