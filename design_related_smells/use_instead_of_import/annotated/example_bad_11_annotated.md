# example_bad_11_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Ecommerce.OrderProcessor` module, `use Ecommerce.PricingHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Ecommerce.OrderProcessor` module)
- **Short Explanation:** `Ecommerce.OrderProcessor` uses `use Ecommerce.PricingHelpers` to access price-calculation and discount functions. The `__using__/1` macro also silently injects `import Ecommerce.DiscountUtils` into the caller, propagating discount calculation helpers into `OrderProcessor` without any explicit declaration. Since only the pricing functions are needed, `import Ecommerce.PricingHelpers` would be transparent and sufficient.
...
defmodule Ecommerce.OrderProcessor do
  ...
  # VALIDATION: This is a smell because `use Ecommerce.PricingHelpers` triggers
  # VALIDATION: `__using__/1`, which injects `import Ecommerce.DiscountUtils`
  # VALIDATION: into `OrderProcessor`. Discount primitives like `apply_percentage/2`,
  # VALIDATION: `apply_flat/2`, and `coupon_valid?/1` silently enter this module's
  # VALIDATION: namespace. The processor only needs the pricing helper functions;
  # VALIDATION: `import Ecommerce.PricingHelpers` would keep the dependency surface
  # VALIDATION: explicit and predictable.
