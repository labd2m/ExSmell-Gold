...
- **Expected smell location:** `AuditLogger` module, top-level directive
- **Affected function(s):** `log_event/3`, `serialize_context/1`, `build_entry/4`
- **Short explanation:** `AuditLogger` calls `use EventHelpers` to obtain event-serialization and context-enrichment utilities. The `__using__/1` macro of `EventHelpers` silently injects an `import` of `JsonSerializer` into `AuditLogger`, making `encode!/1`, `decode!/1`, and `sanitize/1` available without any visible declaration. Replacing `use EventHelpers` with `import EventHelpers` would eliminate the hidden dependency and let readers understand the module without consulting `EventHelpers` internals.
...
defmodule EventHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 injects `import JsonSerializer`
      # VALIDATION: into AuditLogger. encode!/1, decode!/1, and sanitize/1 become
      # VALIDATION: available in AuditLogger with no visible import declaration.
      # VALIDATION: A reader cannot know these come from JsonSerializer without
      # VALIDATION: inspecting EventHelpers. Using `import EventHelpers` instead would
      # VALIDATION: make only EventHelpers functions available and keep the dependency
      # VALIDATION: surface explicit and auditable.
