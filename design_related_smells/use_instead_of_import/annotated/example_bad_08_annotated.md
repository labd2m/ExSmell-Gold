# example_bad_08_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Scheduling.JobDispatcher` module, `use Scheduling.CronHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Scheduling.JobDispatcher` module)
- **Short Explanation:** `Scheduling.JobDispatcher` uses `use Scheduling.CronHelpers` to access cron-expression parsing and scheduling utilities. The `__using__/1` macro also silently injects `import Scheduling.TimeUtils` into the caller, making time-arithmetic helpers available in `JobDispatcher` without any explicit declaration. Since only the cron helpers are needed, `import Scheduling.CronHelpers` would make the dependency visible and avoid injecting the transitive `TimeUtils` dependency.
...
defmodule Scheduling.JobDispatcher do
  ...
  # VALIDATION: This is a smell because `use Scheduling.CronHelpers` triggers
  # VALIDATION: `__using__/1`, which injects `import Scheduling.TimeUtils` into
  # VALIDATION: `JobDispatcher`. Functions like `seconds_until/1`,
  # VALIDATION: `format_duration/1`, and `within_window?/3` from `TimeUtils`
  # VALIDATION: are silently brought into scope. The dispatcher only needs the
  # VALIDATION: cron helpers; `import Scheduling.CronHelpers` would be accurate
  # VALIDATION: and would expose no hidden transitive dependencies.
