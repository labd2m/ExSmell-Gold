...
- **Expected smell location:** `WarehouseCoordinator` module, top-level directive
- **Affected function(s):** `allocate_stock/2`, `transfer/3`, `replenishment_plan/1`
- **Short explanation:** `WarehouseCoordinator` calls `use LocationHelpers` to access bin-addressing and distance-calculation utilities. The `__using__/1` macro of `LocationHelpers` silently injects an `import` of `BinAddressing` into `WarehouseCoordinator`, propagating hidden access to functions like `parse_address/1`, `adjacent?/2`, and `sort_by_proximity/2`. A reader of `WarehouseCoordinator` cannot identify the origin of these functions without inspecting `LocationHelpers` internals. Using `import LocationHelpers` at the call site would be explicit and sufficient.
...
defmodule LocationHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 injects `import BinAddressing`
      # VALIDATION: into WarehouseCoordinator. parse_address/1, adjacent?/2,
      # VALIDATION: sort_by_proximity/2, and format_address/1 appear in
      # VALIDATION: WarehouseCoordinator as if they are local functions, but they
      # VALIDATION: actually originate from BinAddressing. This hidden propagation
      # VALIDATION: reduces readability: the module's dependency surface is opaque
      # VALIDATION: without reading LocationHelpers internals. A plain
      # VALIDATION: `import LocationHelpers` would be the appropriate alternative.
