# example_bad_03_annotated.md

## Metadata

- **Smell Name:** "Use" instead of "import"
- **Expected Smell Location:** `Auth.SessionManager` module, `use Auth.TokenHelpers` directive
- **Affected Function(s):** Module-level directive (affects the entire `Auth.SessionManager` module)
- **Short Explanation:** `Auth.SessionManager` uses `use Auth.TokenHelpers` to access token-building and verification helpers. The `__using__/1` macro also injects `import Auth.CryptoUtils` into the caller, which is a hidden dependency `SessionManager` never knowingly opted into. An `import Auth.TokenHelpers` would expose only the intended API surface without propagating the crypto dependency.
...
defmodule Auth.SessionManager do
  ...
  # VALIDATION: This is a smell because `use Auth.TokenHelpers` invokes
  # VALIDATION: `__using__/1`, which injects `import Auth.CryptoUtils` into
  # VALIDATION: `Auth.SessionManager`. The manager only needs `build_token/3`,
  # VALIDATION: `decode_token/1`, and `token_expired?/1`; it never needs raw
  # VALIDATION: crypto primitives. Using `import Auth.TokenHelpers` would make
  # VALIDATION: the dependency explicit and avoid leaking `Auth.CryptoUtils`
  # VALIDATION: functions into this module's namespace.
