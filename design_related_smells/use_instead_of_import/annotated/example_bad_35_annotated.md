...
- **Expected smell location:** `EventStore` module, top-level directive
- **Affected function(s):** `append/3`, `project/2`, `reconstruct/2`
- **Short explanation:** `EventStore` calls `use SerializationHelpers` to access event-encoding and version-migration utilities. The `__using__/1` macro of `SerializationHelpers` silently injects an `import` of `BinaryCodec` into the caller, propagating a hidden dependency on functions like `encode_term/1`, `decode_term/1`, and `checksum/1`. Replacing `use SerializationHelpers` with `import SerializationHelpers` would eliminate the hidden dependency and keep `EventStore` self-documenting.
...
defmodule SerializationHelpers do
  defmacro __using__(_opts) do
    quote do
      ...
      # VALIDATION: This is a smell because __using__/1 injects `import BinaryCodec`
      # VALIDATION: into EventStore. encode_term/1, decode_term/1, checksum/1, and
      # VALIDATION: encode_json/1 become available in EventStore with no visible import
      # VALIDATION: for BinaryCodec. A developer reading EventStore alone cannot tell
      # VALIDATION: where these functions originate without inspecting SerializationHelpers.
      # VALIDATION: `import SerializationHelpers` at the call site would be transparent
      # VALIDATION: and avoid this hidden injection.
      import BinaryCodec
