...
- **Expected smell location:** `Paginator.paginate/2` and `Paginator.paginate_query/2`
- **Affected function(s):** `paginate/2`, `paginate_query/2`, `metadata/3`
- **Short explanation:** The library fetches `:page_size`, `:max_page_size`, and `:overflow_policy` from the global `Application` environment rather than accepting them as keyword options. Any application depending on this library is locked into one page size globally, making it impossible to paginate different resource types with different page sizes.
...
defmodule Paginator do
  ...
  # VALIDATION: This is a smell because page_size, max_page_size, and
  # overflow_policy are read from Application.fetch_env!/2 instead of being
  # accepted as function parameters, preventing different callers in the same
  # application from paginating with different sizes.
