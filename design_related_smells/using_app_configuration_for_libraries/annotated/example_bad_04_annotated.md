...
- **Expected smell location:** `Notifier.dispatch/2`
- **Affected functions:** `dispatch/2`
- **Short explanation:** The function reads `:default_channel` and `:rate_limit_per_minute` from the Application Environment. Because these are global, different parts of the application (e.g., transactional alerts vs. marketing blasts) cannot use different channels or rate limits without changing the global config, restricting the library's reusability.
...
defmodule Notifier do
  ...
  # VALIDATION: This is a smell because default_channel and rate_limit_per_minute
  # are read from the Application Environment inside the library function.
  # An application sending urgent security alerts (push, no rate limit) and
  # bulk newsletters (email, strict rate limit) cannot configure these per-call
  # through the library API, since they are globally fixed at config time.
