...
- **Expected smell location:** `RateLimiter.check/2`
- **Affected function(s):** `check/2`, `increment/2`
- **Short explanation:** The library reads `:window_seconds` and `:max_requests` from the global `Application Environment` instead of accepting them as arguments. This forces all rate-limiting rules in a dependent application to share the same window and request cap, making it impossible to enforce a strict limit (e.g., 5 req/min) on login attempts while allowing a generous limit (e.g., 1000 req/min) on search queries.
...
defmodule RateLimiter do
  ...
  def check(key, bucket \\ "default") when is_binary(key) do
    ...
    # VALIDATION: This is a smell because the library reads :window_seconds and
    # :max_requests from the global Application Environment instead of accepting
    # them as parameters. An application that needs different rate limit rules
    # per endpoint (e.g., 5/min for login, 1000/min for search) cannot use the
    # same library instance with different policies per call site — all calls
    # share the one global window and max, making per-resource throttling
    # impossible without routing through separate libraries or wrappers.
