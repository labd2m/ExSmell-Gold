...
- **Affected function(s):** `hash/1`, `verify/2`
- **Short explanation:** The library reads `:hash_rounds` and `:algorithm` from the global `Application Environment` instead of accepting them as optional parameters. This prevents dependent applications from using a lower cost factor for test environments or a higher one for admin accounts within the same codebase without touching global config.
...
defmodule PasswordHasher do
  ...
  def hash(plaintext) when is_binary(plaintext) do
    with :ok <- validate_password_strength(plaintext) do
      ...
      # VALIDATION: This is a smell because the library reads :algorithm and
      # :hash_rounds from the global Application Environment instead of
      # accepting them as function parameters. Applications that need different
      # cost factors in different contexts (e.g., lower rounds in CI tests,
      # higher rounds for privileged accounts) cannot do so without modifying
      # global config, which affects every single call site simultaneously.
