...
- **Affected functions:** `paginate/2`
- **Short explanation:** The library fetches `:page_size` and `:max_page_size` from the Application Environment. Different API endpoints or contexts in the same application (e.g., a mobile endpoint wanting pages of 10, an admin dashboard wanting pages of 100) cannot customize this behavior per call through the library.
...
defmodule Paginator do
  ...
  # VALIDATION: This is a smell because page_size and max_page_size are fetched
  # from the Application Environment rather than from the params or an options
  # keyword list. Callers like a mobile API endpoint (preferring small pages) and
  # an admin batch export (preferring large pages) are forced to use the same
  # globally configured value, making the library inflexible.
