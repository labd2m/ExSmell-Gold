...
- **Expected smell location:** `ShipmentLabel.build/1`
- **Affected function(s):** `build/1`
- **Short explanation:** The library function `build/1` reads `:label_width` and `:max_address_lines` from the `Application` environment to control label formatting. Because these layout constraints are hardwired to global config, any application that needs to generate labels with different dimensions (e.g., 4×6 warehouse labels vs. 2×4 envelope stickers) cannot do so without altering shared global state.
...
defmodule ShipmentLabel do
  ...
  def build(%__MODULE__{} = shipment) do
    ...
    # VALIDATION: This is a smell because build/1 is a library function that fetches
    # layout parameters (:label_width, :max_address_lines) from the Application
    # environment. Any application depending on this library is forced to use the
    # same label dimensions everywhere, making it impossible to generate labels
    # of different sizes in the same application without changing global config.
