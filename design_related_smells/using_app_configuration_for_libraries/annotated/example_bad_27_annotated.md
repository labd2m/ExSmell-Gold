...
- **Expected smell location:** `CronParser.next_run/2`
- **Affected function(s):** `next_run/2`
- **Short explanation:** The library function `next_run/2` reads `:default_timezone` from the `Application` environment instead of accepting it as a parameter. A scheduling application that manages jobs for users in multiple time zones cannot calculate next-run times in different time zones from the same library call without mutating global config, which is a shared-state hazard in a concurrent scheduler.
...
defmodule CronParser do
  ...
  def next_run(parsed, from \\ nil) do
    ...
    # VALIDATION: This is a smell because next_run/2 is a library function that
    # fetches :default_timezone from the Application environment rather than
    # accepting a timezone as a parameter. A scheduling application managing jobs
    # for users in different time zones (e.g., America/New_York vs. Asia/Tokyo)
    # cannot compute timezone-aware next-run times from the same library in
    # concurrent contexts without mutating shared global state.
