...
- **Affected functions:** `convert/3`
- **Short explanation:** The library retrieves `:rounding_strategy` and `:decimal_precision` from the Application Environment. Applications that need standard rounding for display and truncation for fee calculations cannot call this library with different rounding strategies in different contexts.
...
defmodule CurrencyConverter do
  ...
  # VALIDATION: This is a smell because rounding_strategy and decimal_precision
  # are fetched from the Application Environment. A checkout module that needs
  # :floor rounding (never charge more than owed) and a reporting module that
  # needs :round rounding cannot both be served by this library at the same time
  # without hacking the global configuration between calls.
