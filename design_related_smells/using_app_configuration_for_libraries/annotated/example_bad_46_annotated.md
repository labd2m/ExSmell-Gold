...
- **Expected smell location:** `DateTimeFormatter.format/1`
- **Affected function(s):** `format/1`, `format_relative/1`
- **Short explanation:** The library reads `:timezone` and `:date_format` from the global `Application Environment` instead of accepting them as call-site parameters. A dependent application that must display timestamps in the user's local timezone cannot do so per-user without a global reconfiguration that would affect every concurrent request.
...
defmodule DateTimeFormatter do
  ...
  def format(dt) do
    ...
    # VALIDATION: This is a smell because the library fetches :timezone and
    # :date_format from the global Application Environment rather than accepting
    # them as keyword arguments. A multi-tenant application that must display
    # timestamps in each user's own timezone (e.g., "America/Chicago" for one
    # user and "Europe/Berlin" for another) cannot do so by calling format/1;
    # every concurrent request would share the same global timezone, making
    # per-user localization impossible without abandoning the library.
