...
- **Expected smell location:** `TokenBuilder.generate/2`
- **Affected function(s):** `generate/2`
- **Short explanation:** The library function `generate/2` reads `:token_ttl_seconds` and `:signing_algorithm` from the `Application` environment. Because these security parameters are locked in global config, an application cannot generate short-lived tokens for one flow (e.g., password reset, 15 min) and long-lived tokens for another (e.g., remember-me, 30 days) without mutating global state, which is not thread-safe and defeats the purpose of a reusable library.
...
defmodule TokenBuilder do
  ...
  def generate(secret, claims) when is_binary(secret) and is_map(claims) do
    with :ok <- validate_claims(claims) do
      ...
      # VALIDATION: This is a smell because generate/2 is a library function that
      # fetches :token_ttl_seconds and :signing_algorithm from the Application
      # environment rather than accepting them as parameters. This prevents the
      # same application from generating tokens with different lifetimes or
      # algorithms for different use-cases (e.g., session tokens vs. reset tokens)
