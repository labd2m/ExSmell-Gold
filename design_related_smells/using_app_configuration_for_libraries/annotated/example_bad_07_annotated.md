...
- **Affected functions:** `validate/1`
- **Short explanation:** The library reads `:min_length`, `:require_uppercase`, `:require_digit`, and `:require_special` from the Application Environment. An application that enforces different password policies for different user roles (e.g., stricter rules for admin accounts) cannot use this library for both scenarios simultaneously.
...
defmodule PasswordPolicy do
  ...
  # VALIDATION: This is a smell because min_length, require_uppercase,
  # require_digit, and require_special are all retrieved from the Application
  # Environment. An application that needs a strict policy for admin users and a
  # lenient policy for guest accounts cannot call this library with different
  # rules in different contexts without changing global app config.
