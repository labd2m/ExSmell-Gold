...
- **Expected smell location:** `LeaderboardServer.start/1`
- **Affected function(s):** `LeaderboardServer.start/1`, `GameScoring.create_leaderboard/2`
- **Short explanation:** Each game tournament leaderboard runs as a `GenServer` holding live ranking data. `GenServer.start/3` is used with no supervisor, meaning a crash silently wipes all rankings and score submissions, with no way to recover or alert game clients.
...
defmodule LeaderboardServer do
  ...
  # VALIDATION: This is a smell because leaderboard processes hold the authoritative
  # live ranking data for active tournaments. Using `GenServer.start/3` without a
  # supervisor means a crash destroys all in-memory ranking data. The leaderboard
  # silently disappears, score submissions fail with no process error, and
  # connected game clients receive no notification of the failure.
