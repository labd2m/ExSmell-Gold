# Annotated Example 14 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `FeatureFlags.Store.start/0`
- **Affected function(s):** `start/0`
- **Short explanation:** The feature flag store is started via `GenServer.start/3` outside any supervision tree. This single long-running process is a global dependency for every feature gate check across the entire application. A crash brings all flag evaluation down with no automatic restart.
...
defmodule FeatureFlags.Store do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` creates the feature flag
  # store as a global long-running process outside any supervision tree. Every feature
  # gate check in the application depends on this single process. If it crashes (e.g.,
  # during a malformed flag configuration sync from the remote source), all feature
  # flag evaluations fail for every user. No supervisor is present to restart it,
  # so the outage persists until the application is manually restarted.
  def start do
