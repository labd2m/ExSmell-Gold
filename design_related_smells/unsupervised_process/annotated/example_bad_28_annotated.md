...
- **Expected smell location:** `PaymentSaga.start/2`
- **Affected function(s):** `PaymentSaga.start/2`
- **Short explanation:** Each payment transaction is managed by a saga `GenServer` that coordinates charge, capture, and reconciliation steps. Using `GenServer.start/3` keeps each saga outside any supervisor, so a process crash during a sensitive multi-step payment operation cannot be detected or recovered automatically.
...
defmodule PaymentSaga do
  ...
  # VALIDATION: This is a smell because payment sagas are long-running stateful
  # processes that coordinate critical financial operations. Starting them with
  # `GenServer.start/3` instead of placing them under a supervisor means that
  # a crash at any step (e.g. during capture) cannot be restarted, leaving
  # the transaction in an indeterminate state with no automated recovery.
