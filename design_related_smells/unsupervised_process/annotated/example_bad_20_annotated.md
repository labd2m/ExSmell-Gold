# Annotated Example 20 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `DataImport.PipelineWorker.start/2`
- **Affected function(s):** `start/2`
- **Short explanation:** Each data import job spawns a dedicated long-running GenServer pipeline worker via `GenServer.start/3` outside a supervision tree. If the process crashes mid-import, the progress state and partially written records are lost with no automatic restart or rollback coordination.
...
defmodule DataImport.PipelineWorker do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` spawns a long-running data
  # import pipeline worker outside any supervision tree. Imports can process millions
  # of rows over many minutes. If the process crashes partway through (e.g., due to
  # a memory spike from a very large batch or an unhandled transform error), all
  # progress counters, the error log, and any in-memory row buffer are lost. No
  # supervisor will restart it, so the import job silently stalls and operators have
  # no way to determine whether partial data was written or to resume from a checkpoint.
