# Annotated Example 13 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `Fraud.RuleEngine.start/1`
- **Affected function(s):** `start/1`
- **Short explanation:** The fraud rule engine is a stateful GenServer started via `GenServer.start/3` outside any supervisor. It accumulates rule scores and user risk profiles in memory. A crash resets all risk data and removes fraud protection entirely, with no automatic restart.
...
defmodule Fraud.RuleEngine do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` starts the fraud detection
  # engine outside any supervision tree. This is a single critical long-running process
  # that accumulates user risk profiles and velocity data. If it crashes due to a rule
  # evaluation bug or unexpected data shape, all in-memory fraud history is lost and
  # the engine never restarts. During the outage, all transactions pass through
  # without fraud checks, which is a serious security regression.
