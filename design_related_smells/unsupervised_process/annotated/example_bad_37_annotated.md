...
- **Expected smell location:** `DeviceShadow.start/1`
- **Affected function(s):** `DeviceShadow.start/1`, `DeviceRegistry.register/2`
- **Short explanation:** Each connected IoT device is backed by a "shadow" `GenServer` that holds its desired and reported state. `GenServer.start/3` is used with no supervisor, meaning a shadow process crash is undetectable, the device appears online when it is not, and delta synchronisation is silently broken.
...
defmodule DeviceShadow do
  ...
  # VALIDATION: This is a smell because device shadows are long-lived processes
  # expected to exist for the full connection lifetime of a device (potentially
  # hours or days). By using `GenServer.start/3`, each shadow runs outside any
  # supervision tree. A crash silently removes the device's state, causing
  # stale routing, missed delta syncs, and no automated recovery.
