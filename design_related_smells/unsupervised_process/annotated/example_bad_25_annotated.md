...
- **Affected function(s):** `ShipmentTracker.start/1`, `LogisticsCoordinator.track_shipments/1`
- **Short explanation:** `ShipmentTracker.start/1` spawns a long-running `GenServer` for each shipment using `GenServer.start/3`, never placing it under a supervisor. Multiple concurrent shipment trackers are created without any lifecycle management, making it impossible to monitor, restart, or cleanly shut them down.
...
defmodule ShipmentTracker do
  ...
  # VALIDATION: This is a smell because each shipment tracker is started outside
  # a supervision tree via `GenServer.start/3`. In a logistics system that may
  # have hundreds of active shipments, none of these long-running processes
  # can be monitored or restarted if they crash, making the application
