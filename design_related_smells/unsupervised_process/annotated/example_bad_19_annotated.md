# Annotated Example 19 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `API.QuotaTracker.start_for_tenant/2`
- **Affected function(s):** `start_for_tenant/2`
- **Short explanation:** Each tenant API quota tracker is started as a long-running GenServer via `GenServer.start/3` outside a supervision tree. A crash silently resets all quota accounting for a tenant, potentially allowing unlimited API usage until the process is manually restarted.
...
defmodule API.QuotaTracker do
  ...
  # quota tracker outside any supervision tree. Quota enforcement is a billing-critical
  # function. If this process crashes (e.g., during a burst of concurrent API calls
  # causing a race in the accumulation logic), the in-memory usage state is lost and
  # the process is never automatically restarted. Until it's noticed and restarted
  # manually, the tenant's API calls pass through unchecked, leading to potential
  # over-usage with no billing recourse.
