# Annotated Example 04 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `NotificationDispatcher.start_worker/1`
- **Affected function(s):** `start_worker/1`
- **Short explanation:** Each notification channel (email, SMS, push) spawns a dedicated long-running worker process via `GenServer.start/3` outside any supervision tree. If a worker crashes during message delivery, the entire channel goes dark with no recovery mechanism and no observability.
...
defmodule Notifications.Dispatcher do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` (not start_link) spawns
  # a long-lived channel worker that is deliberately disconnected from any supervisor.
  # Notification infrastructure is business-critical; if the :email worker crashes
  # mid-delivery, queued messages are silently dropped and the process is never
  # automatically restarted. The caller has no reliable way to detect the outage.
