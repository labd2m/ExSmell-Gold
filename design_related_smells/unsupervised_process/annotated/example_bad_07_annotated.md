# Annotated Example 07 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `Reporting.JobWorker.start/1`
- **Affected function(s):** `start/1`
- **Short explanation:** Each long-running report generation job is assigned its own GenServer process spawned with `GenServer.start/3`, never added to a supervision tree. If a worker crashes mid-generation, the job status is lost forever and users receive no feedback about their pending report.
...
defmodule Reporting.JobWorker do
  ...
  # report job process outside any supervision tree. Report generation can take
  # several minutes. If the process crashes partway through (e.g., due to a
  # memory allocation failure on a large dataset), the job status is permanently
  # lost. No supervisor will restart it, so the user's report request silently
  # disappears and they are left waiting indefinitely.
