...
- **Expected smell location:** `CacheWarmer.start/1`
- **Affected function(s):** `CacheWarmer.start/1`, `CacheManager.warm/1`
- **Short explanation:** One `GenServer` is created per cache namespace to periodically refresh cached values in the background. These are started via `GenServer.start/3` with no supervisor. A crash silently stops all refreshes for that namespace, causing staleness without any alert or automatic restart.
...
defmodule CacheWarmer do
  ...
  # VALIDATION: This is a smell because one CacheWarmer process per namespace
  # is started via `GenServer.start/3` outside any supervision tree. These
  # processes are expected to run indefinitely, periodically waking up to
  # refresh cached data. A crash silently ends all future refreshes for that
  # namespace, and the application has no supervisor to detect and restart it.
