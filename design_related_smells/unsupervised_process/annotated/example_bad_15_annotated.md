# Annotated Example 15 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `Auction.Room.open/1`
- **Affected function(s):** `open/1`
- **Short explanation:** Each auction room is a long-running GenServer started via `GenServer.start/3` outside any supervision tree. A crash in an active auction room silently destroys all bidding state — current bids, bidder list, and auction timer — with no automatic restart or recovery.
...
defmodule Auction.Room do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` creates a live auction
  # room outside any supervision tree. An auction room is a stateful, time-sensitive
  # process that holds the authoritative bidding state. If it crashes mid-auction,
  # all bids, the timer state, and bidder list are permanently lost. With no supervisor
  # to restart the room, the auction silently ends in an undefined state and bidders
