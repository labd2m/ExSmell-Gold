...
- **Expected smell location:** `JobRunner.start/1`
- **Affected function(s):** `JobRunner.start/1`, `Scheduler.enqueue/1`
- **Short explanation:** `JobRunner` starts a dedicated `GenServer` for each scheduled job using `GenServer.start/3`. Since these processes are not part of a supervision tree, failed jobs cannot be detected or restarted, and the scheduler has no visibility into running job processes.
...
defmodule JobRunner do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` launches a job runner
  # outside of any supervision tree. Background job runners can fail due to
  # timeouts, bad payloads, or external service errors. Without a supervisor,
  # these failures are invisible to the rest of the application, and jobs are
  # simply lost rather than being rescheduled or retried.
