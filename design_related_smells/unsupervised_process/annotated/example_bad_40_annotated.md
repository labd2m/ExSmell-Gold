...
- **Expected smell location:** `UploadProcessor.start/1`
- **Affected function(s):** `UploadProcessor.start/1`, `UploadService.begin/2`
- **Short explanation:** Each file upload is handled by its own `GenServer` that tracks chunked parts, validates checksums, and triggers post-processing. Because `GenServer.start/3` is used with no supervisor, a crash mid-upload means the upload is silently abandoned — the client receives no error and the partial chunks may linger in storage forever.
...
defmodule UploadProcessor do
  ...
  # VALIDATION: This is a smell because file upload processes are long-lived:
  # a large upload may send chunks over several minutes. By using `GenServer.start/3`
  # without a supervisor, a crash at any point silently abandons the upload.
  # Orphaned chunk data accumulates in storage, the client hangs indefinitely
  # waiting for a completion response, and there is no way to clean up or retry.
