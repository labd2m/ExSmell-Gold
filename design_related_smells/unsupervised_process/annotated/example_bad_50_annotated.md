...
- **Expected smell location:** `ReportScheduler.schedule/2`
- **Affected function(s):** `schedule/2`
- **Short explanation:** Each report job starts its own GenServer via `GenServer.start/3` with no supervisor. These long-lived processes run timed report generation loops for different tenants and can number in the hundreds, yet none of them appear in the application supervision tree. A crash in any worker silently terminates that tenant's reporting indefinitely.
...
defmodule Reporting.ReportScheduler do
  ...
  # VALIDATION: This is a smell because every call to `schedule/2` starts
  # a long-running GenServer with `GenServer.start/3` that is never registered
  # under a supervisor. Each process runs indefinitely on a timer loop.
  # In a multi-tenant system with dozens or hundreds of tenants, this spawns
  # an equal number of untracked processes. If one crashes, its tenant silently
  # stops receiving reports. The system cannot enumerate, restart, or drain
  # these processes during a deployment or shutdown event.
