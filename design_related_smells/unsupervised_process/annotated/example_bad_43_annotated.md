...
- **Expected smell location:** `TenantConfigServer.start/1`
- **Affected function(s):** `TenantConfigServer.start/1`, `TenantBootstrap.boot/1`
- **Short explanation:** Each tenant in a multi-tenant SaaS application gets its own `GenServer` holding live configuration (feature flags, plan limits, custom settings). These are started with `GenServer.start/3` outside any supervision tree. A crash silently removes the tenant's config process; subsequent calls to read feature flags or plan limits will fail with no automatic recovery.
...
defmodule TenantConfigServer do
  ...
  # VALIDATION: This is a smell because one long-lived `GenServer` is created per
  # tenant via `GenServer.start/3` without placing it under any supervision tree.
  # In a multi-tenant platform these processes are expected to run for the full
  # lifetime of the application. A crash removes the tenant's config silently;
  # the next request to `get_flag/2` or `check_limit/2` will raise because the
  # named process no longer exists, and there is no supervisor to restart it.
