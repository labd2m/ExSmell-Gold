...
- **Expected smell location:** `ChatRoom.start/1`
- **Affected function(s):** `ChatRoom.start/1`, `ChatServer.create_room/2`
- **Short explanation:** Each chat room is a `GenServer` maintaining participant lists and message history, started via `GenServer.start/3`. Without supervision, a crash destroys the room's state (messages, participants) permanently and the room silently disappears from the user-facing system.
...
defmodule ChatRoom do
  ...
  # VALIDATION: This is a smell because chat room processes are long-lived —
  # potentially running for the lifetime of the application. Using `GenServer.start/3`
  # instead of starting these under a supervisor means any process crash silently
  # destroys room state (messages, participants), and there is no supervisor to
  # restart the room or signal failure to the application.
