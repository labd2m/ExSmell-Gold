...
- **Expected smell location:** `OrderProcess.start/1`
- **Affected function(s):** `OrderProcess.start/1`, `OrderService.place/1`
- **Short explanation:** Each order is managed by a dedicated `GenServer` process started with `GenServer.start/3`. With potentially thousands of concurrent orders, none of these processes are visible to a supervisor, making it impossible to manage their lifecycle, detect crashes, or gracefully shut down the system.
...
defmodule OrderProcess do
  ...
  # VALIDATION: This is a smell because each customer order spawns a long-lived
  # `GenServer` via `GenServer.start/3` without any supervisor. Orders can take
  # minutes to hours to complete; if a process crashes mid-lifecycle (e.g. during
  # fulfillment), the order state is lost, the customer receives no update, and
  # the system cannot detect or recover the failed order process.
