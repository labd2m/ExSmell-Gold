...
- **Expected smell location:** `SubscriptionProcess.start/1`
- **Affected function(s):** `SubscriptionProcess.start/1`, `SubscriptionManager.activate/1`
- **Short explanation:** Each active subscription is backed by a `GenServer` that handles renewal scheduling, dunning, and cancellation. These are started with `GenServer.start/3` outside any supervision tree, so a crash silently loses the renewal timer and subscription state.
...
defmodule SubscriptionProcess do
  ...
  # VALIDATION: This is a smell because each subscription is a long-running
  # `GenServer` created with `GenServer.start/3` outside any supervision tree.
  # Subscriptions may remain active for months or years; a process crash silently
  # cancels the renewal timer, meaning no renewal attempt will occur and the
  # subscription will simply expire without any recovery mechanism.
