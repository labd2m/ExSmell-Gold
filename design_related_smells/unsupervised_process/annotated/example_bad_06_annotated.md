# Annotated Example 06 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `Payments.GatewayWorker.start/1`
- **Affected function(s):** `start/1`
- **Short explanation:** Each payment gateway (Stripe, PayPal, Adyen) gets its own long-running GenServer process started via `GenServer.start/3` without being added to a supervision tree. A crash in any gateway worker means all payment processing for that provider silently stops with no automatic restart.
...
defmodule Payments.GatewayWorker do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` launches a payment-gateway
  # worker process outside any supervision tree. Payment processing is mission-critical.
  # If this process crashes (e.g., after a malformed response from the provider causes
  # an unhandled error), all in-flight payment requests are lost and the process is
  # never restarted automatically. The crash is also not propagated to any supervisor
  # that could page an on-call engineer.
