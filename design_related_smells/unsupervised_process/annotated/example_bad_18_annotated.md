# Annotated Example 18 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `Collaboration.DocumentSession.open/2`
- **Affected function(s):** `open/2`
- **Short explanation:** Each collaborative document session is a GenServer started via `GenServer.start/3` outside any supervision tree. A crash while users are actively editing means all in-flight changes and the operational transform log are lost, potentially corrupting the document.
...
defmodule Collaboration.DocumentSession do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` creates a collaborative
  # document session process outside any supervision tree. This process is the sole
  # authority for the in-memory document state and all pending operations for an
  # active editing session. If it crashes, all unsaved edits from all connected
  # users are lost and the operation log used for conflict resolution is discarded.
  # Without a supervisor, the session is never restarted and users are left with
  # a split-brain document state.
