# Annotated Example 03 — Unsupervised Process

- **Smell name:** Unsupervised Process
- **Expected smell location:** `SessionStore.start/0`
- **Affected function(s):** `start/0`
- **Short explanation:** The session store is a single long-running GenServer started via `GenServer.start_link/3` but never placed under a supervisor. If this process crashes, all active user sessions are lost and no automatic restart occurs, causing a complete authentication outage.
...
defmodule Auth.SessionStore do
  ...
  # VALIDATION: This is a smell because `GenServer.start_link/3` is called directly
  # from application code with no supervisor parent. A session store is a critical,
  # long-running process. If it crashes (e.g., during a malformed token insert), the
  # entire in-memory session table is permanently lost. No supervisor is present to
  # restart it, so all authenticated users are immediately logged out with no
  # automatic recovery.
  def start do
