...
- **Expected smell location:** `WarehouseStock.start/1`
- **Affected function(s):** `WarehouseStock.start/1`, `InventoryManager.open_warehouse/1`
- **Short explanation:** Each warehouse gets its own `GenServer` holding live stock levels. `WarehouseStock.start/1` uses `GenServer.start/3`, keeping every warehouse process outside any supervision tree. A crash results in permanent loss of in-memory stock data and no automatic recovery.
...
defmodule WarehouseStock do
  ...
  # VALIDATION: This is a smell because `GenServer.start/3` is used to launch a
  # long-lived process per warehouse without placing it under a supervisor.
  # In a real warehouse system with many locations, none of these processes can
  # be automatically restarted after a crash, and the system has no way to
  # enumerate or manage them collectively.
