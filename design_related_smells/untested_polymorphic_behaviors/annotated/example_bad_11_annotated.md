# Annotated Bad Example 11: Untested Polymorphic Behaviors

## Metadata

- **Smell name**: Untested Polymorphic Behaviors
- **Expected smell location**: `Billing.Formatter.format_invoice_reference/1`
- **Affected function(s)**: `format_invoice_reference/1`
- **Short explanation**: The function calls `to_string/1` on a parameter without any guard clause or typespec restricting the accepted types. This relies on the `String.Chars` protocol, which is not implemented by all Elixir data types (e.g., `Map`, `List`, `Tuple`). Callers can accidentally pass incompatible types (like a map or list) and receive a runtime `Protocol.UndefinedError` instead of a clear `FunctionClauseError`.
...
defmodule Billing.Formatter do
  ...
  # protocol. No guard clause restricts which types are accepted. Passing a value
  # such as `%{number: 10042}` or `[10042]` will raise `Protocol.UndefinedError`
  # at runtime instead of a clear `FunctionClauseError`, making debugging harder
  # and leaving the function's valid input space completely undocumented.
