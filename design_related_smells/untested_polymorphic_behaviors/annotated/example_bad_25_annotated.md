# Annotated Bad Example 25: Untested Polymorphic Behaviors

## Metadata

- **Smell name**: Untested Polymorphic Behaviors
- **Expected smell location**: `HR.BadgeGenerator.build_badge_number/2`
- **Affected function(s)**: `build_badge_number/2`
- **Short explanation**: The function calls `to_string/1` on the `employee_id` parameter without a guard clause. While integers and binaries both implement `String.Chars`, passing a `Tuple` (a common mistake when a caller passes `{:ok, id}` from a previous pipeline step) or a `Map` will raise `Protocol.UndefinedError`. Passing an atom like `:temp` silently produces `"DEPT-temp"` as a badge number, which would pass later string validations but be semantically invalid in the HR system. A guard clause such as `is_integer(employee_id) or is_binary(employee_id)` would eliminate both failure modes.
...
defmodule HR.BadgeGenerator do
  ...
  # VALIDATION: This is a smell because `to_string/1` is called on `employee_id`
  # without any guard clause restricting the accepted types. Passing a `Tuple`
  # such as `{:ok, 42}` (a frequent mistake when the ID comes from an unwrapped
  # result tuple) will raise `Protocol.UndefinedError` at runtime. Passing an
  # `Atom` like `:temp` silently produces `"ENG-temp"`, which is syntactically
  # valid but semantically meaningless as a badge number and can corrupt the
  # access-control database. An explicit guard `is_integer(employee_id) or
  # is_binary(employee_id)` would enforce the correct contract.
