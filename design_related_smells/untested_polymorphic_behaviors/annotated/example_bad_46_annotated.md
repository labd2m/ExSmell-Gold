...
- **Expected smell location:** `consolidate_pickup_stops/1` — the `Enum.concat(stop_lists)` call
- **Affected function(s):** `Logistics.DispatchPlanner.consolidate_pickup_stops/1`
- **Short explanation:** `Enum.concat/1` dispatches through the `Enumerable` protocol on both the outer `stop_lists` and on each inner element. No guard clause restricts `stop_lists` to types implementing `Enumerable`. Passing an integer, atom, or binary raises `Protocol.UndefinedError` at runtime.
...
defmodule Logistics.DispatchPlanner do
  ...
  # VALIDATION: This is a smell because `Enum.concat/1` uses the `Enumerable` protocol on
  # VALIDATION: `stop_lists` and on each inner collection. No guard clause restricts
  # VALIDATION: `stop_lists` to types that implement `Enumerable` (e.g., list, map, range).
  # VALIDATION: Passing an integer, atom, binary, PID, or float raises
  # VALIDATION: `Protocol.UndefinedError` at runtime. Even passing a flat list of `Stop`
  # VALIDATION: structs (instead of a list of lists) would crash silently, and there is
  # VALIDATION: nothing to warn the caller about this expectation.
