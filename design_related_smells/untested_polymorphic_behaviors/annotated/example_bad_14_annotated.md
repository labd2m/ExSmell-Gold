# Annotated Bad Example 14: Untested Polymorphic Behaviors

## Metadata

- **Smell name**: Untested Polymorphic Behaviors
- **Expected smell location**: `Notifications.TemplateRenderer.interpolate_variable/2`
- **Affected function(s)**: `interpolate_variable/2`
- **Short explanation**: The function uses `to_string/1` on the `value` parameter to inject a variable into a template string. No guard clause restricts what types are accepted as `value`. Types like `Map` and `List` that do not implement `String.Chars` will raise `Protocol.UndefinedError` at runtime. Types that do implement the protocol (such as integers or floats) will silently produce output (e.g., substituting the recipient's name with `"42"`) that is semantically incorrect in a notification context.
...
defmodule Notifications.TemplateRenderer do
  ...
  # VALIDATION: This is a smell because `to_string/1` depends on the `String.Chars`
  # protocol. The function has no guard clause restricting the type of `value`.
  # Passing a `Map` (e.g., a nested user struct), a `List`, or a `Tuple` will
  # raise `Protocol.UndefinedError` at runtime instead of a meaningful error at
  # the function boundary. Even passing a `Float` or `Integer` silently produces
  # output (e.g., injecting `"3.14e0"` as a recipient's first name) that is
  # semantically wrong in a notification template context.
