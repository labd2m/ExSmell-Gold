...
- **Expected smell location:** `build_category_path/1` — the `Enum.join(segments, "/")` call
- **Affected function(s):** `Inventory.CategoryTree.build_category_path/1`
- **Short explanation:** `Enum.join/2` (and `Enum.map/2` before it) uses the `Enumerable` protocol. The `segments` parameter has no guard restricting it to types that implement `Enumerable`. Passing an integer, atom, float, PID, or binary string raises `Protocol.UndefinedError` at runtime.
...
defmodule Inventory.CategoryTree do
  ...
  # VALIDATION: This is a smell because `Enum.map/2` and then `Enum.join/2` both dispatch
  # VALIDATION: through the `Enumerable` protocol on `segments`. No guard clause restricts
  # VALIDATION: `segments` to types that implement `Enumerable` (e.g., list, map, range).
  # VALIDATION: Passing an integer, atom, float, binary, or PID will raise
  # VALIDATION: `Protocol.UndefinedError` at runtime with no helpful context for the caller.
