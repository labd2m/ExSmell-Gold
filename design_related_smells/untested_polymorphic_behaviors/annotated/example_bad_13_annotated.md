# Annotated Bad Example 13: Untested Polymorphic Behaviors

## Metadata

- **Smell name**: Untested Polymorphic Behaviors
- **Expected smell location**: `Auth.TokenClaims.build_subject_claim/1`
- **Affected function(s)**: `build_subject_claim/1`
- **Short explanation**: The function calls `to_string/1` on the `subject` parameter to produce the JWT `sub` claim. There is no guard clause or pattern match restricting what types are accepted. This means passing a `Map`, `List`, or `Tuple` will raise `Protocol.UndefinedError` deep in the call stack, while passing an integer or float will silently produce a numeric string (e.g., `"42"` or `"3.14"`) which may pass validation but produce incorrect subject claims in the JWT. The function's intent is clearly to accept user IDs (binaries or integers), yet it accepts any type silently.
...
defmodule Auth.TokenClaims do
  ...
  # VALIDATION: This is a smell because `to_string/1` is invoked on `subject`
  # without any guard clause. The `String.Chars` protocol is not implemented for
  # all types: passing a `Map`, `List`, or `Tuple` will raise
  # `Protocol.UndefinedError` at runtime. Additionally, passing a `Float` would
  # silently produce `"3.14e0"` as a subject claim, which is semantically wrong
  # for a user identifier. The function should restrict input to `is_binary` or
  # `is_integer` via guard clauses to make the valid input domain explicit.
