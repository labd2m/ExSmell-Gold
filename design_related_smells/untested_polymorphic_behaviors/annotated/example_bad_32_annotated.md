# Annotated Bad Example 32: Untested Polymorphic Behaviors

## Metadata

- **Smell name**: Untested Polymorphic Behaviors
- **Expected smell location**: `Webhooks.PayloadSerializer.collect_headers/1`
- **Affected function(s)**: `collect_headers/1`
- **Short explanation**: The function calls `Enum.into/2` on the `headers` parameter to build a normalized header map, relying on the `Enumerable` protocol. There is no guard clause restricting what types are accepted. Passing a plain binary (e.g., a JSON string that hasn't been decoded yet) or an integer raises `Protocol.UndefinedError` at runtime. Passing a `Map` with non-string values silently iterates over key-value pairs and produces a header map with mixed-type values that will fail HTTP client validation later in the pipeline, making this bug extremely hard to trace back to this function.
...
defmodule Webhooks.PayloadSerializer do
  ...
  # protocol. There is no guard clause on `headers`, so passing a plain binary
  # (e.g., a JSON string that was not decoded before being forwarded here) or
  # an integer raises `Protocol.UndefinedError` deep in the Enum internals instead
  # of a clear `FunctionClauseError` at this function boundary. Passing a `Map`
  # with atom keys silently collects them as-is, producing a mixed atom/string
  # key map that causes subtle `Map.merge/2` collisions downstream in `build_headers`.
  # A guard such as `is_list(headers) or is_map(headers)` would enforce the contract.
