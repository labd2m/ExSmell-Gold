...
- **Expected smell location:** `sum_line_amounts/1` — the `Enum.reduce(charges, ...)` call
- **Affected function(s):** `Billing.SubscriptionCalculator.sum_line_amounts/1`
- **Short explanation:** `Enum.reduce/3` dispatches through the `Enumerable` protocol on `charges`. No guard restricts `charges` to `Enumerable` types. Passing an integer, atom, float, or binary raises `Protocol.UndefinedError`. Passing a single `Charge` struct instead of a list of them would also crash silently at runtime.
...
defmodule Billing.SubscriptionCalculator do
  ...
  # VALIDATION: This is a smell because `Enum.reduce/3` dispatches through the `Enumerable`
  # VALIDATION: protocol on `charges`. No guard clause restricts `charges` to types that
  # VALIDATION: implement `Enumerable`. Passing an integer, atom, binary, float, PID, or
  # VALIDATION: even a single `Charge` struct (instead of a list) raises
  # VALIDATION: `Protocol.UndefinedError` at runtime. The function's polymorphic dependency
  # VALIDATION: is untested and undocumented, hiding a class of runtime crashes from callers.
