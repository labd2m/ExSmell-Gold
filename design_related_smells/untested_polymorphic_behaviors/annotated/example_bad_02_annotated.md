...
- **Expected smell location:** `Logistics.ShipmentExporter.serialize_field/1`
- **Affected function(s):** `serialize_field/1`
- **Short explanation:** `serialize_field/1` calls `to_string/1` on an arbitrary term without any
  guard clause or pattern match. The intent is to flatten any shipment field value to a plain
  string for CSV serialization. Types like `String`, `Integer`, `Float`, and `Atom` happen to
  implement `String.Chars` and produce output, but some produce semantically wrong CSV cells
  (e.g., a `DateTime` struct is coerced silently), while others (e.g., a nested `Map` or a
  `List`) raise `Protocol.UndefinedError` at runtime, crashing the export job.
...
defmodule Logistics.ShipmentExporter do
  ...
  # VALIDATION: This is a smell because serialize_field/1 calls to_string/1 on any
  # VALIDATION: term without a guard or pattern match. The function is meant to
  # VALIDATION: handle shipment field values (strings, numbers, atoms, statuses).
  # VALIDATION: Types like Integer and Atom implement String.Chars and silently
  # VALIDATION: produce plausible-looking output. However, passing a DateTime
  # VALIDATION: struct yields its internal representation rather than an ISO-8601
  # VALIDATION: string, producing semantically incorrect CSV cells. Passing a
  # VALIDATION: nested Map (e.g., if a caller mistakenly passes an Address struct
  # VALIDATION: instead of its postal_code field) raises Protocol.UndefinedError,
  # VALIDATION: crashing the entire export without a useful error site.
  @doc """
