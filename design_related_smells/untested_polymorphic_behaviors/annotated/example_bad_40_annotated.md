...
- **Expected smell location:** `slugify_dimension/1` — the `to_string(dimension)` call
- **Affected function(s):** `Reporting.DashboardExporter.slugify_dimension/1`
- **Short explanation:** `to_string/1` relies on the `String.Chars` protocol. The function has no guard clause restricting `dimension` to compatible types. A `Map`, `Tuple`, or arbitrary struct without the protocol raises `Protocol.UndefinedError`. Even protocol-compatible types like `URI` or integers produce nonsensical dimension slugs without any compile- or run-time warning.
...
defmodule Reporting.DashboardExporter do
  ...
  # VALIDATION: This is a smell because `to_string(dimension)` uses the `String.Chars`
  # VALIDATION: protocol. The function has no guard restricting `dimension` to types that
  # VALIDATION: implement the protocol. Passing a Map, Tuple, or a struct without
  # VALIDATION: `String.Chars` raises `Protocol.UndefinedError`. Passing a URI, float, or
  # VALIDATION: integer succeeds but produces slugs like "42" or a URL-shaped string,
  # VALIDATION: which are semantically wrong for a reporting dimension identifier.
