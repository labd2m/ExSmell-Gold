...
- **Expected smell location:** `format_record_identifier/1` — the `to_string(record_id)` call
- **Affected function(s):** `Reporting.AuditTrail.format_record_identifier/1`
- **Short explanation:** `to_string/1` uses the `String.Chars` protocol. No guard clause restricts `record_id` to types implementing it. Passing a `Map`, `Tuple`, or struct without `String.Chars` raises `Protocol.UndefinedError`. Passing a float or `URI` silently produces a formatted identifier that makes no semantic sense in an audit context.
...
defmodule Reporting.AuditTrail do
  ...
  # VALIDATION: This is a smell because `to_string(record_id)` uses the `String.Chars`
  # VALIDATION: protocol. No guard clause restricts `record_id` to types implementing it.
  # VALIDATION: Passing a Map, Tuple, or struct without `String.Chars` raises
  # VALIDATION: `Protocol.UndefinedError`. Passing a Float like `3.14` or a URI succeeds
  # VALIDATION: silently but produces a nonsensical audit record identifier, corrupting
  # VALIDATION: the audit trail without any runtime warning or validation error.
