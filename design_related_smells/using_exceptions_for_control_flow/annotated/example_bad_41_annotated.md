...
- **Expected smell location:** `InvoiceProcessor.calculate_total/1` and `BillingService.generate_invoice/2`
- **Affected function(s):** `InvoiceProcessor.calculate_total/1`, `BillingService.generate_invoice/2`
- **Short explanation:** `InvoiceProcessor.calculate_total/1` raises a `RuntimeError` when given invalid line items or a negative discount, forcing every caller (like `BillingService.generate_invoice/2`) to wrap calls in `try/rescue` for ordinary control-flow. No `{:ok, result} | {:error, reason}` alternative is provided, so clients have no choice but to use exception handling to distinguish between successful and failed calculations — even when failure is an expected, non-exceptional business scenario.
...
defmodule InvoiceProcessor do
  ...
  # VALIDATION: This is a smell because calculate_total/1 raises RuntimeError
  # VALIDATION: for ordinary business-rule violations (empty items, bad discount),
  # VALIDATION: providing no {:ok, _} | {:error, _} alternative. Every caller
  # VALIDATION: is forced into try/rescue to handle predictable, non-exceptional cases.
