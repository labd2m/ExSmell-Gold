...
- **Expected smell location:** `DataImport.CsvParser.parse_row/2`
- **Affected function(s):** `DataImport.CsvParser.parse_row/2` (library side); `DataImport.ProductImporter.import/2` (client side)
- **Explanation:** `parse_row/2` raises `RuntimeError` for ordinary, expected CSV parsing problems: wrong column count, missing required field, and an unparseable price value. During a bulk import these failures are routine — the importer should collect and report bad rows without aborting. Forcing `try/rescue` means the client must use exception-based branching to achieve that behaviour.
...
defmodule DataImport.CsvParser do
  ...
  # VALIDATION: This is a smell because `parse_row/2` raises RuntimeError for three
  # VALIDATION: entirely routine CSV parsing failures: mismatched column count,
  # VALIDATION: missing required field, and non-numeric price. During a large import
  # VALIDATION: these are expected, per-row conditions. Callers cannot simply
  # VALIDATION: pattern-match on {:ok, record} | {:error, reason} — they must
  # VALIDATION: wrap every call in try/rescue to keep processing after a bad row.
