...
- **Expected smell location:** `SlotRegistry.book!/3`
- **Affected function(s):** `SlotRegistry.book!/3`, `AppointmentService.create_appointment/3`
- **Short explanation:** `SlotRegistry.book!/3` raises `SlotUnavailableError` and `SlotNotFoundError` for scenarios that are entirely predictable in a scheduling context — slots are frequently full or requested times don't exist. Because no non-raising `book/3` variant returning `{:ok, booking} | {:error, reason}` is offered, `AppointmentService.create_appointment/3` is forced to use `try/rescue` as its only mechanism for distinguishing between success and ordinary booking failure.
...
defmodule SlotRegistry do
  ...
  # VALIDATION: This is a smell because book!/3 raises SlotUnavailableError and
  # VALIDATION: SlotNotFoundError for routine scheduling outcomes. In a real booking
  # VALIDATION: system, slots being already taken or missing are everyday events
  # VALIDATION: (race conditions, expired links, user error). Since no book/3
  # VALIDATION: returning {:ok, _} | {:error, _} exists, all callers must use
  # VALIDATION: try/rescue for plain conditional logic, not exceptional scenarios.
