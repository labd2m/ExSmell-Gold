...
- **Expected smell location:** `Catalog.PriceEngine.resolve_price/3`
- **Affected function(s):** `Catalog.PriceEngine.resolve_price/3` (library side); `Catalog.CartPricer.price_items/2` (client side)
- **Explanation:** `resolve_price/3` raises `RuntimeError` for expected catalog and pricing conditions: unknown product, unavailable pricing for a currency, and a discontinued product. When pricing a shopping cart with multiple items, any of these can occur for a given line item. Callers cannot pattern-match on individual item results — they must use `try/rescue` per item, forcing exception-based control-flow for a routine lookup.
...
defmodule Catalog.PriceEngine do
  ...
  # VALIDATION: This is a smell because `resolve_price/3` raises RuntimeError for
  # VALIDATION: three predictable catalog-lookup failures: unknown product,
  # VALIDATION: discontinued product, and no price available for the requested
  # VALIDATION: currency/type combination. When pricing a cart with multiple items,
  # VALIDATION: callers encounter these situations routinely. The only way to
  # VALIDATION: tolerate a missing price for one item without aborting the entire
  # VALIDATION: cart is to catch a RuntimeError — forcing exception-based control-flow.
