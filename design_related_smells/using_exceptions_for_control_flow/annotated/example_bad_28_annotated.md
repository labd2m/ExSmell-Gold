...
- **Expected smell location:** `AppointmentBook.schedule/1` (library) and `BookingService.book/2` (client)
- **Affected function(s):** `AppointmentBook.schedule/1`, `BookingService.book/2`
- **Short explanation:** `AppointmentBook.schedule/1` raises exceptions for slot conflicts, outside-of-hours requests, and unknown providers — conditions that occur constantly in any booking system. Without a tuple-returning counterpart, `BookingService.book/2` is obliged to use `try...rescue` for completely ordinary scheduling decisions.
...
defmodule AppointmentBook do
  ...
  # VALIDATION: This is a smell because slot conflicts, out-of-hours requests,
  # and unknown providers are everyday occurrences when booking appointments.
  # Surfacing all of these exclusively through exceptions — without a
  # tuple-returning variant — means the client must use try...rescue just to
  # decide whether a booking was successful or not.
...
defmodule BookingService do
  ...
  def book(patient_id, %{provider_id: _, start: _, duration_minutes: _} = request) do
    ...
    # VALIDATION: SMELL START - Using exceptions for control-flow
    # VALIDATION: This is a smell because the client is forced to rescue
