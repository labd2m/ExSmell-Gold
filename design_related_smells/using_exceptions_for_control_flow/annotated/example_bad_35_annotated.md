...
- **Expected smell location:** `ProfileStore.update/2` (library) and `AccountSettings.apply_changes/2` (client)
- **Affected function(s):** `ProfileStore.update/2`, `AccountSettings.apply_changes/2`
- **Short explanation:** `ProfileStore.update/2` raises exceptions for prohibited username changes, duplicate display names, and invalid field formats — all of which are routine outcomes when users edit their profile settings. Without a tuple-returning variant, `AccountSettings.apply_changes/2` must use `try...rescue` to differentiate between a successful update and a common validation failure.
...
defmodule ProfileStore do
  ...
  # VALIDATION: This is a smell because duplicate display names, username
  # cooldown violations, and invalid field values are all ordinary, predictable
  # outcomes when a user submits profile edits. Raising exceptions for them —
  # with no {:ok, _}/{:error, _} alternative — forces every profile-edit
  # handler to use try...rescue for what is standard form validation logic.
...
defmodule AccountSettings do
  ...
  def apply_changes(user_id, raw_changes) do
    ...
    # VALIDATION: SMELL START - Using exceptions for control-flow
    # VALIDATION: This is a smell because duplicate display names and cooldown
