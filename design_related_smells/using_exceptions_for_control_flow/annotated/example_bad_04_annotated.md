...
- **Expected smell location:** `Notifications.EmailDispatcher.dispatch/1`
- **Affected function(s):** `Notifications.EmailDispatcher.dispatch/1` (library side); `Notifications.CampaignSender.send_batch/2` (client side)
- **Explanation:** `dispatch/1` raises `RuntimeError` for predictable delivery issues such as missing recipient, empty subject, and provider rejection. Callers processing a batch of notifications cannot simply pattern-match on results; they must wrap every dispatch call in `try/rescue`, which turns ordinary notification failures into forced exception-based branching.
...
defmodule Notifications.EmailDispatcher do
  ...
  # VALIDATION: This is a smell because `dispatch/1` raises RuntimeError for
  # VALIDATION: routine, foreseeable failures: missing/invalid recipient, blank
  # VALIDATION: subject, and provider rejection. These are expected outcomes in
  # VALIDATION: any email delivery pipeline. Callers are given no {:error, reason}
  # VALIDATION: path and must use try/rescue to react to delivery failures.
