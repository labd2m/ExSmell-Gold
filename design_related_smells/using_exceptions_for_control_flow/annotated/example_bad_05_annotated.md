...
- **Expected smell location:** `Logistics.TrackingClient.fetch_status/1`
- **Affected function(s):** `Logistics.TrackingClient.fetch_status/1` (library side); `Logistics.ShipmentDashboard.load_statuses/1` (client side)
- **Explanation:** `fetch_status/1` raises `RuntimeError` for expected situations such as an unknown tracking number and a carrier timeout. Callers building a dashboard of multiple shipments have no way to recover from an individual failure without a `try/rescue` block, making exception handling the only available mechanism for normal tracking-lookup control-flow.
...
defmodule Logistics.TrackingClient do
  ...
  # VALIDATION: This is a smell because `fetch_status/1` raises RuntimeError for
  # VALIDATION: predictable, non-exceptional tracking lookup outcomes: invalid
  # VALIDATION: format, unsupported carrier, unknown tracking number, and carrier
  # VALIDATION: timeout. None of these represent a bug — they are expected runtime
  # VALIDATION: conditions. Callers must use try/rescue to handle them, losing the
  # VALIDATION: ability to treat them as ordinary {:error, reason} data.
