...
- **Expected smell location:** `Security.TokenValidator.validate/2`
- **Affected function(s):** `Security.TokenValidator.validate/2` (library side); `Security.ApiAuthPlug.call/2` (client side)
- **Explanation:** `validate/2` raises `RuntimeError` for expected token authentication outcomes: malformed token, unknown issuer, expired token, and invalid signature. In an API authentication plug, these are everyday occurrences on every unauthenticated or expired request. Callers have no structured result to pattern-match on and must use `try/rescue` simply to return the appropriate HTTP 401 response, making exceptions the sole mechanism for authentication control-flow.
...
defmodule Security.TokenValidator do
  ...
  # VALIDATION: This is a smell because `validate/2` raises RuntimeError for four
  # VALIDATION: entirely routine authentication outcomes: malformed token, untrusted
  # VALIDATION: issuer, expired token, and bad signature. On every API request with
  # VALIDATION: a missing, stale, or forged token, the authentication middleware
  # VALIDATION: must handle these as normal control-flow — not as crashes. Callers
  # VALIDATION: are given no {:error, reason} path and must rescue RuntimeError just
  # VALIDATION: to decide whether to return a 401 or proceed.
