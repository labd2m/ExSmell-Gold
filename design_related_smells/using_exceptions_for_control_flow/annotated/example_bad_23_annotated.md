...
- **Expected smell location:** `InventoryStore.reserve/3` (library) and `OrderFulfillment.allocate_items/1` (client)
- **Affected function(s):** `InventoryStore.reserve/3`, `OrderFulfillment.allocate_items/1`
- **Short explanation:** `InventoryStore.reserve/3` raises exceptions for out-of-stock and invalid SKU conditions, which are routine occurrences in an e-commerce inventory system, rather than providing an `{:ok, _}/{:error, _}` tuple. The client module `OrderFulfillment` is therefore forced to use `try...rescue` to handle stock checks during normal order processing.
...
defmodule InventoryStore do
  ...
  # VALIDATION: This is a smell because out-of-stock and unknown-SKU conditions
  # are normal, predictable business events during order fulfillment. Raising
  # exceptions for them — without an alternative tuple-returning version —
  # forces every caller to use try...rescue for routine inventory checks.
  ...
  def allocate_items(%{id: order_id, line_items: line_items} = order) do
    ...
    results =
      Enum.map(line_items, fn item ->
        # VALIDATION: SMELL START - Using exceptions for control-flow
        # VALIDATION: This is a smell because the client is forced into
