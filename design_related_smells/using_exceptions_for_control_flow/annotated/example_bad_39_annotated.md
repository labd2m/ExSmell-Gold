...
- **Expected smell location:** `PlanUpgradePolicy.evaluate/2` (library) and `SubscriptionService.upgrade/2` (client)
- **Affected function(s):** `PlanUpgradePolicy.evaluate/2`, `SubscriptionService.upgrade/2`
- **Short explanation:** `PlanUpgradePolicy.evaluate/2` raises exceptions for downgrade attempts, mid-cycle upgrades on locked accounts, and unsupported plan transitions — all of which are routine business decisions users encounter when changing their subscription tier. The absence of a tuple-based result forces `SubscriptionService.upgrade/2` to use `try...rescue` for everyday subscription management logic.
...
defmodule PlanUpgradePolicy do
  ...
  # VALIDATION: This is a smell because trying to downgrade, switching to the
  # same plan, or attempting an unsupported transition are completely ordinary
  # user actions on a plan-management page. These outcomes should be
  # distinguishable via result tuples, not by rescuing exceptions — but the
  # library provides no alternative.
...
defmodule SubscriptionService do
  ...
  def upgrade(subscription, to_plan_str) do
    ...
    # VALIDATION: SMELL START - Using exceptions for control-flow
    # VALIDATION: This is a smell because the user clicking "upgrade" on a
