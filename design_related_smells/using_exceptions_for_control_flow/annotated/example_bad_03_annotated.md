...
- **Expected smell location:** `Inventory.StockManager.reserve/3`
- **Affected function(s):** `Inventory.StockManager.reserve/3` (library side); `Inventory.OrderFulfillment.fulfill_items/2` (client side)
- **Explanation:** `reserve/3` raises `RuntimeError` when stock is insufficient or an SKU is unknown. These are routine, foreseeable inventory conditions that callers need to handle gracefully. By raising instead of returning `{:error, reason}`, the function forces clients into `try/rescue` blocks for what is fundamentally ordinary control-flow.
...
defmodule Inventory.StockManager do
  ...
  # VALIDATION: This is a smell because `reserve/3` raises RuntimeError for two
  # VALIDATION: completely expected inventory outcomes: unknown SKU and insufficient
  # VALIDATION: stock. Callers processing a list of order lines cannot distinguish
  # VALIDATION: individual line failures from a crash without wrapping each call in
  # VALIDATION: try/rescue, forcing exception-based control-flow.
