...
- **Expected smell location:** `SmsProvider.send/2` (library) and `AlertDispatcher.notify/2` (client)
- **Affected function(s):** `SmsProvider.send/2`, `AlertDispatcher.notify/2`
- **Short explanation:** `SmsProvider.send/2` raises exceptions for invalid phone numbers, opted-out recipients, and messages exceeding the character limit. These are common, expected conditions in any SMS notification system. Because no tuple-based alternative is offered, `AlertDispatcher.notify/2` must use `try...rescue` to route every notification attempt through standard delivery logic.
...
defmodule SmsProvider do
  ...
  # VALIDATION: This is a smell because opted-out phone numbers, invalid
  # formats, and messages that are too long are expected, routine states when
  # dispatching SMS alerts in bulk. Raising exceptions for them — without
  # offering {:ok, _}/{:error, _} — leaves the caller no choice but to
  # use try...rescue for what should be simple branching logic.
...
defmodule AlertDispatcher do
  ...
  def notify(phone_number, %{type: type, body: body} = _alert) do
    ...
    # VALIDATION: SMELL START - Using exceptions for control-flow
    # VALIDATION: This is a smell because the dispatcher regularly encounters
