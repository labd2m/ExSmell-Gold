# Annotated Example 40

- **Smell name:** Using exceptions for control-flow
- **Expected smell location:** `DeliveryStatusUpdater.record_event/2` (library) and `WarehouseWebhookHandler.handle/1` (client)
- **Affected function(s):** `DeliveryStatusUpdater.record_event/2`, `WarehouseWebhookHandler.handle/1`
- **Short explanation:** `DeliveryStatusUpdater.record_event/2` raises exceptions for duplicate events, unknown shipment IDs, illegal state transitions, and unrecognised event codes — all of which arrive routinely in a high-volume warehouse webhook stream. With no tuple-based alternative, `WarehouseWebhookHandler.handle/1` is forced to use `try...rescue` to decide how to acknowledge or reject each incoming webhook — a completely ordinary control-flow task.
...
defmodule DeliveryStatusUpdater do
  ...
  # VALIDATION: This is a smell because in a warehouse webhook stream,
  # duplicate events (at-least-once delivery), unknown event codes from
  # new carrier integrations, and stale out-of-order events are all
  # routine occurrences — not exceptional errors. Raising exceptions for
  # them forces every webhook consumer into try...rescue for what is
  # standard event-processing control flow.
...
defmodule WarehouseWebhookHandler do
  ...
  def handle(%{shipment_id: shipment_id, event: event} = _payload) do
    ...
    # VALIDATION: SMELL START - Using exceptions for control-flow
    # VALIDATION: This is a smell because duplicate events, stale events,
