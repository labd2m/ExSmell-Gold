...
- **Expected smell location:** `ReportExporter.to_csv/2`
- **Affected function(s):** `ReportExporter.to_csv/2`, `ReportingPipeline.export_monthly_report/3`
- **Short explanation:** `ReportExporter.to_csv/2` raises a `RuntimeError` for two perfectly normal reporting scenarios: an empty dataset and unrecognised column keys. These are expected pre-conditions that any scheduler or pipeline must handle gracefully. Because no `{:ok, csv} | {:error, reason}` path is offered, `ReportingPipeline.export_monthly_report/3` must rely on `try/rescue` for what should be straightforward conditional logic.
...
defmodule ReportExporter do
  ...
  # VALIDATION: This is a smell because to_csv/2 raises RuntimeError for predictable,
  # VALIDATION: non-exceptional business conditions: an empty report dataset is a
  # VALIDATION: legitimate outcome (e.g., a new customer with no transactions yet),
  # VALIDATION: and an unrecognised column key is a routine misconfiguration that
  # VALIDATION: callers should be able to handle without exceptions. Callers like
  # VALIDATION: ReportingPipeline are forced into try/rescue for normal control-flow.
