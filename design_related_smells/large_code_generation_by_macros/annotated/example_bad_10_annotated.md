# Annotated Example 10 — Large Code Generation by Macros

## Metadata

- **Smell name:** Large code generation by macros
- **Expected smell location:** `defmacro endpoint/3` inside `Api.RouterDSL`
- **Affected function(s):** `endpoint/3`
- **Short explanation:** The macro expands a large block of HTTP method validation, path format checks, middleware list checks, rate-limit validation, and module-attribute writes for every endpoint declaration. Every call to `endpoint/3` causes the compiler to expand and compile the whole body again rather than delegating to a helper function compiled once.
...
defmodule Api.RouterDSL do
  ...
  # VALIDATION: This is a smell because every call to endpoint/3 causes the
  # VALIDATION: compiler to expand validations for HTTP method, path pattern,
  # VALIDATION: controller atom, middleware list, rate limiting, and scope
  # VALIDATION: requirements, plus the @api_routes attribute write—all inline.
  # VALIDATION: This large body should instead be delegated to a plain
  # VALIDATION: function, compiled once, and called at each site.
