...
- **Expected smell location:** `defmacro define_role/2` inside `MyApp.UserManagement.RoleDSL`
- **Affected function(s):** `define_role/2` macro
- **Short explanation:** Every call to `define_role/2` expands a large `quote` block that checks argument types, validates permission lists, verifies parent-role consistency, guards against duplicate declarations, and writes to module attributes — all inline at the call site. This causes the compiler to produce a large repeated expansion for every role defined in the module.
...
defmodule MyApp.UserManagement.RoleDSL do
  ...
  # VALIDATION: This is a smell because each call to define_role/2 expands this
  # VALIDATION: entire block inline: name atom check, permissions list/atom
  # VALIDATION: validation, permission-atom-list content check, inherits option
  # VALIDATION: atom validation, description type check, deduplication guard,
  # VALIDATION: and struct construction. With many roles, the compiler repeats
  # VALIDATION: all of this code at each call site rather than dispatching once
