# Annotated Example 16 — Large Code Generation by Macros

## Metadata

- **Smell name:** Large code generation by macros
- **Expected smell location:** `defmacro defpermission/2` inside `Access.PermissionDSL`
- **Affected function(s):** `defpermission/2`
- **Short explanation:** The macro expands a large quoted body—resource type check, action check, conditions list validation, implicit-deny check, and audit log flag—inline for every permission declaration. Each use of the macro re-expands and re-compiles this entire block instead of delegating to a function that compiles once.
...
defmodule Access.PermissionDSL do
  ...
  # VALIDATION: This is a smell because defpermission/2 expands all validation
  # VALIDATION: for resource, action, effect, conditions, audit, and
  # VALIDATION: justification_required directly in the quoted block.
  # VALIDATION: Every one of the many permission declarations causes the
  # VALIDATION: compiler to expand and compile this code body separately,
  # VALIDATION: rather than delegating to a single helper function.
