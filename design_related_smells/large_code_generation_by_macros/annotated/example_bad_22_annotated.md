...
- **Expected smell location:** `defmacro notify_via/2` inside `MyApp.Notifications.ChannelRegistry`
- **Affected function(s):** `notify_via/2` macro
- **Short explanation:** Every call to `notify_via/2` expands a large `quote` block that performs module existence checks, callback verification, option validation, deduplication logic, and attribute writes entirely at the call site. The macro should delegate most of this work to a plain function, emitting only the minimal code needed in the `quote`.
...
defmodule MyApp.Notifications.ChannelRegistry do
  ...
  # VALIDATION: This is a smell because every invocation of notify_via/2 causes
  # VALIDATION: the compiler to expand and compile argument validation, priority
  # VALIDATION: enumeration checks, retry-value guards, module-callback
  # VALIDATION: verification, deduplication logic, and struct construction
  # VALIDATION: directly at the call site. With many channels, this means
  # VALIDATION: duplicating all of this code in the expanded bytecode for each
  # VALIDATION: call rather than delegating to a shared function.
