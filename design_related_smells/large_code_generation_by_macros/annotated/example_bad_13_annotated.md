# Annotated Example 13 — Large Code Generation by Macros

## Metadata

- **Smell name:** Large code generation by macros
- **Expected smell location:** `defmacro defevent/2` inside `Events.EventDSL`
- **Affected function(s):** `defevent/2`
- **Short explanation:** The macro places a large inline block—payload key validation, schema version check, topic format check, retention policy check, and module attribute registration—inside the quoted expression. Every event declaration re-expands and recompiles all of this code instead of delegating to a plain helper function compiled once.
...
defmodule Events.EventDSL do
  ...
  # VALIDATION: This is a smell because defevent/2 expands full validation
  # VALIDATION: logic—topic format, schema version, payload keys, retention
  # VALIDATION: policy, ordered flag, and description—inside the quoted block
  # VALIDATION: on every event declaration. All call sites re-expand and
  # VALIDATION: recompile this whole body instead of sharing a single
  # VALIDATION: compiled helper function.
