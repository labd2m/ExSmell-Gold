# Annotated Example 01 — Large Code Generation by Macros

## Metadata

- **Smell name:** Large code generation by macros
- **Expected smell location:** `defmacro define_invoice_type/2` inside `Billing.InvoiceTypes`
- **Affected function(s):** `define_invoice_type/2`
- **Short explanation:** The macro expands a large block of validation, state-machine logic, and attribute registration inline on every invocation. Each call site causes the compiler to expand and compile all of that code again, inflating compilation time and binary size instead of delegating the bulk of the work to a plain function.
...
defmodule Billing.InvoiceTypes do
  ...
  # VALIDATION: This is a smell because the macro expands a large body of
  # VALIDATION: validation, transition checks, and attribute writes on every
  # VALIDATION: call site. All of this code is duplicated by the compiler for
  # VALIDATION: each invocation instead of being delegated to a helper function.
