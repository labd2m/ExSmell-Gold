...
- **Expected smell location:** `defmacro payment_gateway/2` inside `MyApp.Payments.GatewayRegistry`
- **Affected function(s):** `payment_gateway/2` macro
- **Short explanation:** Every invocation of `payment_gateway/2` expands a large `quote` block performing module compilation checks, callback existence checks, currency list validation, fee structure guards, deduplication logic, and attribute writes at the call site. Delegating this work to a plain function would dramatically reduce the volume of code expanded per call.
...
defmodule MyApp.Payments.GatewayRegistry do
  ...
  # VALIDATION: This is a smell because every call to payment_gateway/2 expands
  # VALIDATION: this entire block: atom checks, Code.ensure_compiled!, callback
  # VALIDATION: presence checks for charge/2 and refund/2, currency list format
  # VALIDATION: validation, fee value checks, boolean flag validation,
  # VALIDATION: deduplication guard, and map construction. In a processor config
  # VALIDATION: module with several gateways, all of this code is duplicated
  # VALIDATION: at each call site instead of being shared via a function.
