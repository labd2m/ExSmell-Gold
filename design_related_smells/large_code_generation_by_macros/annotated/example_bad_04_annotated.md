# Annotated Example 04 — Large Code Generation by Macros

## Metadata

- **Smell name:** Large code generation by macros
- **Expected smell location:** `defmacro defchannel/2` inside `Notifications.ChannelDSL`
- **Affected function(s):** `defchannel/2`
- **Short explanation:** The macro inlines extensive argument validation (template key, retry policy, timeout, priority, enabled flag) and module-attribute writes for every notification channel declaration. The compiler expands and compiles this block on each call instead of once through a delegated function.
...
defmodule Notifications.ChannelDSL do
  ...
  # VALIDATION: This is a smell because defchannel/2 inlines validation for
  # VALIDATION: adapter, retry, timeout, priority, and template_key into the
  # VALIDATION: quoted block. Each of the many channel declarations causes the
  # VALIDATION: compiler to expand and compile all of this code again, instead
  # VALIDATION: of delegating to a function that is compiled once.
