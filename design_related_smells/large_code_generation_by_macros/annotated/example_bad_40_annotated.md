...
- **Expected smell location:** `defmacro pipeline_stage/2` inside `MyApp.Pipeline.StageDSL`
- **Affected function(s):** `pipeline_stage/2` macro
- **Short explanation:** Every call to `pipeline_stage/2` expands a large `quote` block that validates the stage name, processor module callbacks, concurrency option, timeout, error strategy, ordering, and deduplication — all inline. A data pipeline module that declares many stages forces the compiler to expand and compile this entire block at each call site instead of once inside a shared function.
...
defmodule MyApp.Pipeline.StageDSL do
  ...
  # VALIDATION: This is a smell because every call to pipeline_stage/2 expands
  # VALIDATION: this entire block inline: name atom check, processor compilation
  # VALIDATION: and process/1 callback check, concurrency positive-integer check,
  # VALIDATION: timeout integer check, on_error strategy enumeration check,
  # VALIDATION: max_retries dependency-on-on_error check, deduplication guard,
  # VALIDATION: and stage struct construction. Pipelines with many stages compile
  # VALIDATION: all of this code at each declaration site rather than delegating
  # VALIDATION: to a shared helper function.
