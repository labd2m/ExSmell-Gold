...
| **Expected smell location** | `defmacro item_type/2`, lines ~55–130                                                       |
| **Affected function(s)** | `item_type/2`                                                                                   |
| **Explanation**          | Every `item_type/2` call expands a `quote` block that performs atom validation on the name, validates the unit-of-measure against an allowlist, checks numeric option boundaries for `:reorder_threshold` and `:max_stock`, accumulates a module attribute, and generates three public functions (`item_TYPE_unit/0`, `item_TYPE_reorder_threshold/0`, `item_TYPE_storable?/1`). A warehouse catalogue with 20+ item types compiles all this validation and function-generation code 20+ times instead of delegating to a single plain function, unnecessarily enlarging the compiled artefact. |
...
defmodule MyApp.Inventory.CatalogueRegistry do
  ...
  # VALIDATION: This is a smell because every call to item_type/2 expands this entire quote
  # block into the calling module. The expansion performs: an atom check on the type name,
  # a unit-of-measure membership check against @valid_units, reorder_threshold extraction
  # with an integer-and-positive guard, max_stock extraction with a boundary guard relative
  # to reorder_threshold, a hazardous boolean check, a module-attribute accumulation, and
  # three complete function definitions (item_TYPE_unit/0, item_TYPE_reorder_threshold/0,
  # item_TYPE_storable?/1). A catalogue with 20 item types compiles all this logic 20 times
  # in the .beam instead of once inside a `__register_item_type__/3` plain function.
