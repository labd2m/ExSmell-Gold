# Annotated Example 09 — Large Code Generation by Macros

## Metadata

- **Smell name:** Large code generation by macros
- **Expected smell location:** `defmacro defrole/2` inside `UserManagement.RoleDSL`
- **Affected function(s):** `defrole/2`
- **Short explanation:** The macro places a large block of argument validation, permission scope checks, role hierarchy validation, and module-attribute registration directly in the quoted expression. Every role declaration causes the compiler to expand and compile this entire body again, whereas a plain helper function would be compiled only once.
...
defmodule UserManagement.RoleDSL do
  ...
  # VALIDATION: This is a smell because defrole/2 expands an extensive quoted
  # VALIDATION: block that validates label, parent, scopes list, UI visibility,
  # VALIDATION: assignable flag, and max_session_minutes on each call site.
  # VALIDATION: Each role declaration re-expands and recompiles all of this
  # VALIDATION: code instead of delegating the work to a plain function.
