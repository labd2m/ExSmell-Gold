...
- **Expected smell location:** `defmacro validates/2` inside `MyApp.Validation.SchemaDSL`
- **Affected function(s):** `validates/2` macro
- **Short explanation:** Every call to `validates/2` expands a substantial `quote` block with type checking, required-flag logic, format-regex validation, length-option guards, numeric-range checks, allowed-values enumeration, deduplication guards, and rule struct registration — all inline at the call site. The bulk of this work should be done in a function, not re-emitted as compiled code for every field declaration.
...
defmodule MyApp.Validation.SchemaDSL do
  ...
  # VALIDATION: This is a smell because each call to validates/2 expands the
  # VALIDATION: entire body inline: field atom check, allowed-types enumeration,
  # VALIDATION: required-flag boolean check, format-regex check, length option
  # VALIDATION: checks, min/max numeric range checks, allowed-values list check,
  # VALIDATION: deduplication guard, and rule struct construction. A schema with
  # VALIDATION: 20 fields causes 20 full expansions of this code at compile time
  # VALIDATION: rather than 20 lightweight calls to a shared function.
