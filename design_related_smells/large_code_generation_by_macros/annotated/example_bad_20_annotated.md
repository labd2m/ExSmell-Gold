...
- **Expected smell location:** `defmacro validates_line_item/2` inside `MyApp.Billing.InvoiceSchema`
- **Affected function(s):** `validates_line_item/2` macro
- **Short explanation:** The macro expands a large `quote` block on every invocation, inlining field-presence checks, numeric validations, currency assertions, and attribute registration at the call site. Any module that calls this macro dozens of times will have all of that validation logic duplicated in the compiled bytecode, bloating compilation time and output size.
...
defmodule MyApp.Billing.InvoiceSchema do
  ...
  # VALIDATION: This is a smell because every invocation of validates_line_item/2
  # VALIDATION: causes the compiler to expand and compile all of this inline
  # VALIDATION: validation logic — type resolution, option parsing, range checks,
  # VALIDATION: and the duplicate-rule guard — directly at the call site, rather
  # VALIDATION: than delegating to a helper function. A module calling this macro
  # VALIDATION: ten times generates ten full copies of this expanded code.
