# Annotated Example 17 — Large Code Generation by Macros

## Metadata

- **Smell name:** Large code generation by macros
- **Expected smell location:** `defmacro defwebhook/2` inside `Webhooks.WebhookDSL`
- **Affected function(s):** `defwebhook/2`
- **Short explanation:** The macro inlines extensive validation—signature algorithm, event type list checks, retry policy, delivery timeout, secret format, and failure threshold—inside the quoted block for every webhook endpoint declaration. Every call site re-expands and re-compiles this large body rather than delegating to a helper function compiled once.
...
defmodule Webhooks.WebhookDSL do
  ...
  # VALIDATION: This is a smell because defwebhook/2 expands all validation—
  # VALIDATION: endpoint URL, event types, signature algorithm, secret size,
  # VALIDATION: content type, retry count, timeout, failure threshold, and
  # VALIDATION: enabled flag—inside the quoted block. Each webhook declaration
  # VALIDATION: re-expands and recompiles this body instead of calling a
  # VALIDATION: plain helper function compiled once.
