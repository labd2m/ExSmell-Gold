...
- **Expected smell location:** `defmacro config_key/2` inside `MyApp.Config.AccessorDSL`
- **Affected function(s):** `config_key/2` macro
- **Short explanation:** Every call to `config_key/2` expands a large `quote` block inlining key validation, OTP-app existence checks, value-type enforcement, default-value type-consistency checks, required-flag handling, environment-overridability checks, deduplication guards, and public accessor function generation. A config module with many keys compiles this entire block for each key rather than delegating to a shared function.
...
defmodule MyApp.Config.AccessorDSL do
  ...
  # VALIDATION: This is a smell because every call to config_key/2 causes the
  # VALIDATION: Elixir compiler to expand this entire block at the call site:
  # VALIDATION: key atom check, type enumeration check, required boolean check,
  # VALIDATION: default value type-consistency check, allowed-values list check,
  # VALIDATION: numeric min/max guards, env_var string check, doc string check,
  # VALIDATION: deduplication guard, struct registration, and accessor-function
  # VALIDATION: definition. A config module with 20 keys compiles all of this
  # VALIDATION: code 20 times rather than once inside a shared helper.
