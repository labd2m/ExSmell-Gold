# Annotated Example 18 — Large Messages

| Field                  | Value                                                                        |
|------------------------|------------------------------------------------------------------------------|
| **Smell name**         | Large messages                                                               |
| **Expected location**  | `Payroll.TaxCalculationClient.calculate_all/2`                              |
| **Affected function(s)**| `calculate_all/2`, `handle_call/3` (GenServer)                             |
| **Explanation**        | The payroll runner assembles a full payroll run structure — covering every employee's compensation breakdown, benefit deductions, historical earnings for annualisation, and tax-residency details — and passes it synchronously to the `TaxCalculator` GenServer via `GenServer.call/2`. Because payroll runs execute under strict deadline windows (employees must be paid on time), the blocking induced by copying thousands of richly-structured employee records is particularly harmful: it delays the entire payroll cycle and leaves little slack for retries if downstream steps fail. |
...
defmodule Payroll.TaxCalculationClient do
  ...
  def calculate_all(calculator_pid, {run_id, period}) do
    ...
    # VALIDATION: This is a smell because `run` is a PayrollRun struct
    # containing 6 000 EmployeeRecord structs, each with a
    # CompensationBreakdown struct (holding two nested maps and a list of
    # 4 BenefitDeduction structs), a TaxResidency struct, a bank_details map,
    # a 12-element EarningsHistory list, and a custom_fields map. Sending
    # this entire, richly-nested graph through GenServer.call/2 forces a
    # complete deep copy of all terms from the client's heap into the
    # calculator's heap before any tax logic can run. Because GenServer.call
    # is synchronous, the client process is entirely blocked during the copy.
    # In payroll systems, where runs must complete before banking cut-off
    # times, this blocking directly threatens on-time payment delivery.
