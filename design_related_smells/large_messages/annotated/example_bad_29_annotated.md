...
| **Expected smell location** | `Ecommerce.CatalogPublisher.publish_to_search_indexer/1` |
| **Affected function(s)** | `publish_to_search_indexer/1` |
| **Short explanation** | The publisher loads the entire product catalogue—including full descriptions, variant matrices, image manifests, and attribute sets—and ships it to the search indexer process in one message. The catalogue can be very large, and each dispatch causes a full deep-copy that blocks the publisher. |
...
defmodule Ecommerce.CatalogPublisher do
  ...
  def publish_to_search_indexer(indexer_pid) do
    ...
    # VALIDATION: This is a smell because `products` is a list of 25,000 product
    # structs, each with 8 variants (containing image lists), 10 reviews
    # (with body text), an SEO metadata map, a description string, and related
    # ID lists. Sending this enormous structure to the indexer in one message
    # causes the BEAM to deep-copy potentially hundreds of megabytes, blocking
    # the CatalogPublisher process and causing the system to become unresponsive.
