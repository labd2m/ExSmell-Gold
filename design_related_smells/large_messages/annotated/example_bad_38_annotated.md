...
| **Expected smell location** | `Subscriptions.UsageSync.sync_to_billing_engine/1` |
| **Affected function(s)** | `sync_to_billing_engine/1` |
| **Short explanation** | The usage sync process loads every usage event for the current billing period—potentially millions of metered usage records across all tenants—and sends the full list to the billing engine GenServer in one message. This causes a very large heap-to-heap copy that blocks the sync process and can overflow the billing engine's mailbox. |
...
defmodule Subscriptions.UsageSync do
  ...
  def sync_to_billing_engine(billing_pid) do
    ...
    # VALIDATION: This is a smell because `records` is a list of 10,000
    # UsageRecord structs. Each record embeds a Subscription struct (with
    # metered feature and add-on lists), a list of 200 UsageEvent structs
    # (each with metadata maps), pre-computed period totals, and computed
    # charges. Sending this entire structure in one process message results
    # in a deep-copy of potentially hundreds of megabytes, blocking the
    # UsageSync process for a significant time and starving the billing
    # engine's mailbox with one enormous message.
