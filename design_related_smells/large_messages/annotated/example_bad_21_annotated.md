...
| **Expected smell location** | `Auth.SessionManager.replicate_snapshot/1` |
| **Affected function(s)** | `replicate_snapshot/1` |
| **Short explanation** | The function grabs the complete in-memory session table (potentially hundreds of thousands of session records, each with large permission sets and audit trails) and sends it in a single message to a standby replication process. This one message can be enormous, causing the sender to block for the entire duration of the heap-to-heap copy. |
...
defmodule Auth.SessionManager do
  ...
  def replicate_snapshot(standby_pid) do
    ...
    # VALIDATION: This is a smell because `sessions` is a map with up to 50,000
    # entries, each containing 30 permission structs and 15 audit entries (deeply
    # nested maps). Sending this entire structure to another process causes a
    # full deep-copy across process heaps, which can block the caller for a
    # significant amount of time and degrade system responsiveness under load.
