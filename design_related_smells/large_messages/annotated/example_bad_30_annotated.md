...
| **Expected smell location** | `AuditLog.Exporter.ship_to_archiver/2` |
| **Affected function(s)** | `ship_to_archiver/2` |
| **Short explanation** | The exporter drains the entire audit log buffer—potentially hundreds of thousands of log entries with full request/response blobs—and sends the list in one message to an archival process. The size of each entry (due to embedded request bodies and stack traces) makes this message extremely large, causing extended blocking of the sender. |
...
defmodule AuditLog.Exporter do
  ...
  def ship_to_archiver(archiver_pid, tenant_id) do
    ...
    # VALIDATION: This is a smell because `tenant_entries` can be a very large
    # list of Entry structs, each embedding full RequestContext (with header
    # list and body map) and ResponseContext structs, plus a multi-line stack
    # trace string and a metadata map. Sending all of these in one process
    # message forces the BEAM to deep-copy the entire structure, blocking the
    # Exporter process and potentially causing it to fall behind on incoming
    # audit events.
