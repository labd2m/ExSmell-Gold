# Annotated Example 08 — Large Messages

| Field                  | Value                                                                        |
|------------------------|------------------------------------------------------------------------------|
| **Smell name**         | Large messages                                                               |
| **Expected location**  | `UserManagement.ArchivalJob.enqueue/1`                                      |
| **Affected function(s)**| `enqueue/1`, `handle_info/2` (GenServer)                                   |
| **Explanation**        | The archival job fetches the entire user base — tens of thousands of user structs with rich profile data, permission sets, and activity summaries — and forwards the whole collection to the `ArchivalWorker` GenServer via `send/2`. The message copy is proportional to the total size of the user data; at scale this blocks the job process long enough to trigger timeout warnings and delay other scheduled jobs that share the same scheduler process. |
...
defmodule UserManagement.ArchivalJob do
  ...
  def enqueue(worker_pid) do
    ...
    # VALIDATION: This is a smell because `users` is a list of 40 000 User
    # structs. Each struct contains an Address, an ActivitySummary with a
    # feature_flags map, a list of 8 Permission structs, a custom_attributes
    # map, and a gdpr_consents map. Sending this entire collection to the
    # ArchivalWorker via `send/2` copies every byte of this data between
    # process heaps. The job process is blocked for the duration of the copy,
    # which — at this data size — can easily consume hundreds of milliseconds,
    # starving other scheduled jobs of their execution window.
