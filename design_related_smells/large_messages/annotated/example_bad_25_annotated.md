...
| **Expected smell location** | `Payments.ReconciliationServer.request_settlement_run/2` |
| **Affected function(s)** | `request_settlement_run/2` |
| **Short explanation** | The reconciliation server fetches a complete transaction ledger for a payment gateway (potentially hundreds of thousands of records) and sends the entire list to a settlement worker process in a single message, causing a very large heap-to-heap copy that blocks the server. |
...
defmodule Payments.ReconciliationServer do
  ...
  def request_settlement_run(settlement_pid, gateway_id) do
    ...
    # VALIDATION: This is a smell because `transactions` is a list that may
    # contain up to 80,000 Transaction structs, each with a payment_method map,
    # metadata map, and a list of 5 event structs. Sending this list in a single
    # process message forces the BEAM to deep-copy the entire structure to the
    # settlement worker's heap, potentially blocking the server for several
    # seconds and delaying other reconciliation work.
