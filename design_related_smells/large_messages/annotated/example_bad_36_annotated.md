...
| **Expected smell location** | `EventStore.ProjectionBuilder.rebuild_projection/2` |
| **Affected function(s)** | `rebuild_projection/2` |
| **Short explanation** | When rebuilding a projection from scratch, the builder loads the complete event stream for an aggregate—potentially hundreds of thousands of domain events each with full payload maps—and sends the entire list to a projection worker process in one message. This large message blocks the builder for the entire duration of the copy. |
...
defmodule EventStore.ProjectionBuilder do
  ...
  def rebuild_projection(projection_pid, stream_id) do
    ...
    # VALIDATION: This is a smell because `events` is a list of up to 60,000
    # DomainEvent structs. Each event has a variably-shaped payload map (some
    # with nested item lists and address maps) and an EventMetadata struct.
    # Sending the entire event log in one process message forces the BEAM to
    # deep-copy all of it across heap boundaries, blocking the ProjectionBuilder
    # process entirely during the copy, which in a busy system can starve other
    # streams from being rebuilt in a timely manner.
