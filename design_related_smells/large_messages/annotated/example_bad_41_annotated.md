...
- **Expected smell location:** `TrackingIngester.ingest/2` — the `send/2` call dispatching the full list of GPS event structs to the aggregator process
- **Affected function(s):** `TrackingIngester.ingest/2`, `TrackingAggregator.handle_info/2`
- **Short explanation:** A full batch of GPS tracking events — each containing coordinates, telemetry maps, and timestamps — is sent as a single message to another process. Copying this large list on every ingestion cycle blocks the ingester and can cause the aggregator mailbox to back up under high fleet load.
...
defmodule TrackingIngester do
  ...
  def ingest(aggregator_pid, vehicle_id) do
    ...
    # VALIDATION: This is a smell because the entire list of GPS event structs
    # (up to 20 000 per batch, each carrying coordinates, speed, heading,
    # and a nested telemetry map) is deep-copied into the aggregator process's
    # mailbox as one message. Copying tens of megabytes of data on every
    # ingestion cycle blocks the ingester process and degrades throughput when
    # many vehicles are reporting simultaneously.
