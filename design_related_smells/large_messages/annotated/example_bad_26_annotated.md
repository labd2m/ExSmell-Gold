...
| **Expected smell location** | `Scheduling.CalendarSync.push_bulk_update/2` |
| **Affected function(s)** | `push_bulk_update/2` |
| **Short explanation** | The sync module loads the full calendar event dataset for all employees across all rooms for the next quarter (thousands of events with attendee lists, recurrence rules, and metadata) and sends the entire structure to a consolidation worker as a single process message, causing a large and potentially blocking deep-copy. |
...
defmodule Scheduling.CalendarSync do
  ...
  def push_bulk_update(worker_pid, %Date{} = quarter_start) do
    ...
    # VALIDATION: This is a smell because `events` is a list of up to 7,200
    # CalendarEvent structs, each with 12 Attendee sub-structs, a
    # RecurrenceRule, a tag list, and a long description string. Sending
    # this entire list in a single message forces the BEAM to deep-copy the
    # full structure across process heaps, blocking the CalendarSync process
    # for the duration of the copy and adding latency to the pipeline.
