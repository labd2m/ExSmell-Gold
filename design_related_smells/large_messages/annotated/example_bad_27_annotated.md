...
| **Expected smell location** | `UserManagement.RoleAuditor.dispatch_full_user_list/1` |
| **Affected function(s)** | `dispatch_full_user_list/1` |
| **Short explanation** | The auditor loads the complete user roster—including all role assignments, group memberships, and activity history for every user—and sends the entire list as one message to the audit worker process. This single message can be very large when the organisation has tens of thousands of users, causing a prolonged heap-to-heap copy that blocks the auditor process. |
...
defmodule UserManagement.RoleAuditor do
  ...
  def dispatch_full_user_list(audit_worker_pid) do
    ...
    # VALIDATION: This is a smell because `users` is a list of up to 30,000
    # User structs, each containing 5 Role structs (with 10-permission lists),
    # 8 GroupMembership structs, and 20 ActivityRecord structs. Sending this
    # giant nested structure as a single message requires the BEAM to copy all
    # of it to the audit worker's heap, blocking the RoleAuditor process for
    # a significant amount of time and degrading system responsiveness.
