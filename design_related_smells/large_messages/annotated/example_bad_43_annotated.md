...
- **Expected smell location:** `ReconciliationRunner.run/2` — the `send/2` call that forwards the full list of transaction records to the ledger auditor process
- **Affected function(s):** `ReconciliationRunner.run/2`, `LedgerAuditor.handle_info/2`
- **Short explanation:** The complete list of transaction records for a reconciliation window — potentially hundreds of thousands of maps with nested breakdowns — is copied into the ledger auditor's mailbox in one `send/2` call. This blocks the runner process during the copy and, when scheduled every few minutes across multiple payment providers, causes sustained memory-copy pressure on the BEAM runtime.
...
defmodule ReconciliationRunner do
  ...
  def run(auditor_pid, provider) do
    ...
    # VALIDATION: This is a smell because the entire list of transaction records
    # (up to 80 000 maps, each containing amounts, fee breakdowns, metadata,
    # and timestamps) is deep-copied into the LedgerAuditor process mailbox as
    # one message. The runner process is blocked until the copy finishes.
    # Scheduled reconciliation jobs for multiple providers running concurrently
    # amplify the blocking and can cause VM-wide scheduling latency spikes.
