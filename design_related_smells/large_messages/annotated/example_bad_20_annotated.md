...
| **Expected smell location** | `Logistics.FleetCoordinator.broadcast_route_manifest/2` |
| **Affected function(s)** | `broadcast_route_manifest/2` |
| **Short explanation** | The coordinator collects every route, stop, and GPS waypoint for an entire fleet's daily manifest and sends the full nested map to each driver process in one message. This results in repeated large deep-copies across process heaps, one per driver, blocking the sender for a prolonged time. |
...
defmodule Logistics.FleetCoordinator do
  ...
  def broadcast_route_manifest(%Date{} = date, driver_pids) when is_list(driver_pids) do
    ...
    Enum.each(driver_pids, fn pid ->
      ...
      # VALIDATION: This is a smell because `manifest` is a large nested map
      # containing 120 routes, each with 35 stops (with packages) and 500
      # GPS waypoints. Sending the ENTIRE manifest to EVERY driver process
      # causes the full structure to be deep-copied once per driver process,
      # blocking the coordinator for a long time and creating excessive GC pressure.
