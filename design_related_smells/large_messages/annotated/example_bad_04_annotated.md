# Annotated Example 04 — Large Messages

| Field                  | Value                                                                        |
|------------------------|------------------------------------------------------------------------------|
| **Smell name**         | Large messages                                                               |
| **Expected location**  | `Notifications.CampaignDispatcher.broadcast/2`                              |
| **Affected function(s)**| `broadcast/2`, `handle_info/2`                                             |
| **Explanation**        | The `broadcast/2` function sends a message containing the full recipient list — a list of subscriber maps that can easily reach tens of thousands of entries — to a broadcaster process. Passing this large collection through `send/2` forces an expensive heap-to-heap copy, blocking the dispatcher process and creating backpressure that delays subsequent campaigns when many are queued. |
...
defmodule Notifications.CampaignDispatcher do
  ...
  def broadcast(broadcaster_pid, %Notifications.Campaign{} = campaign) do
    ...
    # VALIDATION: This is a smell because `subscribers` contains up to 30 000
    # Subscriber structs, each with nested maps, lists of device tokens, and
    # segment IDs. Sending this entire collection in a single `send/2` call
    # copies all of that data from the dispatcher process heap into the
    # Broadcaster's heap at once. This copy operation blocks the dispatcher
    # process, and when multiple campaigns are dispatched concurrently the
    # blocking periods overlap, causing the system to fall behind its
