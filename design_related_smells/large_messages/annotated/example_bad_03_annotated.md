# Annotated Example 03 — Large Messages

| Field                  | Value                                                                      |
|------------------------|----------------------------------------------------------------------------|
| **Smell name**         | Large messages                                                             |
| **Expected location**  | `Auth.AuditFlusher.flush/1`                                               |
| **Affected function(s)**| `flush/1`, `handle_cast/2` (GenServer)                                   |
| **Explanation**        | The `flush/1` function collects a large buffer of audit-log entries in the calling process and ships the entire list to the `AuditWriter` GenServer in one `GenServer.cast`. Each entry is a moderately-sized map including request headers, IP metadata, and session payloads. Sending thousands of such entries at once copies a large binary-heavy structure between processes, blocking the flusher process and delaying subsequent flushes. |
...
defmodule Auth.AuditFlusher do
  ...
  def flush(writer_pid) do
    ...
    # VALIDATION: This is a smell because `entries` is a list of thousands of
    # AuditEntry structs, each carrying multiple maps (request_headers,
    # geo_data) and binary strings. Casting the entire list at once copies
    # all of this data from the flusher's heap into the AuditWriter process's
    # heap, blocking the flusher for as long as the copy takes. Under peak
    # load — when auth events are generated at high frequency — this blocking
    # introduces measurable latency in the flush cycle.
