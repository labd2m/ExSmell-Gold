# Annotated Example 02 — Large Messages

| Field                  | Value                                                                      |
|------------------------|----------------------------------------------------------------------------|
| **Smell name**         | Large messages                                                             |
| **Expected location**  | `Logistics.DispatchCoordinator.assign_routes/2`                           |
| **Affected function(s)**| `assign_routes/2`, `handle_cast/2` (GenServer)                           |
| **Explanation**        | A complete snapshot of every route — including full waypoint lists, driver profiles, and vehicle telemetry — is assembled in the coordinator process and cast as a single message to each worker GenServer. Sending megabyte-sized maps through `GenServer.cast/2` copies the data between processes, blocking the coordinator and degrading throughput when many workers are active simultaneously. |
...
defmodule Logistics.DispatchCoordinator do
  ...
  def assign_routes(worker_pid, routes) do
    ...
    # VALIDATION: This is a smell because `routes` is a large list of fully
    # hydrated Route structs — each containing an 80-waypoint list, a
    # DriverProfile, and a VehicleTelemetry struct with nested maps. Casting
    # this entire structure to the worker process copies every byte of the
    # data into the worker's heap, blocking the coordinator process during the
    # copy and causing noticeable latency when many workers receive their
    # route assignments at dispatch time.
