# Annotated Example 05 — Large Messages

| Field                  | Value                                                                        |
|------------------------|------------------------------------------------------------------------------|
| **Smell name**         | Large messages                                                               |
| **Expected location**  | `Inventory.ReconciliationJob.run/1`                                         |
| **Affected function(s)**| `run/1`, `handle_call/3` (GenServer)                                       |
| **Explanation**        | The job process fetches a full warehouse inventory snapshot — a large list of SKU structs with nested stock-level and supplier information — and passes it via `GenServer.call/2` to the reconciler. Because `GenServer.call` is synchronous, the calling process is blocked until the message is copied, the server processes it, and the reply is sent back. With a snapshot of many thousands of SKUs, the copy alone adds significant latency to what is already a synchronous round-trip. |
...
defmodule Inventory.ReconciliationJob do
  ...
  def run(warehouse_id) do
    ...
    # VALIDATION: This is a smell because `snapshot` is a list of 15 000 SKU
    # structs, each with nested stock_levels and a list of three SupplierInfo
    # structs. Sending all of this via GenServer.call/2 triggers a full heap
    # copy from the job process into the Reconciler process before the call
    # can even be handled. Because GenServer.call is synchronous, the job
    # process is completely blocked during this copy, stalling the entire
    # reconciliation pipeline for the duration of the data transfer.
