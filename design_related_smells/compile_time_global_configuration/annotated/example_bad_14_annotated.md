...
- **Expected smell location:** Module attribute `@default_ttl_seconds` defined at the top of `Cache.RedisStore`
- **Affected function(s):** `put/3`, `get_or_fetch/3`, `invalidate_namespace/1`
- **Short explanation:** `Application.fetch_env!/2` is used in the module body to bind the default TTL to a module attribute. Module attributes are evaluated at compile-time; the `:cache` application configuration may not yet be loaded then, causing a compilation warning or `ArgumentError`.
...
defmodule Cache.RedisStore do
  ...
  # VALIDATION: module body to populate a module attribute. This call happens at
  # VALIDATION: compile-time, before the :cache OTP application is guaranteed to be
  # VALIDATION: loaded. Elixir may therefore raise a warning or an ArgumentError
