...
- **Expected Smell Location:** Module attribute `@min_password_length` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `validate_password/1`, `change_password/3`
- **Explanation:** `Application.fetch_env!/2` is called at compile-time to set `@min_password_length`. During compilation the `:user_management` application is not yet loaded, causing a potential `ArgumentError`. The minimum length becomes a compile-time constant, so tightening the policy in a deployed system requires recompiling rather than updating application configuration.
...
defmodule UserManagement.PasswordPolicy do
  ...
  # VALIDATION: during compilation. At that stage :user_management has not
  # VALIDATION: been loaded, so Elixir raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION:     :min_password_length for application :user_management
  # VALIDATION: The resolved integer is also frozen in the beam, meaning the
  # VALIDATION: policy cannot be tightened at runtime without recompilation.
