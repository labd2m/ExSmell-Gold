...
- **Expected Smell Location:** Module attribute `@smtp_host` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `deliver/1`, `test_connection/0`
- **Explanation:** `Application.fetch_env!/2` is called at compile-time to populate `@smtp_host` with the mail server hostname. Because `:mailer` has not been started during compilation, Elixir raises an `ArgumentError` or a compiler warning. The hostname is frozen in the bytecode, making runtime failover to a backup SMTP relay impossible without recompiling.
...
defmodule Mailer.SmtpRelay do
  ...
  # VALIDATION: at module-compilation time. The :mailer application is not yet
  # VALIDATION: loaded, so Elixir raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION:     :smtp_host for application :mailer
  # VALIDATION: The hostname binary is also baked into the .beam, preventing
  # VALIDATION: runtime failover to a backup relay without recompiling.
