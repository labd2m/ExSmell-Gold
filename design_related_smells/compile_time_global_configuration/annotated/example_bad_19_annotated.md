...
- **Expected smell location:** Module attribute `@webhook_signing_secret` defined at the top of `Webhooks.SignatureVerifier`
- **Affected function(s):** `verify_signature/3`, `sign_payload/1`
- **Short explanation:** `Application.fetch_env!/2` is invoked in the module body to read the webhook signing secret into a module attribute. Since module attributes are frozen during compilation and the `:webhooks` application configuration is only available at runtime, this can raise a warning or `ArgumentError` at compile-time.
...
defmodule Webhooks.SignatureVerifier do
  ...
  # VALIDATION: module body to assign the signing secret to a module attribute.
  # VALIDATION: Module attributes are evaluated at compile-time; if the :webhooks
  # VALIDATION: application has not been loaded by then, Elixir will emit a
  # VALIDATION: warning or raise ArgumentError during compilation.
