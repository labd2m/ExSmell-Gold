...
- **Expected smell location:** Module body, lines where `@storage_bucket`, `@report_ttl_days`, and `@max_rows_per_export` are defined
- **Affected function(s):** `export_sales_report/2`, `write_to_storage/2`, `build_metadata/2`
- **Short explanation:** Three module attributes are populated via `Application.fetch_env!/2` in the module body, which executes at compile time. Since the application environment is not loaded during compilation, this can produce a compiler warning or raise an `ArgumentError`, making the module fragile in environments where configuration is injected at runtime (e.g., releases, Docker containers). The values should be fetched at runtime inside the functions that use them.
...
defmodule Reporting.SalesExporter do
  ...
  # VALIDATION: This is a smell because Application.fetch_env!/2 is called in
  # VALIDATION: the module body at compile time, before the application
  # VALIDATION: environment is loaded. If the configuration keys are absent or
  # VALIDATION: the app is not yet started, an ArgumentError is raised (or a
  # VALIDATION: compiler warning is emitted), breaking the build. These reads
  # VALIDATION: should happen at runtime inside the functions that need them.
