...
- **Expected Smell Location:** Module attribute `@poll_interval_ms` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `init/1`, `handle_info/2` (the `:tick` handler)
- **Explanation:** `Application.fetch_env!/2` is invoked during compilation to set `@poll_interval_ms`. The application environment is not available at that stage, so Elixir may raise an `ArgumentError` or a warning. The value is frozen in the beam, so adjusting the interval in a running system is impossible without recompiling.
...
defmodule Scheduler.JobPoller do
  ...
  # VALIDATION: while the module is being compiled by the Elixir compiler.
  # VALIDATION: At that point :scheduler has almost certainly not been started,
  # VALIDATION: so the environment key is missing and Elixir raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION: The millisecond integer is also hard-coded into the .beam,
  # VALIDATION: so tuning the interval on a live node requires recompilation.
