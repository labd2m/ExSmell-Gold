...
- **Expected Smell Location:** Module attribute `@retention_days` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `purge_expired/0`, `record/3`
- **Explanation:** `Application.fetch_env!/2` is called at compile-time to read the audit log retention period into `@retention_days`. Because `:audit` has not been loaded during compilation, Elixir raises an `ArgumentError` or a warning. The integer is frozen in the bytecode, so operators cannot change the retention window at runtime to comply with updated data-retention policies without recompiling.
...
defmodule Audit.EventLogger do
  ...
  # VALIDATION: at module-compilation time. The :audit application is not yet
  # VALIDATION: loaded when the compiler processes this module, so Elixir
  # VALIDATION: raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION:     :retention_days for application :audit
  # VALIDATION: The integer is also baked into the .beam; updating the
  # VALIDATION: retention period to meet a new compliance requirement requires
  # VALIDATION: recompiling rather than just updating application config.
