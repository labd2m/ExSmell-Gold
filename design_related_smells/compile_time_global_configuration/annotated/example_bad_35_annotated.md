...
- **Expected Smell Location:** Module attribute `@bucket_name` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `upload/3`, `download/1`, `delete/1`, `presign_url/2`
- **Explanation:** `Application.fetch_env!/2` is called at compile-time to obtain the S3 bucket name. Because `:file_storage` has not been loaded during compilation, Elixir raises an `ArgumentError` or a compiler warning. The bucket name is frozen in the bytecode, preventing environment-specific bucket routing at runtime without recompiling.
...
defmodule FileStorage.S3Adapter do
  ...
  # VALIDATION: during module compilation, at which point the :file_storage
  # VALIDATION: application has not been started. Elixir raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION:     :s3_bucket for application :file_storage
  # VALIDATION: The bucket string is also baked into the .beam, making
  # VALIDATION: per-environment bucket routing impossible at runtime.
