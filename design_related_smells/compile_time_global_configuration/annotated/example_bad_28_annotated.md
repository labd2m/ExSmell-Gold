...
- **Expected Smell Location:** Module attribute `@default_warehouse_id` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `reserve_stock/3`, `release_reservation/2`, `adjust_quantity/3`
- **Explanation:** `Application.fetch_env!/2` is called at compile-time to capture the default warehouse identifier. Because the application environment is not available during compilation, this can raise an `ArgumentError` or produce a compiler warning. The warehouse ID becomes a compile-time constant, breaking multi-warehouse deployments that rely on runtime configuration.
...
defmodule Inventory.StockManager do
  ...
  # VALIDATION: This is a smell because Application.fetch_env!/2 executes
  # VALIDATION: at compile-time during module loading. If :inventory has not
  # VALIDATION: been loaded into the application environment yet, Elixir raises
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION:     :default_warehouse_id for application :inventory
  # VALIDATION: The string is also embedded in the .beam bytecode, preventing
  # VALIDATION: runtime reconfiguration in multi-warehouse deployments.
