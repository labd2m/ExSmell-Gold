...
- **Expected smell location:** Module attribute `@enabled_features` defined at the top of `FeatureFlags.Registry`
- **Affected function(s):** `enabled?/1`, `list_enabled/0`, `assert_enabled!/1`
- **Short explanation:** `Application.fetch_env!/2` is called inside the module body to assign the list of enabled features to a module attribute. Since module attributes are evaluated at compile-time and the Application Environment is not necessarily loaded at that point, this can produce a warning or `ArgumentError` during compilation.
...
defmodule FeatureFlags.Registry do
  ...
  # VALIDATION: module body to populate a module attribute with the list of enabled
  # VALIDATION: feature flags. Module attributes are resolved during compilation;
  # VALIDATION: the :feature_flags application may not be loaded at that point,
  # VALIDATION: causing Elixir to emit a warning or raise ArgumentError at compile-time.
