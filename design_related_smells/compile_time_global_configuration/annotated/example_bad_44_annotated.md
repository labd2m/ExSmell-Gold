...
- **Expected Smell Location:** Module attribute `@apns_topic` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `push/3`, `push_bulk/2`
- **Explanation:** `Application.fetch_env!/2` is called at compile-time to resolve the APNs topic (iOS app bundle identifier) and assign it to `@apns_topic`. Because `:push_notifications` has not been loaded when the module is compiled, Elixir raises an `ArgumentError` or a compiler warning. The topic string is frozen in the bytecode, so switching between development and production APNs environments at runtime is impossible without recompiling.
...
defmodule PushNotifications.ApnsAdapter do
  ...
  # VALIDATION: during module compilation. At that point :push_notifications
  # VALIDATION: has not been loaded into the application environment, so
  # VALIDATION: Elixir raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION:     :apns_topic for application :push_notifications
  # VALIDATION: The topic string (iOS bundle ID) is also frozen in the .beam,
  # VALIDATION: preventing sandbox/production routing at runtime.
