...
- **Expected smell location:** Module attribute `@sender_email` defined at the top of `Notifications.Mailer`
- **Affected function(s):** `send_welcome/1`, `send_password_reset/2`, `send_invoice/2`
- **Short explanation:** `Application.fetch_env!/2` is called in the module body to set `@sender_email`. This call happens at compile-time when the Application Environment is not guaranteed to be loaded, which can trigger a compilation warning or an `ArgumentError`.
...
defmodule Notifications.Mailer do
  ...
  # VALIDATION: This is a smell because Application.fetch_env!/2 is used inside the
  # VALIDATION: module body to set a module attribute. Module attributes are resolved
  # VALIDATION: during compilation. If the :notifications OTP application has not yet
  # VALIDATION: been loaded, Elixir will raise a warning or ArgumentError at compile-time.
