...
- **Expected smell location:** Module attribute `@pool_size` defined at the top of `Database.ConnectionPool`
- **Affected function(s):** `checkout/1`, `with_connection/2`, `pool_stats/0`
- **Short explanation:** `Application.fetch_env!/2` is called in the module body to populate the `@pool_size` module attribute. Module attributes are evaluated at compile-time; the `:database` Application Environment may not be available at that stage, which can trigger a warning or `ArgumentError` during compilation.
...
defmodule Database.ConnectionPool do
  ...
  # VALIDATION: module body, causing the pool size to be resolved from the Application
  # VALIDATION: Environment at compile-time. Module attributes are frozen during
  # VALIDATION: compilation; the :database application may not have been loaded by
  # VALIDATION: then, causing Elixir to emit a warning or raise ArgumentError.
