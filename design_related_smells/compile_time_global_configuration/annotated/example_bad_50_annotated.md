...
- **Expected smell location:** Module body, lines where `@base_rate_cents`, `@free_shipping_threshold_cents`, and `@carrier_api_timeout_ms` are defined
- **Affected function(s):** `calculate_rate/2`, `apply_discount/2`, `fetch_carrier_quote/2`
- **Short explanation:** The module attributes `@base_rate_cents`, `@free_shipping_threshold_cents`, and `@carrier_api_timeout_ms` are assigned by calling `Application.fetch_env!/2` in the module body at compile time. The application environment is not yet available when module attributes are evaluated, so this can raise an `ArgumentError` or produce a compiler warning. These values should be read inside the functions that require them, at runtime.
...
defmodule Logistics.ShippingRateCalculator do
  ...
  # VALIDATION: module body to populate module attributes. At compile time the
  # VALIDATION: OTP application has not been started, so the application
  # VALIDATION: environment is not available. This causes an ArgumentError or a
  # VALIDATION: compiler warning. The correct fix is to read these values at
  # VALIDATION: runtime from within the functions that need them.
