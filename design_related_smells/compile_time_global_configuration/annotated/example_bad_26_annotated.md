...
- **Expected Smell Location:** Module attribute `@default_sender` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `deliver_welcome/1`, `deliver_password_reset/2`, `deliver_invoice/3`
- **Explanation:** `Application.fetch_env!/2` is invoked at compile-time to populate `@default_sender`. Because the application environment is not yet available when the module is compiled, Elixir may emit a warning or raise an `ArgumentError`. The sender address is then fixed in the bytecode and cannot be changed at runtime without recompiling the module.
...
defmodule Notifications.EmailDispatcher do
  ...
  # VALIDATION: during compilation of this module. If the :notifications
  # VALIDATION: application has not been started yet (which is the normal state
  # VALIDATION: during Mix compilation), Elixir raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION: The resolved address is also frozen in the compiled beam,
  # VALIDATION: making runtime sender changes impossible without recompiling.
