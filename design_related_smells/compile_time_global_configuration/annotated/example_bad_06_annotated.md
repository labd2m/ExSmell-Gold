...
- **Expected smell location:** Module attribute `@twilio_account_sid` defined at the top of `Notifications.SMSDispatcher`
- **Affected function(s):** `send_otp/2`, `send_alert/2`, `send_delivery_update/3`
- **Short explanation:** `Application.fetch_env!/2` is called at module-body level to assign the Twilio account SID to a module attribute. Module attributes are evaluated during compilation; the Application Environment may not be loaded at that point, potentially causing a warning or `ArgumentError`.
...
defmodule Notifications.SMSDispatcher do
  ...
  # VALIDATION: module body to populate a module attribute. Module attributes are frozen
  # VALIDATION: at compile-time; if the :notifications application has not been fully
  # VALIDATION: loaded by then, Elixir raises a warning or ArgumentError at compile-time.
