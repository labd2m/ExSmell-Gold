...
- **Expected smell location:** Module attribute `@audit_service_url` defined at the top of `Audit.EventLogger`
- **Affected function(s):** `log_event/3`, `log_bulk/1`, `flush/0`
- **Short explanation:** `Application.fetch_env!/2` is called in the module body to set `@audit_service_url` as a module attribute. Module attributes are frozen at compile-time; the `:audit` OTP application configuration may not yet be loaded at that stage, causing Elixir to emit a warning or raise `ArgumentError` during compilation.
...
defmodule Audit.EventLogger do
  ...
  # VALIDATION: module body to set the audit service URL as a module attribute.
  # VALIDATION: Module attributes are resolved during compilation, but the :audit
  # VALIDATION: application configuration may not be available at that point,
  # VALIDATION: causing Elixir to raise a warning or ArgumentError at compile-time.
