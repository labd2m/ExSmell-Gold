...
- **Expected Smell Location:** Module attribute `@default_ttl_seconds` defined via `Application.fetch_env!/2` at the top of the module body
- **Affected Function(s):** `put/3`, `get_or_fetch/3`
- **Explanation:** `Application.fetch_env!/2` is called at compile-time to resolve the cache TTL and assign it to `@default_ttl_seconds`. Because `:cache` has not been loaded at compilation time, Elixir raises an `ArgumentError` or emits a warning. The TTL is frozen in the beam, so tuning cache expiry in production without recompiling is not possible.
...
defmodule Cache.LocalStore do
  ...
  # VALIDATION: at module-compilation time. If :cache has not been started
  # VALIDATION: (which is always the case during Mix compilation), Elixir
  # VALIDATION: raises:
  # VALIDATION:   ** (ArgumentError) could not fetch application environment
  # VALIDATION:     :default_ttl_seconds for application :cache
  # VALIDATION: Baking the TTL into the beam also prevents runtime tuning.
