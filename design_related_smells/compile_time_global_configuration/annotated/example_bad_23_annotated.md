...
- **Expected smell location:** Module attribute `@index_prefix` defined at the top of `Search.ElasticsearchClient`
- **Affected function(s):** `index_document/3`, `search/2`, `delete_document/2`, `bulk_index/2`
- **Short explanation:** `Application.fetch_env!/2` is used in the module body to read the Elasticsearch index prefix into a module attribute. Since module attributes are evaluated at compile-time and the `:search` application environment may not be loaded then, Elixir can raise a warning or `ArgumentError` during compilation.
...
defmodule Search.ElasticsearchClient do
  ...
  # VALIDATION: module body to bind the index prefix to a module attribute. Module
  # VALIDATION: attributes are resolved during compilation; the :search application
  # VALIDATION: may not have been loaded at that point, causing Elixir to emit a
  # VALIDATION: warning or raise ArgumentError at compile-time.
