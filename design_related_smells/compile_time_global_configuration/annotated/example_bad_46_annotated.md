...
- **Expected smell location:** Module body, lines where `@max_retry_attempts`, `@default_currency`, and `@charge_timeout_ms` are defined
- **Affected function(s):** `charge/2`, `retry_failed_charge/2`, `build_charge_request/3`
- **Short explanation:** The module attributes `@max_retry_attempts`, `@default_currency`, and `@charge_timeout_ms` are assigned via `Application.fetch_env!/2` at compile time. Because the application environment may not yet be loaded during compilation, this can trigger warnings or `ArgumentError` crashes at compile time, making the configuration fragile and environment-dependent during the build phase.
...
defmodule Billing.PaymentProcessor do
  ...
  # VALIDATION: application environment is not guaranteed to be loaded, so this
  # VALIDATION: can raise an ArgumentError or emit a compiler warning. The values
  # VALIDATION: should instead be read at runtime inside the functions that need them.
