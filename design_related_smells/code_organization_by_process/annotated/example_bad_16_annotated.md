...
- **Expected smell location**: `Scheduling.DateUtils` module
- **Affected function(s)**: `business_days_between/3`, `add_business_days/3`, `next_business_day/2`, `is_business_day?/2`
- **Short explanation**: Business-day calculations are pure date arithmetic based on a fixed set of weekend rules and a static holiday list. There is no shared state that changes at runtime. The `GenServer` is used purely to namespace these functions behind a process, causing all scheduling computations to serialize through one process when they could run freely in parallel as plain module functions.
...
defmodule Scheduling.DateUtils do
  ...
  # VALIDATION: This is a smell because DateUtils routes pure date arithmetic
  # VALIDATION: through a GenServer process. Holiday and business-day computation
  # VALIDATION: depends only on the input date and compile-time constants. No
  # VALIDATION: mutable state is needed between calls. SLA and scheduling services
  # VALIDATION: call these utilities frequently and concurrently; serializing all
  # VALIDATION: of them through one process is a bottleneck caused purely by
  # VALIDATION: organizing this logic as a process rather than a plain module.
