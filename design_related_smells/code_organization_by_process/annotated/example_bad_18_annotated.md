...
- **Expected smell location**: `Commerce.OrderSummarizer` module
- **Affected function(s)**: `build/2`, `line_items_display/2`, `status_label/2`, `fulfillment_eta/2`
- **Short explanation**: Building an order summary is a pure transformation of an order struct into a display-ready data structure, with no mutable state and no I/O. The `GenServer` state is unused beyond initialization. High-traffic order confirmation pages would call these functions thousands of times per minute; serializing them through one process is a pure architectural bottleneck with no benefit.
...
defmodule Commerce.OrderSummarizer do
  ...
  # VALIDATION: This is a smell because OrderSummarizer uses a GenServer solely
  # VALIDATION: to organize order-display-formatting logic. The process state is
  # VALIDATION: an empty map that participates in no operation. All computation
  # VALIDATION: is pure: label lookups in compile-time constant maps, date
  # VALIDATION: arithmetic, and struct-to-map transformations. The order
  # VALIDATION: confirmation flow would be severely throttled by funneling
  # VALIDATION: every summary build through this one serialized process.
