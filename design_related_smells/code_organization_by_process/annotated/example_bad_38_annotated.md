...
| **Expected smell location** | `TokenParser` module — entire GenServer structure |
| **Affected function(s)** | `parse/2`, `verify_expiry/2`, `extract_claims/2`, `audience_valid?/3` |
| **Short explanation** | JWT parsing and claim verification are pure operations on a token string and a secret/config. No state changes between calls, no resource is shared, and no scheduling is needed. Wrapping these operations in a GenServer creates a single-process bottleneck for every authenticated request in the system. |
...
defmodule Auth.TokenParser do
  ...
  # VALIDATION: This is a smell because JWT parsing is a pure, stateless
  # cryptographic operation. The process holds no shared mutable state —
  # its state is always %{}. Every API request that requires authentication
  # must wait in the GenServer's message queue. Under high traffic this single
  # process becomes a serious bottleneck with no offsetting benefit.
