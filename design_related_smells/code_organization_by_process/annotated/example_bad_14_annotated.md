...
- **Expected smell location**: `Reporting.SalesAggregator` module
- **Affected function(s)**: `by_period/3`, `by_product/2`, `by_region/2`, `top_n/3`
- **Short explanation**: Sales report aggregation is a pure transformation of in-memory data structures with no side effects and no shared state. The `GenServer` process carries no state between calls. Encoding these aggregation operations as `handle_call` clauses instead of plain functions means every concurrent report generation request blocks on this one process, creating an unnecessary bottleneck in the reporting pipeline.
...
defmodule Reporting.SalesAggregator do
  ...
  # VALIDATION: This is a smell because SalesAggregator wraps pure, stateless
  # VALIDATION: data aggregation logic inside a GenServer. The process state is
  # VALIDATION: an empty map never consulted in any handle_call. Aggregation is
  # VALIDATION: entirely functional: it transforms a list of transactions passed
  # VALIDATION: in on each call. Report generation requests from multiple
  # VALIDATION: dashboard users serialize through this one process unnecessarily.
