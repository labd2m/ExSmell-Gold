...
| **Expected smell location** | `GrossPayCalculator` module — entire GenServer structure |
| **Affected function(s)** | `calculate/2`, `overtime_pay/3`, `bonus_amount/3`, `annual_to_period/3` |
| **Short explanation** | Gross pay computation is a pure function of an employee's compensation profile and a pay period record. No state is mutated or accumulated between payroll runs, no external resource is locked, and no scheduling is required. Routing every payroll calculation through a single GenServer serialises what could be fully parallel per-employee computations. |
...
defmodule Payroll.GrossPayCalculator do
  ...
  # VALIDATION: This is a smell because gross pay calculation is a pure,
  # side-effect-free computation. Given an employee profile and a pay period
  # record, the functions apply arithmetic rules and return a result. The
  # GenServer holds no meaningful state between calls (always %{}). During
  # a payroll run that processes hundreds or thousands of employees, routing
  # all calculations through a single process mailbox introduces a serial
  # bottleneck where concurrent execution would be trivially safe.
