...
- **Expected smell location**: `Payments.FeeCalculator` module
- **Affected function(s)**: `processing_fee/3`, `net_payout/3`, `fee_breakdown/3`, `compare_methods/3`
- **Short explanation**: Payment processing fee computation is a deterministic formula applied to amount and payment method—purely arithmetic on a static fee schedule. No mutable state is required, and no resource needs protecting. Routing all payment fee queries through one `GenServer` creates a bottleneck at the point in checkout where fee display is needed for every transaction.
...
defmodule Payments.FeeCalculator do
  ...
  # VALIDATION: This is a smell because FeeCalculator routes pure fee arithmetic
  # VALIDATION: through a GenServer process. The fee schedules are a compile-time
  # VALIDATION: constant map; the process state holds this map but never updates it.
  # VALIDATION: All four operations are deterministic formulas on their input
  # VALIDATION: arguments. The checkout flow queries fee information for every
  # VALIDATION: payment attempt; serializing all of these through one process
  # VALIDATION: creates a bottleneck with no concurrency or statefulness benefit.
