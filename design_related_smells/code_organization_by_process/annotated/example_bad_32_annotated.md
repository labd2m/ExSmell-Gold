...
| **Expected smell location** | `ShippingCalculator` module — entire GenServer structure |
| **Affected function(s)** | `calculate/2`, `cheapest_option/2`, `estimated_delivery/3` |
| **Short explanation** | Shipping cost computation is a pure function of package dimensions, weight, and destination zone. No runtime state is maintained between calls, no I/O is performed, and no concurrency primitive is needed. Wrapping these calculations in a GenServer forces all requests to serialise through one process unnecessarily. |
...
defmodule Logistics.ShippingCalculator do
  ...
  # VALIDATION: This is a smell because every public function computes a
  # shipping cost or delivery estimate from pure inputs, with no shared state
  # and no need for process-level concurrency. All calls are serialised through
  # a single GenServer, which can become a bottleneck when many orders are
  # processed simultaneously at peak periods.
