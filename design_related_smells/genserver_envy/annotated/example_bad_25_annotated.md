...
- **Expected smell location:** `OnboardingAgent` module — `Agent` orchestrating multi-step onboarding workflows
- **Affected function(s):** `start_onboarding/2`, `advance_step/2`, `complete_step/3`
- **Short explanation:** Onboarding workflows involve step sequencing, external service calls (email, provisioning, HR systems), and status transitions — orchestration responsibilities that should live in a `GenServer`.
...
defmodule MyApp.OnboardingAgent do
  ...
  # VALIDATION: This is a smell because the Agent orchestrates a complex, multi-step
  # onboarding process: it manages workflow state machines, triggers external
  # provisioning and HR systems, sends notification emails at each transition,
  # logs audit trails, and persists step results. This coordinated, side-effectful
  # workflow management exceeds the Agent's intended purpose of shared state access
  # and belongs in a GenServer.
