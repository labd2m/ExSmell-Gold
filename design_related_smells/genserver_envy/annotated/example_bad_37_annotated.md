...
- **Expected smell location:** `ContractNegotiationAgent` module — `Agent` orchestrating contract lifecycle workflows
- **Affected function(s):** `submit_proposal/3`, `counter_propose/3`, `finalise/2`
- **Short explanation:** Contract negotiation involves validating offer terms, routing to legal reviewers, generating documents, and transitioning through a negotiation state machine — orchestration that belongs in a `GenServer`.
...
defmodule MyApp.ContractNegotiationAgent do
  ...
  # VALIDATION: This is a smell because the Agent orchestrates a full contract
  # negotiation lifecycle: it validates term completeness, submits proposals to
  # external legal review queues, sends email notifications to counter-parties,
  # enforces round limits, generates final contract documents, and manages a
  # multi-step state machine. This coordinated, side-effectful business logic
  # belongs in a GenServer, not in an Agent designed only to share state.
