# Annotated Bad Example 46

- **Smell name:** GenServer Envy
- **Expected smell location:** `PaymentLedger` module — `Agent`-based process
- **Affected functions:** `reconcile_transactions/1`, `generate_settlement/1`, `flag_suspicious/2`
- **Short explanation:** While the `Agent` is correctly used to maintain a shared transaction ledger, `reconcile_transactions/1`, `generate_settlement/1`, and `flag_suspicious/2` execute complex, isolated business logic entirely for the benefit of the calling process. They perform no state mutation other than bookkeeping flags and should live in a plain module or `GenServer` that separates state from computation.
...
defmodule PaymentLedger do
  ...
  # VALIDATION: This is a smell because reconcile_transactions/1,
  # generate_settlement/1, and flag_suspicious/2 run isolated computations
  # (reconciliation math, settlement aggregation, anomaly detection with
  # side-effectful status updates) inside Agent callbacks. These tasks benefit
  # only the calling process and introduce heavy work into the Agent's
  # serialized execution queue, blocking all other accesses. This
  # pattern belongs in a GenServer or a plain functional module.
