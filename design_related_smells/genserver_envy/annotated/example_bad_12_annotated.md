...
- **Expected smell location:** `WebhookDeliveryTask` — `Task` acting as a persistent delivery manager
- **Affected function(s):** `start_delivery_manager/1`, `delivery_loop/1`
- **Short explanation:** This `Task` runs a persistent receive loop that queues, retries, and tracks webhook deliveries across many requests and responses. It has the full structure of a `GenServer`, not a single-shot async task.
...
defmodule MyApp.WebhookDeliveryTask do
  ...
  def start_delivery_manager(config) do
    ...
    # persistent webhook delivery manager. The process maintains a retry queue,
    # handles multiple message types with replies, tracks delivery history, and
    # coordinates retries with backoff — all of which are GenServer responsibilities.
    # A Task should only be used to run a single asynchronous computation once.
    Task.start_link(fn ->
      state = %{
        config: config,
        pending: :queue.new(),
