...
- **Expected smell location:** `PromoCodeAgent` — `Agent` enforcing promotion business rules
- **Affected function(s):** `redeem/3`, `create_promo/2`, `expire_promo/1`
- **Short explanation:** Promotion redemption involves eligibility checks, usage limits, discount calculations, and audit logging — business logic that should live in a `GenServer`, not an `Agent`.
...
defmodule MyApp.PromoCodeAgent do
  ...
  # VALIDATION: This is a smell because the Agent implements full promotion
  # redemption workflows — including eligibility validation, per-user and
  # global usage limit enforcement, discount calculation, database persistence,
  # and audit logging. An Agent should only expose shared state; this level of
  # orchestrated business logic belongs in a GenServer.
