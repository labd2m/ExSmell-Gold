...
- **Expected smell location:** `ApprovalWorkflowAgent` module — `Agent` orchestrating document approval processes
- **Affected function(s):** `submit_for_approval/3`, `approve/3`, `reject/3`
- **Short explanation:** Approval workflows involve routing to reviewers, sending notification emails, tracking multi-stage approvals, and enforcing business rules — workflow orchestration that belongs in a `GenServer`.
...
defmodule MyApp.ApprovalWorkflowAgent do
  ...
  # VALIDATION: This is a smell because the Agent orchestrates a full multi-stage
  # document approval process: it determines review routing via a policy engine,
  # advances approval stages, sends email notifications to reviewers and submitters,
  # persists decisions, and transitions workflow status through a state machine.
  # This coordinated, side-effectful business logic is the domain of a GenServer,
  # not an Agent whose purpose is simply to share state.
