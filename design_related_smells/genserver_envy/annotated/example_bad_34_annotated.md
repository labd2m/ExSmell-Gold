...
- **Expected smell location:** `LoadBalancerTask` — `Task` acting as a stateful load balancer server
- **Affected function(s):** `start_balancer/1`, `balancer_loop/1`
- **Short explanation:** A load balancer must handle route requests, track backend health, drain nodes for maintenance, and update registrations — a persistent, multi-client server that is the domain of `GenServer`, not `Task`.
...
defmodule MyApp.LoadBalancerTask do
  ...
  def start_balancer(backends) do
    ...
    # load balancer that handles route/register/deregister/drain requests from many
    # callers, maintains backend health state, tracks failure counts, and sends
    # replies. Routing decisions require synchronous request/reply semantics with
    # shared state across all clients — precisely what GenServer models. A Task is
    # only for isolated one-shot computations.
    Task.start_link(fn ->
      indexed =
        backends
        |> Enum.with_index()
