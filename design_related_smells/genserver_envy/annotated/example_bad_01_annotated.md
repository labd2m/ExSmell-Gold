...
- **Expected smell location:** `BillingAgent` module — `Agent` used beyond shared-state purposes
- **Affected function(s):** `process_charge/2`, `retry_failed_charges/1`, `send_receipt/2`
- **Short explanation:** An `Agent` is meant solely for sharing state between processes. Here it is also used to orchestrate multi-step billing logic, retry loops, and side-effectful receipt dispatch — responsibilities that belong in a `GenServer`.
...
defmodule MyApp.BillingAgent do
  ...
  # VALIDATION: This is a smell because an Agent is being used to run complex,
  # multi-step business logic (charge processing, retries, receipt emails) that
  # requires orchestrating side effects and communicating results. An Agent should
  # only share state; this behavior belongs in a GenServer.
