...
- **Expected smell location:** `MetricsPipelineTask` — `Task` acting as a persistent metrics aggregation server
- **Affected function(s):** `start_pipeline/1`, `pipeline_loop/1`
- **Short explanation:** A metrics pipeline must accept ingestion from many sources, maintain rolling windows, answer query requests, and flush to storage on schedule — a multi-client stateful server that belongs in a `GenServer`.
...
defmodule MyApp.MetricsPipelineTask do
  ...
  def start_pipeline(config) do
    ...
    # metrics aggregation server. The process receives data points from many
    # concurrent producers, maintains rolling window state per metric key,
    # answers ad-hoc query requests, triggers anomaly alerts, and handles
    # periodic flush to a time-series store. This sustained multi-client
    # interaction with diverse message types and shared state is exactly what
    # GenServer is designed for, not Task.
    Task.start_link(fn ->
      state = %{
        config: config,
        windows: %{},
