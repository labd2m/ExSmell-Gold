...
- **Expected smell location:** `CircuitBreakerTask` — `Task` acting as a persistent circuit breaker server
- **Affected function(s):** `start_breaker/2`, `breaker_loop/1`
- **Short explanation:** A circuit breaker must track failure counts, manage state transitions (closed/open/half-open), respond to probe requests, and enforce timeouts — a long-lived, stateful server that should be a `GenServer`.
...
defmodule MyApp.CircuitBreakerTask do
  ...
  def start_breaker(service_name, config \\ %{}) do
    ...
    # VALIDATION: This is a smell because a Task is used to implement a persistent
    # circuit breaker server. The process maintains state machine transitions
    # (closed/open/half-open), handles allow?/record_success/record_failure
    # requests from many callers with replies, manages timers for half-open
    # transitions, and sends alerts. A circuit breaker is a canonical long-lived
    # server with multi-client request/reply semantics — precisely what GenServer
    # models. A Task should only perform a single async computation.
    Task.start_link(fn ->
      state = %{
