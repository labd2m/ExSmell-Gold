...
- **Expected smell location:** `LoyaltyPointsAgent` module — `Agent` executing loyalty program business rules
- **Affected function(s):** `earn_points/3`, `redeem_points/3`, `expire_points/1`
- **Short explanation:** Loyalty program management involves tier upgrades, expiry policy enforcement, redemption validation, and external notifications — coordinated business logic that belongs in a `GenServer`.
...
defmodule MyApp.LoyaltyPointsAgent do
  ...
  # VALIDATION: This is a smell because the Agent implements a full loyalty points
  # management system: it applies earning multipliers per tier, upgrades customer
  # tiers based on lifetime point accumulation, enforces redemption minimums,
  # calculates expiry windows, persists ledger entries, and sends email notifications
  # at each transition. This complex, side-effectful orchestration exceeds the
  # Agent's intended role of sharing state and belongs in a GenServer.
