...
- **Expected smell location:** `PaymentAgent` module — `Agent` executing multi-step payment workflows
- **Affected function(s):** `initiate_payment/3`, `capture_payment/2`, `refund/3`
- **Short explanation:** Payment workflows involve side effects, external calls, and complex state transitions — all patterns that belong in a `GenServer`. The `Agent` here far exceeds its simple state-sharing purpose.
...
defmodule MyApp.PaymentAgent do
  ...
  # VALIDATION: This is a smell because the Agent is used to orchestrate complex
  # multi-step payment workflows that include external API calls (fraud scoring,
  # gateway), conditional branching, and error handling. These are general-purpose
  # server responsibilities, not simple shared-state access that defines an Agent's
  # intended purpose.
