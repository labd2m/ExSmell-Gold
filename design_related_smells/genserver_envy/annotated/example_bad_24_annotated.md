...
- **Expected smell location:** `MessageBrokerTask` — `Task` acting as a persistent pub/sub broker
- **Affected function(s):** `start_broker/1`, `broker_loop/1`
- **Short explanation:** A publish/subscribe broker must maintain subscriber registries, route messages, handle subscribe/unsubscribe requests, and reply to publishers — a long-lived server with client interactions that should be a `GenServer`.
...
defmodule MyApp.MessageBrokerTask do
  ...
  def start_broker(config) do
    ...
    # VALIDATION: This is a smell because a Task is used to implement a
    # full publish/subscribe message broker. The process maintains subscriber
    # registries, queues messages per topic, handles subscribe/unsubscribe/publish
    # requests from many callers, sends delivery confirmations, and manages
    # slow-consumer backpressure. This persistent, multi-client server pattern
    # with diverse message types is precisely what GenServer exists for;
    # a Task should only perform a single isolated async computation.
    Task.start_link(fn ->
      state = %{
        config: config,
        subscriptions: %{},
