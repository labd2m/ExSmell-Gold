...
- **Expected smell location:** `JobSchedulerTask` module — `Task`-based process
- **Affected functions:** `start_scheduler/1`, internal `scheduler_loop/2`
- **Short explanation:** A `Task` is intended for a single async execution. Here the task runs an indefinite scheduling loop, accepts messages to dynamically add or cancel jobs, and replies to status queries — all patterns that define a `GenServer`, not a `Task`.
...
defmodule JobSchedulerTask do
  ...
  def start_scheduler(initial_jobs \\ []) do
    Task.start(fn ->
      ...
      # VALIDATION: This is a smell because scheduler_loop/2 makes the Task
      # persist indefinitely, accept add/cancel/status messages from other
      # processes, and manage evolving state across ticks. A Task should run
      # one async action and terminate. State-holding, multi-message-type
      # communication loops are the responsibility of a GenServer.
