# Annotated Bad Example 41

- **Smell name:** GenServer Envy
- **Expected smell location:** `NotificationWorker` module — `Task`-based process
- **Affected functions:** `send_and_track/3`, `query_status/1`, `retry_failed/1`
- **Short explanation:** A `Task` is meant for fire-and-forget async actions. Here the task is kept alive and actively receives messages via `send/2` + `receive` to query its status and trigger retries, turning it into an ad-hoc `GenServer` without the proper structure.
...
defmodule NotificationWorker do
  ...
  def send_and_track(notification, caller_pid, opts \\ []) do
    ...
    Task.start(fn ->
      ...
      # VALIDATION: This is a smell because the Task enters a receive loop to
      # handle incoming messages from other processes — querying state and
      # triggering retries. A Task should only execute an async action and
      # terminate; persistent message-passing belongs in a GenServer.
