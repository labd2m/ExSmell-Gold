...
- **Expected smell location:** `CartAgent` module — `Agent` running shopping cart business logic
- **Affected function(s):** `add_item/3`, `apply_discount/3`, `checkout/2`
- **Short explanation:** Cart management involves pricing calculations, discount application, stock validation, and order submission — coordinated business workflows that belong in a `GenServer`, not in an `Agent` whose role is simply to share state.
...
defmodule MyApp.CartAgent do
  ...
  # VALIDATION: This is a smell because the Agent executes full shopping cart
  # business workflows: stock validation via external service calls, dynamic
  # pricing lookups, tax computation, discount enforcement, and order submission.
  # These multi-step side-effectful operations are far beyond the simple
  # state-sharing purpose of an Agent and belong in a GenServer.
