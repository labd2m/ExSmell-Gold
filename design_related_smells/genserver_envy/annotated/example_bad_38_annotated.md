...
- **Expected smell location:** `ABTestingTask` — `Task` acting as a persistent experiment assignment server
- **Affected function(s):** `start_experiment_server/1`, `experiment_loop/1`
- **Short explanation:** An A/B test server must handle variant assignment for many users, track exposure counts, enforce traffic caps, and respond to result queries — a long-lived interactive server that belongs in a `GenServer`.
...
defmodule MyApp.ABTestingTask do
  ...
  def start_experiment_server(config) do
    ...
    # A/B testing server. It handles variant assignment, exposure tracking,
    # result logging, and experiment lifecycle commands (pause/resume/conclude)
    # from many concurrent callers with reply semantics. This is a stateful
    # server with a diverse client API — precisely GenServer's purpose.
    # A Task is designed for a single one-shot async operation.
    Task.start_link(fn ->
      experiments =
        config.experiments
