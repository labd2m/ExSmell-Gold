...
- **Expected smell location:** `LockManagerTask` — `Task` acting as a persistent distributed lock server
- **Affected function(s):** `start_lock_manager/1`, `lock_manager_loop/1`
- **Short explanation:** A lock manager must handle acquire/release requests, enforce TTLs, queue waiters, and sweep expired locks — a server with persistent multi-client request/reply that belongs in a `GenServer`.
...
defmodule MyApp.LockManagerTask do
  ...
  def start_lock_manager(config \\ %{}) do
    ...
    # lock management server: it handles acquire/release/query requests from
    # many concurrent callers, manages a waiter queue for each resource,
    # enforces TTL expiry, runs periodic sweeps, and sends back replies.
    # This is a textbook GenServer use case — a stateful server with client
    # request/response semantics. A Task should only perform a single
    # isolated async computation, not serve as a multi-client process server.
    Task.start_link(fn ->
      state = %{
