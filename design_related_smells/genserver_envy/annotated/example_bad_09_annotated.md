...
- **Expected smell location:** `SchedulerAgent` module — `Agent` managing job scheduling workflows
- **Affected function(s):** `schedule_job/3`, `tick/0`, `cancel_job/1`
- **Short explanation:** Scheduling involves timing logic, job dispatch to workers, and state coordination — server-level orchestration that should live in a `GenServer`, not an `Agent`.
...
defmodule MyApp.SchedulerAgent do
  ...
  # VALIDATION: This is a smell because the Agent executes job scheduling logic:
  # it receives timer ticks, evaluates which jobs are due, dispatches them to
  # worker pools, and tracks run history. This orchestration — especially
  # responding to process messages and coordinating side effects — is
  # characteristic of a GenServer, not the simple state-sharing role of an Agent.
