# Annotated Bad Example 48

- **Smell name:** GenServer Envy
- **Expected smell location:** `ShipmentRegistry` module — `Agent`-based process
- **Affected functions:** `compute_optimal_route/1`, `estimate_delivery_window/2`, `generate_dispatch_manifest/1`
- **Short explanation:** The `Agent` legitimately stores shared shipment state, but `compute_optimal_route/1`, `estimate_delivery_window/2`, and `generate_dispatch_manifest/1` run isolated, CPU-bound computations exclusively for the benefit of the calling process. They do not share their results with other processes and occupy the Agent's serialized callback queue unnecessarily. These belong in a plain module or a `GenServer`.
...
defmodule ShipmentRegistry do
  ...
  # VALIDATION: This is a smell because compute_optimal_route/1,
  # estimate_delivery_window/2, and generate_dispatch_manifest/1 perform
  # isolated computations inside Agent callbacks. Their results are produced
  # solely for the calling process and are of no interest to other processes
  # sharing this Agent's state. Running these inside the Agent's serialized
  # callback queue blocks concurrent state access and crosses into GenServer
  # (or plain-module) territory, where computation and state management are
  # properly separated.
