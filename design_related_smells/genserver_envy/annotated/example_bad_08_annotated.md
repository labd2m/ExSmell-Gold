...
- **Expected smell location:** `ReportWorkerTask` — `Task` acting as a multi-request report server
- **Affected function(s):** `start_worker/1`, `worker_loop/1`
- **Short explanation:** Rather than performing a single async computation, this `Task` runs a receive loop handling report requests, status polls, and cancellations — a server pattern that should use `GenServer`.
...
defmodule MyApp.ReportWorkerTask do
  ...
  def start_worker(config) do
    ...
    # VALIDATION: This is a smell because Task.start_link is used to create a
    # long-running process that receives and processes many different message types,
    # maintains state (queue, results cache), and sends replies to callers.
    # This is a GenServer pattern implemented inside a Task, which is only
    # intended for isolated, one-shot asynchronous computations.
    Task.start_link(fn ->
      state = %{
        config: config,
        queue: :queue.new(),
