...
- **Expected smell location:** `DeviceRegistryAgent` module — `Agent` orchestrating device provisioning workflows
- **Affected function(s):** `provision_device/3`, `send_command/3`, `decommission/2`
- **Short explanation:** IoT device provisioning involves credential generation, firmware targeting, command dispatch, and telemetry tracking — server-level orchestration that exceeds an `Agent`'s state-sharing purpose.
...
defmodule MyApp.DeviceRegistryAgent do
  ...
  # VALIDATION: This is a smell because the Agent executes a full IoT device
  # lifecycle: it generates TLS certificates via a certificate authority,
  # registers devices with an MQTT broker, targets firmware versions, dispatches
  # commands to remote devices, and decommissions certificates. These multi-step,
  # side-effectful workflows involving external services are exactly the kind of
  # server-side orchestration that belongs in a GenServer, not in an Agent whose
  # purpose is limited to sharing state between processes.
