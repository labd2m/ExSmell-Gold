...
- **Expected smell location:** `ConnectionPoolMonitorTask` — `Task` acting as a persistent pool health monitor
- **Affected function(s):** `start_monitor/1`, `monitor_loop/1`
- **Short explanation:** This `Task` maintains a connection registry, handles check-in/check-out requests from multiple callers, monitors health, and sends back replies — a persistent server role that `GenServer` is built for.
...
defmodule MyApp.ConnectionPoolMonitorTask do
  ...
  def start_monitor(config) do
    ...
    # VALIDATION: This is a smell because the Task implements a persistent
    # connection pool monitor: it tracks individual connections, handles
    # checkout/checkin requests from many callers, enforces timeout policies,
    # runs periodic health checks, and sends responses. This multi-client
    # server pattern with mutable state is the responsibility of a GenServer,
    # not a Task — which should perform only a single async computation.
    Task.start_link(fn ->
      conns =
