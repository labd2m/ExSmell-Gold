...
- **Expected smell location:** `TenantConfigAgent` module — `Agent` running config validation and deployment logic
- **Affected function(s):** `update_config/3`, `deploy_config/2`, `rollback_config/2`
- **Short explanation:** Configuration management involves schema validation, versioning, deployment coordination, and rollback — server-level orchestration that an `Agent` is not designed for.
...
defmodule MyApp.TenantConfigAgent do
  ...
  # VALIDATION: This is a smell because the Agent performs full configuration
  # lifecycle management: it validates config schemas against tenant constraints,
  # creates versioned snapshots, deploys to active config, sends admin notifications,
  # and handles rollback to previous versions. These multi-step operations with
  # external calls and complex state transitions belong in a GenServer, not in
  # an Agent intended only to share state between processes.
