# Annotated Bad Example 47

- **Smell name:** GenServer Envy
- **Expected smell location:** `OrderProcessorTask` module — `Task`-based process
- **Affected functions:** `start_for_order/2`, internal `processor_loop/1`
- **Short explanation:** The `Task` is started once per order but never completes its work — it sits in a message loop waiting for commands (`apply_coupon`, `add_item`, `confirm`, `cancel`) from the controller layer. This is a stateful, multi-step protocol that a `GenServer` is explicitly designed for.
...
defmodule OrderProcessorTask do
  ...
  def start_for_order(order_id, customer_id) do
    ...
    Task.start(fn ->
      ...
      # VALIDATION: This is a smell because processor_loop/1 makes the Task
      # idle between messages and handle a multi-command protocol (add_item,
      # apply_coupon, confirm, cancel) with stateful replies. The Task holds
      # evolving order state and communicates with multiple callers over its
      # entire lifetime. All of this is GenServer territory; a Task should
      # run a single async operation and exit.
