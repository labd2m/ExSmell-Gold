...
- **Expected smell location:** `ReplicationAgent` module — `Agent` orchestrating multi-region data sync
- **Affected function(s):** `replicate/3`, `confirm_replica/3`, `trigger_repair/2`
- **Short explanation:** Replication management involves fanning out writes to remote regions, tracking acknowledgements, detecting divergence, and triggering repair jobs — distributed coordination that should be in a `GenServer`.
...
defmodule MyApp.ReplicationAgent do
  ...
  # VALIDATION: This is a smell because the Agent orchestrates a full multi-region
  # replication workflow: it fans out writes to remote region clients, tracks
  # acknowledgements per replica, detects replication lag and divergence, sends
  # alert notifications, and schedules repair jobs via external services.
  # This coordinated, side-effectful multi-step orchestration far exceeds the
  # simple state-sharing purpose of an Agent and belongs in a GenServer.
