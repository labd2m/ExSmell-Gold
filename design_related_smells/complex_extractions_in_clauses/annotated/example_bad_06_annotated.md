# Annotated Example 06 — Complex Extractions in Clauses
...
| **Expected location**  | `Inventory.AdjustmentHandler.apply/1`                                                        |
| **Affected function**  | `apply/1`                                                                                     |
| **Short explanation**  | Each clause head binds `reason` (for clause selection) and `delta` (for the guard), along with `adjustment_id`, `sku`, `warehouse_id`, `operator_id`, and `notes` — five fields used only inside the function body. Across four clauses, this forces the reader to parse up to seven field bindings per clause just to understand what controls dispatching, which is only `reason` and `delta`. |
...
defmodule Inventory.AdjustmentHandler do
  ...
  # VALIDATION: This is a smell because `adjustment_id`, `sku`, `warehouse_id`,
  # `operator_id`, and `notes` are destructured in the head of all four clauses
  # but none of them participate in clause selection or guard evaluation. Only
  # `reason` is matched against a literal atom to pick the clause, and `delta`
  # appears in the guards. A reader trying to understand the branching logic
  # must sift through seven bindings per clause to locate the two that actually
  # control flow.
