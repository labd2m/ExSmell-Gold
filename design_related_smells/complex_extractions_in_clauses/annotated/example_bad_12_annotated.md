# Annotated Example 12 — Complex Extractions in Clauses
...
| **Expected location**  | `Warehouse.ReceivingDock.accept_delivery/1`                                                        |
| **Affected function**  | `accept_delivery/1`                                                                                |
| **Short explanation**  | The clause dispatch logic is determined exclusively by `condition` (atom matching) and `quantity` (guard), yet the function head of every clause also extracts `delivery_id`, `purchase_order_id`, `supplier_id`, `sku`, and `dock_id` — five bindings that are never referenced in any guard or pattern match, and are only used inside the bodies. With four clauses and seven extractions per head, a developer needs to read the full pattern of every clause to identify the two bindings that actually drive dispatch. |
...
defmodule Warehouse.ReceivingDock do
  ...
  # VALIDATION: This is a smell because `delivery_id`, `purchase_order_id`,
  # `supplier_id`, `sku`, and `dock_id` are all extracted in the function head
  # of every clause despite playing no role in determining which clause fires.
  # Clause selection depends solely on `condition`, and `quantity` is used in
  # the guards. Across four clauses, each carrying seven field bindings, the
  # two meaningful dispatch bindings are buried among five body-only bindings,
  # significantly reducing the readability of the receiving logic.
