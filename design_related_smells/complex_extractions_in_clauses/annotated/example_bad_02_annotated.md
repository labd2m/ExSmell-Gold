# Annotated Example 02 — Complex Extractions in Clauses
...
| **Expected location**  | `Auth.TokenValidator.validate/1`                                                         |
| **Affected function**  | `validate/1`                                                                             |
| **Short explanation**  | Each clause head destructures `token_type` (used for clause selection), `expires_at` (used in the guard), and also `token_id`, `user_id`, `scopes`, and `issued_by` which are never referenced in the clause expression or guard — they are only used inside the body. Reading across three clauses, it requires careful inspection to determine which extracted fields influence dispatch versus which are passive body bindings. |
...
defmodule Auth.TokenValidator do
  ...
  # VALIDATION: This is a smell because `token_id`, `user_id`, `scopes`, and
  # `issued_by` are all extracted in the function heads of every clause, yet
  # none of them participate in pattern matching selection or guard evaluation.
  # Only `token_type` determines which clause fires, and `expires_at` feeds the
  # guard. The remaining four bindings exist purely for body use. Across three
  # clauses, this creates visual noise that hides the actual dispatch logic.
