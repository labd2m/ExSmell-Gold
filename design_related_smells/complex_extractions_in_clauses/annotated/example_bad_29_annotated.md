...
- **Expected smell location**: `dispatch_notification/1`, all three clauses
- **Affected function(s)**: `dispatch_notification/1`
- **Short explanation**: Each clause of `dispatch_notification/1` extracts `id`, `recipient_id`, `subject`, `body`, and `metadata` from `%Notification{}` for body usage only. Guards rely solely on `channel` and `priority`. This pattern, repeated across three clauses with identical exhaustive extractions, obscures which fields drive clause selection and which are just needed for the notification payload.
...
defmodule Notifications.Dispatcher do
  ...
  # VALIDATION: This is a smell because `id`, `recipient_id`, `subject`, `body`, and `metadata`
  # VALIDATION: are extracted in every clause for use only in the body. Only `channel` and
  # VALIDATION: `priority` are used in guards to select the clause. Repeating all five body-only
  # VALIDATION: bindings across three clauses makes clause dispatch intent hard to read.
