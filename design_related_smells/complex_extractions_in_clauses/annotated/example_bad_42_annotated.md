...
- **Expected smell location**: `Billing.DiscountEngine.apply_discount/1` — all three clauses
- **Affected function(s)**: `apply_discount/1`
- **Explanation**: Each clause of `apply_discount/1` destructures the `%Order{}` struct and binds many fields directly in the function signature. Only `status` and `total` drive the guard expressions; `customer_id`, `promo_code`, `items`, `placed_at`, and `order_ref` are bound there solely for use inside the function body. Across three clauses, this mixed intent makes it hard to determine at a glance which bindings participate in clause selection and which are just data pulled in for convenience.
...
defmodule Billing.DiscountEngine do
  ...
  # VALIDATION: This is a smell because `status` and `total` drive the guard
  # expressions, while `customer_id`, `promo_code`, `items`, `placed_at`, and
  # `order_ref` are extracted in the same signature purely for use in the body.
  # Repeating this mixed-purpose extraction across every clause makes it
  # progressively harder to reason about clause selection.
