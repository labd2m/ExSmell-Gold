# Annotated Example 04 — Complex Extractions in Clauses
...
| **Expected location**  | `Notifications.Dispatcher.dispatch/1`                                                          |
| **Affected function**  | `dispatch/1`                                                                                   |
| **Short explanation**  | The function uses `channel` for clause selection and `priority` for the guard, but also destructures `notification_id`, `recipient`, `subject`, `body`, and `metadata` — all of which are only referenced inside the function bodies. Across three clauses, each with seven field extractions, identifying which bindings drive dispatch versus which are body-only requires careful per-clause analysis. |
...
defmodule Notifications.Dispatcher do
  ...
  # VALIDATION: This is a smell because `notification_id`, `recipient`,
  # `subject`, `body`, and `metadata` are extracted in the function head of
  # every clause even though they play no role in selecting which clause
  # executes. Only `channel` drives clause selection and `priority` feeds the
  # guard. A reader scanning three clauses with seven extractions each must
  # mentally filter out five body-only bindings to understand the dispatch
  # logic, which significantly reduces readability.
