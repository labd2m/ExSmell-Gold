...
- **Expected smell location:** `Scheduling.AppointmentBook.confirm_appointment/1`
- **Affected function(s):** `confirm_appointment/1`
- **Explanation:** Each of the three clauses of `confirm_appointment/1` destructures
  eight fields from the `%Appointment{}` struct in the function head (`type`,
  `duration_minutes`, `patient_id`, `provider_id`, `preferred_date`, `location`,
  `notes`, `insurance_id`), but only `type` is used in pattern matching to select
  the clause. The remaining seven fields are used only inside the function bodies.
  Across three clauses this means 21 pattern extractions in function heads, the vast
  majority of which serve no routing purpose, severely obscuring which condition
...
defmodule Scheduling.AppointmentBook do
  ...
  # VALIDATION: This is a smell because confirm_appointment/1 has three clauses each
