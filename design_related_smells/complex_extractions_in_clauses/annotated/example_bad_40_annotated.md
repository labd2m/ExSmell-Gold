# Annotated Example 40 — Complex Extractions in Clauses
...
| **Expected location**  | `Tax.FilingProcessor.process/1`                                                                    |
| **Affected function**  | `process/1`                                                                                        |
| **Short explanation**  | The function selects a clause based on `filing_type` and guards on `gross_income`, but every clause head also binds `filing_id`, `taxpayer_id`, `tax_year`, `deductions`, and `withholdings` — five fields consumed only inside the bodies. Four clauses with seven extractions each means the actual dispatch conditions (`filing_type` and `gross_income`) are hidden in plain sight among body-only bindings. |
...
defmodule Tax.FilingProcessor do
  ...
  # VALIDATION: This is a smell because `filing_id`, `taxpayer_id`, `tax_year`,
