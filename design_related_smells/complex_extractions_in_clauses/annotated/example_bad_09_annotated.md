# Annotated Example 09 — Complex Extractions in Clauses
...
| **Expected location**  | `UserManagement.ProfileUpdater.apply_update/2`                                                   |
| **Affected function**  | `apply_update/2`                                                                                  |
| **Short explanation**  | The function dispatches via `role` (first argument, clause selection) and `account_status` (guard). However, it also extracts `user_id`, `email`, `display_name`, `locale`, and `preferences` in every clause head — fields that never appear in guards and are used only inside the bodies. With three clauses and seven per-clause extractions the reader cannot quickly see what actually drives the branching. |
...
defmodule UserManagement.ProfileUpdater do
  ...
  # VALIDATION: This is a smell because `user_id`, `email`, `display_name`,
  # `locale`, and `preferences` are bound in the function head of every clause
  # but serve no role in clause selection or guard evaluation. The first
  # argument pattern matches on `role` to select the clause, and the guard
  # checks `account_status`. The five extra bindings in each head are purely
  # for body convenience, making it hard to see at a glance that only two
  # bindings actually drive dispatch.
  def apply_update(
        %UserManagement.User{
          user_id: user_id,
          email: email,
