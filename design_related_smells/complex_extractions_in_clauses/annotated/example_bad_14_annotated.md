# Annotated Bad Example 14

**Smell name:** Complex extractions in clauses
**Expected smell location:** `Billing.InvoiceProcessor.process_invoice/1`
**Affected functions:** `process_invoice/1`
**Explanation:** Every clause of `process_invoice/1` destructures six fields from the `Invoice` struct — `id`, `status`, `amount`, `customer_id`, `due_date`, and `currency` — directly in the function head. However, only `status` and `amount` are ever referenced in the guard expressions. The remaining four fields (`id`, `customer_id`, `due_date`, `currency`) are extracted in the clause head purely for convenience inside the function body. Across four clauses, this mixture of guard-oriented and body-only extractions makes it hard to determine at a glance which bindings drive the dispatching logic and which are incidental.
...
defmodule Billing.InvoiceProcessor do
  ...
  # VALIDATION: This is a smell because all four clauses destructure `id`, `status`,
  # `amount`, `customer_id`, `due_date`, and `currency` from the Invoice struct in
  # the function head. Only `status` and `amount` are needed by the guard expressions.
  # `id`, `customer_id`, `due_date`, and `currency` are extracted solely for use
  # inside the function body, mixing guard-oriented and body-only bindings in every
  # clause head and making the dispatching intent harder to read at a glance.
