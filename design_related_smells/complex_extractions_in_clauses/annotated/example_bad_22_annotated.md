...
- **Expected smell location:** `schedule_job/1` function, multi-clause heads
- **Affected function(s):** `schedule_job/1`
- **Short explanation:** All clause heads destructure `%Job{}` extracting `type`, `priority`, `run_at`, `job_id`, `payload`, and `owner_id`. Only `type` (pattern match) and `priority` (guard) control clause selection. `run_at`, `job_id`, `payload`, and `owner_id` are purely body-only bindings that needlessly appear in every single clause head, blurring the distinction between dispatch logic and body usage.
...
defmodule Scheduler.JobDispatcher do
  ...
  # VALIDATION: This is a smell because `run_at`, `job_id`, `payload`, and
