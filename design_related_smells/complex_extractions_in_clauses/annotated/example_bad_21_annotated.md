...
- **Expected smell location:** `process_transaction/1` function, multi-clause heads
- **Affected function(s):** `process_transaction/1`
- **Short explanation:** The clause heads extract `method`, `amount`, `currency`, `transaction_id`, `account_id`, and `metadata` from `%Transaction{}` in every clause. Only `method` is used for structural matching and `amount` for a guard; `currency`, `transaction_id`, `account_id`, and `metadata` are body-only bindings. The reader must scan through all extractions in every head just to discover what actually controls dispatch.
...
defmodule Payments.TransactionProcessor do
  ...
  # VALIDATION: This is a smell because `currency`, `transaction_id`,
