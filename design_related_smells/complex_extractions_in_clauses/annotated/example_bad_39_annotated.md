# Annotated Example 39 — Complex Extractions in Clauses
...
| **Expected location**  | `Pharmacy.PrescriptionValidator.validate/1`                                                        |
| **Affected function**  | `validate/1`                                                                                       |
| **Short explanation**  | Clause selection is controlled by `drug_schedule` and the guard uses `days_supply`, but the function head also extracts `prescription_id`, `patient_id`, `prescriber_id`, `drug_name`, and `quantity` across all three clauses — five bindings that are never consulted for dispatch and only appear in the body. With three clauses and seven extractions per head, the dispatch logic is needlessly obscured by the body-only bindings. |
...
defmodule Pharmacy.PrescriptionValidator do
  ...
  # VALIDATION: This is a smell because `prescription_id`, `patient_id`,
