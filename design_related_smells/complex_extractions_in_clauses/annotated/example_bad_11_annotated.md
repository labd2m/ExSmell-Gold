# Annotated Example 11 — Complex Extractions in Clauses
...
| **Expected location**  | `Subscriptions.PlanChanger.apply_change/1`                                                         |
| **Affected function**  | `apply_change/1`                                                                                    |
| **Short explanation**  | Each clause head extracts `current_plan` and `target_plan` (used for clause selection via literal atom matching) and `billing_cycle_end` (used in the guard), but also eagerly binds `subscription_id`, `account_id`, `seats`, `billing_interval`, and `coupon_code` — five fields only consumed inside the function bodies. Across three clauses with eight extractions each, the reader must mentally discard five body-only bindings per clause to understand the actual dispatch logic. |
...
defmodule Subscriptions.PlanChanger do
  ...
  # VALIDATION: This is a smell because `subscription_id`, `account_id`,
  # `seats`, `billing_interval`, and `coupon_code` are destructured in every
  # clause head but never appear in the guard or influence clause selection.
  # Only the combination of `current_plan` and `target_plan` atoms selects the
  # clause, while `billing_cycle_end` feeds the guard. The five body-only
  # bindings spread across three clauses make it non-trivial to quickly grasp
  # the branching logic from the function signatures alone.
