...
- **Expected smell location:** `change/0` and `assign_default_tax_rates/0`, `tax_rate_for/2`
- **Affected functions:** `change/0`, `assign_default_tax_rates/0`, `tax_rate_for/2`
- **Short explanation:** This migration adds `tax_rate` and `tax_category` columns to `vendors` (structural change) and then applies region-based default tax rates to every existing vendor row (data manipulation). Encoding tax policy logic inside a migration conflates infrastructure concerns with business rules, making the migration harder to test and the business logic harder to evolve independently.
...
defmodule Marketplace.Repo.Migrations.AddTaxFieldsToVendors do
  ...
  def change do
    ...
    # VALIDATION: This is a smell because the migration reads all vendor rows and
    # applies a state-based tax rate lookup, writing the results back to the vendors
    # table. Encoding and executing tax policy logic inside a migration is data
    # manipulation; this logic belongs in a dedicated Mix task.
