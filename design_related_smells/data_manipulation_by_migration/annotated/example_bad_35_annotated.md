# Code Smell Annotation

- **Smell name:** Data manipulation by migration
- **Expected smell location:** `change/0` function
- **Affected function(s):** `change/0`, `populate_display_names/0`, `format_display_name/1`
- **Short explanation:** The migration adds a `display_name` column to `user_profiles` and then queries all rows to derive and write a formatted display name from the existing `username` field. Building and persisting derived presentation values from existing data is a data manipulation responsibility that belongs in a Mix task.

---

```elixir
defmodule Identity.Repo.Migrations.AddDisplayNameToUserProfiles do
  use Ecto.Migration

  import Ecto.Query
  alias Identity.Repo

  def change do
    alter table("user_profiles") do
      add :display_name,           :string, null: true
      add :display_name_source,    :string, default: "username", null: false
      add :display_name_updated_at, :utc_datetime, null: true
    end

    create index("user_profiles", [:display_name])

    flush()

    # VALIDATION: SMELL START - Data manipulation by migration
    # VALIDATION: This is a smell because the migration calls populate_display_names/0,
    # which fetches all user_profile rows, derives a formatted display_name
    # from the username field, and writes it back to the database. Generating
    # and persisting derived presentation data for existing rows is a data
    # manipulation step that should not live inside an Ecto.Migration module;
    # it should be a separate Mix task.
    populate_display_names()
    # VALIDATION: SMELL END

    alter table("user_profiles") do
      modify :display_name, :string, null: false
    end
  end

  defp populate_display_names do
    now = DateTime.utc_now() |> DateTime.truncate(:second)

    rows =
      from(p in "user_profiles",
        select: %{id: p.id, username: p.username}
      )
      |> Repo.all()

    Enum.each(rows, fn %{id: id, username: username} ->
      display_name = format_display_name(username)

      from(p in "user_profiles", where: p.id == ^id)
      |> Repo.update_all(
        set: [display_name: display_name, display_name_updated_at: now]
      )
    end)
  end

  defp format_display_name(username) when is_binary(username) do
    username
    |> String.replace("_", " ")
    |> String.replace("-", " ")
    |> String.split()
    |> Enum.map(&String.capitalize/1)
    |> Enum.join(" ")
  end

  defp format_display_name(_), do: "User"
end
```
