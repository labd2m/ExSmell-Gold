...
- **Expected smell location:** `parse_event_type/1` function
- **Affected function(s):** `parse_event_type/1`
- **Short explanation:** The function receives an event type string from an external webhook payload and converts it directly to an atom using `String.to_atom/1`. Because webhook event types originate from a third-party system, this is an unbounded, user-controlled input that can generate an arbitrary number of atoms, exhausting the BEAM atom table over time.
...
defmodule Billing.WebhookProcessor do
  ...
  # value that comes directly from an external webhook payload. An attacker or
  # a misbehaving third-party service can send arbitrary strings, creating
  # unbounded atoms in the BEAM VM and eventually exhausting the atom table.
