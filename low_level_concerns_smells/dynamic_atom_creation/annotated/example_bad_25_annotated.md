...
| **Expected smell location** | `WorkflowEngine.transition/3`, line where `String.to_atom/1` converts the target state name string |
| **Affected function(s)** | `WorkflowEngine.transition/3` |
| **Short explanation** | Workflow state names are loaded from a YAML/JSON workflow definition file stored in the database and supplied as strings at runtime. Each call to `transition/3` converts the target state name to an atom to record the current step. Because workflow definitions are user-configurable, new state names can be introduced at any time, each permanently claiming an atom slot. |
...
defmodule MyApp.Workflows.WorkflowEngine do
  ...
  # VALIDATION: This is a smell because `String.to_atom/1` converts the target
  # state name string that was loaded from a user-configurable workflow definition
  # stored in the database. Workflow states are domain concepts defined by business
  # administrators (e.g., "awaiting_legal_review", "pending_cfo_approval_v2",
  # "escalated_to_board"). Each unique state name in any workflow definition across
  # any tenant becomes a permanent atom. In a multi-tenant SaaS platform where
  # customers define their own workflows, the number of distinct state name strings
  # is entirely unbounded and grows with customer count and workflow complexity.
