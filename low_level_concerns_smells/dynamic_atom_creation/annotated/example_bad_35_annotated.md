...
- **Expected smell location:** `decode_payment_method/1` function
- **Affected function(s):** `decode_payment_method/1`
- **Short explanation:** The function converts the payment method type string returned by the payment gateway's API response into an atom using `String.to_atom/1`. Because the payment gateway may add new payment method types or return locale-specific variants, this creates atoms from an open-ended, externally controlled set of strings.
...
defmodule Payments.ChargeProcessor do
  ...
  # payment method type string returned directly by the payment gateway API.
  # Gateways periodically introduce new payment methods (e.g. "klarna",
  # "afterpay_clearpay", regional wallet names), and each unique string type
  # creates a new permanent atom. This puts atom table growth under the control
  # of the external provider rather than the developer.
