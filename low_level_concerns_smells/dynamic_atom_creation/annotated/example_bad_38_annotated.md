...
- **Expected smell location:** `header_to_key/1` function
- **Affected function(s):** `header_to_key/1`, `normalize_headers/1`
- **Short explanation:** The function converts HTTP response header names (received from an upstream service) into atoms using `String.to_atom/1`. HTTP headers are determined by the upstream server and can include custom, vendor-specific, or arbitrary header names, making this an uncontrolled source of atom creation.
...
defmodule Gateway.UpstreamProxy do
  ...
  defp normalize_headers(headers) do
    ...
    # VALIDATION: This is a smell because `header_to_key/1` calls
    # `String.to_atom/1` on HTTP header names received from an upstream
    # service. HTTP header names are set by the remote server and can be
    # arbitrary strings (custom headers, vendor extensions, etc.). Each unique
    # header name creates a new permanent atom. Upstreams may add headers at
    # any time, leading to unbounded atom table growth.
    normalized =
