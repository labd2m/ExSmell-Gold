...
| **Expected smell location** | `ReportEngine.build_column_map/1`, line where `String.to_atom/1` converts column header strings |
| **Affected function(s)** | `ReportEngine.build_column_map/1` |
| **Short explanation** | Column header names are read from a CSV file uploaded by an end user. Each header is converted to an atom to serve as a map key. Because the content is user-controlled, an attacker or malformed file can introduce thousands of unique column names, each permanently consuming an atom table slot. |
...
defmodule MyApp.Reporting.ReportEngine do
  ...
  # column header string parsed from a user-uploaded CSV file. The header row is
  # entirely user-controlled: a malicious or careless upload with hundreds of
  # unique column names will permanently fill BEAM's atom table with those strings.
  # The atom table has a hard limit of 1_048_576 entries and is never garbage-
  # collected, so repeated uploads of varied CSVs degrade and can crash the node.
