...
| **Expected smell location** | `TaskRunner.parse_args/1`, line where `String.to_atom/1` converts the CLI command name |
| **Affected function(s)** | `TaskRunner.parse_args/1` |
| **Short explanation** | The Mix task / CLI runner accepts a command name as a plain string argument from the shell environment. Converting it to an atom via `String.to_atom/1` before validation means that any invocation of the binary with an arbitrary command string permanently allocates an atom, making the node susceptible to atom table exhaustion through repeated shell invocations with unique command names. |
...
defmodule MyApp.CLI.TaskRunner do
  ...
  # VALIDATION: This is a smell because `String.to_atom/1` is applied to the first
  # CLI argument—a free-form string provided directly by a shell user or automation
  # script. Although a guard below validates the atom against `@registered_commands`,
  # the atom is created before that check runs, meaning every typo, every novel
  # command string tried by a user, or every iteration of a fuzzing script results
  # in a new permanent atom. In scripted environments where the binary is called
  # many times with varied arguments, this steadily depletes the atom table.
