...
- **Expected smell location:** `cast_field_name/1` function
- **Affected function(s):** `cast_field_name/1`, `build_report_row/2`
- **Short explanation:** The function converts column header strings from a user-uploaded CSV report file into atoms using `String.to_atom/1`. Since column names come from user-supplied files, they are unbounded and uncontrolled, risking atom table exhaustion under high upload volumes.
    ...
    if length(values) != length(headers) do
      ...
      attrs =
        ...
        |> Enum.into(%{}, fn {header, value} ->
          ...
          # VALIDATION: This is a smell because `cast_field_name/1` calls
          # `String.to_atom/1` on column headers extracted from a user-supplied
          # CSV file. Each unique column name in any uploaded file creates a
          # new atom permanently. Malicious or varied files can exhaust the atom
          # table without any upper bound.
