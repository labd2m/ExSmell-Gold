...
- **Expected smell location:** `build_index_name/1` function
- **Affected function(s):** `build_index_name/1`
- **Short explanation:** The function builds an Elasticsearch index name by combining a tenant slug (received from a multi-tenant API request) with a suffix, and then converts the result to an atom using `String.to_atom/1`. Tenant slugs are submitted by users during onboarding and are unbounded in variety, causing uncontrolled atom growth.
...
defmodule Search.IndexRouter do
  ...
  # string composed from a tenant slug, which is supplied by users during
  # account registration. Each unique tenant slug produces a new permanent atom.
  # In a SaaS product with many tenants, or if slugs are generated
  # programmatically, the atom table can accumulate atoms without bound.
