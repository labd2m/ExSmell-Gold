...
- **Expected smell location:** `Billing.InvoiceLineParser.parse/1`
- **Affected function(s):** `parse/1`, `parse_qty_price/1`
- **Short explanation:** `parse/1` splits the raw line item string on `"|"` and extracts each field
  by position using `Enum.at/2`, with no pattern matching or structural validation. If the
  description segment itself contains `"|"` characters, or if a segment is missing entirely,
  the function silently assigns `nil` or the wrong value to each field — returning a plausible-looking
  struct instead of crashing. The caller receives fabricated data without any indication of failure.
...
defmodule Billing.InvoiceLineParser do
  ...
  # VALIDATION: to extract each field by its positional index. If the description
  # VALIDATION: contains a "|" character (e.g., "Setup|Migration Included"), the
  # VALIDATION: segment indices shift silently: `tax_code` receives the description
  # VALIDATION: text and `description` receives the overflow fragment or nil — all
  # VALIDATION: without raising any error or log warning. The caller obtains a
  # VALIDATION: fully-populated struct whose values are incorrect, creating a false
  # VALIDATION: impression that parsing succeeded.
