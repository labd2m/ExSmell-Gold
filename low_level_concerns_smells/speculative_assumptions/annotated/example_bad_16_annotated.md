...
- **Expected smell location:** `Logistics.FreightLegParser.extract_leg_detail/1`
- **Affected function(s):** `extract_leg_detail/1`
- **Short explanation:** The function splits a route leg descriptor on `":"` and reads the
  airport code, event type, and timestamp at fixed positions via `Enum.at/2`. The assumption is
  that no segment will contain `":"`. ISO 8601 timestamps with timezone offsets such as
  `"2024-03-15T09:00:00+10:00"` contain an extra `":"` in the UTC offset portion. When such a
  timestamp is embedded in the leg string, `Enum.at(parts, 2)` silently captures only the first
  portion of the timestamp and the full datetime is lost — without any error being raised.
...
defmodule Logistics.FreightLegParser do
  ...
  # VALIDATION: This is a smell because `extract_leg_detail/1` splits on ":" and uses
  # VALIDATION: `Enum.at/2` at positions 0, 1, 2 to extract location, event type, and
  # VALIDATION: timestamp. ISO 8601 timestamps with UTC offset notation contain an extra
  # VALIDATION: colon (e.g. "2024-03-15T09:00:00+10:00"). For the input
  # VALIDATION: "SYD:DEP:2024-03-15T09:00:00+10:00", the split yields five segments.
  # VALIDATION: `Enum.at(parts, 2)` silently returns "2024-03-15T09" instead of the full
  # VALIDATION: timestamp string. `parse_timestamp/1` then fails on this truncated value
  # VALIDATION: and returns nil. The struct is populated with a nil timestamp and no error
  # VALIDATION: is raised, giving the caller a plausible-looking but incorrect leg record.
