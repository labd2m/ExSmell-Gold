...
- **Expected smell location:** `Logistics.ConsignmentParser.extract_carrier_info/1`
- **Affected function(s):** `extract_carrier_info/1`
- **Short explanation:** The function splits a consignment reference string on `"-"` and extracts
  fields by index using `Enum.at/2`. The assumption is that the carrier name is always a single
  hyphen-free token at index 2. Carrier names like `"DB-SCHENKER"` or `"TNT-FEDEX"` contain
  hyphens, causing all subsequent field indices to shift silently. The function returns a plausible
  but incorrect map — with the wrong carrier code, service level, and tracking suffix — instead of
  crashing and alerting the operator.
...
defmodule Logistics.ConsignmentParser do
  ...
  # VALIDATION: at fixed indices (2, 3, 4) to extract carrier, service level, and tracking
  # VALIDATION: suffix. The assumption is that all three tokens are hyphen-free. When a
  # VALIDATION: real carrier code such as "DB-SCHENKER" or "COURIERS-PLEASE" is embedded
  # VALIDATION: in the reference, the split produces extra segments and every subsequent
  # VALIDATION: `Enum.at` silently picks the wrong token. The function returns `{:ok, info}`
  # VALIDATION: with fabricated values rather than `{:error, reason}`, making the failure
  # VALIDATION: invisible to the caller.
