...
- **Expected smell location:** `Payments.TransactionReferenceParser.parse/1`
- **Affected function(s):** `parse/1`
- **Short explanation:** The function splits a composite transaction reference on `"_"` and
  extracts gateway, environment, and gateway-native ID at fixed indices via `Enum.at/2`. Gateway
  native IDs (e.g. Stripe payment intent IDs such as `"pi_3OvHbcFoo123_secret_abc"`) themselves
  contain underscores. When such an ID is embedded in the composite reference, the split produces
  more segments than expected and the function silently returns the wrong value for every field
  after the first underscore boundary, rather than detecting the malformed input and crashing.
...
defmodule Payments.TransactionReferenceParser do
  ...
  # VALIDATION: This is a smell because the function splits on "_" and uses `Enum.at/2`
  # VALIDATION: at positions 0, 1, and 2 to extract the gateway, environment, and gateway
  # VALIDATION: transaction ID. Stripe payment intent IDs already contain underscores
  # VALIDATION: (e.g. "pi_3OvHbc2eZvKYlo2C0123456"), so the composite reference
  # VALIDATION: "stripe_live_pi_3OvHbc2eZvKYlo2C0123456" splits into
  # VALIDATION: ["stripe", "live", "pi", "3OvHbc2eZvKYlo2C0123456"]. `Enum.at(parts, 2)`
  # VALIDATION: silently returns "pi" instead of the full gateway ID. No error is raised;
  # VALIDATION: the caller receives a plausible-looking map whose :gateway_tx_id is wrong.
