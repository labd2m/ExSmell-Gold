...
- **Expected smell location:** `Scheduling.SlotIdentifierParser.decode/1`
- **Affected function(s):** `decode/1`
- **Short explanation:** The function splits an appointment slot identifier on `"/"` and reads
  the date, time, duration, and resource at fixed positions via `Enum.at/2`. The unplanned
  assumption is that none of these components will ever contain `"/"`. Resource identifiers,
  however, commonly follow path-like conventions (e.g. `"rooms/boardroom-a"` or
  `"staff/dr-smith"`). When such a resource ID is embedded in the slot string, the extra `"/"`
  causes all indices after position 2 to be silently wrong. The function returns a fully
  populated struct with an incorrect resource ID and nil duration — with no crash or warning.
...
defmodule Scheduling.SlotIdentifierParser do
  ...
  # VALIDATION: uses `Enum.at/2` at positions 0, 1, 2, 3 to read date, time, duration,
  # VALIDATION: and resource ID. The planned assumption is that resource IDs are flat
  # VALIDATION: tokens like "consult-room-3". Real-world resource IDs in the system
  # VALIDATION: follow path conventions ("rooms/boardroom-a", "staff/dr-smith"), which
  # VALIDATION: introduce extra "/" characters. For input "2024-04-10/09:00/60/rooms/boardroom-a",
  # VALIDATION: `Enum.at(parts, 3)` silently returns "rooms" and `Enum.at(parts, 2)` returns
  # VALIDATION: "60" correctly, but the actual resource "rooms/boardroom-a" is never captured.
  # VALIDATION: The struct is returned with an incomplete resource_id and no error raised.
