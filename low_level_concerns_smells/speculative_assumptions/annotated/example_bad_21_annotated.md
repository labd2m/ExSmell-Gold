...
- **Expected smell location:** `Payments.WebhookEventTypeParser.parse/1`
- **Affected function(s):** `parse/1`
- **Short explanation:** The function splits a webhook event type string on `"."` and reads
  the resource and event action at fixed positions via `Enum.at/2`. The assumption is that
  event types have exactly two dot-separated segments. Real payment gateway webhooks use deeper
  hierarchies such as `"payment_intent.payment_failed.insufficient_funds"` or
  `"invoice.payment_action_required.card_declined"`. When such strings arrive, `Enum.at(parts, 1)`
  returns only the middle segment, not the full action qualifier, and the function silently
  routes the event to the wrong handler instead of crashing with an unknown event error.
...
defmodule Payments.WebhookEventTypeParser do
  ...
  # VALIDATION: This is a smell because `parse/1` splits on "." and uses `Enum.at/2`
  # VALIDATION: at positions 0 and 1 to extract the resource and action. For nested event
  # VALIDATION: types like "customer.subscription.deleted" — which are real Stripe events —
  # VALIDATION: the split yields ["customer", "subscription", "deleted"]. `Enum.at(parts, 0)`
  # VALIDATION: returns "customer" and `Enum.at(parts, 1)` silently returns "subscription"
  # VALIDATION: instead of the full action qualifier "subscription.deleted". The event is
  # VALIDATION: then routed as if it were an unknown "subscription" action on "customer",
  # VALIDATION: not as a subscription deletion — without any error being raised. The routing
  # VALIDATION: logic silently discards real billing events.
