...
- **Expected smell location:** `Auth.ApiKeyParser.parse/1`
- **Affected function(s):** `parse/1`
- **Short explanation:** The function splits an API key string on `"_"` and reads the key type,
  environment, and secret material at fixed indices using `Enum.at/2`. The critical assumption
  is that the secret portion contains no underscores. In practice the platform generates secrets
  using a Base58 encoder which does not produce underscores, but a legacy key generation path
  uses random hex with `"_"` separators for readability. For those keys the split produces extra
  segments and `Enum.at(parts, 2)` silently returns only the first chunk of the secret, creating
  a truncated key that will fail HMAC verification — but only at a later stage, not here.
...
defmodule Auth.ApiKeyParser do
  ...
  # VALIDATION: This is a smell because `parse/1` splits on "_" with no limit and uses
  # VALIDATION: `Enum.at/2` at positions 0, 1, 2 to extract key type, environment, and
  # VALIDATION: secret. The legacy key generation path produces secrets that include
  # VALIDATION: underscores as human-readable chunk separators, e.g.
  # VALIDATION: "sk_live_9kPq_NmV2_aLsB6c". The split yields
  # VALIDATION: ["sk", "live", "9kPq", "NmV2", "aLsB6c"]. `Enum.at(parts, 2)` silently
  # VALIDATION: returns only "9kPq" — a truncated, incorrect secret. Validation of the
  # VALIDATION: type and environment passes successfully, and the function returns {:ok,
  # VALIDATION: struct} with a secret field that cannot possibly verify any HMAC. The
  # VALIDATION: failure is deferred silently to the HMAC layer rather than detected here.
