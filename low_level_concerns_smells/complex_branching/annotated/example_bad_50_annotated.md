...
- **Expected smell location:** `interpret_disbursement_response/2` function
- **Affected function(s):** `interpret_disbursement_response/2`
- **Short explanation:** The function is the sole handler for every outcome of a payroll disbursement API call — processed, pending bank clearance, rejected by bank, insufficient balance, compliance holds, AML flags, and server faults — all in one deeply nested `case`. Centralising all this branching inflates cyclomatic complexity and creates one fragile point of failure for all payroll operations.
...
defmodule Payroll.DisbursementClient do
  ...
  # VALIDATION: This is a smell because `interpret_disbursement_response/2` is
  # the single function responsible for every HTTP status and body variant
  # returned by one payroll disbursement endpoint. The 200 path branches across
  # processed (with and without bank reference), pending_clearance, and scheduled
  # body shapes. The 400 path branches across invalid_routing_number,
  # invalid_account_number, unsupported_country, invalid_amount, and generic
  # errors. Further arms handle insufficient_balance (402), AML/compliance holds
  # (403), duplicate disbursements (409), regulatory blocks (451), and two server
  # error shapes. All of this branching is packed into one function, making it
  # very long, hard to test, and fragile: a MatchError in any single arm
  # (e.g., missing "bank_reference") prevents all other response types
  # from being handled.
