...
- **Expected smell location:** `apply_adjustment/2` function, the `case` expression over the inventory management API response
- **Affected function(s):** `apply_adjustment/2`
- **Short explanation:** The function takes on the full burden of interpreting every possible response from one inventory adjustment API endpoint — successful adjustments, partial fills, reservations, warehouse errors, out-of-stock conditions, quarantine flags, and connectivity failures — all inside one `case`, making the function unwieldy and difficult to test in isolation per scenario.
...
defmodule Inventory.StockAdjustment do
  ...
  # VALIDATION: This is a smell because the function consolidates all possible
  # response variants from a single WarehouseOS API endpoint — accepted, partial,
  # reserved, out of stock, damaged quarantine, warehouse errors — into one long
  # case expression. This creates high cyclomatic complexity, reduces readability,
  # and means a bug in any one branch could destabilise handling of all others.
