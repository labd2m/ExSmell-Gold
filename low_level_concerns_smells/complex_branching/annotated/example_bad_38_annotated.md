...
- **Expected smell location:** `enroll_employee/2` function, the `case` expression over the HR platform API response
- **Affected function(s):** `enroll_employee/2`
- **Short explanation:** All outcome variations from one HR platform API endpoint — successful onboarding, pending background check, duplicate detection, role conflicts, license violations, missing department, and connectivity failures — are all handled inside a single `case` expression in one function, creating an excessively high cyclomatic complexity.
...
defmodule UserManagement.EmployeeOnboarding do
  ...
  # VALIDATION: This is a smell because all possible API response variants from
  # the HRCore employee enrolment endpoint are handled inside a single function
  # using a large case expression. Successful enrolment, background check holds,
  # duplicate detection, role conflicts, licensing issues, and infrastructure
  # errors each deserve their own handler, but instead they're all piled into
  # one case body, inflating cyclomatic complexity and mixing concerns.
