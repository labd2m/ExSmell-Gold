...
- **Expected smell location:** `handle_token_response/2` function
- **Affected function(s):** `handle_token_response/2`
- **Short explanation:** The function handles every possible outcome from a single OAuth token endpoint in one monolithic `case` expression, including success paths (access token, refresh token, partial scopes), client errors (invalid_grant, invalid_client, invalid_scope, mfa_required), and server errors. Each outcome requires different fields, making the branching both wide and deep. This concentrates all error-handling responsibility in one place, raising cyclomatic complexity and making each branch fragile when schema changes occur.
...
defmodule Auth.IdentityProviderClient do
  ...
  # VALIDATION: This is a smell because `handle_token_response/2` is the sole
  # function responsible for interpreting every HTTP status and every OAuth error
  # code returned by a single token endpoint. The outer `case` on HTTP status
  # expands into nested `case` blocks on the body, covering success variants
  # (full token, missing refresh_token, partial scopes) and error variants
  # (invalid_grant, invalid_client, invalid_scope, mfa_required, server errors).
  # All of this branching is packed into one function, making it very long,
  # difficult to test branch-by-branch, and fragile — a pattern-match error in
  # any single branch (e.g., a missing "mfa_token" key) will raise an exception
  # and prevent all other response types from being handled.
