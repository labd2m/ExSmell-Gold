...
- **Expected smell location:** `GatewayClient.process_charge/2`, the large `case` block handling every possible gateway response
- **Affected function(s):** `process_charge/2`
- **Short explanation:** A single function assumes full responsibility for distinguishing every HTTP status code, network error, fraud signal, card decline code, and partial-success response returned by the payment gateway. The resulting `case` has eleven clauses spanning multiple dimensions (HTTP status, body shape, nested decline sub-code), making cyclomatic complexity very high. A typo in any one pattern silently falls through to a wrong branch, and testing every path in isolation is impractical.
...
defmodule MyApp.Payments.GatewayClient do
  ...
  def process_charge(charge_params, opts \\ []) do
    ...
    # VALIDATION: This is a smell because `process_charge/2` alone handles every
    # VALIDATION: possible gateway response: success, card declines by sub-code,
    # VALIDATION: fraud blocks, gateway errors, rate limits, timeouts, and network
    # VALIDATION: failures — all within one deeply nested case expression. The
    # VALIDATION: cyclomatic complexity is very high, making this function hard to
    # VALIDATION: test, maintain, and extend without introducing regressions.
