...
- **Expected smell location:** `sync_shipment_status/2` function, the `case` expression over the carrier API response
- **Affected function(s):** `sync_shipment_status/2`
- **Short explanation:** All possible carrier tracking response variants (in-transit, delivered, returned, customs-held, lost, failed, network issues) are handled by a single massive `case` inside one function, making it difficult to understand, extend, or independently test each branch.
...
defmodule Logistics.CarrierSync do
  ...
  # VALIDATION: This is a smell because every possible response variant from a
  # single carrier API endpoint is handled inside one function using a large case
  # expression with many arms, inflating cyclomatic complexity. A bug or typo in
  # any single arm could silently affect the entire function's reliability, and
  # each scenario is difficult to test in isolation.
