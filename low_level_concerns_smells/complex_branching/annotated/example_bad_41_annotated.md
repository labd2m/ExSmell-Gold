...
- **Expected smell location:** `interpret_tax_calculation_response/2` function
- **Affected function(s):** `interpret_tax_calculation_response/2`
- **Short explanation:** The function handles every variant returned by a single tax-calculation API endpoint in one massive `case` expression — success with full breakdown, partial results, jurisdiction errors, exemption decisions, nexus mismatches, rate-limit, and server faults. All response-handling responsibility is concentrated here, creating high cyclomatic complexity and a single fragile point of failure for all callers.
...
defmodule Billing.TaxCalculationClient do
  ...
  # VALIDATION: This is a smell because `interpret_tax_calculation_response/2`
  # is the single function responsible for every possible HTTP status and body
  # variant from the tax calculation endpoint. The outer `case` on HTTP status
  # fans out into deeply nested inner `case` blocks covering: fully calculated
  # results with jurisdiction breakdowns, partial results with warnings, full
  # exemptions, nexus-mismatch errors, unsupported-jurisdiction errors,
  # validation failures, auth errors, quota exhaustion, and server faults.
  # Packing all of this into one function makes it extremely long and hard to
  # test each branch independently. A runtime error in any single arm (e.g., a
  # missing key in "jurisdictions") raises an exception that prevents every
  # other response type from being handled.
