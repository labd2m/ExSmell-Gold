...
- **Expected smell location:** `verify_identity/2` function, the `case` expression over the identity provider response
- **Affected function(s):** `verify_identity/2`
- **Short explanation:** A single function concentrates handling of all identity verification response types from one API endpoint — approved, pending review, rejected for many distinct reasons, expired sessions, rate limits, and network failures — into one long `case`, raising cyclomatic complexity and making isolated testing difficult.
...
defmodule Auth.IdentityVerification do
  ...
  # VALIDATION: This is a smell because all possible response variants from the
  # IDVerify document submission endpoint are handled inside one function using a
  # single case expression with many branches, each representing a distinct
  # business scenario. This concentrates too much responsibility in one place,
  # making it hard to test each branch independently and risky to change.
