...
smell_location: Reports.MetricsFetcher.parse_metrics_response/1
affected_functions:
  - parse_metrics_response/1
explanation: >
  `parse_metrics_response/1` matches against 16 distinct response shapes from
  a single analytics platform API call within one `case` expression. The
  function fuses HTTP status discrimination, body pattern matching, domain
  error tagging, and logging into a single block. This makes it hard to
  locate which branch handles a particular error, hard to test each code path
  independently, and especially risky because a runtime error in any arm would
  leave metric snapshots unrecorded for the entire dashboard report.
---
...
defmodule Reports.MetricsFetcher do
  ...
  # VALIDATION: This is a smell because one function absorbs all 16 possible
  # response shapes from one API endpoint: two success variants, empty 204,
  # three distinct 400 codes, 401, two 403 variants, 404, 422, 429, 500, 503
  # with retry header, a catch-all status, :timeout, and generic client error.
  # The resulting cyclomatic complexity makes maintenance and testing extremely
  # difficult; a bug in one arm can affect all other code paths if the function
  # raises an exception.
