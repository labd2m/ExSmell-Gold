...
smell_location: Billing.ChargeProcessor.handle_charge_response/1
affected_functions:
  - handle_charge_response/1
explanation: >
  The `handle_charge_response/1` function uses a single `case` expression to
  handle 14 distinct response shapes returned by the same payment provider API
  call. This concentrates all error-path logic in one place, raises cyclomatic
  complexity dramatically, and makes the function fragile: a typo or raised
  exception in any one branch can silently break processing for every other
  response type. Each branch also conflates concerns (logging, error tagging,
  and status normalisation) that would be easier to reason about in isolation.
---
...
defmodule Billing.ChargeProcessor do
  ...
  # VALIDATION: This is a smell because a single private function assumes sole
  # responsibility for interpreting 14 structurally distinct responses from one
  # API call. Every new failure code or status variant forces a change here,
  # increasing cyclomatic complexity and the risk that an unrelated edit
  # corrupts an existing branch.
