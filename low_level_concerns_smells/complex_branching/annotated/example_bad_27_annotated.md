...
- **Expected smell location:** `fetch_shipment_status/2` function
- **Affected function(s):** `fetch_shipment_status/2`
- **Short explanation:** The function uses a deeply nested `case` expression to handle every possible status code and response variant from a single carrier API endpoint. All branching logic is concentrated in one function, making it long, hard to test independently, and prone to bugs if any single branch introduces a runtime error that collapses the entire match.
...
defmodule Logistics.CarrierClient do
  ...
  # handling every possible HTTP status code and response body variant returned
  # by one carrier API endpoint. The deeply nested `case` on the outer HTTP
  # result, combined with inner `case` blocks per status code, creates high
  # cyclomatic complexity. Any runtime error in one branch (e.g., a missing key
  # in the body map) raises an exception that prevents all other branches from
  # executing, making the entire function fragile and very hard to unit-test in
  # isolation.
