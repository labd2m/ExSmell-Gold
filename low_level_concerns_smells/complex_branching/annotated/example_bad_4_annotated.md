...
- **Expected smell location:** `KYCClient.submit_verification/2`, the large nested `case`/`cond` block handling every verification outcome
- **Affected function(s):** `submit_verification/2`
- **Short explanation:** A single function handles every response from the KYC provider: approved, manually-reviewed, rejected with multiple sub-reasons (document issues, sanctions, age, PEP status), missing documents, expired sessions, duplicate submissions, and network failures. Each branch also triggers different downstream workflows (flagging, notifying, storing). This concentration of branching logic in one function makes it extraordinarily hard to test individual paths or add new rejection reasons without risking the entire classification logic.
...
defmodule MyApp.Compliance.KYCClient do
  ...
  def submit_verification(user_id, verification_payload) do
    ...
    # VALIDATION: This is a smell because `submit_verification/2` handles every
    # VALIDATION: possible verification outcome inside one function: approved,
    # VALIDATION: manual review required, rejected for multiple document or identity
    # VALIDATION: reasons, sanctions hits, PEP flags, session expiry, duplicate
    # VALIDATION: submissions, provider errors, and network failures — each with
    # VALIDATION: different side effects (onboarding flows, alerts, logging).
    # VALIDATION: The resulting cyclomatic complexity is unmanageable and makes
    # VALIDATION: exhaustive testing practically impossible.
