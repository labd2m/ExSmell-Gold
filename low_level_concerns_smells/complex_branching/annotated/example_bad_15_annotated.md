...
- **Expected smell location:** `apply_calendar_response/3`, inside the `case` that handles all response variants from `CalendarProviderClient.upsert_event/2`
- **Affected function(s):** `apply_calendar_response/3`
- **Short explanation:** `apply_calendar_response/3` collapses all possible outcomes from a calendar provider API into one `case` expression: created, updated, conflict detected, attendee limit exceeded, organiser not found, resource unavailable, recurring rule invalid, event in the past, and two transport errors. Each branch carries its own side-effects: calendar store writes, conflict resolution queuing, attendee notifications, and audit logs. The high cyclomatic complexity makes the function fragile and difficult to test; a failure in any one branch's side-effect hides which calendar response was being processed.
...
defmodule Scheduling.CalendarSyncHandler do
  ...
  # VALIDATION: This is a smell because `apply_calendar_response/3` handles
  # every possible response from `CalendarProviderClient.upsert_event/2` in
  # a single `case` block with ten branches. Created, updated, scheduling
  # conflict, attendee capacity, missing organiser, resource unavailability,
  # invalid recurrence rule, past-event rejection, and two transport failures
  # all live in one function. The cyclomatic complexity is very high; testing
  # the "conflict" branch, for example, requires exercising a function that
  # also handles recurrence validation, capacity checks, and network errors.
  # A bug introduced while fixing the "past_event" branch can inadvertently
  # break the "created" path if the function raises before returning.
