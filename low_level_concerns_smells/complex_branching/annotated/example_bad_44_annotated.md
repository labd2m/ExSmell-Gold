...
- **Expected smell location:** `interpret_send_response/2` function
- **Affected function(s):** `interpret_send_response/2`
- **Short explanation:** The function is the sole handler for every HTTP response variant from a transactional email API endpoint — queued, delivered, throttled, bounced, spam-blocked, invalid address, attachment errors, template errors, and server faults — all in one `case` expression. This concentrates all branching responsibility in one place, making the function long, fragile, and very difficult to test per branch.
...
defmodule Notifications.EmailDeliveryClient do
  ...
  # VALIDATION: This is a smell because `interpret_send_response/2` concentrates
  # every possible API outcome from a single email send endpoint in one function.
  # The outer `case` fans into deeply nested inner `case` blocks: the 200 path
  # handles queued, delivered, and scheduled shapes; the 400 path branches on
  # invalid_email, suppressed_recipient, invalid_attachment, template_not_found,
  # template_variable_missing, and generic errors. Further arms cover quota
  # exhaustion (402), domain authentication failures (403), legal blocks (451),
  # and two shapes of server errors. The resulting function is very long and each
  # arm is tightly coupled to a specific body schema, so any missing key in a
  # pattern raises a MatchError that collapses handling for all response types.
