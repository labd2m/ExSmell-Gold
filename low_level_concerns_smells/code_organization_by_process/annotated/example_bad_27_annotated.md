...
- **Expected smell location**: `Api.PaginationCalculator` module
- **Affected function(s)**: `metadata/3`, `page_range/3`, `offset/2`, `visible_pages/3`
- **Short explanation**: Pagination metadata is purely arithmetic on three integers (total count, page number, page size). No shared state is needed between calls and no resource needs serialized access. The `GenServer` state is never populated or consulted. Because this module is invoked on every paginated API response, funneling all calls through one process creates a bottleneck in the API layer for something that could be a zero-overhead plain module.
...
defmodule Api.PaginationCalculator do
  ...
  # VALIDATION: This is a smell because PaginationCalculator uses a GenServer
  # VALIDATION: to organize pure integer arithmetic. The process carries no
  # VALIDATION: meaningful state; all five operations derive their result
  # VALIDATION: entirely from arguments. Every paginated API endpoint calls
  # VALIDATION: this module on each request; routing them all through one
  # VALIDATION: process serializes trivially parallel work and creates a
  # VALIDATION: bottleneck purely due to process-based code organization.
