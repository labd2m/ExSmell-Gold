...
| **Expected smell location** | `OrderTotalCalculator` module — entire GenServer structure |
| **Affected function(s)** | `calculate/2`, `apply_coupon/3`, `breakdown/2`, `validate_items/2` |
| **Short explanation** | Computing order totals, applying coupons, and validating items are pure arithmetic and data-validation operations. None of them requires shared mutable state or a process boundary. Routing every checkout through a single GenServer queues concurrent requests unnecessarily. |
...
defmodule Commerce.OrderTotalCalculator do
  ...
  # VALIDATION: This is a smell because order total computation is a pure
  # calculation over item lists and coupon data supplied by the caller on
  # each call. The GenServer state is always empty. This design serialises
  # all checkout computations through one process, reducing throughput and
  # adding latency without any concurrent-access benefit.
