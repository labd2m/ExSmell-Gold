...
- **Expected smell location:** `Commerce.DiscountEngine` module — `GenServer` implementation
- **Affected functions:** `apply_percentage/3`, `apply_fixed/3`, `apply_tiered/3`, `best_of/3`, `stack_discounts/3`
- **Short explanation:** Discount calculation is pure arithmetic applied to order totals and discount rule data structures. None of the calculations mutate shared state, require exclusive resource access, or involve I/O. Using a `GenServer` to organize these computations serializes discount evaluations — a critical-path operation during checkout — through a single process without any architectural justification.
...
defmodule Commerce.DiscountEngine do
  ...
  # VALIDATION: This is a smell because discount calculations are pure functions:
  # they receive numeric inputs and discount rule structs, then return numbers.
  # No shared mutable resource is accessed and no side effects occur. Wrapping
  # them in a GenServer introduces a serial bottleneck on the checkout path,
  # where high request concurrency is expected.
