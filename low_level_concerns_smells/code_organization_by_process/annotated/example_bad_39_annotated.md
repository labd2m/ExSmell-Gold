...
| **Expected smell location** | `AddressFormatter` module — entire GenServer structure |
| **Affected function(s)** | `format/2`, `format_single_line/2`, `validate/2`, `normalize/2` |
| **Short explanation** | Address formatting and validation are pure transformations of input maps to output strings or validated structs. No state is shared between calls, no external resource is held, and no scheduling is involved. Every call through the GenServer adds message-passing overhead and prevents parallelism for what is trivially concurrent work. |
...
defmodule Logistics.AddressFormatter do
  ...
  # VALIDATION: This is a smell because address formatting is a pure
  # string-building function over a map input. There is no state to
  # maintain between requests and no concurrency concern. The GenServer
  # process forces all formatting work to be serial, limiting throughput
  # when many addresses must be formatted concurrently (e.g., during
