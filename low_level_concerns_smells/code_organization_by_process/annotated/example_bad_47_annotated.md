...
| **Expected smell location** | `RouteCalculator` module — entire GenServer structure |
| **Affected function(s)** | `haversine_distance/3`, `total_route_distance/2`, `fuel_cost/4`, `co2_emission/4` |
| **Short explanation** | All operations in this module are pure mathematical computations over coordinate data (haversine formula, summation over waypoints, multiplications by factors). No state is maintained between calls, no I/O is required, and no external resource is accessed. Routing these calculations through a GenServer serialises concurrent route calculations unnecessarily. |
...
defmodule Logistics.RouteCalculator do
  ...
  # VALIDATION: This is a smell because route calculations are pure
  # mathematical operations on coordinate data. The GenServer maintains
  # no shared state between calls (state is always {}). When the fleet
  # service needs to price dozens of routes simultaneously, all calculations
  # are queued through one process mailbox, a bottleneck that could be
  # avoided by using plain module functions.
