...
- **Expected smell location**: `Scheduling.ConflictDetector` module
- **Affected function(s)**: `overlaps?/3`, `find_conflicts/2`, `merge_overlapping/2`, `free_slots/3`
- **Short explanation**: Detecting scheduling conflicts is pure date-range arithmetic over lists. No shared mutable state is involved and no resource needs serialized access. The `GenServer` holds no state after initialization. A calendar service detecting conflicts for many users simultaneously would need to serialize all checks through this one process, creating an artificial bottleneck from a design choice that should have been a plain module.
...
defmodule Scheduling.ConflictDetector do
  ...
  # VALIDATION: This is a smell because ConflictDetector uses a GenServer to
  # VALIDATION: organize pure time-range comparison logic. The process carries
  # VALIDATION: no persistent state; init simply acknowledges :ok. Overlap
  # VALIDATION: detection is deterministic and depends only on the time ranges
  # VALIDATION: passed to each call. Booking services checking availability for
  # VALIDATION: many resources in parallel must serialize through this single
  # VALIDATION: process, introducing contention where none is necessary.
