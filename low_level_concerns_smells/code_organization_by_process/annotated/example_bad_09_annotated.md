...
- **Expected smell location**: `Auth.PasswordPolicy` module
- **Affected function(s)**: `evaluate/2`, `score/2`, `meets_policy?/2`, `violations/2`
- **Short explanation**: Password policy evaluation is entirely stateless string analysis. The `GenServer` holds no mutable state and performs no I/O. Every call to evaluate a password must wait for the single process, turning what should be a parallel, embarrassingly stateless operation into a serialized queue—purely because the developer chose to organize this logic as a process rather than a plain module.
...
defmodule Auth.PasswordPolicy do
  ...
  # VALIDATION: This is a smell because Auth.PasswordPolicy wraps purely functional
  # VALIDATION: password validation logic inside a GenServer. The process state is
  # VALIDATION: the policy map set at init and never mutated. None of the operations
  # VALIDATION: require shared mutable state or access serialization. Under high
  # VALIDATION: registration throughput every evaluation serializes through this one
  # VALIDATION: process, creating a bottleneck with no runtime justification.
