...
| **Expected smell location** | `StructuredLogFormatter` module — entire GenServer structure |
| **Affected function(s)** | `format_entry/2`, `format_batch/2`, `redact_sensitive/2`, `to_json_line/2` |
| **Short explanation** | Log formatting is a pure data-to-string transformation over an event map and a config. No state is stored between formatting calls, no I/O is performed by the formatter itself, and no shared resource is accessed. Routing all log-formatting work through a single GenServer can throttle high-throughput logging pipelines. |
...
defmodule Observability.StructuredLogFormatter do
  ...
  # VALIDATION: This is a smell because log formatting is a pure transformation
  # from an event map to a JSON string. No mutable state is accumulated between
  # calls, no resource is shared, and no scheduling is needed. In a high-volume
  # service that logs thousands of events per second, serialising all formatting
  # through a single GenServer creates an easily avoidable bottleneck.
