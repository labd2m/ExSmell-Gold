...
- **Expected smell location**: `Commerce.DiscountEngine` module
- **Affected function(s)**: `apply/3`, `preview/3`, `applicable_promotions/2`, `validate_coupon/2`
- **Short explanation**: Discount and coupon evaluation is purely functional: given a cart and a set of promotion rules, compute the discount. The `GenServer` process state is set once at startup and never updated. All high-frequency cart operations serialize through this single process, which is wasteful given that the work is completely parallelizable and stateless.
...
defmodule Commerce.DiscountEngine do
  ...
  # VALIDATION: This is a smell because DiscountEngine uses a GenServer solely to
  # VALIDATION: hold a static promotion list and perform arithmetic on cart totals.
  # VALIDATION: The process state (@promotions) never changes at runtime. Discount
  # VALIDATION: computation has no side effects and no shared mutable resources.
  # VALIDATION: Every cart evaluation in the checkout service waits on this one
  # VALIDATION: process, turning a trivially parallel calculation into a serial queue.
