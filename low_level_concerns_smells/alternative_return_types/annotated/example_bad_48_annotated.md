...
- **Expected smell location:** `CRM.CustomerRepository.find/2`, around the `opts[:with_contacts]` and `opts[:ids_only]` checks
- **Affected function(s):** `find/2`
- **Short explanation:** The function returns a list of integer IDs, a list of `%Customer{}` structs, or a list of `%Customer{}` with preloaded `:contacts` association depending on the options — different list element types that break any downstream code that treats the list uniformly.
...
defmodule CRM.CustomerRepository do
  ...
  # VALIDATION: This is a smell because the list elements change type depending
  # VALIDATION: on options: plain integers, %Customer{} structs, or %Customer{}
  # VALIDATION: structs with preloaded contact lists. Downstream code that pipes
  # VALIDATION: the result (e.g., Enum.each(&send_email(&1.email, ...))) silently
  # VALIDATION: crashes when :ids_only was used, because there is no .email field
  # VALIDATION: on an integer.
