...
- **Expected smell location:** `Catalog.Product.search/2`
- **Affected function(s):** `search/2`
- **Short explanation:** The `:wrap` option makes the function return either a raw list of `%Product{}` structs, a `{results, meta}` tuple with pagination metadata, or a single integer count. These are fundamentally different types that cannot be handled by a common downstream pattern-match.
...
defmodule MyApp.Catalog.Product do
  ...
  # VALIDATION: This is a smell because opts[:wrap] changes the return type
  # from a plain list of structs (:none), to a two-element tuple of
  # {[%Product{}], pagination_map} (:page), to a bare integer (:count).
  # Each case is consumed differently: list for simple rendering, tuple for
  # paginated APIs, integer for count badges. But the single function signature
  # conceals which type comes out, causing subtle bugs when opts change.
