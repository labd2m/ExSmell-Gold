...
- **Expected smell location:** `FeatureFlags.Evaluator.check/3`, around the `opts[:explain]` and `opts[:variant]` checks
- **Affected function(s):** `check/3`
- **Short explanation:** The function returns a `boolean`, a `string` variant name, or a `%{enabled: boolean, variant: string | nil, reason: string, rule_matched: string | nil}` explanation map depending on the options — three incompatible types from what appears to be a simple flag check.
...
defmodule FeatureFlags.Evaluator do
  ...
  # VALIDATION: This is a smell because the return type is a boolean, a string,
  # VALIDATION: or a detailed map depending on opts. Code that gates a feature
  # VALIDATION: with `if check("flag", actor)` will silently always be truthy
  # VALIDATION: when :variant is used (non-nil strings are truthy) but falsy
  # VALIDATION: when the variant is nil — a subtle bug that stems directly from
  # VALIDATION: the unstable return contract.
