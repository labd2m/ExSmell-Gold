...
- **Expected smell location:** `Pricing.Calculator.compute/2`
- **Affected function(s):** `compute/2`
- **Short explanation:** The `:breakdown` option changes the return from a single `Decimal` value, to a `{net, tax, total}` three-element tuple, or a full map with line-item detail. These are structurally incompatible return types despite all representing a price computation result.
...
defmodule MyApp.Pricing.Calculator do
  ...
  # VALIDATION: This is a smell because opts[:breakdown] changes what is returned:
  # :total returns a single Decimal (just the final amount), :summary returns a
  # {net, tax, total} three-element tuple, and :detailed returns a map with
  # per-line breakdowns, applied promos, and tax details. Each return shape
  # is consumed by a completely different part of the system (display labels,
  # accounting records, receipts), making this a single function with three
  # incompatible type contracts.
