...
- **Expected smell location:** `Payments.Gateway.charge/3`
- **Affected function(s):** `charge/3`
- **Short explanation:** The `:response_mode` option causes the function to return either a plain `:ok`, an `{:ok, transaction_id}` tuple, or a full `{:ok, %Transaction{}}` struct. Each shape is structurally different and callers are burdened with knowing the mode before safely unwrapping the result.
...
defmodule MyApp.Payments.Gateway do
  ...
  # VALIDATION: This is a smell because opts[:response_mode] changes the return
  # type dramatically: :minimal returns plain :ok on success (and {:error, reason}
  # on failure), :id returns {:ok, transaction_id} string, and :full returns
  # {:ok, %Transaction{}} with all fields populated. The error path is consistent,
  # but the success path is completely different for each mode, creating an
  # asymmetric and unpredictable function contract.
