...
- **Expected smell location:** `Billing.SubscriptionPlan.for_customer/2`, around the `opts[:include_usage]` and `opts[:summarize]` checks
- **Affected function(s):** `for_customer/2`
- **Short explanation:** The function returns a `%Plan{}` struct, a `{%Plan{}, %UsageSummary{}}` tuple, or a compact `%{name: string, status: atom, next_billing: Date.t()}` map depending on options. The caller cannot predict the type without inspecting opts.
...
defmodule Billing.SubscriptionPlan do
  ...
  # VALIDATION: This is a smell because the return type of for_customer/2 is
  # VALIDATION: either a %Plan{} struct, a {%Plan{}, %UsageSummary{}} tuple, or
  # VALIDATION: a plain summary map. Downstream code — controllers, background
  # VALIDATION: jobs — must always repeat the option check to safely use the
  # VALIDATION: return value, coupling callers to the internal branching logic.
