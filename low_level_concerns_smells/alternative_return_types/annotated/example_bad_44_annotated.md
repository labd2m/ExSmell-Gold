...
- **Expected smell location:** `HR.EmployeeDirectory.lookup/2`, around the `opts[:by]` key matching
- **Affected function(s):** `lookup/2`
- **Short explanation:** The function returns a single `%Employee{}`, a `[%Employee{}]` list, or `nil` / raises depending on the `:by` option — different callers expecting a single record vs a collection will silently get the wrong shape if they don't track the option.
...
defmodule HR.EmployeeDirectory do
  ...
  # VALIDATION: This is a smell because the :by option radically changes the
  # VALIDATION: return type: a single struct (possibly nil or raising), versus
  # VALIDATION: a list of structs. A caller that passes the result to
  # VALIDATION: Enum.each/2 will crash when :by is :id or :email. Callers
  # VALIDATION: must always inspect the option they used to handle the result.
