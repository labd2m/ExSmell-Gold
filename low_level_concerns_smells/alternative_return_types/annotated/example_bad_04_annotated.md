...
- **Expected smell location:** `Notifications.Dispatcher.send/3`
- **Affected function(s):** `send/3`
- **Short explanation:** The `:ack` option causes the function to return either `:ok`, an `{:ok, receipt_id}` tuple, or a `{:ok, %DeliveryReport{}}` struct depending on how much acknowledgment is requested. Callers cannot pattern-match uniformly on the result.
...
defmodule MyApp.Notifications.Dispatcher do
  ...
  # VALIDATION: This is a smell because the opts[:ack] parameter changes what the
  # function returns in an incompatible way: :none returns plain :ok, :receipt
  # returns an {:ok, string} tuple, and :report returns {:ok, %DeliveryReport{}}.
  # Every caller must know at compile/call time which ack level was used before
  # safely handling the result, making the function's type contract effectively
  # invisible from its signature alone.
