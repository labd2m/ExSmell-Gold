...
- **Expected smell location:** `Media.Uploader.upload/2`
- **Affected function(s):** `upload/2`
- **Short explanation:** The `:result` option causes the function to return a plain `url` string, a `%MediaAsset{}` struct, or an `{:ok, url, asset}` three-element tuple. Callers cannot write uniform post-upload handling without inspecting which result mode was requested.
...
defmodule MyApp.Media.Uploader do
  ...
  # VALIDATION: This is a smell because opts[:result] causes three incompatible
  # return types: :url returns a plain binary URL string, :asset returns
  # a %MediaAsset{} struct, and :full returns {:ok, url, %MediaAsset{}} tuple.
  # Each type requires completely different handling in the caller. The error
  # path is consistent ({:error, reason}) but success diverges entirely based on
  # a dynamically-set option that is not visible in the function signature.
