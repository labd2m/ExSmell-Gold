...
- **Expected smell location:** `Integrations.WebhookParser.parse/2`, around the `opts[:raw]` and `opts[:validate]` checks
- **Affected function(s):** `parse/2`
- **Short explanation:** The function returns a raw map (decoded JSON), a `%WebhookEvent{}` struct, or an `{:ok, %WebhookEvent{}} | {:error, reason}` tuple depending on the options, making it impossible to pattern-match the result without knowing which options were used.
...
defmodule Integrations.WebhookParser do
  ...
  # VALIDATION: This is a smell because the return type is either a plain decoded
  # VALIDATION: map, a %WebhookEvent{} struct, or an {:ok/:error, ...} tagged
  # VALIDATION: tuple — three incompatible shapes. Middleware that pipes the
  # VALIDATION: result into a processing function must always reproduce the
  # VALIDATION: option-check logic to dispatch correctly.
