...
- **Expected smell location:** `Logistics.AddressValidator.validate/2`, around the `opts[:enrich]` and `opts[:strict]` checks
- **Affected function(s):** `validate/2`
- **Short explanation:** The function returns a boolean, a `{:ok, %Address{}}` / `{:error, reasons}` tuple, or a `%{valid: bool, normalized: map, suggestions: list}` enriched map depending on options — three incompatible types from a single public function.
...
defmodule Logistics.AddressValidator do
  ...
  # VALIDATION: This is a smell because the function returns a boolean, an
  # VALIDATION: {:ok/:error, ...} tuple, or a rich map depending on which opts
  # VALIDATION: are provided. These are fundamentally incompatible shapes;
  # VALIDATION: every call-site must hard-code knowledge of the opts used
  # VALIDATION: before it can safely use the return value.
