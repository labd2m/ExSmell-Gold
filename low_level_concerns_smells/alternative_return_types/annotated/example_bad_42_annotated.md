...
- **Expected smell location:** `Ecommerce.PricingEngine.get_price/3`, around the `opts[:with_breakdown]` and `opts[:as_cents]` checks
- **Affected function(s):** `get_price/3`
- **Short explanation:** The function returns a `Decimal.t()`, an integer (cents), or a `%{base: Decimal, discount: Decimal, tax: Decimal, total: Decimal}` breakdown map depending on the options, making it impossible to write a single arithmetic pipeline without knowing the opts upfront.
...
defmodule Ecommerce.PricingEngine do
  ...
  # VALIDATION: This is a smell because the return type shifts between a
  # VALIDATION: Decimal scalar, a plain integer, and a rich breakdown map.
  # VALIDATION: Any code doing arithmetic on the result (e.g., summing a cart)
  # VALIDATION: must first branch on the options used, replicating the same
  # VALIDATION: conditional logic at every call-site.
