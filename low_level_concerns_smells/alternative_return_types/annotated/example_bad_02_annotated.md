...
- **Expected smell location:** `Billing.Invoice.generate/2`
- **Affected function(s):** `generate/2`
- **Short explanation:** The `:output` option changes the return type from a structured `%Invoice{}` struct, to an `{:ok, binary}` PDF blob, to an `iolist` of lines. Callers cannot reliably pipe this function's result without first knowing which option was set, breaking composability.
...
defmodule MyApp.Billing.Invoice do
  ...
  # VALIDATION: This is a smell because the return type changes entirely based on
  # opts[:output]. With :struct it returns a %Invoice{} struct, with :pdf it returns
  # an {:ok, binary} tuple (or {:error, reason}), and with :text it returns a plain
  # list of strings (iolist). Each return shape requires completely different
  # handling by the caller, making the function contract ambiguous.
