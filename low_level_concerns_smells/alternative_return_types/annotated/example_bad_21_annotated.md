...
- **Expected smell location:** `Audit.Trail.record_event/2`
- **Affected function(s):** `record_event/2`
- **Short explanation:** The `:persist` option changes the return from a plain `%AuditEvent{}` struct (built but not saved), to an `{:ok, %AuditEvent{}}` tuple (saved synchronously), to `:async` (enqueued for background persistence). These three shapes represent entirely different states of the data and cannot be handled uniformly downstream.
...
defmodule MyApp.Audit.Trail do
  ...
  # VALIDATION: This is a smell because opts[:persist] changes the return type:
  # :none returns a plain %AuditEvent{} struct (no DB interaction at all),
  # :sync returns {:ok, %AuditEvent{}} or {:error, reason} after a blocking
  # DB write, and :async returns the atom :async after enqueuing the event
  # for background processing. Each value represents a completely different
  # operation outcome — not just a different level of detail about the same
  # outcome — making the function contract deeply ambiguous.
