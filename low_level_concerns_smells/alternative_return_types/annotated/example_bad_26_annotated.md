...
- **Expected smell location:** `Warehouse.PutawayEngine.suggest/2`
- **Affected function(s):** `suggest/2`
- **Short explanation:** The `:output` option changes the return from a single `bin_code` string, to a list of `{bin_code, score}` ranked tuples, to a `%PutawayPlan{}` struct with routing instructions. Each shape represents a totally different level of information, and callers must know which was requested to handle the result correctly.
...
defmodule MyApp.Warehouse.PutawayEngine do
  ...
  # VALIDATION: This is a smell because opts[:output] produces incompatible types:
  # :best returns a plain binary string (the top bin code), :ranked returns a
  # list of {bin_code, float} tuples ordered by score, and :plan returns a
  # %PutawayPlan{} struct with routing, equipment, and timing detail.
  # A caller who passes :best cannot use the result as a %PutawayPlan{},
  # and the function signature provides no indication of which type to expect.
