...
- **Expected smell location:** `Inventory.ProductCatalog.search/2`, around the `opts[:paginate]` and `opts[:count_only]` checks
- **Affected function(s):** `search/2`
- **Short explanation:** The function returns an integer count, a `%Scrivener.Page{}` pagination struct, or a plain `[%Product{}]` list depending on the options, making the return type opaque to the caller without inspecting options.
...
defmodule Inventory.ProductCatalog do
  ...
  # VALIDATION: This is a smell because the function can return an integer,
  # VALIDATION: a Scrivener pagination struct, or a plain list of Product structs
  # VALIDATION: depending on the opts. There is no single type that describes
  # VALIDATION: what `search/2` returns, making it difficult to compose safely.
