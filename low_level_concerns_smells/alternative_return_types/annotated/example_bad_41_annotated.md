...
- **Expected smell location:** `Compliance.AuditLog.query/2`, around the `opts[:aggregate]` and `opts[:export]` checks
- **Affected function(s):** `query/2`
- **Short explanation:** The function returns a `[%AuditEntry{}]` list, a `%{total: integer, by_actor: map, by_action: map}` aggregate map, or a NDJSON binary depending on the `:aggregate` and `:export` options — three completely different types.
...
defmodule Compliance.AuditLog do
  ...
  # VALIDATION: This is a smell because the function returns structurally
  # VALIDATION: incompatible types: a list of AuditEntry structs, an aggregate
  # VALIDATION: statistics map, or a raw NDJSON binary string. Code that calls
  # VALIDATION: query/2 must always know which option flags were passed to safely
  # VALIDATION: handle the result, spreading this coupling across the codebase.
