# Annotated Example 26 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with identical names
- **Expected smell location:** Both `defmodule Inventory.StockManager` declarations
- **Affected functions:** `Inventory.StockManager.reserve/3`, `Inventory.StockManager.release/2`, `Inventory.StockManager.commit/2`, `Inventory.StockManager.available_quantity/1`, `Inventory.StockManager.reorder_alert/1`
- **Short explanation:** Two different source files both declare `defmodule Inventory.StockManager`. The BEAM can hold only one definition per module name at a time. When both files are compiled, the second definition silently replaces the first, causing `UndefinedFunctionError` for any caller that relies on functions from the overwritten definition.
...
# VALIDATION: This is a smell because `Inventory.StockManager` is declared here
# and again in a second block below. BEAM will drop one definition, causing
# silent data-correctness and runtime failures in the inventory system.
...
# ── file: lib/inventory/stock_manager_audit.ex  (audit trail added later;
#    developer accidentally reused the parent module name) ───────────────────

# VALIDATION: SMELL START - Modules with identical names
# VALIDATION: This second `defmodule Inventory.StockManager` replaces the
# first at runtime. All callers that use `reserve/3`, `release/2`, `commit/2`,
# `available_quantity/1`, or `reorder_alert/1` will get `UndefinedFunctionError`
# because those functions vanish from BEAM's module table.

defmodule Inventory.StockManager do
  @moduledoc """
  Audit trail utilities for stock management operations.
  Should have been `Inventory.StockManager.Audit` but was mistakenly given
  the same name as the stock management module.
  """
