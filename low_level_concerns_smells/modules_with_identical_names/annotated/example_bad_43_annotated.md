# Annotated Example 43 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with identical names
- **Expected smell location:** Both `defmodule Compliance.Checker` declarations
- **Affected functions:** `Compliance.Checker.run/2`, `Compliance.Checker.check_gdpr/1`, `Compliance.Checker.check_pci/1`, `Compliance.Checker.violations/1`, `Compliance.Checker.generate_report/2`
- **Short explanation:** Two separate source files both declare `defmodule Compliance.Checker`. BEAM silently drops one definition at load time. In a compliance context this is particularly dangerous: if the enforcement functions are discarded, GDPR or PCI violations may go undetected, creating regulatory and legal risk.
...
# VALIDATION: This is a smell because `Compliance.Checker` is declared here
# and also in a second block below. BEAM will discard one definition, silently
# disabling compliance checks that carry serious legal and regulatory weight.
...
# ── file: lib/compliance/checker_remediation.ex  (remediation workflows added
#    later; developer accidentally reused the parent module name) ─────────────

# VALIDATION: SMELL START - Modules with identical names
# VALIDATION: This second `defmodule Compliance.Checker` replaces the first in
# BEAM. `run/2`, `check_gdpr/1`, `check_pci/1`, `violations/1`, and
# `generate_report/2` all vanish, silently disabling all compliance enforcement.
