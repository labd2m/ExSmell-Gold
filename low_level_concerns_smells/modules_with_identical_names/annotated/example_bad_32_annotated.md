# Annotated Example 32 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with identical names
- **Expected smell location:** Both `defmodule Orders.Processor` declarations
- **Affected functions:** `Orders.Processor.submit/1`, `Orders.Processor.confirm/1`, `Orders.Processor.cancel/2`, `Orders.Processor.fulfil/1`, `Orders.Processor.status_history/1`
- **Short explanation:** Two separate source files both declare `defmodule Orders.Processor`. When compiled, only one definition survives in BEAM. The discarded module's functions become permanently inaccessible, breaking the order lifecycle pipeline for any caller that depends on the lost functions.
...
# VALIDATION: This is a smell because `Orders.Processor` is declared here and
# again in a second block below. BEAM will silently drop one version, causing
# order management operations to become permanently unavailable.
...
# ── file: lib/orders/processor_notifications.ex  (notification hooks added
#    later; developer accidentally reused the parent module name) ─────────────

# VALIDATION: SMELL START - Modules with identical names
# VALIDATION: This second `defmodule Orders.Processor` replaces the first in
# BEAM. Functions `submit/1`, `confirm/1`, `cancel/2`, `fulfil/1`, and
# `status_history/1` all become permanently unavailable, breaking the order
# lifecycle.

defmodule Orders.Processor do
  @moduledoc """
  Notification hooks triggered by order lifecycle transitions.
  Was intended to be `Orders.Processor.Notifications` but received the
  same module name as the core processor by mistake.
  """
