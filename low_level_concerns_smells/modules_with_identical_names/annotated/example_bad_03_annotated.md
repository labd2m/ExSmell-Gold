# Annotated Example 03 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Logistics.Shipment`
- **Affected functions:** `Logistics.Shipment.create/1` (file one) and `Logistics.Shipment.track/1` (file two)
- **Explanation:** `Logistics.Shipment` is declared in both `lib/logistics/shipment.ex` and `lib/logistics/shipment_tracking.ex`. Only one can survive in BEAM's module table. Whichever file compiles second wins, and all functions unique to the first file are silently lost.
...
defmodule Logistics.Shipment do
  ...
  # VALIDATION: This is a smell because the module `Logistics.Shipment` is
  # defined a second time in `lib/logistics/shipment_tracking.ex`. BEAM will
  # only keep one version. `create/1` may become unreachable if that file wins.
