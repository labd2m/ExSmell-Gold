# Annotated Example 02 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Auth.Session`
- **Affected functions:** `Auth.Session.create/2` (module_one) and `Auth.Session.revoke/1` (module_two)
- **Explanation:** The module `Auth.Session` is declared twice across two files. The BEAM will silently drop whichever is compiled first, making the functions in that version unreachable at runtime and creating a subtle security gap where, for instance, session creation logic may disappear while revocation logic survives — or vice versa.
...
defmodule Auth.Session do
  ...
  # VALIDATION: This is a smell because `Auth.Session` is defined again in
  # `lib/auth/session_revocation.ex`. BEAM loads one and silently discards the
  # other. `create/2` will be unavailable if the revocation file compiles last.
