# Annotated Example 05 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Payments.Processor`
- **Affected functions:** `Payments.Processor.charge/2` (file one) and `Payments.Processor.refund/2` (file two)
- **Explanation:** The module name `Payments.Processor` is reused across `lib/payments/processor.ex` and `lib/payments/refund_processor.ex`. At runtime, only one definition can exist; whichever file is compiled last takes over, making the other's functions invisible to the system.
...
defmodule Payments.Processor do
  ...
  # VALIDATION: This is a smell because `Payments.Processor` is also defined in
  # `lib/payments/refund_processor.ex`. BEAM can only hold one module with this
  # name. If the refund file wins, `charge/2` and its helpers vanish entirely.
