# Annotated Example 44 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with identical names
- **Expected smell location:** Both `defmodule Fraud.Detector` declarations
- **Affected functions:** `Fraud.Detector.evaluate/2`, `Fraud.Detector.score/1`, `Fraud.Detector.flag/2`, `Fraud.Detector.clear_flag/1`, `Fraud.Detector.risk_summary/1`
- **Short explanation:** Two separate source files both declare `defmodule Fraud.Detector`. BEAM silently discards the first definition when the second is loaded. Losing fraud detection functions means financial transactions skip risk checks entirely, which can result in undetected fraudulent activity and direct financial harm.
...
# VALIDATION: This is a smell because `Fraud.Detector` is declared here and
# again in a second block below. BEAM will drop one definition, silently
# disabling fraud detection with potentially severe financial consequences.
...
# ── file: lib/fraud/detector_allowlist.ex  (allowlist management added later;
#    developer accidentally reused the parent module name) ────────────────────

# VALIDATION: SMELL START - Modules with identical names
# VALIDATION: This second `defmodule Fraud.Detector` replaces the first in
# BEAM. `evaluate/2`, `score/1`, `flag/2`, `clear_flag/1`, and `risk_summary/1`
# all vanish, letting every transaction skip fraud detection entirely.
