# Annotated Example 20 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Support.Ticket`
- **Affected functions:** `Support.Ticket.open/2` (file one) and `Support.Ticket.escalate/2` (file two)
- **Explanation:** `Support.Ticket` appears in both `lib/support/ticket.ex` and `lib/support/ticket_escalation.ex`. BEAM silently drops the first definition when the second is loaded. Depending on compilation order, either ticket creation or escalation will stop working, impacting the customer support queue.
...
defmodule Support.Ticket do
  ...
  # VALIDATION: This is a smell because `Support.Ticket` is defined again in
  # `lib/support/ticket_escalation.ex`. BEAM keeps only the last-compiled
  # version. If escalation compiles second, `open/2` vanishes, making it
  # impossible to create new support tickets on the platform.
