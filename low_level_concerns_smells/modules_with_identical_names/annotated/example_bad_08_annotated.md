# Annotated Example 08 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Reports.Generator`
- **Affected functions:** `Reports.Generator.generate/2` (file one) and `Reports.Generator.export/2` (file two)
- **Explanation:** `Reports.Generator` is defined in `lib/reports/generator.ex` and again in `lib/reports/export_generator.ex`. The BEAM cannot hold two distinct modules under the same atom key. One will silently shadow the other, making either report generation or export functionality unreachable.
...
defmodule Reports.Generator do
  ...
  # VALIDATION: This is a smell because `Reports.Generator` is defined again
  # in `lib/reports/export_generator.ex`. When BEAM loads both files, only one
  # definition survives. `generate/2` may become unavailable, breaking scheduled
  # report jobs and dashboard data pipelines.
