# Annotated Example 15 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Compliance.AuditLog`
- **Affected functions:** `Compliance.AuditLog.record/3` (file one) and `Compliance.AuditLog.export/2` (file two)
- **Explanation:** `Compliance.AuditLog` appears in `lib/compliance/audit_log.ex` and again in `lib/compliance/audit_export.ex`. The BEAM only loads one module per name, so one definition will silently win. Either the `record/3` write path or the `export/2` read path will vanish, creating a compliance gap.
...
defmodule Compliance.AuditLog do
  ...
  # VALIDATION: This is a smell because `Compliance.AuditLog` is defined again
  # in `lib/compliance/audit_export.ex`. BEAM cannot load both; the second
  # replaces the first. `record/3` — the core write path — may become
  # unreachable, leaving security-sensitive operations unlogged.
