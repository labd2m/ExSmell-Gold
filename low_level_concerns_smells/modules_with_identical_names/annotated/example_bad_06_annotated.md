# Annotated Example 06 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Inventory.Stock`
- **Affected functions:** `Inventory.Stock.reserve/2` (file one) and `Inventory.Stock.replenish/2` (file two)
- **Explanation:** `Inventory.Stock` is defined in both `lib/inventory/stock.ex` and `lib/inventory/stock_replenishment.ex`. Because the BEAM module table uses the module name as a key, one file's functions will be silently discarded at load time, leading to runtime errors in either the reservation or replenishment path.
...
defmodule Inventory.Stock do
  ...
  # VALIDATION: This is a smell because `Inventory.Stock` is defined again in
  # `lib/inventory/stock_replenishment.ex`. The BEAM module loader keeps only
  # the last-compiled definition. `reserve/2` and `release/1` may disappear,
  # causing silent failures in the order checkout pipeline.
