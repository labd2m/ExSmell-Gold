# Annotated Example 23 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with identical names
- **Expected smell location:** Both `defmodule Auth.Token` declarations
- **Affected functions:** `Auth.Token.generate/2`, `Auth.Token.verify/1`, `Auth.Token.revoke/1`, `Auth.Token.refresh/1`, `Auth.Token.claims_for/1`
- **Short explanation:** Two separate source files both declare `defmodule Auth.Token`. Because BEAM loads only one module definition per name, the second file's definition silently replaces the first, making any function exclusive to the first module impossible to call — a critical failure in an authentication context.
...
# VALIDATION: This is a smell because `Auth.Token` is defined here and again
# in a second block below. The BEAM will silently drop one definition at load
# time, causing hard-to-diagnose runtime errors in authentication flows.
...
# ── file: lib/auth/token_claims.ex  (a developer added claim-inspection helpers
#    but forgot to namespace the module as Auth.Token.Claims) ─────────────────

# VALIDATION: SMELL START - Modules with identical names
# VALIDATION: This second `defmodule Auth.Token` redefines the module.
# After compilation, BEAM retains only this version, making `generate/2`,
# `verify/1`, `revoke/1`, and `refresh/1` from the first block unreachable.
# This is catastrophic for the authentication subsystem.

defmodule Auth.Token do
  @moduledoc """
  Helper utilities for inspecting and extracting claims from Auth tokens.
  Was intended to be defined as `Auth.Token.Claims` but was accidentally
  given the parent module's name.
  """
