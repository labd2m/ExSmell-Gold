# Annotated Example 36 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with identical names
- **Expected smell location:** Both `defmodule RateLimiter.Policy` declarations
- **Affected functions:** `RateLimiter.Policy.check/3`, `RateLimiter.Policy.consume/3`, `RateLimiter.Policy.remaining/2`, `RateLimiter.Policy.reset/2`, `RateLimiter.Policy.block_until/2`
- **Short explanation:** Two source files both declare `defmodule RateLimiter.Policy`. BEAM can only load one definition per module name; the second definition replaces the first, making the rate-limiting enforcement functions permanently unavailable and potentially allowing unlimited API calls or resource abuse.
...
# VALIDATION: This is a smell because `RateLimiter.Policy` is defined here and
# again in a second block below. BEAM will discard one definition, breaking
# rate-limit enforcement and potentially exposing APIs to abuse.
...
# ── file: lib/rate_limiter/policy_admin.ex  (admin reset tooling added later;
#    developer accidentally reused the module name) ──────────────────────────

# VALIDATION: SMELL START - Modules with identical names
# VALIDATION: This second `defmodule RateLimiter.Policy` replaces the first.
# `check/3`, `consume/3`, `remaining/2`, `reset/2`, and `block_until/2` all
# vanish from BEAM, disabling rate limit enforcement across the application.

defmodule RateLimiter.Policy do
  @moduledoc """
  Admin tooling for inspecting and managing rate limit policies at runtime.
  Was intended to be `RateLimiter.Policy.Admin` but was accidentally given the
  same name as the core policy module.
  """
