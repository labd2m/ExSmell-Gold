# Annotated Example 12 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `HR.Employee`
- **Affected functions:** `HR.Employee.onboard/1` (file one) and `HR.Employee.terminate/2` (file two)
- **Explanation:** `HR.Employee` is defined in `lib/hr/employee.ex` and again in `lib/hr/employee_offboarding.ex`. BEAM tracks modules by atom name; loading the second file causes it to silently replace the first. Either onboarding or offboarding logic will be completely unreachable depending on compilation order.
...
defmodule HR.Employee do
  ...
  # VALIDATION: This is a smell because `HR.Employee` is also defined in
  # `lib/hr/employee_offboarding.ex`. BEAM can only keep one version loaded.
  # If the offboarding file compiles second, `onboard/1` disappears entirely,
  # breaking new-hire provisioning without any compile-time warning.
