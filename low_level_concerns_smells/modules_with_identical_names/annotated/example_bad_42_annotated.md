# Annotated Example 42 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with identical names
- **Expected smell location:** Both `defmodule Media.Transcoder` declarations
- **Affected functions:** `Media.Transcoder.submit_job/3`, `Media.Transcoder.status/1`, `Media.Transcoder.cancel_job/1`, `Media.Transcoder.outputs/1`, `Media.Transcoder.supported_formats/0`
- **Short explanation:** Two source files both declare `defmodule Media.Transcoder`. BEAM can only keep one definition loaded; the second file's definition replaces the first at compile time, permanently removing all functions from the overwritten version and causing media transcoding pipelines to crash at runtime.
...
# VALIDATION: This is a smell because `Media.Transcoder` is declared here and
# again in a second block below. BEAM will drop one definition, breaking the
# media transcoding pipeline for any caller relying on lost functions.
...
# ── file: lib/media/transcoder_presets.ex  (preset management added later;
#    developer forgot to create a distinct namespace) ────────────────────────

# VALIDATION: SMELL START - Modules with identical names
# VALIDATION: This second `defmodule Media.Transcoder` replaces the first.
# `submit_job/3`, `status/1`, `cancel_job/1`, `outputs/1`, and
# `supported_formats/0` all vanish from BEAM, silently breaking the entire
