# Annotated Example 19 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `IoT.Device`
- **Affected functions:** `IoT.Device.provision/2` (file one) and `IoT.Device.ingest_telemetry/2` (file two)
- **Explanation:** `IoT.Device` is defined in both `lib/iot/device.ex` and `lib/iot/device_telemetry.ex`. BEAM's module loader will overwrite the first with the second. Either device provisioning or telemetry ingestion will become unreachable, breaking the IoT platform's two most critical paths.
...
defmodule IoT.Device do
  ...
  # VALIDATION: This is a smell because `IoT.Device` is defined a second time
  # in `lib/iot/device_telemetry.ex`. BEAM keeps only one version. If the
  # telemetry file compiles last, `provision/2` disappears entirely, halting
  # device onboarding across the manufacturing and self-service pipelines.
