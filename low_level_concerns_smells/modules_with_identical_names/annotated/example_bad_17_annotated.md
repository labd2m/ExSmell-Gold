# Annotated Example 17 — Modules with Identical Names

## Metadata

- **Smell name:** Modules with Identical Names
- **Expected smell location:** Two separate files both define `Fleet.Vehicle`
- **Affected functions:** `Fleet.Vehicle.register/1` (file one) and `Fleet.Vehicle.assign_driver/2` (file two)
- **Explanation:** `Fleet.Vehicle` is defined in both `lib/fleet/vehicle.ex` and `lib/fleet/vehicle_assignments.ex`. When BEAM loads both, the second replaces the first. Either vehicle registration or driver assignment functionality will be silently lost, breaking fleet operations.
...
defmodule Fleet.Vehicle do
  ...
  # VALIDATION: This is a smell because `Fleet.Vehicle` is defined a second
  # time in `lib/fleet/vehicle_assignments.ex`. BEAM cannot load two modules
  # under the same name. `register/1` may be lost if the assignment file
  # compiles last, preventing new vehicles from entering the fleet.
