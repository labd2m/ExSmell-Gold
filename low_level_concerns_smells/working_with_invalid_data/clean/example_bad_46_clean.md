# Example 46: Multi-Currency FX Conversion Service
...
defmodule Finance.FxConverter do
    ...
    with {:ok, account} <- TreasuryAccount.get(account_id),
      ...
      gain_loss_by_currency =
        ...
        |> Enum.map(fn {currency, recs} ->
          ...
          gain_loss = current_value - total_converted
          {currency, %{total_original: total_original, book_value: Float.round(total_converted, 2), current_value: Float.round(current_value, 2), gain_loss: Float.round(gain_loss, 2)}}
        end)
        |> Map.new()
  ...
  defp compute_conversion_fee(amount, _currency, _account_id), do: amount * 0.005
