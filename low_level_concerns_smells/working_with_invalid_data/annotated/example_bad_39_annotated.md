# Example 39: SaaS Subscription Billing Engine - Annotated

## Metadata
- **Smell Name**: Working with invalid data
- **Expected Location**: `Billing.SubscriptionEngine.apply_coupon/3` function
- **Affected Functions**: `apply_coupon/3`
- **Explanation**: The function does not validate the type of `discount_value` from the coupon before passing it to `Decimal.mult/2`. If the value is a raw string or atom instead of a Decimal or numeric, the error will surface deep inside the Decimal library rather than at the function boundary.
...
defmodule Billing.SubscriptionEngine do
  ...
  # VALIDATION: This is a smell because the `coupon` map's `discount_value` field
  # VALIDATION: is not validated to be a Decimal or numeric type before being
  # VALIDATION: passed to Decimal.mult/2. If a caller retrieves a coupon whose
  # VALIDATION: discount_value was stored as a string (e.g., "15.00"), the error
  # VALIDATION: will emerge inside the Decimal library with no indication that the
  # VALIDATION: problem originated from the coupon parameter in apply_coupon/3.
  def apply_coupon(subscription_id, coupon_code, invoice_amount) do
    with {:ok, subscription} <- Subscription.get(subscription_id),
      ...
      # discount_value is used in Decimal arithmetic without type validation
      discounted_amount =
