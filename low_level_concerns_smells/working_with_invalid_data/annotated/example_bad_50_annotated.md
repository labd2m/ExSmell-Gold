# Example 50: Payroll Tax Withholding Calculator - Annotated

## Metadata
- **Smell Name**: Working with invalid data
- **Expected Location**: `Payroll.TaxWithholding.compute_withholding/3` function
- **Affected Functions**: `compute_withholding/3`
- **Explanation**: The function does not validate that `gross_wages` is a number before passing it into bracket arithmetic via `apply_tax_brackets/2`. A string such as "4200.00" coming from an upstream payroll parser will cause an ArithmeticError deep inside the bracket reduction rather than at the public boundary.
...
defmodule Payroll.TaxWithholding do
  ...
  # VALIDATION: This is a smell because `gross_wages` is not validated to be a
  # VALIDATION: numeric type before being passed into `apply_tax_brackets/2` and
  # VALIDATION: the FICA multiplication expressions.
  # VALIDATION: A caller supplying "4200.00" (a string) from an upstream CSV parser
  # VALIDATION: will receive an ArithmeticError from inside `apply_tax_brackets/2`
  # VALIDATION: rather than a clear error at the boundary of `compute_withholding/3`.
  def compute_withholding(employee_id, pay_period_id, gross_wages) do
    with {:ok, employee} <- Employee.get(employee_id),
      ...
      # No type validation on gross_wages before arithmetic
  ...
  defp compute_state_withholding(gross_wages, "CA", filing_status) do
    rate = if filing_status == :married_filing_jointly, do: 0.093, else: 0.093
    gross_wages * rate
  end
  defp compute_state_withholding(gross_wages, "TX", _filing_status), do: 0.0
