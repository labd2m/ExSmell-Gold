# Example 48: Ad Campaign Budget Pacing Service - Annotated

## Metadata
- **Smell Name**: Working with invalid data
- **Expected Location**: `Advertising.BudgetPacer.update_daily_cap/3` function
- **Affected Functions**: `update_daily_cap/3`
- **Explanation**: The function does not validate that `total_budget` and `flight_days` are numbers before using them in division and comparison. Passing a string or nil for either will raise an ArithmeticError or FunctionClauseError inside the calculation rather than at the boundary.
...
defmodule Advertising.BudgetPacer do
  ...
  # VALIDATION: This is a smell because `total_budget` and `flight_days` are used
  # VALIDATION: in division and comparison without any type validation at the boundary.
  # VALIDATION: If a caller passes total_budget as a string like "50000.00" or
  # VALIDATION: flight_days as an atom like :thirty, the ArithmeticError will surface
  # VALIDATION: inside the division expression rather than at this function's entry point,
  # VALIDATION: making it harder to trace the root cause.
  def update_daily_cap(campaign_id, total_budget, flight_days) do
    with {:ok, campaign} <- Campaign.get(campaign_id),
      ...
      # No type validation on total_budget or flight_days before arithmetic
