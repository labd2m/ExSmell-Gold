# Example 42: Telematics GPS Route Scoring Service - Annotated

## Metadata
- **Smell Name**: Working with invalid data
- **Expected Location**: `Telematics.RouteScorer.score_trip/3` function
- **Affected Functions**: `score_trip/3`
- **Explanation**: The function does not validate that `telemetry_samples` is a list of maps with numeric fields before piping them into arithmetic aggregation. Non-numeric field values will cause confusing ArithmeticError crashes inside the reduction, not at the function boundary.
...
defmodule Telematics.RouteScorer do
  ...
  # VALIDATION: This is a smell because `telemetry_samples` is not validated
  # VALIDATION: before its fields are used in arithmetic aggregation inside
  # VALIDATION: `aggregate_metrics/1` and the scoring functions. If any sample
  # VALIDATION: contains a string speed, nil acceleration, or a binary timestamp,
  # VALIDATION: the error will emerge deep in the reduction pipeline rather than
  # VALIDATION: at the boundary of this public function.
  def score_trip(trip_id, driver_id, telemetry_samples) do
    with {:ok, trip} <- Trip.get(trip_id),
      ...
      # telemetry_samples is used directly without type or structure validation
