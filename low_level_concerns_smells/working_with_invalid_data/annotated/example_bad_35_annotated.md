...
- **Expected smell location:** `CurrencyConverter.convert/4`, where `exchange_rate` is passed to `Decimal.mult/2`
- **Affected function(s):** `convert/4`
- **Short explanation:** The `exchange_rate` is fetched from an external rate provider and passed directly to `Decimal.mult/2` (via `Decimal.new/1`) without checking that it is a numeric value or a valid numeric string. External rate APIs occasionally return error sentinel values or `null` for illiquid pairs. If `exchange_rate` is `nil` or an atom like `:unavailable`, `Decimal.new/1` raises a `Decimal.Error` deep in the library, with no reference to the `convert/4` boundary.
    ...
    with :ok <- validate_currencies(from_currency, to_currency),
      ...
      if from_currency == to_currency do
        ...
        with {:ok, exchange_rate, rate_fetched_at} <-
          ...
          # VALIDATION: This is a smell because `adjusted_rate` (derived from
          # VALIDATION: `exchange_rate`) is passed to `Decimal.new/1` inside
          # VALIDATION: `Decimal.mult/2` without any type check. The external
          # VALIDATION: rate provider may return nil for illiquid pairs or an
          # VALIDATION: atom like :unavailable on degraded service. The resulting
          # VALIDATION: Decimal.Error will surface deep in the Decimal library,
          # VALIDATION: with no trace to this function boundary.
          converted =
            Decimal.new(amount)
