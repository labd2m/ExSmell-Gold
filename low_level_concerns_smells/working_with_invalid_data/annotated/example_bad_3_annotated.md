...
- **Expected smell location:** `TokenService.issue_token/3`, where `expires_in` is passed to `DateTime.add/3`
- **Affected function(s):** `issue_token/3`
- **Short explanation:** The `expires_in` parameter is expected to be an integer number of seconds, but it is passed directly to `DateTime.add/3` without any type validation. If a caller passes a string like `"3600"` or an atom, `DateTime.add/3` will raise a `FunctionClauseError` deep in the DateTime module, making the root cause (invalid input at the boundary) hard to diagnose.
...
defmodule MyApp.Auth.TokenService do
  ...
  def issue_token(user, type, opts \\ []) do
    ...
    # VALIDATION: This is a smell because `expires_in` is passed directly to
    # VALIDATION: `DateTime.add/3` assuming it is an integer. If a caller
    # VALIDATION: overrides this via opts and passes a non-integer (e.g. a string),
    # VALIDATION: the error will be raised inside DateTime.add with a FunctionClauseError
    # VALIDATION: that does not mention the original invalid input.
