...
- **Expected smell location:** `ShippingCalculator.quote/3`, where `weight_kg` is used in arithmetic to compute the base rate
- **Affected function(s):** `quote/3`
- **Short explanation:** The `weight_kg` parameter is used directly in a multiplication against `rate_per_kg` without any type check at the function boundary. If a caller passes a string such as `"3.5"` — common when weight values come from parsed CSV manifests or form inputs — the `ArithmeticError` surfaces inside the multiplication expression, pointing at a private helper rather than the public entry point where invalid data was accepted.
...
defmodule MyApp.Logistics.ShippingCalculator do
  ...
  def quote(weight_kg, origin_address, destination_address, opts \\ []) do
    ...
    with :ok <- validate_service_level(service_level),
      ...
      # VALIDATION: This is a smell because `weight_kg` is used in arithmetic
      # VALIDATION: (`billable_weight * rate_table.rate_per_kg`) without any
      # VALIDATION: type check. CSV import pipelines or web form handlers often
      # VALIDATION: pass this as a string like "3.5". The ArithmeticError will be
      # VALIDATION: raised inside `compute_base_rate/2`, not at this boundary.
      dimensional_weight =
