# Example 36: Real Estate Property Valuation System - Annotated

## Metadata
- **Smell Name**: Working with invalid data
- **Expected Location**: `PropertyValuation.calculate_appraisal/3` function
- **Affected Functions**: `calculate_appraisal/3`
- **Explanation**: The function does not validate the `adjustments` parameter (expected to be a keyword list of numeric values) before passing each value into arithmetic operations. If a caller passes a string, atom, or nil for any adjustment, the error will surface deep inside a calculation helper rather than at the boundary.
...
defmodule RealEstate.PropertyValuation do
  ...
  # VALIDATION: This is a smell because `adjustments` is never validated before
  # VALIDATION: its values are passed to arithmetic operators. If any adjustment
  # VALIDATION: value is a string or atom (e.g., adjustments: [lot_size: "big"]),
  # VALIDATION: the ArithmeticError will originate inside `apply_adjustments/2`,
  # VALIDATION: making it hard to trace back to the original caller.
  def calculate_appraisal(property_id, comparable_ids, adjustments) do
    with {:ok, property} <- Property.get(property_id),
      ...
      # adjustments is used directly without any type or shape validation
  ...
  defp apply_adjustments(base_price, adjustments) do
    Enum.reduce(adjustments, base_price, fn {key, value}, acc ->
      ...
      # value is used here in arithmetic without prior validation
      acc + value * weight
    end)
  end
  ...
  defp validate_comparables_coverage(comparables, property) do
