...
- **Expected smell location:** `InvoiceCalculator.calculate_total/1`, lines where `unit_price` and `quantity` are used in arithmetic
- **Affected function(s):** `calculate_total/1`, `apply_line_discount/2`
- **Short explanation:** The function accepts a list of line items and directly performs arithmetic on `unit_price` and `quantity` without validating that they are numbers. If a caller passes strings or nil values, the error will surface deep inside arithmetic operations, making it hard to trace back to the invalid input.
...
defmodule MyApp.Billing.InvoiceCalculator do
  ...
  def calculate_total(invoice) do
    ...
    # VALIDATION: This is a smell because `line_items` elements are used directly
    # VALIDATION: in arithmetic without validating that `unit_price` and `quantity`
    # VALIDATION: are actual numbers. If invalid types are passed, errors will surface
    # VALIDATION: inside Enum/arithmetic calls far from the entry point.
    subtotal =
      invoice.line_items
