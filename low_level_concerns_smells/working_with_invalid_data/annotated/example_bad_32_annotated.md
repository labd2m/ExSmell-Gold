...
- **Expected smell location:** `LoanAmortizer.generate_schedule/4`, where `principal` is used in `:math.pow/2` and arithmetic
- **Affected function(s):** `generate_schedule/4`
- **Short explanation:** The `principal` parameter is fed into the standard loan amortization formula involving `:math.pow/2` and multiplication without any type check at the function boundary. If a caller passes a string such as `"250000"` (common when parsing form data), the arithmetic expression raises a `BadArgumentError` inside the Erlang `:math` module, with no message pointing back to the `generate_schedule/4` boundary where the invalid value was accepted.
...
defmodule MyApp.Finance.LoanAmortizer do
  ...
  def generate_schedule(principal, annual_rate, term_months, opts \\ []) do
    ...
    with :ok <- validate_loan_type(loan_type),
      ...
      # VALIDATION: This is a smell because `principal` is used directly in
      # VALIDATION: `:math.pow/2` and multiplication without any type check.
      # VALIDATION: Loan origination systems often receive form-submitted data
      # VALIDATION: where the principal is a string like "250000". Passing this
      # VALIDATION: to `:math.pow/2` causes a BadArgumentError inside the Erlang
      # VALIDATION: :math module with no trace to this function boundary.
      monthly_payment =
