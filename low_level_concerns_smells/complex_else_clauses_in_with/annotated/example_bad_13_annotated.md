...
- **Expected smell location:** `fulfill_order/2`, inside the `with` block's `else` clause
- **Affected function(s):** `fulfill_order/2`
- **Short explanation:** Order fulfillment involves six sequential operations—validating the order state, locking stock, generating a packing slip, assigning a picker, posting to the WMS, and triggering the carrier API. All six produce different errors, and every error is merged into one flat `else` block.
...
defmodule Fulfillment.OrderFulfiller do
  ...
  def fulfill_order(order_id, fulfillment_center_id) do
    with {:ok, order} <- fetch_ready_order(order_id),
      ...
      # VALIDATION: This is a smell because errors from six steps (order validation, stock
      # locking, packing slip generation, picker assignment, WMS posting, and carrier API)
      # are all flattened into a single `else` block. Errors like `:not_found`,
      # `:already_fulfilled`, `:stock_lock_failed`, `:partial_lock`, `:slip_render_error`,
      # `:no_picker_available`, `:wms_timeout`, `:carrier_rejected` each originate from
      # a distinct step, yet nothing in the `else` block reflects that structure.
      {:error, :not_found} ->
