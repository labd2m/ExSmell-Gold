...
- **Expected smell location:** `submit_prescription/3`, inside the `with` block's `else` clause
- **Affected function(s):** `submit_prescription/3`
- **Short explanation:** Prescription submission involves five steps—validating the prescriber's credentials, verifying the patient's record, checking for drug interactions, applying formulary rules, and creating the prescription record. Each step fails with its own distinct error value, yet all errors are handled in a single `else` block, providing no structural clue about which step produced any given failure.
...
defmodule Healthcare.PrescriptionSubmitter do
  ...
  def submit_prescription(prescriber_id, patient_id, rx_params) do
    ...
    with {:ok, prescriber} <- fetch_licensed_prescriber(prescriber_id, medication_class),
      ...
      # VALIDATION: This is a smell because five unrelated pipeline steps each produce
      # different error shapes, all merged into one `else` block. `:prescriber_not_found`,
      # `:license_inactive`, and `:dea_required` come from prescriber validation;
      # `:patient_not_found` and `:patient_inactive` from patient fetching;
      # `{:interaction_warning, _}` and `:interaction_contraindicated` from interaction
      # checking; `:not_on_formulary` and `:prior_auth_required` from formulary evaluation;
      # and `:prescription_db_error` from persistence — with no grouping in the `else` block.
