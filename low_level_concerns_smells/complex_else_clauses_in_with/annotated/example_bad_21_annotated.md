...
- **Expected smell location:** `transfer_asset/3`, inside the `with` block's `else` clause
- **Affected function(s):** `transfer_asset/3`
- **Short explanation:** Asset transfer chains five operations—fetching the asset, verifying ownership, checking the recipient account, validating transfer eligibility, and recording the transfer. All error shapes from these steps are flattened into one `else` block, making it impossible to tell which step caused a given failure without tracing each helper individually.
...
defmodule AssetManagement.AssetTransferService do
  ...
  def transfer_asset(asset_id, from_account_id, to_account_id) do
    with {:ok, asset} <- fetch_transferable_asset(asset_id),
      ...
      # VALIDATION: This is a smell because errors from five independent pipeline steps are
      # all handled in one `else` block. `:asset_not_found` and `:asset_not_transferable`
      # come from asset fetching; `:not_the_owner` from ownership verification;
      # `:recipient_not_found` and `:recipient_suspended` from recipient lookup;
      # `:compliance_hold`, `:cross_border_restricted`, and `:cooldown_active` from
      # compliance checking; and `:record_failed` from transfer recording.
      # The `else` block gives no structural hint about which step each error belongs to.
