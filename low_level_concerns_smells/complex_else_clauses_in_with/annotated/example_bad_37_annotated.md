# Annotated Example 37 — Complex else clauses in with

## Metadata

- **Smell name:** Complex else clauses in with
- **Expected smell location:** `renew_subscription/1`, inside the `with` expression's `else` block
- **Affected function(s):** `renew_subscription/1`
- **Short explanation:** Five sequential renewal steps each produce different error structures. All are flattened into a single `else` block, making it challenging to identify which step originated a given error pattern without examining all five clauses in the `with`.
...
defmodule Billing.SubscriptionRenewal do
  ...
  def renew_subscription(subscription_id) do
    ...
    # VALIDATION: This is a smell because five with-clauses produce errors of
    # completely different shapes (nil, {:error, :invalid, _},
    # {:declined, _, _}, {:error, :extend, _}, {:error, :mail}).
    # Collecting all in one else block makes it impossible to tell which step
    # a given error pattern came from at a glance.
