# Annotated Example 28 — Complex else clauses in with

## Metadata

- **Smell name:** Complex else clauses in with
- **Expected smell location:** `process_invoice/2`, inside the `with` expression's `else` block
- **Affected function(s):** `process_invoice/2`
- **Short explanation:** The `with` statement chains four distinct steps, each of which can fail with a different error shape. All failure cases are collapsed into a single `else` block, making it impossible to tell at a glance which pipeline step produced a given error pattern. A reader must mentally trace every clause to understand the error provenance, hurting readability and maintainability.
...
defmodule Billing.InvoiceProcessor do
  ...
  def process_invoice(customer_id, raw_payload) do
    ...
    # VALIDATION: This is a smell because four with-clauses can each fail with
    # a structurally different error value ({:error, :not_found},
    # {:error, :validation, _}, {:error, :tax, _}, {:error, changeset}),
    # yet all cases are handled in one flat else block. The reader cannot tell
    # which clause produced {:error, :not_found} vs {:error, changeset} without
    # carefully re-reading every step.
