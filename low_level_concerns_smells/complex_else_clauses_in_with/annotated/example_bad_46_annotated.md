# Annotated Example 46 — Complex else clauses in with

## Metadata

- **Smell name:** Complex else clauses in with
- **Expected smell location:** `handle_command/2`, inside the `with` expression's `else` block
- **Affected function(s):** `handle_command/2`
- **Short explanation:** Five steps in the command-handling pipeline each fail with structurally different shapes. Consolidating all in one `else` block destroys the per-step error attribution, making the error-handling logic hard to reason about and extend.
...
defmodule EventStore.CommandHandler do
  ...
  def handle_command(aggregate_id, command) do
    ...
    # unique error shape ({:error, :schema, _}, {:error, :not_found},
    # {:error, :rule, _}, {:error, :factory, _}, {:error, :conflict, _}).
    # The undifferentiated else block couples all five failure paths together,
    # obscuring which step produced a given error.
