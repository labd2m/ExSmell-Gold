# Annotated Bad Example 2

- **Smell name:** Complex else clauses in with
- **Expected smell location:** `authenticate_user/2`, inside the `with` block's `else` clause
- **Affected function(s):** `authenticate_user/2`
- **Short explanation:** Four sequential authentication steps each produce distinct error values. All of them are handled in a single monolithic `else` block, obscuring which step failed and making the control flow harder to reason about.
...
defmodule Auth.SessionManager do
  ...
  def authenticate_user(credentials, request_metadata) do
    with {:ok, user} <- resolve_user(credentials.identifier),
      ...
      # VALIDATION: This is a smell because the `else` block mixes errors from four completely
      # different steps: user lookup (`:not_found`), password check (`:invalid_password`),
      # account status (`:locked`, `:suspended`, `:unverified`), and MFA
      # (`:mfa_required`, `:invalid_mfa_code`, `:mfa_expired`). It is not clear which
      # step produced each error without reading all helper functions in parallel.
      {:error, :not_found} ->
