# Annotated Bad Example 5

- **Smell name:** Complex else clauses in with
- **Expected smell location:** `execute_refund/3`, inside the `with` block's `else` clause
- **Affected function(s):** `execute_refund/3`
- **Short explanation:** A chain of five refund-pipeline steps each produces its own error shape. All of them funnel into a single `else` block with eight pattern-match clauses, making it hard to associate any given error with the pipeline step that produced it.
...
defmodule Payments.RefundProcessor do
  ...
  def execute_refund(order_id, amount_cents, requested_by) do
    with {:ok, order} <- fetch_completed_order(order_id),
      ...
      # VALIDATION: This is a smell because the `else` block aggregates errors from five
      # unrelated steps: order fetching, payment fetching, refund window validation,
      # amount validation, and gateway calls. The error atoms `:not_found`,
      # `:not_completed`, `:no_successful_payment`, `:refund_window_expired`,
      # `:exceeds_original_amount`, `:already_fully_refunded`, `:gateway_declined`,
      # and `:gateway_unavailable` each come from a different step, making the
      # `else` block a single point of confusion for all failure modes.
      {:error, :not_found} ->
