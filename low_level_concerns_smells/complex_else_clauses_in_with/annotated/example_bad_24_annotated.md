...
- **Expected smell location:** `moderate_post/2`, inside the `with` block's `else` clause
- **Affected function(s):** `moderate_post/2`
- **Short explanation:** Post moderation runs five checks in sequence—fetching the post, verifying the moderator's role, running the automated classifier, applying manual moderation rules, and persisting the decision. Each check fails with distinct error shapes, all of which are collapsed into one `else` block, obscuring the failure's originating step.
...
defmodule Moderation.PostModerationService do
  ...
  def moderate_post(post_id, moderator_id) do
    with {:ok, post} <- fetch_pending_post(post_id),
      ...
      # VALIDATION: This is a smell because errors from five sequential steps are all
      # merged into a single `else` block. `:post_not_found` and `:already_moderated`
      # come from post fetching; `:moderator_not_found` and `:insufficient_permissions`
      # from moderator validation; `:classifier_unavailable` and `{:classifier_error, _}`
      # from the classifier call; `:rule_conflict` and `:rule_engine_error` from rule
      # application; and `:decision_record_failed` from persistence —
      # none of them attributable to a specific step from the `else` block alone.
