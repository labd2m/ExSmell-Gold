# Annotated Bad Example 9

- **Smell name:** Complex else clauses in with
- **Expected smell location:** `generate_report/3`, inside the `with` block's `else` clause
- **Affected function(s):** `generate_report/3`
- **Short explanation:** Report generation involves validating parameters, loading data from a warehouse, applying filters, rendering a template, and uploading to object storage. Each step may fail in distinct ways, yet all failures feed into a single `else` block, mixing together unrelated error causes.
...
defmodule Reporting.ReportGenerator do
  ...
  def generate_report(report_type, params, requester_id) do
    with {:ok, validated_params} <- ParamValidator.validate(report_type, params),
      ...
      # VALIDATION: This is a smell because all five steps produce different error shapes,
      # but they are all caught in one `else` block. `:unknown_report_type` and
      # `{:invalid_params, _}` come from validation; `:warehouse_timeout` and
      # `:insufficient_permissions` from data fetching; `:filter_config_error` from
      # filtering; `{:render_error, _}` from rendering; and `:upload_failed` from storage.
      # There is no clear mapping from error to step within the `else` block.
