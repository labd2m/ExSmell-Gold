...
- **Expected smell location:** `process_upload/3`, inside the `with` block's `else` clause
- **Affected function(s):** `process_upload/3`
- **Short explanation:** A document upload pipeline with six steps—scanning for viruses, validating the file type, extracting metadata, persisting the record, indexing the content, and notifying collaborators—has all its error paths collapsed into one `else` block. Errors from unrelated concerns are grouped together with no indication of their source step.
...
defmodule Documents.UploadPipeline do
  ...
  def process_upload(upload, workspace_id, uploader_id) do
    with {:ok, scan_result} <- VirusScanner.scan(upload.path),
      ...
      # VALIDATION: This is a smell because the six pipeline steps each produce distinct
      # error shapes, but they are all caught in a single `else` block.
      # `:virus_detected` and `:scan_failed` come from virus scanning;
      # `:unsupported_type` from MIME validation; `{:metadata_error, _}` from extraction;
      # `{:db_error, _}` from persistence; `:index_unavailable` from search indexing;
      # and `:notification_failed` from collaborator notification.
      # Reading the `else` block alone gives no hint about which step each error belongs to.
