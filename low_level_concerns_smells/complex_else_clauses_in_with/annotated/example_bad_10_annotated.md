# Annotated Bad Example 10

- **Smell name:** Complex else clauses in with
- **Expected smell location:** `upgrade_subscription/3`, inside the `with` block's `else` clause
- **Affected function(s):** `upgrade_subscription/3`
- **Short explanation:** Five sequential subscription-upgrade steps each produce different kinds of errors. All are handled in a single `else` block with nine distinct clauses, obscuring which step in the upgrade pipeline is responsible for which failure.
...
defmodule Subscriptions.UpgradeManager do
  ...
  def upgrade_subscription(account_id, target_plan_id, payment_method_id) do
    with {:ok, account} <- fetch_active_account(account_id),
      ...
      # VALIDATION: This is a smell because errors from five distinct steps—account lookup,
      # subscription resolution, plan eligibility, proration calculation, and payment charging—
      # are all collapsed into one `else` block. The clauses `:not_found`, `:suspended`,
      # `:no_active_subscription`, `:plan_not_found`, `:downgrade_not_allowed`,
      # `:same_plan`, `:proration_error`, `:payment_declined`, and `:payment_method_invalid`
      # each originate from a different step, but there is nothing in the `else` block to
      # indicate which step produces which error.
      {:error, :not_found} ->
