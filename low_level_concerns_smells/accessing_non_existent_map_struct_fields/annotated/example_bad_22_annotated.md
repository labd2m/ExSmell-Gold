...
- **Expected smell location:** `Compliance.AuditLogger.log_action/2`, lines where `action` map keys are accessed dynamically
- **Affected function(s):** `log_action/2`
- **Short explanation:** `action[:actor_id]`, `action[:resource_type]`, `action[:resource_id]`, and `action[:outcome]` use dynamic bracket access. When `:actor_id` is absent, `nil` is stored in the audit record, creating an untraceable log entry that silently passes integrity checks designed only for malformed (non-nil) values.
...
defmodule Compliance.AuditLogger do
  ...
  def log_action(action, context \\ %{}) do
    ...
    # VALIDATION: This is a smell because `action[:actor_id]`,
    # `action[:resource_type]`, `action[:resource_id]`, and `action[:outcome]`
    # use dynamic bracket access on a plain map. When `:actor_id` is absent,
    # `nil` is stored in the audit entry without raising an error; the
    # integrity validator only checks for obviously wrong values (non-string
    # types), so a missing actor silently produces an untraceable record.
    # When `:outcome` is absent, `nil` is stored as the outcome atom, which
    # later breaks downstream compliance queries that match on known atoms.
