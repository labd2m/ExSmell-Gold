...
- **Expected smell location:** `Reporting.SalesAggregator.build_report/1`, where optional report filters are accessed dynamically
- **Affected function(s):** `build_report/1`
- **Short explanation:** The function reads `:region_filter`, `:rep_id`, and `:include_refunds` from the report spec using bracket access. Absent keys return `nil`, causing filter predicates to silently match all records (treating no filter the same as an empty filter), which produces incorrect report data.
...
defmodule Reporting.SalesAggregator do
  ...
  defp filter_sales(sales, spec) do
    ...
    # VALIDATION: This is a smell because `spec[:region_filter]`, `spec[:rep_id]`,
    # and `spec[:include_refunds]` use dynamic bracket access on a plain map.
    # When these keys are absent, all three expressions return `nil`.
    # For `region_filter` and `rep_id`, a `nil` filter makes the `==` comparison
    # always false, effectively excluding all sales — the opposite of the intended
    # "no filter" behaviour. For `include_refunds`, `nil` is treated as falsy,
    # silently excluding refunds even when the caller never specified a preference.
