...
- **Expected smell location:** `Subscription.PlanUpgrader.upgrade/3`, lines where `upgrade_request` map keys are accessed dynamically
- **Affected function(s):** `upgrade/3`
- **Short explanation:** `upgrade_request[:target_plan]`, `upgrade_request[:billing_cycle]`, and `upgrade_request[:prorate]` use dynamic bracket access. When `:prorate` is absent, the silent `nil` is treated as falsy and proration is silently skipped, potentially over-charging the customer rather than raising a clear configuration error.
...
defmodule Subscription.PlanUpgrader do
  ...
  def upgrade(subscription, upgrade_request, account) do
    ...
    # VALIDATION: This is a smell because `upgrade_request[:target_plan]`,
    # `upgrade_request[:billing_cycle]`, and `upgrade_request[:prorate]` use
    # dynamic bracket access on a plain map. When `:prorate` is absent, `nil`
    # is returned and treated as falsy, silently skipping proration credit
    # calculation and causing the customer to be over-charged for the
    # remaining billing period. There is no way to distinguish "prorate: nil"
    # (intentional) from a missing `:prorate` key (accidental omission).
