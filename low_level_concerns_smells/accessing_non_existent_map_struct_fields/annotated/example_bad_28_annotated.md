...
- **Expected smell location:** `RealEstate.ListingPublisher.publish/2`, lines where `listing` map keys are accessed dynamically
- **Affected function(s):** `publish/2`
- **Short explanation:** `listing[:price]`, `listing[:property_type]`, `listing[:area_sqm]`, and `listing[:geo]` use dynamic bracket access on a plain map. When `:price` is absent, `nil` flows into price-formatting and range-indexing arithmetic, silently producing a malformed listing record. A missing `:geo` key causes location-based search indexing to be skipped without any error.
...
defmodule RealEstate.ListingPublisher do
  ...
  def publish(listing, publisher_config) do
    ...
    # VALIDATION: This is a smell because `listing[:price]`,
    # `listing[:property_type]`, `listing[:area_sqm]`, and `listing[:geo]`
    # use dynamic bracket access on a plain map. When `:price` is absent,
    # `nil` is returned and passed into `validate_price/1` as well as
    # `compute_price_per_sqm/2`; the validator catches `nil` but only after
    # `nil / area_sqm` is attempted in the computation branch, raising
    # `ArithmeticError`. When `:geo` is absent, the silent `nil` causes
    # spatial indexing to be skipped without any warning, producing listings
    # that are invisible to location-based searches.
    price         = listing[:price]
