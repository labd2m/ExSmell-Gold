...
- **Expected smell location:** `IoT.DeviceProvisioner.provision/1`, where optional device configuration fields are accessed dynamically
- **Affected function(s):** `provision/1`
- **Short explanation:** The function reads `:firmware_version`, `:heartbeat_interval_sec`, and `:encryption_key` from the device map using bracket access. Absent keys return `nil`, so devices are provisioned with a `nil` firmware version and missing encryption key — neither triggers a validation error, silently producing an insecure or mis-configured device record.
...
defmodule IoT.DeviceProvisioner do
  ...
  defp build_provisioning_record(device) do
    ...
    # VALIDATION: This is a smell because `device[:firmware_version]`,
    # `device[:heartbeat_interval_sec]`, and `device[:encryption_key]` use
    # dynamic bracket access on a plain map. When these keys are absent, `nil`
    # is returned silently. A `nil` firmware version is written into the
    # provisioning record without error, and a missing encryption key produces
    # a provisioned device without security credentials — indistinguishable
    # from a device where the key was intentionally left blank.
