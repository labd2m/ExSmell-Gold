...
- **Expected smell location:** `Compliance.AuditLogger.record_event/1`, where optional event context fields are accessed dynamically
- **Affected function(s):** `record_event/1`
- **Short explanation:** The function reads `:session_id`, `:ip_address`, and `:actor_role` from the event map using bracket access. Absent keys return `nil`, so audit log entries are written with `nil` in mandatory compliance fields (IP address, actor role) with no error raised, producing incomplete records that fail regulatory audits.
...
defmodule Compliance.AuditLogger do
  ...
  defp build_log_entry(event) do
    ...
    # VALIDATION: This is a smell because `event[:session_id]`, `event[:ip_address]`,
    # `event[:actor_role]`, and `event[:reason]` use dynamic bracket access on a
    # plain map. When any of these keys is absent, `nil` is returned silently.
    # For a compliance audit log, `nil` in `:ip_address` or `:actor_role` produces
    # an incomplete record that violates regulatory requirements (e.g., HIPAA §164.312).
    # The code cannot distinguish "field intentionally omitted" from "field was never
    # populated by the caller", masking data quality issues.
