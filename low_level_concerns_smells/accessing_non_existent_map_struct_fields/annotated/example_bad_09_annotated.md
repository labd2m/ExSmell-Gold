...
- **Expected smell location:** `Auth.SessionManager.create_session/2`, lines where `options` map keys are accessed dynamically
- **Affected function(s):** `create_session/2`
- **Short explanation:** `options[:ttl_seconds]`, `options[:single_use]`, and `options[:ip_binding]` use dynamic bracket access. If any key is absent the function silently uses `nil` in expressions such as `System.os_time(:second) + nil`, producing a runtime error that is hard to trace, or silently skips security-critical behaviour (like IP binding) without alerting the developer.
...
defmodule Auth.SessionManager do
  ...
  def handle_call({:create_session, user, options}, _from, state) do
    ...
    # VALIDATION: This is a smell because `options[:ttl_seconds]`,
    # `options[:single_use]`, and `options[:ip_binding]` use dynamic bracket
    # access on a plain map. When `:ttl_seconds` is missing, `nil` is added to
    # the current timestamp (`now + nil`), crashing at runtime with an
    # `ArithmeticError`. When `:ip_binding` is missing, the security feature is
    # silently skipped without any warning, making it impossible to distinguish
    # "no binding desired" from "the key was accidentally omitted".
