...
- **Expected smell location:** `UserManagement.ProfileUpdater.apply_changes/2`, where optional profile update fields are accessed dynamically
- **Affected function(s):** `apply_changes/2`
- **Short explanation:** The function reads optional changeset fields (`:avatar_url`, `:locale`, `:notification_frequency`) using bracket access on a plain map. Missing keys return `nil`, and subsequent logic silently keeps stale values or applies invalid defaults instead of detecting that the caller did not supply those fields.
...
defmodule UserManagement.ProfileUpdater do
  ...
  defp merge_changes(user, changeset) do
    ...
    # VALIDATION: This is a smell because `changeset[:avatar_url]`,
    # `changeset[:locale]`, and `changeset[:notification_frequency]` use dynamic
    # bracket access on a plain map. When a caller omits any of these keys, the
    # access returns `nil`. The `|| user.field` fallback then silently keeps the
    # old value, but a caller who explicitly passes `avatar_url: nil` to clear the
    # avatar is treated identically to one who never mentioned it at all — the
    # intended `nil` value is lost and the stale URL is preserved.
