...
- **Expected smell location:** `Auth.SessionManager.build_session/2`, where user capability flags are accessed dynamically
- **Affected function(s):** `build_session/2`
- **Short explanation:** The function uses bracket notation on the `user` map to read optional permission-related keys. When those keys are absent, `nil` is returned silently and treated as falsy, making it impossible to distinguish a user with no MFA configured from one whose MFA key is explicitly set to `nil`.
...
defmodule Auth.SessionManager do
  ...
  defp check_account_status(user) do
    ...
    # VALIDATION: This is a smell because `user[:suspended]` and
    # `user[:force_password_reset]` use dynamic bracket access on a plain map.
    # If either key is absent the expression evaluates to `nil`, which is
    # treated as falsy — indistinguishable from the key being explicitly `false`.
    # A suspended user whose map was built without the `:suspended` key would
