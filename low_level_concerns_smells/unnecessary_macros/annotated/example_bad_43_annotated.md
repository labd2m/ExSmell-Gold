...
| **Expected smell location** | `UserManagement.AddressHelper` module, `merge_address/2` macro |
| **Affected function(s)** | `merge_address/2` |
| **Short explanation** | `merge_address/2` merges two map values at runtime using `Map.merge/3`. Both arguments are runtime values; no compile-time transformation is achievable here. A plain function is the natural and simpler solution. |
...
defmodule UserManagement.AddressHelper do
  ...
  # VALIDATION: This is a smell because `merge_address/2` takes two address
  # maps at runtime and merges them, rejecting nil values. Both arguments
  # are runtime values and `Map.merge/3` is a runtime function. There is no
  # compile-time expansion to be gained; a `def` would be simpler, directly
  # testable, and free of `quote/unquote` indirection. Callers are also
  # unnecessarily forced to `require` the module.
  defmacro merge_address(base, override) do
    quote do
      base_map = unquote(base)
      override_map = unquote(override)

      Map.merge(base_map, override_map, fn _key, v1, v2 ->
