# Annotated Example 17 — Unnecessary Macros

## Metadata

- **Smell name:** Unnecessary macros
- **Expected smell location:** `defmacro fetch_required/2` inside `Auth.ClaimsUtils`
- **Affected function(s):** `fetch_required/2`
- **Short explanation:** The macro looks up a key in a runtime map and raises or returns a value — a standard map access pattern with no compile-time component. A plain function (or even a direct `Map.fetch!/2` call) is all that is needed.
...
defmodule Auth.ClaimsUtils do
  ...
  # VALIDATION: This is a smell because fetch_required/2 only calls Map.fetch
  # on a runtime map and pattern-matches the result. This is a basic runtime
  # map lookup; a regular def function is simpler and does not require callers
