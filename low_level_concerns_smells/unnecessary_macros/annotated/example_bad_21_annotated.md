# Annotated Example 21 — Unnecessary Macros

## Metadata

- **Smell name:** Unnecessary macros
- **Expected smell location:** `defmacro cents_to_float/1` inside `Payments.CurrencyUtils`
- **Affected function(s):** `cents_to_float/1`
- **Short explanation:** The macro converts an integer amount in cents to a float by dividing by 100.0 — a single arithmetic expression evaluated entirely at runtime. A regular function is the correct and far simpler abstraction.
...
defmodule Payments.CurrencyUtils do
  ...
  # VALIDATION: This is a smell because cents_to_float/1 performs a single
  # integer-to-float division at runtime. There is no compile-time
  # transformation or AST manipulation needed; a plain def function is the
  # correct and idiomatic Elixir approach.
