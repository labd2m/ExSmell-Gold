# Annotated Example 23 — Unnecessary Macros

## Metadata

- **Smell name:** Unnecessary macros
- **Expected smell location:** `defmacro parse_iso_date/1` inside `Scheduling.InputParser`
- **Affected function(s):** `parse_iso_date/1`
- **Short explanation:** The macro calls `Date.from_iso8601/1` and pattern-matches the result — a standard runtime parsing operation. No AST transformation takes place; a plain function with the same body would be cleaner and would not require callers to `require` the module.
...
defmodule Scheduling.InputParser do
  ...
  # VALIDATION: This is a smell because parse_iso_date/1 only calls
  # Date.from_iso8601/1 and wraps the result — a pure runtime string-parsing
  # operation. No compile-time transformation is needed; a regular def
  # function is the appropriate and simpler solution.
