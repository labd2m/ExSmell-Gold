# Annotated Example 06 — Unnecessary Macros

## Metadata

- **Smell name:** Unnecessary macros
- **Expected smell location:** `defmacro paginate/2` inside `Reporting.Pagination`
- **Affected function(s):** `paginate/2`
- **Short explanation:** The macro splits a list into a page using only `Enum.drop/2` and `Enum.take/2` — standard runtime enumerable operations. A regular function is the right abstraction and would be just as efficient and far easier to understand.
...
defmodule Reporting.Pagination do
  ...
  # VALIDATION: This is a smell because paginate/2 only calls Enum.drop and
  # Enum.take on a runtime list. These are pure runtime operations with no
  # compile-time component; a def function would be clearer and idiomatic.
