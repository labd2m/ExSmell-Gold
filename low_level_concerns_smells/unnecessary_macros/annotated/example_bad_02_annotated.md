# Annotated Example 02 — Unnecessary Macros

## Metadata

- **Smell name:** Unnecessary macros
- **Expected smell location:** `defmacro validate_email/1` inside `AuthValidator`
- **Affected function(s):** `validate_email/1`
- **Short explanation:** The macro runs a plain regex match at runtime — there is no AST manipulation or compile-time benefit. A regular function achieves exactly the same result with less complexity.
...
defmodule AuthValidator do
  ...
  # VALIDATION: This is a smell because validate_email/1 only applies a regex
  # match and returns a boolean. There is no compile-time transformation
  # involved; a regular def function would be simpler and clearer.
