...
| **Expected smell location** | `UserManagement.RoleChecker` module, `has_permission?/2` macro |
| **Affected function(s)** | `has_permission?/2` |
| **Short explanation** | `has_permission?/2` checks whether a role atom is present in a list of permitted roles. This is a plain membership test on runtime values; a regular function (or even an inline `in` expression) would be the correct, simpler approach. |
...
defmodule UserManagement.RoleChecker do
  ...
  # VALIDATION: This is a smell because determining whether a role has a
  # permission is a runtime list-membership check. The `role` and
  # `required_roles` values are only available at runtime, so there is no
  # benefit to compile-time code generation. A `def` function would be
  # idiomatic, testable with ExUnit, and much easier to read.
