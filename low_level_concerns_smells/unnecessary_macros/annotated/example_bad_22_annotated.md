# Annotated Example 22 — Unnecessary Macros

## Metadata

- **Smell name:** Unnecessary macros
- **Expected smell location:** `defmacro count_by/2` inside `Inventory.QueryUtils`
- **Affected function(s):** `count_by/2`
- **Short explanation:** The macro groups a runtime list by a key function and counts the members of each group using `Enum.group_by/2` and `Map.new/2`. This is a straightforward runtime enumerable operation with no compile-time component; a regular function is the correct tool.
...
defmodule Inventory.QueryUtils do
  ...
  # VALIDATION: This is a smell because count_by/2 only calls Enum.group_by/2
  # and Map.new/2 on a runtime list with a runtime function. There is no
  # compile-time transformation involved; a def function is simpler,
  # more composable, and equally expressive.
