# Annotated Example 09 — Unnecessary Macros

## Metadata

- **Smell name:** Unnecessary macros
- **Expected smell location:** `defmacro build_error/2` inside `Logistics.ErrorFactory`
- **Affected function(s):** `build_error/2`
- **Short explanation:** The macro constructs a plain error map from two runtime values. Map construction is entirely a runtime operation, so a regular function would be equally expressive with far less ceremony.
...
defmodule Logistics.ErrorFactory do
  ...
  # VALIDATION: This is a smell because build_error/2 only constructs a plain
  # map from two runtime values. Map construction requires no compile-time
  # transformation; a regular def function would be cleaner and idiomatic.
