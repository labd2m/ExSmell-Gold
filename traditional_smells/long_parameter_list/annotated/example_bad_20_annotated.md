# Annotated Example 20 — Long Parameter List
...
| **Expected smell location** | `Commerce.Orders.place_order/10` |
| **Affected function(s)** | `place_order/10` |
| **Explanation** | The function takes 10 positional parameters representing customer identity (customer_id, customer_email), shipping address (ship_street, ship_city, ship_country), order content (items, coupon_code), and payment info (payment_method_id, currency, notes). These map to at least three natural groupings (`%ShippingAddress{}`, `%OrderOptions{}`, and the customer reference), but are all passed as a flat list. |

---

```elixir
# VALIDATION: SMELL START - Long Parameter List
# VALIDATION: This is a smell because `place_order/10` has ten positional
# arguments. The parameters cover customer identity (customer_id,
# customer_email), a full shipping address (ship_street, ship_city,
# ship_country), order content and discounts (items, coupon_code),
# and payment/preference details (payment_method_id, currency, notes).
# Structuring these into focused data structures would make invocations
# self-documenting and resistant to positional mix-ups.
