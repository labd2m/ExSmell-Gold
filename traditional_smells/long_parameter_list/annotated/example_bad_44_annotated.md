...
| **Expected smell location** | `Payments.Processor.charge_card/11` |
| **Affected function(s)** | `charge_card/11` |
| **Short explanation** | Eleven parameters for a card charge span card details, amount, merchant context, and idempotency options. These could be consolidated into `%CardDetails{}` and `%ChargeRequest{}` structs. Their flat listing forces callers to remember the exact position of values like `card_cvv` versus `card_expiry`, which are high-stakes parameters to get wrong. |
...
defmodule Payments.Processor do
  ...
  # VALIDATION: This is a smell because 11 positional parameters are passed
  # in a security-sensitive context. Swapping card_number with card_holder
  # or card_expiry with card_cvv would not cause a compile error but would
  # cause a failed or fraudulent charge. A %CardDetails{} struct would
  # enforce named access and make the charge call site self-documenting.
  def charge_card(
        card_number,
        card_holder,
        card_expiry_month,
