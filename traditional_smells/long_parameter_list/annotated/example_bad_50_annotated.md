...
| **Expected smell location** | `HR.Onboarding.onboard_employee/13` |
| **Affected function(s)** | `onboard_employee/13` |
| **Short explanation** | Thirteen parameters cover personal details, employment terms, payroll information, and IT provisioning flags. The breadth of concerns is a clear signal that the data should be grouped into structs such as `%PersonalDetails{}`, `%EmploymentTerms{}`, and `%ProvisioningOptions{}` instead of being threaded as a 13-item positional list. |
...
defmodule HR.Onboarding do
  ...
  # VALIDATION: This is a smell because 13 heterogeneous parameters span
  # personal data (full_name, personal_email, start_date), employment terms
  # (department, job_title, employment_type, salary_cents, manager_id), and
  # provisioning options (laptop_required, access_level, send_welcome_email,
  # enroll_in_benefits, remote_worker). Even with careful documentation,
  # callers mixing positional arguments of the same type (e.g., two string
  # IDs) will create hard-to-trace onboarding errors.
