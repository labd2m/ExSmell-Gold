...
| **Expected smell location** | `UserManagement.Profiles.update_profile/10` |
| **Affected function(s)** | `update_profile/10` |
| **Short explanation** | Ten parameters describe a profile update that mixes personally identifiable fields, contact preferences, and display settings. A `%ProfileChangeset{}` or a plain map would allow callers to provide only the fields they are changing, rather than being forced to supply all ten arguments—including unchanged ones—on every call. |
...
defmodule UserManagement.Profiles do
  ...
  # VALIDATION: This is a smell because callers must supply all 10
  # parameters even when updating only one or two fields. There is no
  # way to express "change only the display_name" without passing the
  # current value of every other parameter in the correct position.
  # A %ProfileUpdateParams{} struct with optional fields would fix this.
  def update_profile(
        user_id,
        display_name,
        bio,
