# Annotated Example 11 — Long Parameter List
...
| **Expected smell location** | `Billing.create_invoice/9` |
| **Affected function(s)** | `create_invoice/9` |
| **Explanation** | The function accepts 9 individual scalar parameters that collectively describe an invoice and its recipient. These values clearly belong to two cohesive data structures (a customer and an invoice configuration) but are instead threaded through the function signature as loose, unrelated arguments, making call sites error-prone and the function interface hard to understand. |

---

```elixir
# VALIDATION: SMELL START - Long Parameter List
# VALIDATION: This is a smell because `create_invoice/9` receives nine individual
# parameters (customer_name, customer_email, customer_tax_id, due_days,
# currency, line_items, discount_percent, notes, send_email) that could
# be naturally grouped into a %Customer{} and an %InvoiceConfig{} struct,
# making the interface significantly clearer and less error-prone.
