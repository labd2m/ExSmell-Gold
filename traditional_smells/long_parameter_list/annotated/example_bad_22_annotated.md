# Annotated Example 22 — Long Parameter List
...
| **Expected smell location** | `Healthcare.Patients.create_patient_record/10` |
| **Affected function(s)** | `create_patient_record/10` |
| **Explanation** | The function takes 10 individual parameters: demographic info (first_name, last_name, birth_date, gender), contact details (email, phone, emergency_contact_name, emergency_contact_phone), and clinical data (blood_type, allergies). These naturally belong in a `%Demographics{}` and a `%ClinicalInfo{}` struct rather than a flat, positional argument list. |

---

```elixir
# VALIDATION: SMELL START - Long Parameter List
# VALIDATION: This is a smell because `create_patient_record/10` takes ten
# individual positional parameters. Patient demographics (first_name,
# last_name, birth_date, gender), contact information (email, phone,
# emergency_contact_name, emergency_contact_phone), and clinical data
# (blood_type, allergies) are all mixed in one flat signature. The many
# string-typed arguments are easy to transpose, and grouping them into
# focused structs would reduce that risk significantly.
