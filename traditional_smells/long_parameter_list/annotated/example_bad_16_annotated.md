# Annotated Example 16 — Long Parameter List
...
| **Expected smell location** | `Inventory.Products.add_product/11` |
| **Affected function(s)** | `add_product/11` |
| **Explanation** | The function accepts 11 individual parameters describing a product: identity fields (sku, name, description), categorization (category, brand), physical properties (weight_kg, unit), pricing (cost_price, sale_price), and stock settings (initial_quantity, reorder_threshold). These clearly map to two or three domain structs rather than a long flat list. |

---

```elixir
# VALIDATION: SMELL START - Long Parameter List
# VALIDATION: This is a smell because `add_product/11` has eleven individual
# positional parameters. The parameters span multiple conceptual groups:
# product identity (sku, name, description), classification (category, brand),
# physical attributes (weight_kg, unit), pricing (cost_price, sale_price),
# and stock management (initial_quantity, reorder_threshold). Grouping these
# into `%ProductInfo{}` and `%StockConfig{}` would greatly reduce the
# cognitive burden of each call site.
