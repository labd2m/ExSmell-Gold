# Annotated Example 36 — Long Parameter List
...
| **Expected smell location** | `Telemedicine.Consultations.book_consultation/11` |
| **Affected function(s)** | `book_consultation/11` |
| **Explanation** | The function accepts 11 individual parameters covering patient identity (patient_id, patient_name, patient_email, patient_phone), physician details (physician_id, specialty), session configuration (scheduled_at, duration_minutes, modality), and billing info (insurance_provider, insurance_member_id). These naturally form a `%PatientContact{}`, a `%PhysicianRef{}`, and a `%SessionConfig{}` struct rather than a flat positional list of eleven arguments. |

---

```elixir
# VALIDATION: SMELL START - Long Parameter List
# VALIDATION: This is a smell because `book_consultation/11` takes eleven
# individual positional parameters. Patient contact data (patient_id,
# patient_name, patient_email, patient_phone), physician selection
# (physician_id, specialty), session setup (scheduled_at, duration_minutes,
# modality), and insurance billing details (insurance_provider,
# insurance_member_id) are all passed as a flat argument list. The
# function signature reveals no structure, and callers must track the
# exact position of every argument — including two adjacent IDs and
# two adjacent insurance strings — with no compiler assistance.
