...
| **Expected smell location** | `Analytics.Events.track/10` |
| **Affected function(s)** | `track/10` |
| **Short explanation** | Ten parameters are needed to record a single analytics event, covering actor identity, context metadata, and routing flags. These parameters form at least two logical groups (`%ActorContext{}`, `%EventOptions{}`) whose flat enumeration in the signature makes it easy to mix up positionally similar string arguments and hard to add new optional fields in the future. |
...
defmodule Analytics.Events do
  ...
  # VALIDATION: This is a smell because 10 positional parameters cover
  # actor identity (user_id, session_id, device_type), event description
  # (event_name, event_category, properties), context (ip_address,
  # user_agent), and routing flags (send_to_stream, send_to_experiments).
  # These groups belong in structs; as positional args they are brittle and
  # hard to mock or pattern-match in tests.
  def track(
        user_id,
        session_id,
        event_name,
