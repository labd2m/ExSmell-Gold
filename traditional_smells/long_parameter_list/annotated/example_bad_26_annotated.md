# Annotated Example 26 — Long Parameter List
...
| **Expected smell location** | `Subscriptions.Manager.create_subscription/10` |
| **Affected function(s)** | `create_subscription/10` |
| **Explanation** | The function takes 10 individual parameters covering account identification (account_id, owner_email), plan details (plan_id, billing_cycle, seats), payment info (payment_method_id, currency), and configuration (trial_days, coupon_code, auto_renew). These naturally belong to a `%PlanSelection{}` and a `%BillingConfig{}` struct rather than a flat positional list. |

---

```elixir
# VALIDATION: SMELL START - Long Parameter List
# VALIDATION: This is a smell because `create_subscription/10` accepts ten
# individual positional parameters. Account identity (account_id,
# owner_email), plan configuration (plan_id, billing_cycle, seats), payment
# method (payment_method_id, currency), and subscription behavior settings
# (trial_days, coupon_code, auto_renew) are all flattened into one long
# signature. This is confusing to call correctly and hides the natural
# grouping of these domain concepts.
