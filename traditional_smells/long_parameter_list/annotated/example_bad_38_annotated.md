...
| **Short explanation** | The function receives 12 individual parameters that cover customer data, invoice metadata, and line-item details. These could be grouped into domain structs such as `%Customer{}`, `%InvoiceOptions{}`, and `%LineItem{}`, reducing the parameter count and making call sites far less error-prone. |
...
defmodule Billing do
  ...
  # VALIDATION: This is a smell because the function accepts 12 positional
  # parameters. Callers must remember the exact order of customer_name,
  # customer_email, customer_tax_id, currency, issue_date, due_date,
  # line_description, unit_price, quantity, discount_percent,
  # send_immediately, and notes – a mistake in ordering silently produces
  # wrong invoices. These could be grouped into %Customer{} and
  # %InvoiceRequest{} structs.
  def create_invoice(
        customer_name,
        customer_email,
        customer_tax_id,
