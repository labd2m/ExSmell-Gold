# Annotated Example 29 — Long Parameter List
...
| **Expected smell location** | `Support.Tickets.create_ticket/10` |
| **Affected function(s)** | `create_ticket/10` |
| **Explanation** | The function accepts 10 individual parameters covering requester info (requester_id, requester_email, requester_name), ticket content (subject, body, attachments), classification (category, priority), and routing (assigned_team_id, send_confirmation). These naturally form a `%RequesterInfo{}` and a `%TicketConfig{}` struct rather than a flat, positional parameter list. |

---

```elixir
# VALIDATION: SMELL START - Long Parameter List
# VALIDATION: This is a smell because `create_ticket/10` takes ten positional
# parameters. Requester identification (requester_id, requester_email,
# requester_name), ticket content (subject, body, attachments), classification
# (category, priority), and routing options (assigned_team_id,
# send_confirmation) are all passed as a flat list of arguments. This makes
# call sites hard to read and maintain, particularly as several parameters
# are optional and could easily be confused with each other.
