...
| **Expected smell location** | `Scheduling.Appointments.book/11` |
| **Affected function(s)** | `book/11` |
| **Short explanation** | Eleven parameters carry patient info, provider info, timing, and communication preferences. These naturally decompose into `%Participant{}` and `%BookingRequest{}` structs. The flat parameter list forces callers to thread boolean flags like `send_sms_reminder` and `send_email_reminder` in precise positional order, inviting silent misconfiguration. |
...
defmodule Scheduling.Appointments do
  ...
  # VALIDATION: This is a smell because the 11 parameters combine patient
  # details, provider details, temporal data, and notification prefs into a
  # single flat list. Adding a new option (e.g., video_link) requires every
  # caller to update its argument list. Structs like %Patient{}, %Provider{},
  # and %BookingOptions{} would isolate concerns and allow safe extension.
  def book(
        patient_id,
        patient_name,
        patient_phone,
