...
- **Expected Smell Location:** `MyApp.Auth.OAuthHandler`, `MyApp.Auth.TokenValidator`, `MyApp.Auth.UserMapper`
- **Affected Functions:** `OAuthHandler.exchange_code/2`, `TokenValidator.verify/2`, `UserMapper.map_profile/2`
- **Explanation:** Adding a new OAuth provider (e.g., `:apple`) requires small but mandatory changes in all three modules: a code-exchange clause in `OAuthHandler`, a token verification clause in `TokenValidator`, and a profile normalization clause in `UserMapper`. The concept of a provider is fragmented across modules instead of being encapsulated in a provider-specific abstraction.

```elixir
# VALIDATION: SMELL START - Shotgun Surgery
# VALIDATION: This is a smell because supporting a new OAuth provider (e.g., :apple)
# VALIDATION: forces simultaneous changes in OAuthHandler.exchange_code/2,
# VALIDATION: TokenValidator.verify/2, and UserMapper.map_profile/2. All three modules
# VALIDATION: must be kept in sync for every new provider, making omissions error-prone.
