...
- **Expected Smell Location:** `MyApp.Reports.Builder`, `MyApp.Reports.AccessControl`, `MyApp.Reports.Exporter`
- **Affected Functions:** `Builder.build/2`, `AccessControl.authorize/2`, `Exporter.export/3`
- **Explanation:** Adding a new report type (e.g., `:compliance`) requires small but mandatory changes in all three modules: data assembly in `Builder`, role authorization in `AccessControl`, and format-specific export logic in `Exporter`. Report-type knowledge is scattered rather than consolidated in a single module.

```elixir
# VALIDATION: SMELL START - Shotgun Surgery
# VALIDATION: This is a smell because adding a new report type (e.g., :compliance) requires
# VALIDATION: changes in Builder.build/2 (data assembly), AccessControl.authorize/2 (role gating),
# VALIDATION: and Exporter.export/3 (format-specific output). All three modules must be touched
# VALIDATION: for every new report type, scattering the concept across the codebase.
