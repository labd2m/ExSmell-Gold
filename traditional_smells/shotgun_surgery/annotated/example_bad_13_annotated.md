# Example Bad 13 — Annotated

## Metadata

- **Smell Name**: Shotgun Surgery
- **Expected Smell Location**: Functions `calculate_monthly_interest/1`, `get_daily_withdrawal_limit/1`, `apply_maintenance_fee/1`, and `get_overdraft_policy/1` inside `Banking.AccountManager`
- **Affected Functions**: `calculate_monthly_interest/1`, `get_daily_withdrawal_limit/1`, `apply_maintenance_fee/1`, `get_overdraft_policy/1`
- **Explanation**: The bank account type logic (`:checking`, `:savings`, `:money_market`) is fragmented across four functions. Adding a new account type (e.g., `:investment`) forces four independent edits across the module — a textbook case of Shotgun Surgery.
...
defmodule Banking.AccountManager do
  ...
  # VALIDATION: This is a smell because adding a new account type (e.g., :investment)
  # requires a new clause here AND in get_daily_withdrawal_limit/1, apply_maintenance_fee/1,
  # and get_overdraft_policy/1 — four scattered changes for one new type.
  ...
  # VALIDATION: This is a smell because a new account type also needs a withdrawal limit
  # clause here, independent of calculate_monthly_interest/1.
  ...
  # VALIDATION: This is a smell because a new account type also requires a maintenance
  # fee clause here, independent of the previous two locations.
  ...
  # VALIDATION: This is a smell because a new account type also requires an overdraft
  # policy clause here, completing the four-location change.
