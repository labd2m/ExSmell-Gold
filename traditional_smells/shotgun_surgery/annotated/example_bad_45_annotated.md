...
- **Expected Smell Location:** `MyApp.Scheduling.BookingEngine`, `MyApp.Scheduling.DurationPolicy`, `MyApp.Scheduling.ReminderService`
- **Affected Functions:** `BookingEngine.book/3`, `DurationPolicy.slots_required/1`, `ReminderService.schedule_reminders/2`
- **Explanation:** Adding a new appointment type (e.g., `:group_session`) requires small but mandatory changes in all three modules: booking validation in `BookingEngine`, slot calculation in `DurationPolicy`, and reminder timing in `ReminderService`. Appointment-type logic is distributed rather than encapsulated, creating high coupling across modules.

```elixir
# VALIDATION: SMELL START - Shotgun Surgery
# VALIDATION: This is a smell because adding a new appointment type (e.g., :group_session)
# VALIDATION: requires changes in BookingEngine.book/3 (availability logic),
# VALIDATION: DurationPolicy.slots_required/1 (duration rules), and
# VALIDATION: ReminderService.schedule_reminders/2 (reminder timing).
# VALIDATION: Each module must be updated independently for a single new type.
