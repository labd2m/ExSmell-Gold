...
- **Expected Smell Location:** Functions `charge/3`, `refund/2`, `webhook_secret/1` in `Payments.GatewayAdapter`; `fee_percentage/1`, `supports_partial_refund?/1` in `Payments.FeePolicy`; `provider_label/1` in `Payments.ReceiptBuilder`
- **Affected Functions:** See above (6 functions across 3 modules)
- **Explanation:** Onboarding a new payment gateway (e.g., `:square`) requires independent changes in three separate modules. Charge logic, fee calculations, and receipt formatting are each spread without a single point of gateway configuration, resulting in multiple scattered modifications for a single new integration.
...
defmodule Payments.GatewayAdapter do
  ...
  # VALIDATION: This is a smell because adding a new gateway (e.g., :square) requires
  # VALIDATION: new clauses in charge/3, refund/2, and webhook_secret/1 here, AND
  # VALIDATION: independent changes in Payments.FeePolicy and Payments.ReceiptBuilder.
...
defmodule Payments.FeePolicy do
  ...
  # VALIDATION: SMELL START - Shotgun Surgery
  # VALIDATION: fee_percentage/1 and supports_partial_refund?/1 each require new
  # VALIDATION: clauses for every new gateway, fully independent from GatewayAdapter.
...
defmodule Payments.ReceiptBuilder do
  ...
  # VALIDATION: SMELL START - Shotgun Surgery
  # VALIDATION: provider_label/1 must also be updated for every new gateway,
  # VALIDATION: adding yet another scattered change point across the codebase.
