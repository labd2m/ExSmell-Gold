...
- **Expected Smell Location:** Functions `authorize_url/2`, `exchange_token/2`, `fetch_profile/2` in `Auth.OAuthProvider`; `provider_display_name/1`, `avatar_url/1` in `Auth.UserProfile`; `log_provider_event/2` in `Auth.AuditLog`
- **Affected Functions:** See above (6 functions across 3 modules)
- **Explanation:** Adding a new OAuth provider (e.g., `:apple`) requires scattered changes across three separate modules, each independently pattern-matching on the provider atom. There is no centralized provider configuration, so every function in every module must be updated individually.
...
defmodule Auth.OAuthProvider do
  ...
  # VALIDATION: This is a smell because adding a new provider (e.g., :apple) requires
  # VALIDATION: new clauses in authorize_url/2, exchange_token/2, and fetch_profile/2
  # VALIDATION: here, AND separate changes in Auth.UserProfile and Auth.AuditLog.
...
defmodule Auth.UserProfile do
  ...
  # VALIDATION: SMELL START - Shotgun Surgery
  # VALIDATION: provider_display_name/1 and avatar_field/1 also require updates for
  # VALIDATION: every new provider, completely independently of OAuthProvider.
...
defmodule Auth.AuditLog do
  ...
  # VALIDATION: SMELL START - Shotgun Surgery
  # VALIDATION: log_provider_event/2 must also be updated per new provider, creating
  # VALIDATION: yet another scattered change point each time the provider list evolves.
