...
- **Expected Smell Location:** `MyApp.Billing.InvoiceGenerator`, `MyApp.Billing.TaxEngine`, `MyApp.Billing.DeliveryService`
- **Affected Functions:** `InvoiceGenerator.generate/2`, `TaxEngine.apply/2`, `DeliveryService.deliver/2`
- **Explanation:** Adding a new invoice type (e.g., `:credit_note`) requires small but mandatory changes scattered across all three modules: document generation logic in `InvoiceGenerator`, tax treatment in `TaxEngine`, and delivery routing in `DeliveryService`. No single module owns the invoice type concept.

```elixir
# VALIDATION: SMELL START - Shotgun Surgery
# VALIDATION: This is a smell because adding a new invoice type (e.g., :credit_note) requires
# VALIDATION: changes in InvoiceGenerator.generate/2 (document structure),
# VALIDATION: TaxEngine.apply/2 (tax treatment rules), and DeliveryService.deliver/2
# VALIDATION: (delivery channel and metadata). Three modules must change simultaneously
# VALIDATION: for a single new invoice type, making omissions and regressions likely.
