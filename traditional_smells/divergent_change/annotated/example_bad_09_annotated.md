...
| **Expected smell location** | `DeviceManager` module |
| **Affected functions** | `register_device/2`, `decommission_device/1`, `update_metadata/2` (device registry reason) and `ingest_telemetry/2`, `get_latest_reading/1`, `aggregate_telemetry/2` (telemetry reason) and `push_firmware_update/2`, `acknowledge_update/2`, `list_pending_updates/1` (firmware OTA reason) |
| **Explanation** | The module manages device registry/metadata, telemetry ingestion and aggregation, and firmware OTA update orchestration — three unrelated IoT concerns. Changes to the device provisioning flow, the telemetry schema, or the OTA update mechanism would each independently drive changes to this module. |
...
defmodule IoT.DeviceManager do
  ...
  # VALIDATION: This is a smell because the module bundles three independent
  # IoT concerns: (1) device registry and lifecycle management, (2) telemetry
  # data ingestion and querying, and (3) firmware OTA update orchestration.
  # Each axis of change is driven by different teams and requirements.
