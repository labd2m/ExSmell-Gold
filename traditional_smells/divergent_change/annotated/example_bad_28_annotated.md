...
- **Expected smell location:** `EmployeeAdmin` module (entire module)
- **Affected functions:** `hire_employee/1`, `terminate_employee/2`, `compute_payroll/2`, `apply_raise/3`, `assign_shift/3`, `swap_shift/3`
- **Explanation:** `EmployeeAdmin` handles HR lifecycle events, payroll computation, and shift scheduling — three entirely separate business domains. A change to termination policies, tax computation rules, or scheduling algorithms would each require edits in unrelated parts of this module, making it a textbook Divergent Change.
...
defmodule MyApp.EmployeeAdmin do
  ...
  # VALIDATION: This is a smell because the module combines three unrelated
  # responsibilities: HR lifecycle (hire/terminate), payroll (compute/raise),
  # and shift scheduling (assign/swap). Each area has distinct reasons to
  # change—new employment laws, updated tax brackets, or shift-coverage
  # policies—leading to constant unrelated modifications in a single module.
