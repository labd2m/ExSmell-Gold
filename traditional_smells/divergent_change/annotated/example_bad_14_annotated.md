...
| **Expected smell location** | `FleetOperations` module |
| **Affected functions** | `register_vehicle/1`, `retire_vehicle/1`, `update_vehicle_details/2` (vehicle registry reason) and `record_location/2`, `get_current_location/1`, `get_route_history/2` (GPS tracking reason) and `schedule_maintenance/2`, `complete_maintenance/2`, `list_due_maintenance/0` (maintenance reason) |
| **Explanation** | The module conflates vehicle registry management, real-time GPS location tracking, and maintenance scheduling — three independent fleet management concerns. A change to the tracking data source, to the maintenance notification rules, or to vehicle record fields would each independently drive changes to this module. |
...
defmodule Fleet.FleetOperations do
  ...
  # to change: (1) vehicle registration and compliance record rules, (2) GPS
  # tracking data ingestion and querying, and (3) preventive/corrective
  # maintenance scheduling and completion tracking. Each concern belongs to
  # a distinct operational domain.
