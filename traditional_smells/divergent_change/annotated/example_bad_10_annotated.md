...
| **Expected smell location** | `TenantAdmin` module |
| **Affected functions** | `provision_tenant/1`, `suspend_tenant/1`, `update_tenant_settings/2` (tenancy management reason) and `set_billing_plan/2`, `record_usage_overage/2`, `generate_usage_invoice/1` (billing reason) and `assign_role/3`, `revoke_role/3`, `list_permissions/2` (RBAC/permissions reason) |
| **Explanation** | The module conflates tenant provisioning/settings, billing plan and overage management, and role-based access control — three independent multi-tenancy concerns. A new billing tier, a change to the permission model, or a tenant lifecycle policy change would each independently require modifications to this module. |
...
defmodule Platform.TenantAdmin do
  ...
  # multi-tenancy concerns: (1) tenant lifecycle and configuration management,
  # (2) billing plan assignment and usage overage tracking, and (3) role-based
  # access control and permissions. Each has an independent reason to change.
