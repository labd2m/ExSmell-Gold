...
- **Expected smell location:** `OrderProcessor` module (entire module)
- **Affected functions:** `place_order/2`, `cancel_order/2`, `reserve_stock/2`, `release_stock/2`, `schedule_shipment/2`, `track_shipment/1`
- **Explanation:** The `OrderProcessor` module combines three distinct concerns: order lifecycle management, inventory reservation, and shipment scheduling/tracking. Each concern has independent reasons to change — for example, order policies may change based on business rules, inventory logic may shift with warehouse changes, and shipment tracking may change when carriers are updated. This causes the module to diverge for unrelated reasons.
...
defmodule MyApp.OrderProcessor do
  ...
  # VALIDATION: This is a smell because the module mixes order lifecycle,
  # inventory reservation, and shipment concerns. Each group has its own
  # independent reasons to change, violating single responsibility and
  # causing unrelated modifications to accumulate in one module.
