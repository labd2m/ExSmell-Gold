...
- **Expected smell location:** `CampaignManager` module (entire module)
- **Affected functions:** `create_campaign/1`, `launch_campaign/1`, `pause_campaign/1`, `send_campaign_email/2`, `record_open/2`, `record_click/3`, `campaign_analytics/1`
- **Explanation:** `CampaignManager` combines marketing campaign lifecycle, email dispatch to recipient lists, engagement event recording (opens/clicks), and analytics aggregation. Marketing strategy changes affect the lifecycle, ESP integrations affect email dispatch, tracking pixel/link encoding changes affect event recording, and BI tool changes affect analytics — four separate reasons to modify one module.
...
defmodule MyApp.CampaignManager do
  ...
  # VALIDATION: This is a smell because campaign lifecycle management,
  # email delivery, engagement tracking, and analytics aggregation are
  # four independent concerns bundled into one module. A new ESP,
  # updated engagement pixel, changed campaign approval workflow, or
  # revised KPI definitions each force unrelated edits to this module.
