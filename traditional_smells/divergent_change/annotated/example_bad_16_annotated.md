...
| **Expected smell location** | `SurveyPlatform` module |
| **Affected functions** | `create_survey/2`, `publish_survey/1`, `close_survey/1`, `add_question/2` (survey construction reason) and `submit_response/2`, `validate_response/2`, `list_responses/1` (response collection reason) and `calculate_nps/1`, `aggregate_responses/1`, `export_results/2` (analytics reason) |
| **Explanation** | The module conflates survey design and lifecycle management, survey response collection and validation, and survey result analytics/export — three independent product research concerns. New question types, a new analytics metric, or a new export format would each independently motivate changes to this module. |
...
defmodule Research.SurveyPlatform do
  ...
  # to change: (1) survey structure and lifecycle (adding question types,
  # publishing rules), (2) response collection mechanics and validation,
  # and (3) analytics calculations and result export formats. Each dimension
  # is driven by different requirements and stakeholders.

  ## ── Survey Construction & Lifecycle ─────────────────────────────────────────
