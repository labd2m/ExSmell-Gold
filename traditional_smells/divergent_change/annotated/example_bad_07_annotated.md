...
| **Expected smell location** | `ReportingEngine` module |
| **Affected functions** | `fetch_sales_data/1`, `fetch_refund_data/1`, `fetch_retention_data/1` (data retrieval reason) and `format_as_csv/1`, `format_as_json/1`, `build_summary_table/1` (formatting reason) and `email_report/2`, `upload_to_s3/2`, `post_to_slack/2` (delivery reason) |
| **Explanation** | The module conflates three independent concerns: querying report data, formatting report output, and delivering the report to recipients. A new data source, a new output format (e.g. XLSX), or a new delivery channel (e.g. webhook) would each independently require changes to this one module. |
...
defmodule Analytics.ReportingEngine do
  ...
  # VALIDATION: This is a smell because the module has three independent
  # reasons to change: (1) changes to how report data is queried or sourced,
  # (2) changes to output format (CSV vs JSON vs XLSX), and (3) changes to
  # report delivery channels or schedules. These are entirely separate concerns.
