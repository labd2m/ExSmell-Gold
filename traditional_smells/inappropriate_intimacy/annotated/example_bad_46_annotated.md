...
- **Expected smell location:** `Scheduling.AppointmentManager.book/3`
- **Affected function(s):** `book/3`, `find_available_slot/3`
- **Short explanation:** `AppointmentManager` directly reads internal fields of `Provider` (`provider.accepting_new_patients`, `provider.specialties`, `provider.max_daily_appointments`) and `Calendar` (`calendar.blocked_dates`, `calendar.working_hours`, `calendar.slot_duration_minutes`) to perform scheduling logic. This detailed knowledge of other modules' internals belongs inside those modules.
...
defmodule Scheduling.AppointmentManager do
  ...
  # VALIDATION: This is a smell because attempt_booking/4 and find_available_slot/3
  # VALIDATION: directly read internal fields of Provider (accepting_new_patients,
  # VALIDATION: specialties, max_daily_appointments) and Calendar (blocked_dates,
  # VALIDATION: working_hours, slot_duration_minutes) to implement scheduling logic
  # VALIDATION: that should be encapsulated inside those modules.
