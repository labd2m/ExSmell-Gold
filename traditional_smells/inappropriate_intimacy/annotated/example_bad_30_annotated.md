...
- **Expected smell location:** `FeatureToggleResolver.resolve/2` function
- **Affected function(s):** `FeatureToggleResolver.resolve/2`
- **Short explanation:** `FeatureToggleResolver.resolve/2` fetches a `TenantConfig` struct and a `RolloutRule` struct and then directly reads their internal fields (`.enabled_features`, `.overrides`, `.beta_cohort_ids`, `.rollout_percentage`, `.target_segments`, `.kill_switch`) to evaluate feature availability. These evaluation details are internal to `TenantConfig` and `RolloutRule` and should be encapsulated in those modules via dedicated functions rather than being examined field-by-field here.
...
defmodule MyApp.Platform.FeatureToggleResolver do
  ...
  def resolve(feature_key, context) do
    ...
    # VALIDATION: This is a smell because resolve/2 directly reads .enabled_features,
    # .overrides, and .beta_cohort_ids from the TenantConfig struct, and .rollout_percentage,
    # .target_segments, and .kill_switch from the RolloutRule struct. The logic for
    # whether a feature is enabled for a given tenant/user is the internal responsibility
    # of TenantConfig and RolloutRule; they should expose a single function like
    # TenantConfig.feature_state/2 and RolloutRule.evaluate/2 instead of leaking
    # their internal fields to this resolver.
