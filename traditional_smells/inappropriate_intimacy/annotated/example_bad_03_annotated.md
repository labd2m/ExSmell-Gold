# Code Smell: Inappropriate Intimacy — Example 03

| Field                    | Value |
|--------------------------|-------|
| **Smell Name**           | Inappropriate Intimacy |
| **Expected Smell Location** | `Notifications.EmailDispatcher.dispatch/3` |
| **Affected Function(s)** | `dispatch/3` |
| **Explanation**          | `dispatch/3` calls `Recipient.load_contact_preferences/1` and `Template.compile/2` from foreign modules, then directly reads their internal struct fields (`prefs.preferred_language`, `prefs.email_address`, `prefs.unsubscribed_topics`, `template.subject`, `template.html_body`, `template.text_body`). `EmailDispatcher` must know the internal shape of `ContactPreferences` and `CompiledTemplate` — implementation details that should remain encapsulated inside the `Recipient` and `Template` modules. |
...
defmodule Notifications.EmailDispatcher do
  ...
  def dispatch(recipient_id, template_name, variables \\ %{}) do
    with {:ok, recipient} <- Recipient.fetch(recipient_id),
      ...
      # VALIDATION: This is a smell because dispatch/3 calls Recipient.load_contact_preferences/1
      # VALIDATION: and Template.compile/2 from foreign modules, then reads internal struct fields
      # VALIDATION: directly: prefs.preferred_language, prefs.email_address,
      # VALIDATION: prefs.unsubscribed_topics, template.subject, template.html_body,
      # VALIDATION: and template.text_body. EmailDispatcher is forced to understand the internal
      # VALIDATION: shape of ContactPreferences and CompiledTemplate, coupling it to those
      # VALIDATION: modules' internal representations rather than relying on their public interfaces.
