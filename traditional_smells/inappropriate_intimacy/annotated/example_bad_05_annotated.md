# Code Smell: Inappropriate Intimacy — Example 05
...
| **Explanation**          | `process/3` calls `Transaction.fetch_payment_source/1` and `PaymentGateway.credentials_for/1` from foreign modules, then reads their internal struct fields directly (`source.gateway_name`, `source.gateway_transaction_id`, `source.captured_amount`, `creds.api_key`, `creds.api_secret`, `creds.endpoint_url`). `RefundProcessor` must know the internal shapes of `PaymentSource` and `GatewayCredentials` — details that should be hidden inside the `Transaction` and `PaymentGateway` modules. |
...
defmodule Payments.RefundProcessor do
  ...
  def process(transaction_id, amount, reason) when is_binary(reason) do
    with {:ok, transaction} <- Transaction.fetch(transaction_id),
      ...
      # VALIDATION: This is a smell because process/3 calls Transaction.fetch_payment_source/1
      # VALIDATION: and PaymentGateway.credentials_for/1 from foreign modules, then directly
      # VALIDATION: reads internal struct fields: source.gateway_name, source.gateway_transaction_id,
      # VALIDATION: source.captured_amount, creds.api_key, creds.api_secret, creds.endpoint_url.
      # VALIDATION: RefundProcessor accumulates knowledge of PaymentSource and GatewayCredentials
      # VALIDATION: internals that should remain encapsulated inside Transaction and PaymentGateway.
