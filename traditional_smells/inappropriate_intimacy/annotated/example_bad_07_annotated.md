...
- **Expected smell location:** `Invoice.generate/2` function
- **Affected function(s):** `Invoice.generate/2`
- **Short explanation:** `Invoice.generate/2` calls `Account.find/1` and `TaxProfile.for_account/1` and then directly accesses internal fields of the returned structs (`.billing_address`, `.vat_number`, `.rate`, `.country_code`), creating tight coupling with the internals of `Account` and `TaxProfile`. These details should be encapsulated behind dedicated functions in those modules.
...
defmodule MyApp.Billing.Invoice do
  ...
  def generate(account_id, line_items) do
    ...
    # VALIDATION: This is a smell because generate/2 fetches internal structs from Account
    # and TaxProfile and then directly reads their internal fields (.billing_address,
    # .vat_number, .rate, .country_code) instead of delegating those queries
    # to the owning modules via dedicated functions.
