...
- **Expected smell location:** `Logistics.DeliveryAssigner.assign/2`
- **Affected function(s):** `assign/2`, `rank_drivers/2`
- **Short explanation:** `DeliveryAssigner` directly reads internal fields of `Driver` (`driver.status`, `driver.current_zone`, `driver.vehicle_type`, `driver.max_parcels`, `driver.shift_ends_at`) and `Vehicle` (`vehicle.load_capacity_kg`, `vehicle.refrigerated`, `vehicle.fuel_type`) to implement assignment logic that should be encapsulated in those modules.
...
defmodule Logistics.DeliveryAssigner do
  ...
  # VALIDATION: This is a smell because find_best_driver/3 and rank_drivers/2 directly
  # VALIDATION: access internal fields of Driver (status, current_zone, vehicle_id,
  # VALIDATION: max_parcels, shift_ends_at) and Vehicle (load_capacity_kg, refrigerated,
  # VALIDATION: fuel_type) to implement suitability and ranking logic that belongs
  # VALIDATION: inside Driver and Vehicle modules.
