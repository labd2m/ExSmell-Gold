...
- **Expected smell location:** `dispatch/2` in `Logistics.ShipmentDispatcher`
- **Affected function(s):** `dispatch/2`
- **Short explanation:** `dispatch/2` directly reads internal fields of `Carrier` (`active`),
  `ServiceLevel` (`availability`, `cutoff_time`), and `Vehicle` (`capacity_kg`, `driver_id`)
  to make dispatch decisions. The function knows the shape of structs that belong to other modules
  and applies business rules that should be encapsulated there, creating excessive coupling across
  the `Carriers` and `Fleet` module boundaries.
...
defmodule Logistics.ShipmentDispatcher do
  ...
  def dispatch(%Shipment{} = shipment, carrier_code) do
    ...
    # VALIDATION: This is a smell because dispatch/2 reads internal fields of Carrier
    # VALIDATION: (active), ServiceLevel (availability, cutoff_time), and Vehicle
    # VALIDATION: (capacity_kg, driver_id) to make routing and eligibility decisions,
    # VALIDATION: rather than delegating "can this carrier accept a shipment?" to
    # VALIDATION: Carrier, "is this service window open?" to ServiceLevel, and
    # VALIDATION: "does this vehicle fit the shipment?" to Vehicle.
