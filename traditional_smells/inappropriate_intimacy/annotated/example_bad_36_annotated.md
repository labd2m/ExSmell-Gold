...
- **Expected smell location:** `transfer_between_bins/4` in `Inventory.StockAdjustmentService`
- **Affected function(s):** `transfer_between_bins/4`
- **Short explanation:** `transfer_between_bins/4` directly reads internal fields of `Bin`
  (`active`, `hazmat_restricted`, `capacity_units`, `used_units`, `zone_id`) and `Zone`
  (`climate_controlled`) to validate transfer eligibility. These checks represent rules that
  belong to the `Bin` and `Zone` modules. The service should ask those modules "can I transfer
  here?" rather than inspecting their internal data fields directly.
...
defmodule Inventory.StockAdjustmentService do
  ...
  def transfer_between_bins(product_id, quantity, from_bin_id, to_bin_id) do
    ...
    # VALIDATION: This is a smell because transfer_between_bins directly reads Bin.active,
    # VALIDATION: Bin.hazmat_restricted, Bin.capacity_units, Bin.used_units, Bin.zone_id,
    # VALIDATION: and Zone.climate_controlled to make transfer eligibility decisions.
    # VALIDATION: The Bin and Zone modules should encapsulate these rules instead of
    # VALIDATION: exposing raw internal fields for callers to interpret.
