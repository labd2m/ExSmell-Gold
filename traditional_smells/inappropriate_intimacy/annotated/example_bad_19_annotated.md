...
- **Expected smell location:** `PayrollProcessor.run/2` function
- **Affected function(s):** `PayrollProcessor.run/2`
- **Short explanation:** `PayrollProcessor.run/2` fetches an `Employee` struct and a `Contract` struct and then directly reads their internal fields (`.base_salary`, `.overtime_hours`, `.tax_withholding_code`, `.pay_frequency`, `.bonus_eligible`, `.commission_rate`) to compute the payslip. These are internal payroll details owned by `Employee` and `Contract` that should be surfaced through their own query API, not accessed as raw fields in this module.
...
defmodule MyApp.HR.PayrollProcessor do
  ...
  def process_employee(employee, period_start, period_end) do
    ...
    # VALIDATION: This is a smell because process_employee/3 directly reads .base_salary,
    # .overtime_hours, and .tax_withholding_code from the Employee struct, and
    # .pay_frequency, .bonus_eligible, and .commission_rate from the Contract struct.
    # These internal compensation details should not be accessed as raw fields here;
    # instead, Employee and Contract should expose dedicated functions for computing
    # gross pay or providing withholding information.
