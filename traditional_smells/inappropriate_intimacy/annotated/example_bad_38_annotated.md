...
- **Expected smell location:** `book/4` in `Scheduling.AppointmentBook`
- **Affected function(s):** `book/4`
- **Short explanation:** `book/4` directly reads internal fields of `Provider`
  (`accepting_new_clients`, `calendar_id`, `timezone`), `Calendar` (`sync_enabled`,
  `external_id`), `Availability` (`slot_duration_minutes`), `ServiceType`
  (`duration_minutes`, `requires_intake_form`), and `BookingPreference`
  (`preferred_reminder_minutes`). The function applies booking-eligibility logic that
  belongs to the owning modules, making it tightly coupled to their internals.
...
defmodule Scheduling.AppointmentBook do
  ...
  def book(customer_id, provider_id, slot_id, service_type_code) do
    ...
    # VALIDATION: This is a smell because book/4 directly reads Provider.accepting_new_clients,
    # VALIDATION: Provider.calendar_id, Provider.timezone, Calendar.sync_enabled,
    # VALIDATION: Calendar.external_id, Availability.slot_duration_minutes,
    # VALIDATION: ServiceType.duration_minutes, ServiceType.requires_intake_form, and
    # VALIDATION: BookingPreference.preferred_reminder_minutes. All of these are internal
    # VALIDATION: details that the respective modules should encapsulate rather than exposing
    # VALIDATION: to be interpreted by an unrelated function.
