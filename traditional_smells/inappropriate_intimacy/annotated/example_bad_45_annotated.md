...
- **Affected function(s):** `process/2`, `determine_refund_method/2`
- **Short explanation:** `RefundProcessor` directly reads internal fields of `Transaction` (`txn.status`, `txn.amount_cents`, `txn.captured_at`, `txn.payment_method`, `txn.gateway_reference`) and `Merchant` (`merchant.refund_policy`, `merchant.auto_approve_threshold_cents`, `merchant.gateway_id`) to drive the refund flow. This detailed knowledge of other modules' internals creates tight coupling.
...
defmodule Payments.RefundProcessor do
  ...
  # VALIDATION: This is a smell because validate_and_refund/4 and determine_refund_method/2
  # VALIDATION: directly access internal fields of Transaction (status, captured_at,
  # VALIDATION: amount_cents, payment_method, gateway_reference) and Merchant
  # VALIDATION: (refund_policy, auto_approve_threshold_cents, gateway_id) to enforce
  # VALIDATION: business rules that should be encapsulated in those modules respectively.
