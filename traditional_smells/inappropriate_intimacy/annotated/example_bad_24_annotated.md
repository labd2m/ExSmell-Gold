...
- **Expected smell location:** `CampaignLauncher.launch/1` function
- **Affected function(s):** `CampaignLauncher.launch/1`
- **Short explanation:** `CampaignLauncher.launch/1` fetches an `EmailCampaign` struct and an `AudienceSegment` struct and then directly reads their internal fields (`.template_id`, `.send_window`, `.ab_test_config`, `.filter_criteria`, `.suppression_list_id`, `.estimated_size`) to orchestrate delivery. The segment filtering logic and campaign configuration should be encapsulated in `AudienceSegment` and `EmailCampaign` respectively.
...
defmodule MyApp.Marketing.CampaignLauncher do
  ...
  def launch(campaign_id) do
    with {:ok, campaign} <- EmailCampaign.fetch(campaign_id),
      ...
      # VALIDATION: This is a smell because launch/1 directly reads .template_id,
      # .send_window, and .ab_test_config from the EmailCampaign struct, and
      # .filter_criteria, .suppression_list_id, and .estimated_size from the
      # AudienceSegment struct. CampaignLauncher should not need to know these internal
      # fields; EmailCampaign should expose a ready-to-launch context and AudienceSegment
      # should expose a resolve_recipients/1 function.
