...
- **Expected smell location:** `SalesReportBuilder.build/2` function
- **Affected function(s):** `SalesReportBuilder.build/2`
- **Short explanation:** `SalesReportBuilder.build/2` fetches a `SalesRegion` struct and a `ReportConfig` struct and directly reads internal fields (`.team_ids`, `.currency_code`, `.conversion_rates`, `.included_metrics`, `.aggregation_level`, `.fiscal_year_start`) to construct the report. This couples the builder tightly to the internal structure of both modules, whose details should be surfaced through their own API.
...
defmodule MyApp.Reporting.SalesReportBuilder do
  ...
  def build(region_id, config_id) do
    with {:ok, region} <- SalesRegion.fetch(region_id),
      ...
      # VALIDATION: This is a smell because build/2 directly reads .team_ids and
      # .currency_code and .conversion_rates from the SalesRegion struct, and
      # .included_metrics, .aggregation_level, and .fiscal_year_start from the
      # ReportConfig struct. This module should not know about these internal fields;
      # it should ask SalesRegion and ReportConfig for derived values through their own
      # dedicated functions.
