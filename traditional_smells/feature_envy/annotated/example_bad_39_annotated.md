...
- **Smell:** Feature Envy
- **Expected smell location:** `Billing.InvoiceFormatter.format_line_item/1`
- **Affected function(s):** `format_line_item/1`
- **Explanation:** `format_line_item/1` calls `LineItem.get_product/1`, `LineItem.get_tax_profile/1`, `LineItem.get_applied_discount/1`, and `LineItem.unit_label/1`, and reads multiple fields directly from the `item` struct (`unit_price`, `quantity`, `region_code`, `fulfillment_notes`). The function uses almost nothing from `InvoiceFormatter`'s own domain; it clearly envies `LineItem` and belongs there.
...
defmodule Billing.InvoiceFormatter do
  ...
  # VALIDATION: This is a smell because format_line_item/1 is defined in
  # VALIDATION: InvoiceFormatter but nearly all of its logic operates on
  # VALIDATION: LineItem's data: it calls LineItem.get_product/1,
  # VALIDATION: LineItem.get_tax_profile/1, LineItem.get_applied_discount/1,
  # VALIDATION: and LineItem.unit_label/1, while also reading item.unit_price,
  # VALIDATION: item.quantity, item.region_code, and item.fulfillment_notes
  # VALIDATION: directly from the LineItem struct. InvoiceFormatter contributes
  # VALIDATION: only the final money/percent formatting calls. Moving this
  # VALIDATION: function into LineItem would increase LineItem's cohesion and
  # VALIDATION: reduce coupling with InvoiceFormatter.
