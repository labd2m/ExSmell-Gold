...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Subscriptions.BillingCycle.generate_renewal_invoice/1` |
| **Affected Function(s)** | `generate_renewal_invoice/1` |
| **Explanation** | `generate_renewal_invoice/1` is defined in `Subscriptions.BillingCycle` but all meaningful operations are performed on `Subscriptions.SubscriptionContract` — calling `get!/1`, `renewal_amount/1`, `next_billing_date/1`, `is_annual?/1`, `discount_for_renewal/1`, and reading struct fields. The function belongs to `SubscriptionContract`, not `BillingCycle`. |
...
defmodule Subscriptions.BillingCycle do
  ...
  # VALIDATION: This is a smell because `generate_renewal_invoice/1` is defined in
  # VALIDATION: `Subscriptions.BillingCycle` but performs all operations through
  # VALIDATION: `Subscriptions.SubscriptionContract`: it calls
  # VALIDATION: `SubscriptionContract.get!/1`, `SubscriptionContract.renewal_amount/1`,
  # VALIDATION: `SubscriptionContract.next_billing_date/1`, `SubscriptionContract.is_annual?/1`,
  # VALIDATION: and `SubscriptionContract.discount_for_renewal/1`. Nothing from
  # VALIDATION: `BillingCycle` itself drives this logic; it should live in
  # VALIDATION: `SubscriptionContract`.
