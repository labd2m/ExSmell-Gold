...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Compliance.AuditReport.format_audit_entry/1` |
| **Affected Function(s)** | `format_audit_entry/1` |
| **Explanation** | `format_audit_entry/1` lives in `Compliance.AuditReport` but all its data and helper calls come from `Compliance.AuditLog` — calling `get!/1`, `actor_label/1`, `action_category/1`, `is_sensitive?/1`, `outcome_label/1`, and reading struct fields. The function is more interested in `AuditLog` than in the report module and should be moved there. |
...
defmodule Compliance.AuditReport do
  ...
  # VALIDATION: This is a smell because `format_audit_entry/1` belongs to
  # VALIDATION: `Compliance.AuditReport` but every operation it performs
  # VALIDATION: comes from `Compliance.AuditLog`: it calls `AuditLog.get!/1`,
  # VALIDATION: `AuditLog.actor_label/1`, `AuditLog.action_category/1`,
  # VALIDATION: `AuditLog.is_sensitive?/1`, and `AuditLog.outcome_label/1`.
  # VALIDATION: The function uses nothing from `AuditReport` itself and
  # VALIDATION: should be moved into `AuditLog`.
