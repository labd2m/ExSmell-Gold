...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Billing.InvoiceSummary.calculate_line_total/1` |
| **Affected Function(s)** | `calculate_line_total/1` |
| **Explanation** | `calculate_line_total/1` lives in `Billing.InvoiceSummary` but fetches the `InvoiceItem` struct and calls `InvoiceItem.get_tax_rate/1`, `InvoiceItem.get_discount/1`, and `InvoiceItem.get_surcharge/1`. All meaningful data and behaviour come from `InvoiceItem`; the function is more interested in that module than in its own. |
...
defmodule Billing.InvoiceSummary do
  ...
  # VALIDATION: This is a smell because `calculate_line_total/1` retrieves the struct
  # VALIDATION: via `InvoiceItem.get!/1` and then calls three more functions on that
  # VALIDATION: module: `get_tax_rate/1`, `get_discount/1`, and `get_surcharge/1`.
  # VALIDATION: All data and behaviour originate in `InvoiceItem`; this function
  # VALIDATION: contributes nothing from `InvoiceSummary` itself and should live in
