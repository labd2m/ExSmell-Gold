...
- **Smell:** Feature Envy
- **Expected smell location:** `Billing.RenewalEngine.compute_renewal_invoice/1`
- **Affected function(s):** `compute_renewal_invoice/1`
- **Explanation:** `compute_renewal_invoice/1` calls `SubscriptionPlan.get_pricing_tiers/1`, `SubscriptionPlan.applicable_addons/1`, `SubscriptionPlan.annual_discount_rate/1`, `SubscriptionPlan.tax_category/1`, and `SubscriptionPlan.grace_period_days/1`, while reading multiple fields from the subscription struct directly. `RenewalEngine` contributes only arithmetic helpers. The function envies `SubscriptionPlan` and should live there.
...
defmodule Billing.RenewalEngine do
  ...
  # VALIDATION: This is a smell because compute_renewal_invoice/1 is defined in
  # VALIDATION: RenewalEngine but nearly all of its logic derives from
  # VALIDATION: SubscriptionPlan. It calls:
  # VALIDATION:   - SubscriptionPlan.get_pricing_tiers/1
  # VALIDATION:   - SubscriptionPlan.applicable_addons/1
  # VALIDATION:   - SubscriptionPlan.annual_discount_rate/1
  # VALIDATION:   - SubscriptionPlan.tax_category/1
  # VALIDATION:   - SubscriptionPlan.grace_period_days/1
  # VALIDATION: and reads subscription.id, subscription.plan_id,
  # VALIDATION: subscription.seat_count, subscription.billing_cycle,
  # VALIDATION: subscription.current_period_end, and subscription.customer_id
  # VALIDATION: directly from the struct. RenewalEngine only supplies
  # VALIDATION: date arithmetic and the wrapping invoice map.
  # VALIDATION: This function should live in SubscriptionPlan.
