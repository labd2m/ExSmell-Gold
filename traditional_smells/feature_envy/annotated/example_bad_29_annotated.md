...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `CRM.DealPipeline.score_deal_probability/1` |
| **Affected Function(s)** | `score_deal_probability/1` |
| **Explanation** | `score_deal_probability/1` lives in `CRM.DealPipeline` but all of its data and helper invocations come from `CRM.DealRecord` — calling `get!/1`, `deal_size_tier/1`, `days_in_stage/1`, `has_champion?/1`, `competitor_count/1`, and `budget_confirmed?/1`. The function has no dependency on `DealPipeline` and should reside in `DealRecord`. |
...
defmodule CRM.DealPipeline do
  ...
  # VALIDATION: This is a smell because `score_deal_probability/1` is placed in
  # VALIDATION: `CRM.DealPipeline` but all logic and data come from `CRM.DealRecord`:
  # VALIDATION: it calls `DealRecord.get!/1`, `DealRecord.deal_size_tier/1`,
  # VALIDATION: `DealRecord.days_in_stage/1`, `DealRecord.has_champion?/1`,
  # VALIDATION: `DealRecord.competitor_count/1`, and `DealRecord.budget_confirmed?/1`.
  # VALIDATION: The function is entirely driven by `DealRecord` and belongs there.
