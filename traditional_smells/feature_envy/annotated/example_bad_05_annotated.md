# Annotated Example 05: Feature Envy

## Metadata

- **Smell**: Feature Envy
- **Expected Smell Location**: `Reporting.SalesReport.summarize_line_item/1`
- **Affected Function(s)**: `summarize_line_item/1`
- **Explanation**: `summarize_line_item/1` exclusively uses functions and data from the
  `LineItem` module (`LineItem.product_name/1`, `LineItem.unit_price_with_tax/1`,
  `LineItem.discount_applied/1`, `LineItem.net_total/1`, `LineItem.category/1`), plus
  direct struct fields. `SalesReport` contributes no logic of its own to this function;
  it would be more cohesive inside the `LineItem` module.
...
defmodule Reporting.SalesReport do
  ...
  # VALIDATION: This is a smell because summarize_line_item/1 exclusively uses functions
  # VALIDATION: and data from the LineItem module: LineItem.product_name/1,
  # VALIDATION: LineItem.unit_price_with_tax/1, LineItem.discount_applied/1,
  # VALIDATION: LineItem.net_total/1, and LineItem.category/1, as well as direct struct fields.
  # VALIDATION: SalesReport contributes no logic of its own to this function;
  # VALIDATION: it would be more cohesive inside the LineItem module.
