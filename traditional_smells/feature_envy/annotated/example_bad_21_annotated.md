...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Reporting.SalesReport.compute_rep_commission/1` |
| **Affected Function(s)** | `compute_rep_commission/1` |
| **Explanation** | `compute_rep_commission/1` lives in `Reporting.SalesReport` but all its logic and data come from `Reporting.SalesRecord` — calling `get!/1`, `gross_revenue/1`, `is_recurring?/1`, `discount_applied?/1`, and `region_multiplier/1`. The function is more interested in `SalesRecord` than in the report module itself. |
...
defmodule Reporting.SalesReport do
  ...
  # VALIDATION: This is a smell because `compute_rep_commission/1` is defined in
  # VALIDATION: `Reporting.SalesReport` but exclusively uses `SalesRecord`'s data
  # VALIDATION: and behaviour: it calls `SalesRecord.get!/1`, `SalesRecord.gross_revenue/1`,
  # VALIDATION: `SalesRecord.is_recurring?/1`, `SalesRecord.discount_applied?/1`, and
  # VALIDATION: `SalesRecord.region_multiplier/1`. The function has no dependency on
  # VALIDATION: anything in `SalesReport` and should be moved to `SalesRecord`.
