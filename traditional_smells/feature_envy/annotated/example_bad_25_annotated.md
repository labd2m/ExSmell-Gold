...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Accounts.ProfileMigrator.build_migration_diff/1` |
| **Affected Function(s)** | `build_migration_diff/1` |
| **Explanation** | `build_migration_diff/1` is defined in `Accounts.ProfileMigrator` but all of its data and helper calls come from `Accounts.UserProfile` — calling `get!/1`, `full_name/1`, `has_avatar?/1`, `marketing_opted_in?/1`, `verified?/1`, and `linked_providers/1`. The function is entirely driven by `UserProfile` and should live there. |
...
defmodule Accounts.ProfileMigrator do
  ...
  # VALIDATION: This is a smell because `build_migration_diff/1` is placed in
  # VALIDATION: `Accounts.ProfileMigrator` yet every meaningful call targets
  # VALIDATION: `Accounts.UserProfile`: it calls `UserProfile.get!/1`,
  # VALIDATION: `UserProfile.full_name/1`, `UserProfile.has_avatar?/1`,
  # VALIDATION: `UserProfile.marketing_opted_in?/1`, `UserProfile.verified?/1`,
  # VALIDATION: and `UserProfile.linked_providers/1`. The function should be
  # VALIDATION: part of `UserProfile`, not the migration orchestrator.
