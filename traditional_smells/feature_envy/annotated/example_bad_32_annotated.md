...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Fulfillment.RouteBuilder.estimate_stop_cost/1` |
| **Affected Function(s)** | `estimate_stop_cost/1` |
| **Explanation** | `estimate_stop_cost/1` lives in `Fulfillment.RouteBuilder` but all its data and helper calls originate from `Fulfillment.DeliveryStop` — calling `get!/1`, `distance_km/1`, `requires_equipment?/1`, `time_window_penalty/1`, `package_weight_kg/1`, and reading struct fields. The function belongs in `DeliveryStop`, not in `RouteBuilder`. |
...
defmodule Fulfillment.DeliveryStop do
  ...
  def distance_km(%__MODULE__{lat: lat, lng: lng, depot_lat: dlat, depot_lng: dlng}) do
    # Simplified Euclidean approximation; production code would use Haversine
    dx = (lat - dlat) * 111.0
...
defmodule Fulfillment.RouteBuilder do
  ...
  # VALIDATION: This is a smell because `estimate_stop_cost/1` belongs to
  # VALIDATION: `Fulfillment.RouteBuilder` yet all its data originates in
  # VALIDATION: `Fulfillment.DeliveryStop`: it calls `DeliveryStop.get!/1`,
  # VALIDATION: `DeliveryStop.distance_km/1`, `DeliveryStop.requires_equipment?/1`,
  # VALIDATION: `DeliveryStop.time_window_penalty/1`, and
  # VALIDATION: `DeliveryStop.package_weight_kg/1`. The function should be
  # VALIDATION: moved into `DeliveryStop`.
