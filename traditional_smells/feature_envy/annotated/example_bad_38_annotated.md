...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Elearning.CertificateIssuer.build_certificate_data/1` |
| **Affected Function(s)** | `build_certificate_data/1` |
| **Explanation** | `build_certificate_data/1` lives in `Elearning.CertificateIssuer` but all its meaningful operations are performed on `Elearning.CourseEnrollment` — calling `get!/1`, `completion_pct/1`, `passed?/1`, `final_grade/1`, `total_study_hours/1`, and `certificate_eligible?/1`. The function is entirely interested in `CourseEnrollment` and should be moved there. |
...
defmodule Elearning.CertificateIssuer do
  ...
  # VALIDATION: This is a smell because `build_certificate_data/1` is defined
  # VALIDATION: inside `Elearning.CertificateIssuer` yet every operation it
  # VALIDATION: performs targets `Elearning.CourseEnrollment`: it calls
  # VALIDATION: `CourseEnrollment.get!/1`, `CourseEnrollment.completion_pct/1`,
  # VALIDATION: `CourseEnrollment.passed?/1`, `CourseEnrollment.final_grade/1`,
  # VALIDATION: `CourseEnrollment.total_study_hours/1`, and
  # VALIDATION: `CourseEnrollment.certificate_eligible?/1`. Nothing from
  # VALIDATION: `CertificateIssuer` drives this logic; the function should live
  # VALIDATION: in `CourseEnrollment`.
