...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Tax.VatEngine.compute_vat_breakdown/1` |
| **Affected Function(s)** | `compute_vat_breakdown/1` |
| **Explanation** | `compute_vat_breakdown/1` is defined in `Tax.VatEngine` but derives all its data and decisions from `Tax.InvoiceTaxLine` — calling `get!/1`, `net_amount/1`, `applicable_rate/1`, `is_exempt?/1`, `reverse_charge?/1`, and `jurisdiction_code/1`. The function has no reliance on `VatEngine`'s own state and belongs in `InvoiceTaxLine`. |
...
defmodule Tax.VatEngine do
  ...
  # VALIDATION: This is a smell because `compute_vat_breakdown/1` belongs to
  # VALIDATION: `Tax.VatEngine` but all data and helper calls come from
  # VALIDATION: `Tax.InvoiceTaxLine`: it calls `InvoiceTaxLine.get!/1`,
  # VALIDATION: `InvoiceTaxLine.net_amount/1`, `InvoiceTaxLine.applicable_rate/1`,
  # VALIDATION: `InvoiceTaxLine.is_exempt?/1`, `InvoiceTaxLine.reverse_charge?/1`,
  # VALIDATION: and `InvoiceTaxLine.jurisdiction_code/1`. The logic is entirely
  # VALIDATION: about the tax line, not the engine, and should be moved there.
