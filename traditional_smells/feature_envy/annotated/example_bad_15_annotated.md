...
| **Smell location**     | `Payments.ChargeProcessor.prepare_charge_details/1`                                       |
| **Affected function**  | `prepare_charge_details/1`                                                                |
| **Explanation**        | The function calls `PaymentMethod.fetch!/1`, `PaymentMethod.type/1`, `PaymentMethod.billing_address/1`, `PaymentMethod.expiry/1`, `PaymentMethod.is_verified?/1`, `PaymentMethod.supported_currencies/1`, `PaymentMethod.network/1`, and `PaymentMethod.issuing_bank/1`, while reading `method.last_four`, `method.holder_name`, `method.fingerprint`, and `method.country`. `ChargeProcessor` contributes only minor date calculations. All domain logic and data belong to `PaymentMethod`. |
...
defmodule Payments.ChargeProcessor do
  ...
  # VALIDATION: This is a smell because prepare_charge_details/1 focuses entirely on data
  # VALIDATION: from the PaymentMethod module. It calls PaymentMethod.fetch!/1,
  # VALIDATION: PaymentMethod.type/1, PaymentMethod.billing_address/1,
  # VALIDATION: PaymentMethod.expiry/1, PaymentMethod.is_verified?/1,
  # VALIDATION: PaymentMethod.supported_currencies/1, PaymentMethod.network/1, and
  # VALIDATION: PaymentMethod.issuing_bank/1, while also reading method.last_four,
  # VALIDATION: method.holder_name, method.fingerprint, and method.country.
  # VALIDATION: ChargeProcessor contributes only minor date arithmetic on top of
  # VALIDATION: PaymentMethod data; this function should live in PaymentMethod.
