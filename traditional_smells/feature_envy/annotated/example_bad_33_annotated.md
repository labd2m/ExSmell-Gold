...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `IAM.PolicyEnforcer.evaluate_resource_access/2` |
| **Affected Function(s)** | `evaluate_resource_access/2` |
| **Explanation** | `evaluate_resource_access/2` is defined in `IAM.PolicyEnforcer` but draws every piece of data and logic from `IAM.AccessPolicy` — calling `get_for_role!/1`, `allows_action?/2`, `resource_scope/1`, `is_time_restricted?/1`, and `within_allowed_hours?/1`. The function should be part of `AccessPolicy`, not `PolicyEnforcer`. |
...
defmodule IAM.PolicyEnforcer do
  ...
  # VALIDATION: This is a smell because `evaluate_resource_access/2` lives in
  # VALIDATION: `IAM.PolicyEnforcer` but performs every check through
  # VALIDATION: `IAM.AccessPolicy`: it calls `AccessPolicy.get_for_role!/1`,
  # VALIDATION: `AccessPolicy.allows_action?/2`, `AccessPolicy.resource_scope/1`,
  # VALIDATION: `AccessPolicy.is_time_restricted?/1`, and
  # VALIDATION: `AccessPolicy.within_allowed_hours?/1`. The function contributes
  # VALIDATION: nothing from `PolicyEnforcer` itself and belongs in `AccessPolicy`.
