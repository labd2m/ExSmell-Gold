...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Catalog.PricingEngine.apply_tier_pricing/1` |
| **Affected Function(s)** | `apply_tier_pricing/1` |
| **Explanation** | `apply_tier_pricing/1` is placed in `Catalog.PricingEngine` but exclusively operates on `Catalog.ProductVariant` — calling `get!/1`, `base_price/1`, `volume_tier/1`, `is_on_clearance?/1`, `markdown_rate/1`, and reading struct fields. The function is more interested in `ProductVariant` than in the engine module. |
...
defmodule Catalog.PricingEngine do
  ...
  # VALIDATION: This is a smell because `apply_tier_pricing/2` is defined in
  # VALIDATION: `Catalog.PricingEngine` but all its operations target
  # VALIDATION: `Catalog.ProductVariant`: it calls `ProductVariant.get!/1`,
  # VALIDATION: `ProductVariant.base_price/1`, `ProductVariant.volume_tier/2`,
  # VALIDATION: `ProductVariant.is_on_clearance?/1`, and
  # VALIDATION: `ProductVariant.markdown_rate/1`. The function belongs in
  # VALIDATION: `ProductVariant`, not in the engine module.
