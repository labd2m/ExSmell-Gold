...
|---|---|
| **Smell** | Feature Envy |
| **Expected Smell Location** | `Scheduling.BookingService.check_slot_availability/2` |
| **Affected Function(s)** | `check_slot_availability/2` |
| **Explanation** | `check_slot_availability/2` is defined in `Scheduling.BookingService` but draws all its data and logic from `Scheduling.CalendarEntry` — calling `get!/1`, `is_open?/1`, `remaining_capacity/1`, `within_booking_window?/1`, and `duration_minutes/1`. Nothing in the function is specific to `BookingService`; it should reside in `CalendarEntry`. |
...
defmodule Scheduling.BookingService do
  ...
  # VALIDATION: This is a smell because `check_slot_availability/2` lives in
  # VALIDATION: `Scheduling.BookingService` but all its work is done through
  # VALIDATION: `Scheduling.CalendarEntry`: it calls `CalendarEntry.get!/1`,
  # VALIDATION: `CalendarEntry.is_open?/1`, `CalendarEntry.remaining_capacity/1`,
  # VALIDATION: `CalendarEntry.within_booking_window?/1`, and
  # VALIDATION: `CalendarEntry.duration_minutes/1`. The `user_id` argument is
  # VALIDATION: only used for a log line; the real logic belongs in `CalendarEntry`.
