...
- **Smell name:** Comments
- **Expected smell location:** `BillingService.generate_invoice/2`
- **Affected function(s):** `generate_invoice/2`
- **Short explanation:** The function is documented with plain `#` comments instead of an `@doc` attribute, making the documentation invisible to `IEx.h/1`, ExDoc, and other tooling.
...
defmodule MyApp.BillingService do
  ...
  # VALIDATION: This is a smell because the function is documented using plain # comments
  # instead of an @doc attribute, making the documentation invisible to tooling like IEx.h/1 and ExDoc.
