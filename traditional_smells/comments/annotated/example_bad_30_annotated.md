...
- **Short explanation:** The function documentation is written with plain `#` comments instead of `@doc`. This bypasses Elixir's documentation toolchain, making it impossible to retrieve the docs via `h/1` in IEx or include them in generated documentation.
...
defmodule MyApp.InventoryManager do
  ...
  # VALIDATION: This is a smell because `reserve_stock/3` is described with plain `#` comments
  # VALIDATION: instead of `@doc`, so the documentation cannot be retrieved via `h/1` in IEx
  # VALIDATION: or included in ExDoc-generated HTML.
