...
- **Smell name:** Comments
- **Expected smell location:** `ReportBuilder.build_revenue_report/2`
- **Affected function(s):** `build_revenue_report/2`
- **Short explanation:** The function is documented with `#` comment lines describing parameters and return values, rather than using `@doc`, which means the documentation is not accessible through Elixir tooling.
...
defmodule MyApp.Reporting.ReportBuilder do
  ...
  # VALIDATION: This is a smell because build_revenue_report/2 is documented using plain
  # # comments instead of @doc, making the documentation unavailable to ExDoc and IEx.h/1.
