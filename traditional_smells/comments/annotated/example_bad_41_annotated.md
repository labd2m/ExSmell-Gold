...
- **Smell name:** Comments
- **Expected smell location:** `ShipmentTracker` module, function `update_shipment_status/3`
- **Affected function(s):** `update_shipment_status/3`
- **Short explanation:** `update_shipment_status/3` is documented with `#` comment lines instead of `@doc`, stripping any developer-facing documentation from the compiled module.
...
defmodule ShipmentTracker do
  ...
  # VALIDATION: This is a smell because the function documents its contract, allowed transitions,
  # VALIDATION: and return values using `#` comments rather than `@doc`. The compiler ignores
  # VALIDATION: these, so no tooling can surface them.
