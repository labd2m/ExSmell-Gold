...
- **Smell name:** Comments
- **Expected smell location:** `ReportGenerator.generate_sales_report/2`
- **Affected function(s):** `generate_sales_report/2`
- **Short explanation:** Documentation for `generate_sales_report/2` is expressed as a multi-line `#` comment block instead of `@doc`. This bypasses Elixir's official documentation mechanism and makes the function's contract undiscoverable through tooling.
...
defmodule MyApp.ReportGenerator do
  ...
  # VALIDATION: This is a smell because documentation for `generate_sales_report/2` is
  # VALIDATION: written using `#` comment lines instead of the `@doc` attribute, making it
  # VALIDATION: unreachable by `h/1` in IEx and invisible to ExDoc.
