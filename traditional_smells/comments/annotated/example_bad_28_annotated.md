...
- **Short explanation:** The function relies on inline `#` comments to explain its behaviour and return values instead of using `@doc`, which is the idiomatic Elixir way to provide machine-readable, tooling-friendly documentation.
...
defmodule MyApp.AuthService do
  ...
  # VALIDATION: This is a smell because plain `#` comments are used to document the function
  # VALIDATION: instead of `@doc`, so the documentation is not accessible via `h/1` in IEx
  # VALIDATION: or rendered by ExDoc.
