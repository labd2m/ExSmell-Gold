...
- **Smell name:** Comments
- **Expected smell location:** `UserManager` module, function `update_account/2`
- **Affected function(s):** `update_account/2`
- **Short explanation:** `update_account/2` documents its behaviour with `#` comment lines rather than a proper `@doc` attribute, so the documentation is stripped at compile time and never available through ExDoc or IEx.
...
defmodule UserManager do
  ...
  # VALIDATION: This is a smell because `update_account/2` is explained entirely through
  # VALIDATION: `#` comment lines rather than an `@doc` attribute. Elixir's documentation
  # VALIDATION: tooling ignores these, so they are never compiled into module docs.
