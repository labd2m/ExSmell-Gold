...
| **Short explanation** | The function is documented using plain `#` comment lines instead of the proper `@doc` attribute, bypassing ExDoc and making the documentation invisible to tooling and `h/1` in IEx. |
...
defmodule MyApp.BillingService do
  ...
  # VALIDATION: This is a smell because the developer documented `calculate_invoice/2`
  # VALIDATION: using plain `#` comment lines instead of an `@doc` attribute,
  # VALIDATION: making the documentation invisible to ExDoc and IEx `h/1`.
