...
- **Smell name:** Comments
- **Expected smell location:** `RevenueReporter` module, function `generate_monthly_report/2`
- **Affected function(s):** `generate_monthly_report/2`
- **Short explanation:** `generate_monthly_report/2` is described exclusively through `#` comment lines rather than `@doc`, so the documentation is never compiled into the module and cannot be consulted by developers via ExDoc or `h/1`.
...
defmodule RevenueReporter do
  ...
  # VALIDATION: This is a smell because `generate_monthly_report/2` provides its entire
  # VALIDATION: specification through `#` comment lines. Elixir discards these at compile
  # VALIDATION: time; the function has no `@doc` and is effectively undocumented to tooling.
