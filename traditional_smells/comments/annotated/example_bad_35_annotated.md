...
- **Short explanation:** The function's expected behaviour, parameters, and return values are described using plain `#` comments instead of `@doc`. Elixir's documentation tooling only processes `@doc` attributes, so this documentation is effectively invisible.
...
defmodule MyApp.ShipmentTracker do
  ...
  # VALIDATION: This is a smell because `update_shipment_status/3` is described via `#` comment
  # VALIDATION: lines instead of `@doc`. The Elixir documentation system (`h/1`, ExDoc) does not
  # VALIDATION: process plain comments, so this documentation is invisible to those tools.
