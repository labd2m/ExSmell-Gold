...
| **Short explanation** | The public `dispatch/2` function is described only through inline `#` comments instead of an `@doc` attribute, bypassing all documentation tooling. |
...
defmodule MyApp.NotificationDispatcher do
  ...
  # VALIDATION: This is a smell because `dispatch/2` is documented exclusively
  # VALIDATION: with plain `#` comment lines. The `@doc` attribute is never used,
  # VALIDATION: so ExDoc won't render this documentation and IEx `h/1` will show nothing.
