...
- **Smell name:** Comments
- **Expected smell location:** `InventoryManager` module, function `reserve_stock/3`
- **Affected function(s):** `reserve_stock/3`
- **Short explanation:** `reserve_stock/3` uses plain `#` comment blocks for its documentation instead of `@doc`, making the documentation invisible to ExDoc and the IEx help system.
...
defmodule InventoryManager do
  ...
  # VALIDATION: This is a smell because `reserve_stock/3` documents itself with plain `#`
  # VALIDATION: comment lines rather than `@doc`. These are treated as code comments by the
  # VALIDATION: Elixir compiler and are not available as structured documentation.
