...
- **Short explanation:** The function uses inline `#` comments to describe its behaviour, parameters, and return values instead of using the `@doc` attribute, so none of that information is available via `IEx.h/1` or generated HTML docs.
...
defmodule MyApp.Inventory.InventoryManager do
  ...
  # VALIDATION: This is a smell because reserve_stock/3 uses # comments for documentation
  # rather than @doc, hiding the documentation from Elixir's documentation tooling.
