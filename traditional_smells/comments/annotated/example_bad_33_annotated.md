...
- **Smell name:** Comments
- **Expected smell location:** `PaymentProcessor.charge/2`
- **Affected function(s):** `charge/2`
- **Short explanation:** `charge/2` has its full contract written as `#` comments. Even though the information is thorough, using `#` comments instead of `@doc` means the documentation is inaccessible via Elixir's built-in tooling and not rendered in any generated docs.
...
defmodule MyApp.PaymentProcessor do
  ...
  # VALIDATION: This is a smell because the function documentation appears as a block of `#`
  # VALIDATION: comments rather than a `@doc` attribute. The Elixir documentation system
  # VALIDATION: (`h/1`, ExDoc) only understands `@doc`, so this text is invisible to it.
