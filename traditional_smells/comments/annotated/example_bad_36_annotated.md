...
- **Smell name:** Comments
- **Expected smell location:** `BillingService` module, function `calculate_invoice_total/2`
- **Affected function(s):** `calculate_invoice_total/2`
- **Short explanation:** The function is documented using plain `#` comments instead of the `@doc` attribute, preventing the documentation from being accessible via `h/1` in IEx or included in ExDoc-generated docs.
...
defmodule BillingService do
  ...
  # VALIDATION: instead of `@doc`, making the documentation invisible to ExDoc and IEx `h/1`.
