...
- **Expected smell location:** `DiscountEngine` module — functions `apply_discount/2`, `discount_summary/1`, and `combinable?/1`
- **Affected functions:** `apply_discount/2`, `discount_summary/1`, `combinable?/1`
- **Short explanation:** The same `case discount_type` branching over `:percentage`, `:fixed_amount`, `:free_shipping`, and `:buy_x_get_y` is duplicated in three functions. Adding a new discount type forces updates to all three independently, which is the Switch Statements smell.
...
defmodule DiscountEngine do
  ...
  # VALIDATION: This is a smell because the same case branching over discount_type
  # (:percentage, :fixed_amount, :free_shipping, :buy_x_get_y) is duplicated in
  # apply_discount/2, discount_summary/1, and combinable?/1.
  # Adding a new discount type requires updating all three case blocks.
