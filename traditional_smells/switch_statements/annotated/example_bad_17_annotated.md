...
- **Expected smell location:** `RefundEngine` module — functions `requires_manager_approval?/1`, `customer_notification_copy/1`, and `accounting_reversal_code/1`
- **Affected functions:** `requires_manager_approval?/1`, `customer_notification_copy/1`, `accounting_reversal_code/1`
- **Short explanation:** The same `case reason` branching over `:duplicate_charge`, `:product_not_received`, `:fraudulent`, and `:customer_request` is duplicated in three distinct functions. Any new refund reason type forces changes in all three, which is the Switch Statements smell.
...
defmodule RefundEngine do
  ...
  # VALIDATION: This is a smell because the same case branching over reason
  # (:duplicate_charge, :product_not_received, :fraudulent, :customer_request)
  # is duplicated in requires_manager_approval?/1, customer_notification_copy/1,
  # and accounting_reversal_code/1. A new refund reason requires updating all three.
