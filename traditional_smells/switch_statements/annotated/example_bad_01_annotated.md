...
- **Expected smell location:** `BillingProcessor` module — functions `calculate_processing_fee/1`, `build_payment_receipt/1`, and `log_payment_attempt/1`
- **Affected functions:** `calculate_processing_fee/1`, `build_payment_receipt/1`, `log_payment_attempt/1`
- **Short explanation:** The same `case payment_method` branching logic over `:credit_card`, `:debit_card`, `:bank_transfer`, and `:pix` is duplicated verbatim across three separate functions. Adding a new payment method requires modifying all three independently, which is the hallmark of the Switch Statements smell.
...
defmodule BillingProcessor do
  ...
  # VALIDATION: This is a smell because the same case branching over payment_method
  # is duplicated in calculate_processing_fee/1, build_payment_receipt/1, and
  # log_payment_attempt/1. Every time a new payment method is introduced, a developer
  # must update all three case expressions independently.
