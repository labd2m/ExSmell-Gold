...
- **Expected smell location:** `RefundPolicy.requires_approval?/1` and `RefundPolicy.ledger_code/1`
- **Affected functions:** `requires_approval?/1`, `ledger_code/1`
- **Short explanation:** The same `case` branching over refund reason (`:duplicate_charge`, `:product_not_received`, `:product_defective`, `:customer_request`, `:fraud`) is duplicated in `requires_approval?/1` and `ledger_code/1`. Every new refund reason must be registered in both.
...
defmodule RefundPolicy do
  ...
  # VALIDATION: This is a smell because the same case branching on `reason`
  # also appears in `ledger_code/1` below. Both enumerate all five refund reason
  # atoms — adding a new reason requires changes in both functions.
  ...
  # VALIDATION: SMELL START - Switch Statements
  # VALIDATION: This is a smell because the same case branching on `reason`
  # already appeared in `requires_approval?/1` above. The refund reason atoms
  # are fully repeated, making the two functions fragile to extend in parallel.
