...
- **Expected smell location:** `FulfillmentRouter.cost_multiplier/1` and `FulfillmentRouter.priority_label/1`
- **Affected functions:** `cost_multiplier/1`, `priority_label/1`
- **Short explanation:** The same `case` branching over delivery priority (`:economy`, `:standard`, `:express`, `:overnight`) is duplicated in `cost_multiplier/1` and `priority_label/1`. Adding a new priority level requires modifying both case blocks.
...
defmodule FulfillmentRouter do
  ...
  # also appears in `priority_label/1` below. Both enumerate :economy, :standard,
  # :express, :overnight — a new priority requires touching both case blocks.
  ...
  # VALIDATION: SMELL START - Switch Statements
  # VALIDATION: This is a smell because the same case branching on `priority`
  # already appeared in `cost_multiplier/1` above. All delivery priority atoms
  # are repeated, creating duplicated branching that must evolve in parallel.
