...
- **Expected smell location:** `TransactionLedger` module — functions `ledger_entry_type/1`, `is_terminal_status?/1`, and `status_display/1`
- **Affected functions:** `ledger_entry_type/1`, `is_terminal_status?/1`, `status_display/1`
- **Short explanation:** The same `case status` branching over `:authorized`, `:captured`, `:refunded`, `:voided`, and `:failed` is repeated three times across the module. Any new transaction status must be added to each case block separately, which is the Switch Statements smell.
...
defmodule TransactionLedger do
  ...
  # (:authorized, :captured, :refunded, :voided, :failed) is duplicated in
  # ledger_entry_type/1, is_terminal_status?/1, and status_display/1.
  # Each new transaction status requires independent changes to all three cases.
