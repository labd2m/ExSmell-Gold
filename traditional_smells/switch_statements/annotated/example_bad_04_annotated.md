...
- **Expected smell location:** `InventoryPolicy` module — functions `reorder_threshold/1`, `storage_zone/1`, and `expiry_alert_days/1`
- **Affected functions:** `reorder_threshold/1`, `storage_zone/1`, `expiry_alert_days/1`
- **Short explanation:** The same `case category` branching over `:electronics`, `:perishable`, `:apparel`, and `:pharmaceutical` is duplicated in three functions. Adding a new product category requires editing all three case blocks, which is the Switch Statements smell.
...
defmodule InventoryPolicy do
  ...
  # VALIDATION: This is a smell because the same case branching over category
  # (:electronics, :perishable, :apparel, :pharmaceutical) appears identically in
  # reorder_threshold/1, storage_zone/1, and expiry_alert_days/1. When a new category
  # is introduced, all three functions must be updated independently.
