...
- **Expected smell location:** `UserOnboarding.welcome_template/1` and `UserOnboarding.landing_path/1`
- **Affected functions:** `welcome_template/1`, `landing_path/1`
- **Short explanation:** The same `case` branching over user status (`:invited`, `:pending_verification`, `:active`, `:suspended`, `:deactivated`) is duplicated in `welcome_template/1` and `landing_path/1`. Any new status requires changes in both functions.
...
defmodule UserOnboarding do
  ...
  # also appears in `landing_path/1` below. Both enumerate :invited,
  # :pending_verification, :active, :suspended, :deactivated — a new status
  # must be added in both functions.
  ...
  # VALIDATION: SMELL START - Switch Statements
  # VALIDATION: This is a smell because the same case branching on `status`
  # already appeared in `welcome_template/1` above. All user status atoms
  # are repeated, making the two functions independently fragile.
