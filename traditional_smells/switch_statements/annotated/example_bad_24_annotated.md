...
- **Expected smell location:** `InvoiceStateMachine` module — functions `overdue_fee_applicable?/1`, `customer_portal_actions/1`, and `dunning_step/1`
- **Affected functions:** `overdue_fee_applicable?/1`, `customer_portal_actions/1`, `dunning_step/1`
- **Short explanation:** The same `case status` branching over `:draft`, `:open`, `:overdue`, `:paid`, and `:voided` is duplicated in three separate functions. Adding a new invoice status requires editing all three case blocks, which is the Switch Statements smell.
...
defmodule InvoiceStateMachine do
  ...
  # (:draft, :open, :overdue, :paid, :voided) is duplicated across
  # overdue_fee_applicable?/1, customer_portal_actions/1, and dunning_step/1.
  # Introducing a new invoice status forces changes in all three independently.
