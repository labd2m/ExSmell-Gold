...
- **Expected smell location:** `ReportExporter` module — functions `content_type/1`, `file_extension/1`, and `serialize/2`
- **Affected functions:** `content_type/1`, `file_extension/1`, `serialize/2`
- **Short explanation:** The same `case format` branching over `:csv`, `:json`, `:xlsx`, and `:pdf` is reproduced in three separate functions. Adding a new export format demands editing all three case blocks, which is the Switch Statements smell.
...
defmodule ReportExporter do
  ...
  # VALIDATION: This is a smell because the same case branching over format
  # (:csv, :json, :xlsx, :pdf) is duplicated verbatim in content_type/1,
  # file_extension/1, and serialize/2. Introducing a new export format requires
  # updating all three case expressions independently.
