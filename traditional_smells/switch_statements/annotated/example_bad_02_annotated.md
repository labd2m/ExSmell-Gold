...
- **Expected smell location:** `AccessControl` module — functions `resolve_dashboard_path/1`, `apply_data_filters/2`, and `audit_access/2`
- **Affected functions:** `resolve_dashboard_path/1`, `apply_data_filters/2`, `audit_access/2`
- **Short explanation:** The same `case role` branching over `:admin`, `:manager`, `:analyst`, and `:viewer` is reproduced in three distinct functions. When a new role is added, all three case expressions must be updated in parallel, which is the Switch Statements smell.
...
defmodule AccessControl do
  ...
  # VALIDATION: This is a smell because the same case branching over the user's role
  # (:admin, :manager, :analyst, :viewer) is duplicated in resolve_dashboard_path/1,
  # apply_data_filters/2, and audit_access/2. Adding a new role forces a developer
  # to update all three case expressions independently.
