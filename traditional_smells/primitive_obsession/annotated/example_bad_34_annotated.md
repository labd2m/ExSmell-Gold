...
| **Expected smell location** | `Catalog.ProductVariantManager` module — variant attributes in all functions |
| **Affected functions** | `create_variant/3`, `update_pricing/3`, `find_matching_variant/3`, `build_variant_matrix/1` |
| **Short explanation** | Product variant attributes (colour and size) are passed as two separate `String.t()` parameters rather than a `%VariantSpec{colour: String.t(), size: String.t(), sku_suffix: String.t()}` struct. This makes the variant identity split across multiple arguments, forces callers to reassemble them when building SKUs or labels, and makes it easy to pass the arguments in the wrong order. |
...
defmodule Catalog.ProductVariantManager do
  ...
  # VALIDATION: This is a smell because a product variant is identified by two
  # separate plain strings: `colour` and `size` (e.g., colour = "BLACK",
  # size = "M"). Instead of a %VariantSpec{colour: "BLACK", size: "M"} struct,
  # these two closely related values are always passed together as loose
  # parameters. Functions that build SKU suffixes, display labels, or matrix
  # keys must reassemble them every time from raw strings.
