...
- **Expected Smell Location**: `authorize/3`, `grant_permission/2`, `revoke_permission/2`, `has_all_permissions?/2`, `merge_permissions/2`
- **Affected Function(s)**: All public functions in `Auth.PermissionGuard`
- **Explanation**: A user's permission set is modelled as a plain `list(String.t())` of raw permission strings (e.g., `"orders:read"`, `"admin:write"`) instead of a dedicated type or struct such as `%PermissionSet{}`. This means that scope parsing, wildcard expansion, and validation logic are all scattered across the module operating on raw strings, with no type-level guarantee that the strings represent valid, well-formed permissions.
...
defmodule Auth.PermissionGuard do
  ...
  # VALIDATION: This is a smell because a permission set is represented as a raw
  # VALIDATION: `list(String.t())` of unvalidated strings instead of a dedicated
  # VALIDATION: `%PermissionSet{}` struct (or at minimum a validated `@type`).
  # VALIDATION: All functions parse, validate, and manipulate plain strings, meaning
  # VALIDATION: typos like `"ordres:read"` are only caught at runtime and business
  # VALIDATION: rules such as wildcard expansion are duplicated across clauses.
