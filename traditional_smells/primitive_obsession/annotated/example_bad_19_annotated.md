...
- **Expected Smell Location**: `register_account/4`, `validate_account/3`, `initiate_transfer/5`, `mask_account/1`
- **Affected Function(s)**: All public functions in `Payments.BankAccountService`
- **Explanation**: A bank account is decomposed into three raw `String.t()` values (`account_number`, `routing_number`, `account_type`) instead of a `%BankAccount{account_number: String.t(), routing_number: String.t(), account_type: :checking | :savings, bank_name: String.t()}` struct. All functions must accept, validate, and forward these three strings independently, masking logic is ad hoc, and callers can silently swap account and routing numbers.
...
defmodule Payments.BankAccountService do
  ...
  # VALIDATION: This is a smell because a bank account is decomposed into three
  # VALIDATION: raw `String.t()` primitives — `account_number`, `routing_number`,
  # VALIDATION: and `account_type` — instead of a `%BankAccount{}` struct.
  # VALIDATION: Every function accepts and threads these three strings individually;
  # VALIDATION: callers can silently transpose `account_number` and `routing_number`;
  # VALIDATION: and masking, validation, and ABA-checksum logic operate on
  # VALIDATION: untyped strings with no structural guarantees.
