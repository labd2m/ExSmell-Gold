...
| **Expected smell location** | `Accounts.ProfileService` module — name representation throughout |
| **Affected functions** | `create_profile/2`, `update_name/2`, `generate_display_name/1`, `search_by_name/2` |
| **Short explanation** | A person's name is passed as a `{String.t(), String.t()}` tuple `{first_name, last_name}` — or as two separate string parameters — rather than a `%PersonName{first: String.t(), last: String.t(), preferred: String.t() | nil}` struct. This makes middle names and preferred names impossible to represent cleanly, requires callers to always destructure the tuple, and causes formatting logic to scatter throughout the module. |
...
defmodule Accounts.ProfileService do
  ...
  # VALIDATION: This is a smell because a person's name is passed as a plain
  # two-element tuple {first_name, last_name} or as two separate binary
  # parameters rather than a dedicated %PersonName{first: _, last: _, preferred: _}
  # struct. Every formatting operation (initials, display, search index) must
  # re-destructure the raw strings, and there is no place to attach a
  # preferred/nickname or middle name without adding more primitive arguments.
