...
- **Expected Smell Location**: `build_report/3`, `validate_range/2`, `split_into_periods/3`, `range_days/2`
- **Affected Function(s)**: All public functions in `Reporting.SalesReportBuilder`
- **Explanation**: A date range is represented as two separate `String.t()` ISO-8601 date values (`from_date` and `to_date`) passed individually to every function instead of being encapsulated in a `%DateRange{}` struct. This forces repeated parsing and validation, scatters boundary-check logic, and allows callers to silently pass start after end.
...
defmodule Reporting.SalesReportBuilder do
  ...
  # VALIDATION: This is a smell because a date range is modelled as two raw
  # VALIDATION: `String.t()` values (`from_date` and `to_date`) instead of a
  # VALIDATION: `%DateRange{from: Date.t(), to: Date.t()}` struct.
  # VALIDATION: Every function independently parses and validates these two
  # VALIDATION: strings, duplicating boundary and ordering checks, and the
  # VALIDATION: caller can pass from > to with no compile-time protection.
