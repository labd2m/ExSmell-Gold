...
- **Expected Smell Location**: `estimate_delivery_cost/3`, `within_free_delivery_radius?/2`, `add_distances/2`, `convert_distance/3`
- **Affected Function(s)**: All public functions in `Logistics.DeliveryDistanceEstimator`
- **Explanation**: A delivery distance is modelled as a raw `float()` value (implicitly in kilometres) rather than a `%Distance{value: float(), unit: :km | :mi | :m}` struct. The unit is enforced only by naming convention, so any caller that passes a distance in miles or metres instead of kilometres will get silently wrong cost estimates, and unit-conversion helpers are standalone functions with no attachment to the distance concept.
...
defmodule Logistics.DeliveryDistanceEstimator do
  ...
  # VALIDATION: This is a smell because delivery distance is represented as a
  # VALIDATION: plain `float()` value (implicitly kilometres) rather than a
  # VALIDATION: `%Distance{value: float(), unit: :km | :mi | :m}` struct.
  # VALIDATION: The unit is only enforced by the naming convention `_km` in
  # VALIDATION: parameter names; a caller passing miles or metres will produce
  # VALIDATION: silently wrong cost estimates, and unit-conversion functions
  # VALIDATION: are scattered as standalone helpers rather than being methods
  # VALIDATION: on the Distance type.
