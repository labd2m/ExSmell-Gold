...
| **Expected smell location** | `Scheduling.AppointmentManager` module — time slot representation in all functions |
| **Affected functions** | `book_appointment/3`, `check_availability/3`, `cancel_appointment/2`, `reschedule/3` |
| **Short explanation** | Time slots are represented as pairs of plain `integer` values for hour and minute (e.g., `start_hour = 9`, `start_minute = 30`) instead of a dedicated `%TimeSlot{start: Time.t(), end: Time.t(), timezone: String.t()}` struct. This splits an inherently cohesive concept across multiple arguments, makes timezone handling implicit, and forces callers to reassemble the slot semantics every time. |
...
defmodule Scheduling.AppointmentManager do
  ...
  # VALIDATION: This is a smell because time slots are expressed as raw integer
  # pairs (start_hour, start_minute) and (end_hour, end_minute) rather than a
  # proper %TimeSlot{start: ~T[...], end: ~T[...], timezone: String.t()} struct.
  # This scatters time arithmetic across every function, loses timezone info,
  # and forces callers to manage four primitives instead of one cohesive value.
