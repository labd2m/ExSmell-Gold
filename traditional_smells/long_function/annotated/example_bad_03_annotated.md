...
- **Expected smell location:** `Logistics.ShipmentHandler.process/1`
- **Affected function(s):** `process/1`
- **Short explanation:** The `process/1` function conflates shipment validation, carrier selection logic, label generation, warehouse task creation, status transitions, and event broadcasting into one unbroken body. Each of those phases is a distinct responsibility and should live in its own helper function.
...
defmodule Logistics.ShipmentHandler do
  ...
  # VALIDATION: This is a smell because `process/1` mixes shipment validation,
  # VALIDATION: carrier selection, weight checking, label creation, warehouse task
  # VALIDATION: scheduling, status update, and event publishing all in one function,
  # VALIDATION: violating single-responsibility and making testing very hard.
