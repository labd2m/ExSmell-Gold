...
| **Location**       | `Reporting.SalesReportGenerator.generate/2`                                        |
| **Affected fn(s)** | `generate/2`                                                                       |
| **Explanation**    | `generate/2` fuses data fetching, period parsing, per-rep aggregation, regional rollup, top-performer ranking, churn rate computation, CSV/PDF rendering and file persistence into one monolithic function body of roughly 100 lines. Every stage is a separable concern with its own data shape and logic. The liberal use of inline comments that name each "section" is a direct symptom that the author recognised multiple responsibilities but chose not to extract them — the defining marker of the Long Function smell. |
...
defmodule Reporting.SalesReportGenerator do
  ...
  # VALIDATION: This is a smell because `generate/2` mixes date range parsing,
  # raw data loading, per-rep aggregation, regional rollup, ranking, churn
  # calculation, CSV/PDF rendering, file storage, and metadata persistence
  # all in one flat function body of ~100 lines.  Each of these concerns should
  # be a focused private helper.  The inline section comments highlight how many
  # unrelated responsibilities the author crammed into a single function.
