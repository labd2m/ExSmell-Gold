...
- **Expected smell location:** `Inventory.StockManager.receive_purchase_order/2`
- **Affected function(s):** `receive_purchase_order/2`
- **Short explanation:** The `receive_purchase_order/2` function handles PO validation, line-item reconciliation, stock level updates, low-stock threshold checks, supplier acknowledgement recording, and event emission in one monolithic block. Each phase is a cohesive sub-task that warrants its own helper function.
...
defmodule Inventory.StockManager do
  ...
  # VALIDATION: This is a smell because `receive_purchase_order/2` performs PO status
  # VALIDATION: validation, per-line item stock upsert, low-stock alerting, supplier
  # VALIDATION: acknowledgement, PO closure, and event publishing all in one body —
  # VALIDATION: far exceeding a single responsibility and making the function very long.
