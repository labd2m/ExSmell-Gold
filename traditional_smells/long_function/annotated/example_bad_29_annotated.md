...
- **Expected smell location:** `Notifications.Dispatcher.send_campaign/2`
- **Affected function(s):** `send_campaign/2`
- **Short explanation:** `send_campaign/2` conflates audience filtering, template rendering, channel routing (email vs SMS vs push), delivery scheduling, unsubscribe enforcement, and metrics initialisation into one sprawling function, making it hard to test individual concerns in isolation.
...
defmodule Notifications.Dispatcher do
  ...
  # VALIDATION: This is a smell because `send_campaign/2` handles audience
  # selection, unsubscribe filtering, per-recipient template personalisation,
  # multi-channel routing, send-window enforcement, delivery logging, and
  # metrics initialisation — all in one function body that is over 90 lines
  # long and has no focused, single-purpose helpers for any of its steps.
