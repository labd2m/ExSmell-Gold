...
- **Expected smell location:** `Finance.ExpenseReportProcessor.submit/2`
- **Affected function(s):** `submit/2`
- **Short explanation:** `submit/2` handles spending-policy validation, receipt verification, currency conversion, approver routing, ledger-entry creation, ERP sync, reimbursement scheduling, and submitter notification without delegating any step to a dedicated helper function.
...
defmodule Finance.ExpenseReportProcessor do
  ...
  # VALIDATION: This is a smell because `submit/2` runs through policy
  # validation, receipt-attachment checks, currency conversion, approver
  # routing, ledger creation, ERP synchronisation, reimbursement
  # scheduling, and employee notification — eight separate concerns
  # stitched together into one function body exceeding 110 lines.
