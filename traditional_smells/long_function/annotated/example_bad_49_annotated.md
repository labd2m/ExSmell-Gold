...
| **Location**       | `Logistics.ShipmentDispatcher.dispatch_shipment/2`                                 |
| **Affected fn(s)** | `dispatch_shipment/2`                                                              |
| **Explanation**    | `dispatch_shipment/2` is a low-cohesion function that inlines inventory reservation, carrier selection, rate calculation, label generation, shipment record creation, warehouse task assignment, and customer notification into one linear body exceeding 90 lines. Each responsibility is a distinct, separable concern. The need for section-dividing comments inside the function body is itself a signal that the code should be split into smaller, named helpers. |
...
defmodule Logistics.ShipmentDispatcher do
  ...
  # VALIDATION: This is a smell because `dispatch_shipment/2` handles inventory
  # reservation, carrier selection, rate comparison, label generation, shipment
  # persistence, warehouse task creation, and customer notification all inline
  # without delegation to focused helpers.  The function body spans roughly
  # 95 lines and uses section comments to guide the reader through unrelated stages,
  # which is a canonical Long Function symptom.
