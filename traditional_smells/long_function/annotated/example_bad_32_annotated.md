...
- **Expected smell location:** `Payments.GatewayService.charge/2`
- **Affected function(s):** `charge/2`
- **Short explanation:** `charge/2` crams idempotency-key resolution, card validation, fraud-scoring, gateway selection, charge attempt, retry logic, receipt generation, and webhook dispatch into a single overgrown function body instead of delegating each concern to focused helpers.
...
defmodule Payments.GatewayService do
  ...
  # VALIDATION: This is a smell because `charge/2` handles idempotency
  # resolution, card/amount validation, fraud evaluation, gateway routing,
  # retry loop, receipt building, and async webhook firing — all within one
  # function of over 110 lines, with each block representing a discrete
  # concern that should be its own private function.
