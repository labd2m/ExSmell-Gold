...
- **Expected smell location:** `Accounts.RegistrationService.register/1`
- **Affected function(s):** `register/1`
- **Short explanation:** `register/1` merges input sanitisation, uniqueness checks, password hashing, plan assignment, profile initialisation, verification-email dispatch, onboarding-task creation, and welcome-notification firing into one unwieldy function instead of composing smaller focused helpers.
...
defmodule Accounts.RegistrationService do
  ...
  # VALIDATION: This is a smell because `register/1` performs input
  # normalisation, duplicate-email check, password-strength enforcement,
  # password hashing, user persistence, plan assignment, profile creation,
  # email-verification token generation and dispatch, onboarding-task
  # seeding, and welcome push-notification — all in one function body
  # that stretches well past 100 lines with no helper functions.
