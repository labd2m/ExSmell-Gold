...
- **Expected smell location:** `Reporting.CustomerLifetimeValue.compute/2`
- **Affected function(s):** `compute/2`
- **Short explanation:** The `compute/2` function fetches order history, calculates purchase frequency and average order value, applies a churn-probability model, computes a discounted future revenue stream, segments the result into LTV tiers, persists the record, and queues a CRM sync — all sequentially inside one function with no extraction. Each phase is a distinct analytical step that deserves its own helper.
...
defmodule Reporting.CustomerLifetimeValue do
  ...
  # VALIDATION: This is a smell because `compute/2` mixes order history fetching,
  # VALIDATION: frequency and AOV calculation, churn probability modelling, DCF
  # VALIDATION: projection, tier classification, CLV record persistence, and CRM
  # VALIDATION: sync queuing all in one function body far exceeding ten lines.
