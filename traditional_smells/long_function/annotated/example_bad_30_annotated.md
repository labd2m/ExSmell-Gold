...
- **Expected smell location:** `Reporting.SalesReport.generate/2`
- **Affected function(s):** `generate/2`
- **Short explanation:** `generate/2` lumps together date-range validation, data fetching, KPI computation (revenue, refunds, AOV, growth), grouping by region and product, formatting rows, CSV building, and storage upload into a single function body that sprawls across many distinct concerns.
...
defmodule Reporting.SalesReport do
  ...
  # VALIDATION: This is a smell because `generate/2` carries out at least
  # eight conceptually separate tasks — parameter validation, raw data
  # retrieval, refund cross-referencing, KPI calculation, region/product
  # grouping, CSV row serialisation, file assembly, and S3 upload — all
  # inside one function without extracting any single-purpose helpers,
  # resulting in a function body well over 100 lines.
