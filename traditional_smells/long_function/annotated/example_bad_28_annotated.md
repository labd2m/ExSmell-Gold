...
- **Expected smell location:** `Logistics.ShipmentService.dispatch/2`
- **Affected function(s):** `dispatch/2`
- **Short explanation:** `dispatch/2` mixes address validation, carrier selection, weight/dimension checks, label generation, inventory deduction, tracking-event creation, and customer notification into one long sequential body, violating the single-responsibility principle and making the function difficult to test or maintain.
...
defmodule Logistics.ShipmentService do
  ...
  # VALIDATION: This is a smell because `dispatch/2` sequentially performs
  # address validation, dimensional checks, carrier negotiation, label
  # generation, stock deduction, tracking-record creation, and e-mail
  # dispatch — each a distinct responsibility that should live in its own
  # dedicated function rather than being concatenated into one >90-line body.
