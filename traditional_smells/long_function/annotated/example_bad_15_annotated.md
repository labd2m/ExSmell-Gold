...
- **Expected smell location:** `Inventory.ReorderAgent.evaluate_and_reorder/1`
- **Affected function(s):** `evaluate_and_reorder/1`
- **Short explanation:** The `evaluate_and_reorder/1` function loads all SKUs, filters those below reorder point, checks for open POs to avoid duplicates, calculates reorder quantities using lead-time demand, builds PO line items, creates the PO, notifies the purchasing team, and logs the activity all inside one function body that is clearly too long.
...
defmodule Inventory.ReorderAgent do
  ...
  # VALIDATION: This is a smell because `evaluate_and_reorder/1` loads stock data,
  # VALIDATION: identifies under-threshold SKUs, checks open POs, computes reorder
  # VALIDATION: quantities, builds PO structures, persists them, and dispatches
  # VALIDATION: notifications — all inside one single function with no delegation.
