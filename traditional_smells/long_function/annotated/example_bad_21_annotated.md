...
- **Expected smell location:** `Payments.PayoutDisbursement.disburse/2`
- **Affected function(s):** `disburse/2`
- **Short explanation:** The `disburse/2` function performs seller eligibility validation, pending earnings aggregation, minimum payout threshold checking, bank account verification, gateway transfer initiation, payout record creation, earnings ledger reconciliation, and seller notification all in a single body. Each of those steps is a meaningful, independently testable concern that should be extracted into its own helper.
...
defmodule Payments.PayoutDisbursement do
  ...
  # VALIDATION: This is a smell because `disburse/2` performs seller status checks,
  # VALIDATION: earnings aggregation, threshold enforcement, bank account validation,
  # VALIDATION: gateway transfer, payout record insertion, ledger reconciliation,
  # VALIDATION: and email notification all without delegating to any helper function.
