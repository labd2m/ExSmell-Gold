...
- **Expected smell location:** `Logistics.FulfillmentCoordinator.fulfill/1`
- **Affected function(s):** `fulfill/1`
- **Short explanation:** The `fulfill/1` function performs order-status validation, multi-warehouse stock allocation across order items, pick-list generation, shipment record bootstrapping, order status transition, fulfillment event publishing, and customer shipping notification all in a single function. The accumulated steps produce a function that is clearly too long and handles too many responsibilities.
...
defmodule Logistics.FulfillmentCoordinator do
  ...
  # VALIDATION: This is a smell because `fulfill/1` handles order status validation,
  # VALIDATION: per-item warehouse stock allocation, allocation failure handling,
  # VALIDATION: pick-list and pick-item persistence, shipment bootstrapping,
  # VALIDATION: order status update, event publishing, and customer notification
  # VALIDATION: all inside one function body that far exceeds a reasonable length.
