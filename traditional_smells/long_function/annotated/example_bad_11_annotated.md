...
- **Expected smell location:** `Logistics.RouteOptimizer.build_daily_route/2`
- **Affected function(s):** `build_daily_route/2`
- **Short explanation:** The `build_daily_route/2` function loads pending deliveries, clusters them by postal zone, applies vehicle capacity constraints, performs a simple greedy ordering, assigns drivers, persists the route plan, and notifies drivers — all without delegating any step to a separate function. The result is a function with too many lines and too many distinct concerns.
...
defmodule Logistics.RouteOptimizer do
  ...
  # VALIDATION: This is a smell because `build_daily_route/2` performs delivery loading,
  # VALIDATION: zone clustering, capacity filtering, greedy stop ordering, driver
  # VALIDATION: assignment, route persistence, and driver notification inside a
  # VALIDATION: single function that is far too long and covers too many responsibilities.
