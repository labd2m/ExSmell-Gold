...
- **Expected smell location:** `Inventory.CycleCountReconciler.reconcile/2`
- **Affected function(s):** `reconcile/2`
- **Short explanation:** The `reconcile/2` function loads expected stock, computes variances, classifies discrepancy severity, creates adjustment records for each SKU, triggers investigation tasks for large variances, updates on-hand quantities, closes the cycle count, and notifies the warehouse manager — all inline. Each phase is a separate concern that inflates the function well beyond a reasonable length.
...
defmodule Inventory.CycleCountReconciler do
  ...
  # VALIDATION: This is a smell because `reconcile/2` mixes cycle count validation,
  # VALIDATION: expected-vs-counted variance computation, severity classification,
  # VALIDATION: adjustment record creation, investigation task spawning, on-hand
  # VALIDATION: stock updates, session closure, and manager notification — all
  # VALIDATION: packed into one function with no helper extraction.
