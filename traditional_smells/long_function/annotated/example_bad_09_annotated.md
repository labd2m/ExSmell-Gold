...
- **Expected smell location:** `UserManagement.Registrar.register/1`
- **Affected function(s):** `register/1`
- **Short explanation:** The `register/1` function performs email deduplication, password strength enforcement, user record insertion, verification token generation, welcome email delivery, default preference seeding, and audit trail creation all in one place. Each is a distinct concern that should be separated into focused helpers.
...
defmodule UserManagement.Registrar do
  ...
  # VALIDATION: This is a smell because `register/1` handles input normalization,
  # VALIDATION: duplicate email check, password policy enforcement, user insertion,
  # VALIDATION: token generation and persistence, welcome email, default preferences
  # VALIDATION: seeding, and audit logging all inside one very long function body.
