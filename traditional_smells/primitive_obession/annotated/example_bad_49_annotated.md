...
- **Expected smell location:** `Scheduling.AppointmentBooker` module — a time slot is represented as two raw `integer` primitives (`start_hour` and `duration_minutes`) plus a raw `string` `day_of_week` in `book_appointment/5`, `is_slot_available?/4`, and `list_available_slots/3` instead of a `TimeSlot` struct
- **Affected functions:** `book_appointment/5`, `is_slot_available?/4`, `list_available_slots/3`, `validate_slot/3`
- **Short explanation:** A bookable time slot is a domain concept with clear constraints (valid hours, valid duration, valid day) that belong together. Instead of a `TimeSlot` struct, the concept is dissolved into three primitives scattered across every function signature. This duplicates validation logic, inflates arities, and makes it trivial to swap `start_hour` and `duration_minutes` without any warning.
...
defmodule Scheduling.AppointmentBooker do
  ...
  # VALIDATION: This is a smell because a bookable time slot — start hour,
  # duration in minutes, and day of week — is a single cohesive domain concept.
  # Instead of being encapsulated in a `TimeSlot` struct (which would carry its
  # own validation and end-time computation), it is broken into three raw
  # primitives: `start_hour` (integer), `duration_minutes` (integer), and
  # `day_of_week` (string). Every function that touches a time slot must accept
  # three extra parameters, validation is duplicated across `validate_slot/3`
  # and `is_slot_available?/4`, and there is nothing preventing a caller from
  # passing duration where a start hour is expected.
