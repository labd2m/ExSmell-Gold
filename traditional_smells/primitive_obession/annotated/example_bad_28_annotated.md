...
| **Expected smell location** | `Inventory.StockManager` module — SKU handling throughout |
| **Affected functions** | `reserve_stock/3`, `replenish/3`, `transfer_between_warehouses/4`, `lookup_product/1` |
| **Short explanation** | Product SKUs are passed as plain `String.t()` values (e.g., `"ELEC-TV-42-BLK-US"`) that encode category, subcategory, size, color, and region inside a hyphen-delimited string. Rather than a `%SKU{category: atom(), product_code: String.t(), variant: map()}` struct, all domain attributes must be extracted by splitting the string everywhere they're needed. |
...
defmodule Inventory.StockManager do
  ...
  # VALIDATION: This is a smell because the SKU is a plain string like
  # "ELEC-TV-42-BLK-US" that encodes multiple domain attributes (category,
  # product code, variant size, colour, region) inside a delimiter-separated
  # format. Every function that needs these attributes must parse the string,
  # which should instead be a %SKU{} struct with named, typed fields.
