...
- **Expected smell location:** `Logistics.RouteOptimizer` module — `latitude` and `longitude` raw `float` pairs used in `calculate_distance/4`, `find_nearest_depot/3`, and `build_waypoint/3` instead of a `Coordinate` or `GeoPoint` struct
- **Affected functions:** `calculate_distance/4`, `find_nearest_depot/3`, `build_waypoint/3`, `estimate_route/2`
- **Short explanation:** Geographic coordinates (latitude + longitude) are a cohesive domain concept, yet they are consistently passed as two separate `float` primitives throughout the module. A `Coordinate` struct would allow validation of valid ranges (lat −90..90, lon −180..180), enable cleaner function signatures, and make the intent of each argument self-documenting.
...
defmodule Logistics.RouteOptimizer do
  ...
  # VALIDATION: This is a smell because a geographic coordinate is a cohesive
  # domain value (latitude + longitude together define one point), yet it is
  # broken into two separate `float` parameters (`lat1`, `lon1`, `lat2`, `lon2`)
  # in every function. A dedicated `Coordinate` struct (or `%{lat: float, lon: float}`)
  # would make the intent clear, prevent argument-order mistakes, and centralise
  # range validation. Callers can easily swap lat/lon and the compiler gives no warning.
