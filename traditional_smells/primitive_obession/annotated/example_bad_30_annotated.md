...
| **Expected smell location** | `Delivery.RouteOptimizer` module — coordinate parameters throughout |
| **Affected functions** | `nearest_depot/2`, `build_route/2`, `within_delivery_zone/2`, `estimate_travel_time/3` |
| **Short explanation** | Geographic coordinates are passed as separate `float` values for latitude and longitude rather than a `%Coordinates{lat: float(), lng: float()}` or `%GeoPoint{latitude: float(), longitude: float(), label: String.t()}` struct. This means callers must always manage two tightly coupled values as if they were independent, and every function re-implements the same lat/lng destructuring pattern. |
...
defmodule Delivery.RouteOptimizer do
  ...
  # VALIDATION: This is a smell because geographic positions are represented as
  # separate `lat` and `lng` float parameters instead of a dedicated struct like
  # %GeoPoint{lat: float(), lng: float()}. Every function accepts and passes
  # two loosely paired floats, making it trivial to swap their order by mistake,
  # and all haversine calculations repeat the same destructuring boilerplate.
