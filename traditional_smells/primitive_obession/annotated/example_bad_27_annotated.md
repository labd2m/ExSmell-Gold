...
| **Expected smell location** | `Notifications.EmailDispatcher` module — email address parameters |
| **Affected functions** | `send_transactional/3`, `send_bulk/2`, `add_to_suppression_list/1`, `validate_recipient/1` |
| **Short explanation** | Email addresses are passed and stored as plain `String.t()` values rather than a `%EmailAddress{local: String.t(), domain: String.t(), display_name: String.t() | nil}` struct. This forces repeated parsing of local parts and domains, makes display-name handling ad-hoc, and spreads RFC-5321 validation logic across multiple unrelated functions. |
...
defmodule Notifications.EmailDispatcher do
  ...
  # VALIDATION: This is a smell because email addresses are plain strings
  # like "alice@example.com" or "Alice Smith <alice@example.com>" instead of
  # a structured %EmailAddress{local: "alice", domain: "example.com",
  # display_name: "Alice Smith"} struct. Parsing the display name, domain,
  # and local part is repeated inline across every function that needs them.
