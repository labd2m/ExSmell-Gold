...
- **Expected Smell Location**: `validate_shipping_address/5`, `format_address_label/5`, `geocode_address/5`, `update_user_address/6`
- **Affected Function(s)**: All public functions in `UserManagement.AddressService`
- **Explanation**: Street, city, state, postal code, and country are carried as five separate `String.t()` primitives rather than a single `%Address{}` struct. This forces every caller to manage and pass five related values in tandem, duplicates validation logic, and makes it impossible to leverage pattern-matching or protocol dispatch on the concept of an address.
...
defmodule UserManagement.AddressService do
  ...
  # VALIDATION: This is a smell because the concept of an "address" is represented
  # VALIDATION: by five separate `String.t()` primitives (street, city, state,
  # VALIDATION: postal_code, country) instead of a single `%Address{}` struct.
  # VALIDATION: All functions in this module are forced to accept and thread these
  # VALIDATION: five values individually, making the interface fragile and verbose.
