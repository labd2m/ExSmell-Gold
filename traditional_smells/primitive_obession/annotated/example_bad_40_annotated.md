...
| **Expected smell location** | `Fleet.FuelManager` module — fuel quantity and capacity representation |
| **Affected functions** | `record_refuel/4`, `compute_fuel_efficiency/3`, `check_fuel_level/2`, `schedule_refuel_alert/2` |
| **Short explanation** | Fuel quantities and vehicle tank capacities are passed as plain `float` values with an implicit unit (always litres) rather than a `%FuelQuantity{value: float(), unit: :litre | :gallon}` struct. This means any caller working in gallons must convert externally before calling, there is no compile-time guarantee about units, and functions that log or format the quantity must repeat the unit assumption. |
...
defmodule Fleet.FuelManager do
  ...
  # VALIDATION: This is a smell because fuel quantities (amount added, current
  # level, tank capacity) are all plain `float` values with an implicit
  # litre unit instead of a dedicated %FuelQuantity{value: float(), unit: :litre}
  # struct. Functions assume litres without enforcement; mixing in a gallon
  # value would produce wrong results silently. The struct would make the
  # unit explicit and centralise conversion logic.
