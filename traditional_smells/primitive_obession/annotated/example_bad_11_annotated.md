...
- **Expected Smell Location**: `volumetric_weight/3`, `fits_in_box?/6`, `classify_parcel/3`, `dimensional_charge/4`
- **Affected Function(s)**: All public functions in `Logistics.ParcelClassifier`
- **Explanation**: A package's three-dimensional size is represented as three separate `float()` parameters (`length_cm`, `width_cm`, `height_cm`) instead of a `%Dimensions{length: float(), width: float(), height: float(), unit: atom()}` struct. Every function must accept and forward all three values, the dimension ordering is only enforced by convention, and there is no single place to attach unit-conversion or validation behaviour.
...
defmodule Logistics.ParcelClassifier do
  ...
  # VALIDATION: This is a smell because a package's 3D dimensions are modelled
  # VALIDATION: as three separate `float()` values — `length_cm`, `width_cm`,
  # VALIDATION: `height_cm` — instead of a `%Dimensions{}` struct. Every function
  # VALIDATION: in the module must accept these three values individually, the
  # VALIDATION: caller-side ordering is purely by convention, and unit handling
  # VALIDATION: (always cm here) is implicit rather than enforced by the type.
