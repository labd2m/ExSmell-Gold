...
| **Expected smell location** | `Accounts.UserProfile` module — phone number handling throughout |
| **Affected functions** | `update_phone/2`, `send_verification_sms/1`, `format_phone_display/1`, `normalize_phone/1` |
| **Short explanation** | Phone numbers are stored and passed as raw `String.t()` values rather than a dedicated `%PhoneNumber{country_code: String.t(), national_number: String.t(), e164: String.t()}` struct. This forces parsing and formatting logic to be duplicated across functions, makes country-code extraction fragile, and provides no domain model for what constitutes a valid phone number. |
...
defmodule Accounts.UserProfile do
  ...
  # VALIDATION: This is a smell because phone numbers are represented as plain
  # strings throughout (e.g., "+5531987654321"). All domain knowledge about a
  # phone number — its country code, national number, formatting rules, and
  # E.164 validity — is scattered across multiple helper functions operating
  # on raw strings. A %PhoneNumber{} struct would centralize this behaviour.
