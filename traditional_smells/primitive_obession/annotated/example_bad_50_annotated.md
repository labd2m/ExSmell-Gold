...
- **Expected smell location:** `HR.CompensationCalculator` module — `base_salary`, `bonus_percent`, `currency`, and `pay_period` are raw primitives (`float`, `float`, `string`, `string`) used in `calculate_net_pay/5`, `apply_deductions/3`, and `generate_payslip/4` instead of `Salary` and `CompensationPeriod` structs
- **Affected functions:** `calculate_net_pay/5`, `apply_deductions/3`, `generate_payslip/4`, `annualise/2`
- **Short explanation:** An employee's compensation package is a rich domain concept — the base salary amount, its currency, and the pay period (monthly/biweekly/weekly) are all tightly related and must be validated together. Representing them as three independent primitives (`float`, `string`, `string`) forces validation to be spread across the module and makes the function signatures verbose and error-prone.
...
defmodule HR.CompensationCalculator do
  ...
  # VALIDATION: This is a smell because an employee's salary is a multi-dimensional
  # domain concept. The annual gross amount, its currency, and the pay period are
  # all related fields that are validated and used together — yet they are split
  # into three separate primitives: `annual_gross` (float), `currency` (string),
  # and `pay_period` (string). A `Salary` struct grouping these three would prevent
  # callers from constructing half-valid combinations (e.g., a negative salary in
  # an unsupported currency with an invalid pay period), and would reduce function
  # arities significantly across the module.
