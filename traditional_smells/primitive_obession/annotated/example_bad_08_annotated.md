...
- **Expected Smell Location**: `calculate_shipping_cost/4`, `convert_weight/3`, `total_shipment_weight/1`, `exceeds_limit?/3`
- **Affected Function(s)**: All public functions in `Inventory.WeightCalculator`
- **Explanation**: A physical weight measurement is represented as two separate primitives (`value :: float()` and `unit :: String.t()`) instead of a `%Weight{value: float(), unit: atom()}` struct. This forces every function to accept and validate the unit string manually, spreads unit-conversion logic across the module, and allows silent bugs when callers pass incompatible units into arithmetic operations.
...
defmodule Inventory.WeightCalculator do
  ...
  # VALIDATION: This is a smell because a weight measurement is decomposed into
  # VALIDATION: two raw primitives — `value :: float()` and `unit :: String.t()` —
  # VALIDATION: instead of a single `%Weight{value: float(), unit: atom()}` struct.
  # VALIDATION: All functions accept the pair individually, duplicate unit
  # VALIDATION: validation, and risk arithmetic errors when units are mixed
  # VALIDATION: without explicit conversion.
