...
- **Expected smell location:** `serialize_rows/2` in `Reporting.ExportBuilder`
- **Affected function(s):** `serialize_rows/2`
- **Short explanation:** `serialize_rows/2` receives an `export_format` atom and routes through a `case` expression, but the only clause is a wildcard. The intent was to support multiple output formats (CSV, JSON, XLSX) by dispatching different serialisers per format. Only CSV serialisation was ever implemented, and the routing structure was never filled in, leaving a `case` that always falls through to a single outcome.
...
defmodule Reporting.ExportBuilder do
  ...
  # VALIDATION: This is a smell because `export_format` is passed into a `case` 
  # expression intended to dispatch different serialisers per format, but only a 
  # wildcard clause exists. The developer planned to add :json and :xlsx branches 
  # but never did. Every export currently produces CSV output regardless of the 
  # `format` value, making the case and the format parameter speculative dead code.
