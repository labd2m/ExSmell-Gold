...
- **Expected Smell Location**: `Scheduling.WorkflowOrchestrator` (module definition)
- **Affected Function(s)**: entire `Scheduling.WorkflowOrchestrator` module
- **Explanation**: `Scheduling.WorkflowOrchestrator` was built speculatively to manage
  multi-step workflows where tasks depend on each other. `Scheduling.TaskManager` —
  the operational module — schedules, runs, and tracks independent tasks without any
  dependency chaining. It never calls `WorkflowOrchestrator`. The module exists as
  unused speculative infrastructure, complicating the codebase with no benefit.
...
# VALIDATION: This is a smell because `Scheduling.WorkflowOrchestrator` was
# implemented speculatively to handle multi-step workflows with dependency
# resolution between tasks. `Scheduling.TaskManager` only manages independent
# tasks and never references this module. The entire module is dead code that
# adds complexity with no practical value.
