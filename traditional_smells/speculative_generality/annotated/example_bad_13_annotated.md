...
- **Expected Smell Location**: `Notifications.AlertManager.send_alert/3`
- **Affected Function(s)**: `send_alert/3`
- **Explanation**: The `severity` parameter is defined with a default of `:info` to
  support tiered alert routing (e.g. `:warning`, `:critical`, `:emergency`) in the
  future. Every call site within this module — `notify_ops_team/2`,
  `broadcast_system_event/2`, `alert_on_threshold/3` — always calls `send_alert/2`
  without supplying a severity level. The parameter is speculative infrastructure
  that has never been exercised with any value other than its default.
...
defmodule Notifications.AlertManager do
  ...
  # VALIDATION: This is a smell because `severity \\ :info` was added to enable
  # routing rules based on alert severity (e.g. send :critical alerts to PagerDuty).
  # Every call site in this module—notify_ops_team/2, broadcast_system_event/2, and
  # alert_on_threshold/3—always calls send_alert/2 without providing a severity.
  # The parameter is speculative and only ever takes its default value of :info.
