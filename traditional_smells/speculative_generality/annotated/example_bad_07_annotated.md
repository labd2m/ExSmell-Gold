...
- **Expected Smell Location**: `Payments.ChargeExecutor.execute_charge/1`
- **Affected Function(s)**: `execute_charge/1`
- **Explanation**: The function pattern-matches on `payment.type` in a `case` expression
  to allow for payment-method-specific charge execution logic (e.g. different gateway
  calls for `:credit_card`, `:debit_card`, and `:bank_transfer`). Every branch,
  including the catch-all, delegates to the same `Gateway.charge/2` call with the same
  arguments. The branching structure is pure speculation — no method-specific behaviour
...
defmodule Payments.ChargeExecutor do
  ...
  def execute_charge(%Charge{} = charge) do
    ...
    # VALIDATION: This is a smell because `payment.type` is extracted and matched to
    # allow different gateway calls per payment method. Every branch—:credit_card,
    # :debit_card, :bank_transfer, and the catch-all—calls Gateway.charge/2 with
    # identical arguments. The differentiated structure is speculative and has never
    # been filled with method-specific logic.
    result =
