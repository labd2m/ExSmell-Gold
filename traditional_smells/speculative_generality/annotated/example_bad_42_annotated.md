...
- **Expected smell location:** `schedule_digest/3` in `Notifications.EmailDispatcher`
- **Affected function(s):** `schedule_digest/3`
- **Short explanation:** `schedule_digest/3` was written speculatively to support batching notifications into periodic digest emails (daily or weekly summaries) rather than sending each one immediately. The function is fully implemented but is never called by `dispatch/2`, the notification worker, or any other module in the codebase. Every notification is dispatched immediately, and the digest pathway was never connected to any consumer.
...
defmodule Notifications.EmailDispatcher do
  ...
  # VALIDATION: This is a smell because `schedule_digest/3` is a fully implemented 
  # function that is never called anywhere in the module or broader codebase. It was 
  # written speculatively to support digest/batching of notifications. Since 
  # `dispatch/2` never delegates to it and no worker or scheduler references it, 
  # the function — along with its `DigestStore` alias — is entirely dead 
  # speculative code.
