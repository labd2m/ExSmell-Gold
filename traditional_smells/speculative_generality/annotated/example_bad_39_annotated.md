...
- **Expected smell location:** `schedule/3` in `Payments.ChargeScheduler`
- **Affected function(s):** `schedule/3`
- **Short explanation:** The `schedule/3` function accepts a `dry_run` parameter with a default of `false`. It was added speculatively to allow callers to simulate charge scheduling without persisting anything — useful for pre-flight validation in a future admin dashboard. In practice every call site uses `schedule/2`, never passing a third argument. No caller has ever exercised the dry-run path, making the parameter and its conditional persistence branch dead speculative flexibility.
...
defmodule Payments.ChargeScheduler do
  ...
  # VALIDATION: This is a smell because the `dry_run` parameter with default `false` 
  # was added speculatively to support a preview mode where charges are validated 
  # but never persisted. Every call site in the codebase calls `schedule/2`, never 
  # passing a third argument. The `if dry_run` branch has never been executed in 
  # production, making the parameter and the branch dead speculative flexibility.
