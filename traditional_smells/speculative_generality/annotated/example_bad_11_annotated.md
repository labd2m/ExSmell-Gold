...
- **Expected Smell Location**: `Logistics.RouteOptimizer.optimize/3`
- **Affected Function(s)**: `optimize/3`
- **Explanation**: The `mode` parameter is defined with a default of `:fastest` to
  allow different optimization strategies (e.g. `:cheapest`, `:greenest`, `:balanced`)
  in the future. Every internal call site in this module — `plan_multi_stop/2`,
  `replan_on_delay/1` — always calls `optimize/2`, never supplying a `mode`.
  The parameter has been exercised only with its default value and exists solely
...
defmodule Logistics.RouteOptimizer do
  ...
  # VALIDATION: This is a smell because `mode \\ :fastest` was added to allow
  # alternative routing strategies such as :cheapest or :greenest. Every call site—
  # plan_multi_stop/2 and replan_on_delay/1—always omits the third argument,
  # so :fastest is the only mode ever applied. The parameter is speculative and
  # adds no real flexibility to the module.
