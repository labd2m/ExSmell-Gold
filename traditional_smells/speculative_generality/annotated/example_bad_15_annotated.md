...
- **Expected Smell Location**: `Inventory.PricingAdjuster.compute_adjusted_price/1`
- **Affected Function(s)**: `compute_adjusted_price/1`
- **Explanation**: `compute_adjusted_price/1` destructures `%Product{product_line: line}`
  from its argument to allow product-line-specific markup rules in the future. The
  `case` expression built around `line` contains only a catch-all clause (`_`) that
  applies a uniform 12 % markup to every product regardless of line. The extraction
  and `case` scaffolding are entirely speculative — the per-line differentiation
...
defmodule Inventory.PricingAdjuster do
  ...
  def compute_adjusted_price(%Product{product_line: product_line} = product) do
    ...
    # VALIDATION: This is a smell because `product_line` is extracted via pattern
    # matching to allow different markup rates per product line (e.g. :electronics,
    # :apparel, :perishables). The case has only a catch-all clause that applies the
    # same @default_markup for every product line. The per-line branching is
    # speculative and was never differentiated.
    markup =
