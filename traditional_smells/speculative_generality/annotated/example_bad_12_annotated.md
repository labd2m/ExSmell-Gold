...
- **Expected Smell Location**: `Auth.AccessPolicy.resolve_permissions/1`
- **Affected Function(s)**: `resolve_permissions/1`
- **Explanation**: `resolve_permissions/1` pattern-matches on `user.account_type` and
  dispatches into a `case` expression to allow different permission sets for each
  account tier (`:free`, `:basic`, `:pro`, `:enterprise`). Every branch returns the
  same static permission map, however, indicating the branching was added speculatively
  in anticipation of per-tier access control that was never differentiated.
...
defmodule Auth.AccessPolicy do
  ...
  def resolve_permissions(%User{account_type: account_type} = _user) do
    ...
    # VALIDATION: This is a smell because `account_type` is extracted to allow
    # different permission sets per account tier (:free, :basic, :pro, :enterprise).
    # Every branch returns the identical permission map, however. The per-tier
    # branching was added speculatively and was never filled with differentiated
    # permission sets.
