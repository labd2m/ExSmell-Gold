...
- **Expected smell location:** `suspend/3` in `UserManagement.AccountSuspender`
- **Affected function(s):** `suspend/3`
- **Short explanation:** The `suspend/3` function defines a `notify_user` parameter with a default of `true`. The intent was to allow callers to suppress the user-facing notification in certain scenarios (e.g., silent fraud-detection suspensions). In practice, every call site in the codebase omits the argument and relies on the default. No caller ever passes `false`, making the parameter dead speculative flexibility.
...
defmodule UserManagement.AccountSuspender do
  ...
  # VALIDATION: This is a smell because the `notify_user` parameter with default 
  # `true` was added speculatively to allow silent suspensions (e.g., during fraud 
  # investigations where alerting the user could compromise the investigation). 
  # However, every single call site in the codebase calls `suspend/2`, never 
  # passing a third argument. The parameter has never been exercised and represents 
  # dead speculative flexibility.
