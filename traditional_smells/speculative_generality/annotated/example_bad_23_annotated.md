...
- **Expected smell location:** `compute_processing_fee/1` in `Payments.FeeCalculator`
- **Affected function(s):** `compute_processing_fee/1`
- **Short explanation:** The function destructures `payment_method` and branches on it through a `case` expression, but every branch returns the same percentage (2.9%). The branching structure was written speculatively to accommodate per-method fee schedules in the future, but no differentiation was ever implemented. The code gives the illusion of complexity while applying a uniform fee.
...
defmodule Payments.FeeCalculator do
  ...
  # VALIDATION: This is a smell because `payment_method` is extracted from the 
  # transaction and matched in a `case` expression, but all three clauses 
  # (:credit_card, :debit_card, :bank_transfer) return exactly the same percentage 
  # (0.029). The branching was written speculatively to support different rates per 
  # method, but that differentiation was never introduced, making all branches 
  # functionally identical dead structure.
