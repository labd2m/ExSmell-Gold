...
- **Expected Smell Location**: `Reporting.SalesReport.build/1`
- **Affected Function(s)**: `build/1`
- **Explanation**: `build/1` extracts the `breakdown_by` field from its `params`
  argument via pattern matching, anticipating that the report would eventually support
  multiple breakdown dimensions (e.g. `:region`, `:product`, `:sales_rep`). In
  practice the `case` contains only a single catch-all branch that applies identical
  aggregation logic regardless of the value. The extraction and branching infrastructure
  are speculative and never used differentially.
...
defmodule Reporting.SalesReport do
  ...
  def build(%{breakdown_by: breakdown_by} = params) do
    ...
    # VALIDATION: This is a smell because `breakdown_by` is extracted from `params`
    # to allow different aggregation strategies (e.g. :region, :product, :sales_rep).
    # However, the case has only a catch-all clause that applies the same aggregation
    # for every value. The branching was added speculatively and never filled with
