...
- **Expected Smell Location**: `Payments.RefundEngine.initiate_refund/3`
- **Affected Function(s)**: `initiate_refund/3`
- **Explanation**: The `reason` parameter is defined with a default of
  `:customer_request` to allow different refund reasons (e.g. `:duplicate_charge`,
  `:fraudulent`, `:product_not_received`) to trigger different processing paths in the
  future. Every call site in this module — `process_full_refund/1`,
  `process_partial_refund/2`, and the dispute handler — always calls `initiate_refund/2`
  without providing a reason. The parameter is speculative and provides no real
  differentiation.
...
defmodule Payments.RefundEngine do
  ...
  # VALIDATION: This is a smell because `reason \\ :customer_request` was added to
  # support different processing paths per refund reason (e.g. auto-approval for
  # :duplicate_charge, additional review for :fraudulent). Every call site in this
  # module—process_full_refund/1, process_partial_refund/2, handle_dispute/2—always
  # omits the third argument. The parameter is speculative and has never been
  # exercised with any value other than its default.
