...
- **Expected Smell Location**: `UserManagement.ProfileManager.update_profile/3`
- **Affected Function(s)**: `update_profile/3`
- **Explanation**: The `audit_level` parameter is defined with a default of `:standard`
  in anticipation of multiple audit granularities (e.g. `:minimal`, `:detailed`,
  `:compliance`). In practice every call site — `update_contact_info/3`,
  `update_preferences/3`, and `update_avatar/2` — calls `update_profile/2` without
  supplying an audit level. The parameter is speculative and has never been exercised
  with any value other than its default.
...
defmodule UserManagement.ProfileManager do
  ...
  # VALIDATION: This is a smell because `audit_level \\ :standard` was added to
  # support configurable audit verbosity (e.g. :minimal logs only the field names,
  # :detailed logs old and new values, :compliance logs everything with a signature).
  # Every call site—update_contact_info/3, update_preferences/3, and
  # update_avatar/2—always omits the third argument. The parameter is speculative
  # and has never been used with any value other than :standard.
