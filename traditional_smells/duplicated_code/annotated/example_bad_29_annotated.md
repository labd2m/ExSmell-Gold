...
- **Expected smell location:** `UserPolicy.can_edit_user?/2` and `UserPolicy.can_delete_user?/2`
- **Affected functions:** `can_edit_user?/2`, `can_delete_user?/2`
- **Short explanation:** Both policy functions independently resolve an actor's effective permissions by combining their base role permissions with any organisation-scoped overrides. This permission-resolution pipeline is duplicated instead of shared.
...
defmodule UserPolicy do
  ...
  def can_edit_user?(%User{} = actor, %User{} = target) do
    ...
    # VALIDATION: This is a smell because the block that fetches the base role
    # permissions, merges org-scoped overrides, and evaluates the specific
    # permission flag is reproduced character-for-character in `can_delete_user?/2`.
    base_permissions =
  ...
  def can_delete_user?(%User{} = actor, %User{} = target) do
    # VALIDATION: SMELL START - Duplicated Code
    # VALIDATION: This is a smell because this permission-resolution block
    # duplicates the one in `can_edit_user?/2`. Any change to how overrides are
    # merged or roles are resolved must be made in both functions.
    base_permissions =
      case Role.fetch_permissions(actor.role) do
        {:ok, perms} -> perms
        _ -> %PermissionSet{}
