...
- **Expected smell location:** `AppointmentScheduler.book/2` and `AppointmentScheduler.reschedule/3`
- **Affected functions:** `book/2`, `reschedule/3`
- **Short explanation:** Both functions repeat the logic that checks whether the requested time slot falls within business hours, is not on a blocked date, and does not conflict with existing bookings. This validation pipeline is duplicated rather than extracted.
...
defmodule AppointmentScheduler do
  ...
  def book(provider_id, booking_params) do
    with {:ok, provider} <- Provider.fetch(provider_id),
         ...
         # VALIDATION: This is a smell because the business-hours check, blocked-date
         # check, and conflict check below are reproduced verbatim in `reschedule/3`.
  ...
  def reschedule(appointment_id, provider_id, new_params) do
    with {:ok, appointment} <- Appointment.fetch(appointment_id),
         ...
         # VALIDATION: SMELL START - Duplicated Code
         # VALIDATION: This is a smell because the same three availability checks
         # (business hours, blocked dates, conflicts) are duplicated from `book/2`.
         # Adding a new constraint (e.g., max bookings per day) requires updating
         # both functions.
