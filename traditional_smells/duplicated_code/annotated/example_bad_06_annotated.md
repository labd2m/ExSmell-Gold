...
| **Expected smell location** | `Scheduling.AppointmentManager.book/3` and `Scheduling.AppointmentManager.reschedule/3` |
| **Affected functions** | `book/3`, `reschedule/3` |
| **Short explanation** | Both functions duplicate the slot availability check: querying existing appointments for the provider within the time window and verifying no overlaps exist. Any change to the overlap detection algorithm would need to be applied in both places. |
...
defmodule Scheduling.AppointmentManager do
  ...
  def book(%Patient{} = patient, %Provider{} = provider, start_time) do
    ...
    # VALIDATION: This is a smell because the overlap check — querying
    # existing appointments and filtering by time overlap — is repeated
    # identically in reschedule/3. Changes to conflict detection (e.g.,
    # adding buffer time) must be applied in both functions.
  ...
  def reschedule(%Appointment{} = appt, %Provider{} = provider, new_start_time) do
    ...
    # VALIDATION: SMELL START - Duplicated Code
    # VALIDATION: This is a smell because the overlap check is a copy of
    # the same logic from book/3, excluding only the appointment being rescheduled.
    existing =
