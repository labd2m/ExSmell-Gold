...
- **Expected smell location:** `ContactRepository.search/1` and `ContactRepository.export/1`
- **Affected functions:** `search/1`, `export/1`
- **Short explanation:** Both functions independently build the same dynamic filter list from the query params (name substring, email domain, tags, date range), then pass the filters to different downstream calls. The filter-building logic is duplicated rather than shared.
...
defmodule ContactRepository do
  ...
  def search(params) do
    ...
    # VALIDATION: This is a smell because the filter accumulation logic below
    # (name, email_domain, tags, created_after, created_before) is reproduced
    # identically at the start of `export/1`. Changes to filter semantics must
  ...
  def export(params) do
    ...
    # VALIDATION: SMELL START - Duplicated Code
    # VALIDATION: This is a smell because this filter-building pipeline is a
    # duplicate of the one in `search/1`. Any new filter field must be added to
    # both functions to remain consistent.
    filters =
      []
      |> maybe_add_filter(:name_contains, Map.get(params, :name))
      |> maybe_add_filter(:email_domain, Map.get(params, :email_domain))
