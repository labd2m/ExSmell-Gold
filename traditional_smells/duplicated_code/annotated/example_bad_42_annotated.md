...
| **Expected smell location** | `UserManagement.InviteService.send_invite/2` and `UserManagement.InviteService.bulk_import_users/2` |
| **Affected functions** | `send_invite/2`, `bulk_import_users/2` |
| **Short explanation** | Both functions reproduce the same email-domain allowlist/blocklist validation (normalising the domain, checking corporate allowlist, checking disposable-domain blocklist). A new blocked domain or a new allowlist entry must be added in two places. |
...
defmodule UserManagement.InviteService do
  ...
  def send_invite(email, %Workspace{} = workspace) do
    with {:ok, normalised} <- normalise_email(email),
         ...
         # VALIDATION: This is a smell because the domain allowlist/blocklist
         # check (extract domain, allowlist check, disposable-domain blocklist)
         # is duplicated verbatim in bulk_import_users/2. Policy changes must
         # be applied in both branches.
      ...
      Enum.map(emails, fn raw_email ->
        outcome =
          with {:ok, normalised} <- normalise_email(raw_email),
               # VALIDATION: SMELL START - Duplicated Code
               # VALIDATION: This is a smell because the identical
               # domain-policy check from send_invite/2 is copy-pasted here.
               # Any domain-policy update must be applied in both functions.
               {:ok, domain}     <- extract_domain(normalised),
               :ok               <- check_domain_allowed(domain, workspace),
               :ok               <- check_domain_not_disposable(domain) do
               # VALIDATION: SMELL END
