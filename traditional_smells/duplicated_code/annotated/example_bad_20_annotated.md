...
| **Expected smell location** | `Ecommerce.Cart.apply_coupon/2` and `Ecommerce.Cart.apply_store_credit/2` |
| **Affected functions** | `apply_coupon/2`, `apply_store_credit/2` |
| **Short explanation** | Both functions duplicate the logic that validates whether a cart meets the minimum order value threshold before applying a discount: summing line item prices (quantity × unit price), and comparing the subtotal against a minimum. If the subtotal computation changes (e.g., excluding gift items), it must be updated in both functions. |
...
defmodule Ecommerce.Cart do
  ...
  def apply_coupon(%Cart{} = cart, coupon_code) do
    with {:ok, coupon} <- fetch_active_coupon(coupon_code) do
      ...
      # VALIDATION: This is a smell because the subtotal computation —
      # summing quantity * unit_price_cents across all line items — is
      # duplicated in apply_store_credit/2. Any change to how the subtotal
      # is computed (e.g., excluding sale items) must be updated in both functions.
      subtotal_cents =
        cart.line_items
  ...
  def apply_store_credit(%Cart{} = cart, %StoreCredit{} = credit) do
    ...
    # VALIDATION: SMELL START - Duplicated Code
    # VALIDATION: This is a smell because this subtotal computation is a copy
    # of the identical Enum.reduce pipeline from apply_coupon/2.
    subtotal_cents =
      cart.line_items
      |> Enum.reduce(0, fn %LineItem{} = li, acc ->
        acc + li.quantity * li.unit_price_cents
