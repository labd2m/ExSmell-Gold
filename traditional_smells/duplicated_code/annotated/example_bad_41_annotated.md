...
| **Expected smell location** | `Scheduling.BookingManager.book_resource/3` and `Scheduling.BookingManager.extend_booking/3` |
| **Affected functions** | `book_resource/3`, `extend_booking/3` |
| **Short explanation** | Both functions duplicate the overlap-detection query against existing bookings (fetching active bookings for the resource and comparing time windows). A change to the overlap definition—e.g., adding buffer time—must be applied in both functions. |
...
defmodule Scheduling.BookingManager do
  ...
  def book_resource(resource_id, %DateTime{} = starts_at, %DateTime{} = ends_at) do
    with {:ok, resource} <- Resource.fetch(resource_id),
      ...
      # VALIDATION: This is a smell because the overlap-detection logic
      # (fetching bookings + comparing start/end) is duplicated verbatim
      # in extend_booking/3. Adding buffer time or changing overlap
      # semantics requires updating both functions.
  ...
  def extend_booking(booking_id, new_ends_at, extended_by_user_id) do
    with {:ok, booking}  <- Repo.fetch_booking(booking_id),
      ...
      # VALIDATION: SMELL START - Duplicated Code
      # VALIDATION: This is a smell because the same overlap-detection
      # block from book_resource/3 is reproduced here with no changes.
      # Both blocks must always be kept in sync.
      existing_bookings = Repo.list_active_bookings(booking.resource_id)

      # Exclude the booking being extended from the overlap check
