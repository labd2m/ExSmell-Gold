...
| **Expected smell location** | `NotificationService` module (entire module) |
| **Affected functions** | All functions across channel dispatch, template rendering, preference management, rate limiting, and delivery tracking |
| **Short explanation** | `NotificationService` manages channel dispatch (email/SMS/push), template rendering, user preference lookup, rate limiting, and delivery tracking — five distinct concerns merged in one module. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because NotificationService handles dispatch
# over multiple channels, template rendering, user preference resolution,
# rate limiting, and delivery record persistence — five separate
# responsibilities that should each belong to their own module.
