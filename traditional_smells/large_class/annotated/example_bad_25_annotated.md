...
| **Expected smell location** | `HRManager` module (entire module) |
| **Affected functions** | All functions: onboarding, payroll, leave management, performance reviews, and offboarding |
| **Short explanation** | `HRManager` handles employee onboarding, payroll calculation, leave request management, performance review workflows, and offboarding — five completely unrelated HR sub-domains packed into one module. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because HRManager conflates onboarding workflows,
# payroll computation, leave/PTO management, performance review cycles, and
# offboarding procedures into a single module. Each is a distinct HR
# sub-domain with its own rules and should live in its own module.
