...
| **Expected smell location** | `InventoryManager` module (entire module) |
| **Affected functions** | All functions: stock tracking, reservation, replenishment, valuation, and reporting |
| **Short explanation** | `InventoryManager` conflates stock level tracking, reservation/hold logic, reorder/replenishment, inventory valuation (FIFO/LIFO), and reporting into a single module, each being a distinct inventory sub-domain. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because InventoryManager handles stock tracking,
# reservation management, automatic replenishment, FIFO/LIFO valuation, and
# reporting — five distinct inventory sub-concerns bundled into one module
# instead of being split into cohesive, focused modules.
