...
- **Expected smell location:** The entire `PaymentProcessor` module
- **Affected functions:** `charge/3`, `authorize/3`, `capture/2`, `void_authorization/2`, `refund/3`, `create_customer_profile/1`, `update_payment_method/2`, `list_transactions/2`, `generate_receipt/1`, `handle_webhook/1`
- **Short explanation:** `PaymentProcessor` combines gateway interaction (charge, authorize, capture, void), refund processing, customer vault management (profiles, stored cards), transaction history, receipt generation, and webhook handling into one module. Each concern represents a separate subdomain (gateway, vault, reporting, webhooks) and should have its own dedicated module.

```elixir
# VALIDATION: SMELL START - Large Class (Large Module)
# VALIDATION: This is a smell because PaymentProcessor handles gateway
# operations (charge, authorize, capture, void), refunds, customer payment
# profile management, transaction reporting, receipt generation, and webhook
# processing — far too many distinct responsibilities for one module.
defmodule PaymentProcessor do
  ...
  # --- Customer profile (vault) management ---
