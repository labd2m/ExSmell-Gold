...
- **Expected smell location:** The entire `WarehouseOperations` module
- **Affected functions:** `receive_goods/2`, `pick_items/2`, `pack_order/2`, `dispatch_shipment/2`, `conduct_cycle_count/1`, `record_damage/2`, `assign_bin_location/2`, `generate_packing_list/1`, `calculate_warehouse_kpis/1`, `alert_low_bin_capacity/1`
- **Short explanation:** `WarehouseOperations` combines inbound receiving, order picking, packing, outbound dispatch, cycle counting, damage recording, bin-location management, packing list generation, KPI calculation, and capacity alerting — ten responsibilities spanning receiving, fulfilment, quality control, slotting, analytics, and alerting domains.

```elixir
# VALIDATION: SMELL START - Large Class (Large Module)
# VALIDATION: This is a smell because WarehouseOperations packs inbound
# receiving, outbound pick/pack/dispatch, cycle counting, damage tracking,
# bin-slot management, packing list rendering, KPI analytics, and capacity
# alerting into one module — ten responsibilities across multiple domains
# that should each have a dedicated module.
