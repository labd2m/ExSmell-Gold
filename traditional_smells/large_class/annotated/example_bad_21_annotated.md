...
| **Expected smell location** | `PaymentProcessor` module (entire module) |
| **Affected functions** | All functions: gateway integration, fraud detection, refunds, dispute handling, and reconciliation |
| **Short explanation** | `PaymentProcessor` merges gateway charge logic, fraud scoring, refund processing, dispute management, and daily reconciliation — five separate financial concerns — into a single non-cohesive module. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because PaymentProcessor includes gateway
# charging, fraud detection/scoring, refund logic, dispute workflows,
# and financial reconciliation all in one place — these are distinct
# payment sub-domains that each merit their own module.
