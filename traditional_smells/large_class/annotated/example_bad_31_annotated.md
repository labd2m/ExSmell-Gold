...
| **Expected smell location** | `EventManager` module (entire module) |
| **Affected functions** | All functions: event creation, ticketing, attendee check-in, sponsor management, and post-event reporting |
| **Short explanation** | `EventManager` combines event creation/scheduling, ticket type/pricing management, attendee check-in workflows, sponsor coordination, and post-event analytics — five unrelated event sub-domains packed into one module. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because EventManager manages event scheduling,
# ticket sales/pricing, attendee check-in, sponsor management, and
# post-event reporting — five distinct event management sub-domains — all
# within a single non-cohesive module instead of split across focused modules.
