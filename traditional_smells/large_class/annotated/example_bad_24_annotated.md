...
| **Affected functions** | All functions: order creation, status transitions, fulfillment, returns, and customer communications |
| **Short explanation** | `OrderManager` encompasses order creation and validation, state-machine transitions, fulfillment coordination, returns/RMA handling, and customer email communications — five unrelated order sub-domains in one module. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because OrderManager merges order creation,
# status-machine management, fulfillment logic, return/RMA workflows, and
# customer communication into one non-cohesive module. Each represents a
# distinct business concern that deserves its own focused module.
