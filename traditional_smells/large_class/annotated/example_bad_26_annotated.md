...
| **Affected functions** | All functions: product CRUD, pricing, categorization, search, and media management |
| **Short explanation** | `ProductCatalog` handles product creation/editing, dynamic pricing rules, category taxonomy, full-text search indexing, and media/image management — five distinct catalog sub-domains merged in one non-cohesive module. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because ProductCatalog conflates product lifecycle
# management, pricing rule evaluation, category/taxonomy handling, search
# index synchronization, and media asset management — all separate concerns
# that should each have a focused, dedicated module.
