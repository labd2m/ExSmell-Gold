...
- **Affected functions:** `create_product/1`, `update_product/2`, `publish_product/1`, `unpublish_product/1`, `add_variant/2`, `update_pricing/2`, `apply_bulk_discount/2`, `index_for_search/1`, `generate_sitemap_entry/1`, `import_from_csv/1`
- **Short explanation:** `ProductCatalog` lumps together product lifecycle (CRUD, publish/unpublish), variant management, pricing and discount logic, full-text search indexing, SEO sitemap generation, and CSV bulk import. Each concern is a separate domain: `Catalog.Products`, `Catalog.Pricing`, `Catalog.Search`, `Catalog.SEO`, `Catalog.Import`.

```elixir
# VALIDATION: SMELL START - Large Class (Large Module)
# VALIDATION: This is a smell because ProductCatalog manages product lifecycle,
# variant handling, pricing rules, discount application, Elasticsearch indexing,
# sitemap entry generation, and CSV bulk import — at least six distinct concerns
# that make the module too large and incoherent.
defmodule ProductCatalog do
  ...
  def generate_sitemap_entry(%Product{status: :published} = product) do
    ...
    <url>
      ...
      <lastmod>#{Date.to_string(DateTime.to_date(product.updated_at))}</lastmod>
