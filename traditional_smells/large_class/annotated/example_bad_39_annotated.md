...
- **Expected smell location:** The entire `AppointmentScheduler` module
- **Affected functions:** `book_appointment/3`, `cancel_appointment/2`, `reschedule_appointment/3`, `list_available_slots/2`, `block_time_slot/3`, `send_reminder/2`, `check_conflicts/2`, `generate_schedule_report/2`, `sync_to_calendar/2`
- **Short explanation:** `AppointmentScheduler` handles slot availability querying, conflict detection, booking and rescheduling lifecycle, time-block management, reminder delivery, utilization reporting, and external calendar synchronization. These are distinct concerns spanning scheduling logic, communication, analytics, and third-party integration that should live in dedicated modules.

```elixir
# VALIDATION: SMELL START - Large Class (Large Module)
# VALIDATION: This is a smell because AppointmentScheduler packs together
# availability querying, conflict detection, booking lifecycle, time-block
# administration, reminder dispatch, schedule reporting, and Google Calendar
# sync — very different responsibilities that bloat the module and hurt cohesion.
