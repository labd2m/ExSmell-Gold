...
- **Affected functions:** `create_subscription/2`, `change_plan/2`, `pause_subscription/2`, `resume_subscription/1`, `cancel_subscription/2`, `apply_proration/2`, `charge_renewal/1`, `send_renewal_notice/1`, `calculate_mrr/0`, `list_expiring_trials/1`
- **Short explanation:** `SubscriptionManager` spans subscription lifecycle (create, change, pause, resume, cancel), billing proration, renewal charging, renewal email notifications, MRR analytics, and trial management. Billing logic, notification delivery, and analytics are clearly separate bounded contexts from subscription state management.

```elixir
# VALIDATION: SMELL START - Large Class (Large Module)
# VALIDATION: This is a smell because SubscriptionManager combines subscription
# lifecycle state management, billing proration, renewal charging, renewal email
# notifications, MRR revenue analytics, and trial expiry tracking — six
# different concerns that bloat the module and reduce cohesion.
