...
- **Expected smell location:** The entire `ReportingDashboard` module
- **Affected functions:** `revenue_summary/2`, `churn_rate/2`, `top_products/2`, `user_acquisition/2`, `export_to_csv/2`, `export_to_json/2`, `cache_report/3`, `schedule_report_delivery/3`, `send_report_email/3`
- **Short explanation:** `ReportingDashboard` blends metric computation (revenue, churn, products, acquisition), data export in multiple formats, report caching, scheduled delivery, and email dispatch. These are orthogonal concerns: analytics belong in `Reports.Analytics`, serialization in `Reports.Export`, caching in `Reports.Cache`, and delivery in `Reports.Delivery`.

```elixir
# VALIDATION: SMELL START - Large Class (Large Module)
# VALIDATION: This is a smell because ReportingDashboard handles multiple
# distinct concerns: computing four different business metrics, exporting data
# in two formats, caching results, scheduling periodic delivery, and sending
# email reports — responsibilities that clearly belong in separate modules.
