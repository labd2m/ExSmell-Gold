...
| **Affected functions** | All functions: plan management, subscription lifecycle, trial handling, usage metering, and dunning |
| **Short explanation** | `SubscriptionManager` combines plan/tier configuration, subscription lifecycle (create/upgrade/cancel), free trial logic, usage-based metering, and dunning/retry campaigns — five distinct subscription billing concerns in one large module. |

```elixir
# VALIDATION: SMELL START - Large Module (Large Class)
# VALIDATION: This is a smell because SubscriptionManager conflates plan
# configuration, subscription lifecycle management, trial workflows,
# usage metering, and dunning campaign logic — each a distinct concern
# in the subscription billing domain — into one non-cohesive module.
